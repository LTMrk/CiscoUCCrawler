...
Share:
Today, we are pleased to announce the launch of the multi-agent software reference application we call as well as an . 
...
> "The coffeeAGNTCY reference application is a great launchpad for us to implement AGNTCY components in our latest project. The easy-to-follow and ready-made Docker compose files are super convenient," said Amogh Tarcar, Senior Data Scientist at Persistent.
...
We recommend starting with the . Here, we demonstrate the integration of an A2A client within a LangGraph workflow with an A2A server agent. It models a simplified agent system that acts as a coffee sommelier.
...
The AGNTCY Application SDK provides a convenient way to integrate with AGNTCY components, beginning with and later expanding to incorporate many others including , and . All this while enabling interoperability with agentic protocols such as A2A and Model Context Protocol.  
