[Jump to Content](https://help.webexconnect.io/docs/wxcc-using-bot-rich-responses-with-livechat#content)
...
v6.20.0
[Home](https://help.webexconnect.io/)[Platform Documentation](https://help.webexconnect.io/docs)[Platform Changelog](https://help.webexconnect.io/changelog)Using bot rich responses with Livechat
Search
`CTRL-K`
All
Platform Documentation
...
Sample flow how a bot interact and responds with LiveChat
[Suggest Edits](https://help.webexconnect.io/edit/wxcc-using-bot-rich-responses-with-livechat)
## 
Sample flow
[](https://help.webexconnect.io/docs/wxcc-using-bot-rich-responses-with-livechat#sample-flow)
<https://imiprodus1org.us.webexconnect.io/flowdesigner/view/public/de56af4a-db1c-4e62-b011-3b67395e35c3> (pass: 123)
![Screenshot of 'bot loop' part of the flow for using rich responses.](https://files.readme.io/7b369b6-Complete_flow.png)
The 'bot loop' part of the flow for using rich responses. The numbers above each box correspond to steps in this documentation.
## 
Steps to use rich responses in a Webex Connect flow
[](https://help.webexconnect.io/docs/wxcc-using-bot-rich-responses-with-livechat#steps-to-use-rich-responses-in-a--flow)
...
![Screenshot of setting the 'botresponse' variable in bot node.](https://files.readme.io/ca63a0f-Task1.png)
Setting the 'botresponse' variable in bot node.
  1. Use an evaluate node for 'Success' node outcome of the bot node to parse the bot output and transform it into the format required by the inApp node. 
...
![Screenshot of setting script outputs in evaluate node.](https://files.readme.io/e145129-Eval_branches.png)
Setting script outputs in evaluate node.
  1. For each of the three node outcomes, use a different In-app Messaging channel node and select the message type based on the appropriate branch. 
    1. Text response 
...
![Screenshot of delivering a generic template/carousel to the user.](https://files.readme.io/8077a04-in_app_carousel.png)
Delivering a generic template/carousel to the user.
  1. Select the appropriate 'Message Type' in subsequent Append Conversation nodes and provide the values from the preceding channel nodes. Set the Timestamp field as `$(timestamp)`.

![Screenshot of appending the conversation with generic template.](https://files.readme.io/cf2b90b-Append.png)
Appending the conversation with generic template.
  1. Connect receive nodes to Append conversation nodes in each of the three branches. Add 'Incoming message' and 'Postback' event names. Update the value of the variable that is being used as an input message to the bot node when exiting these receive nodes.

![Screenshot of event names in the receive nodes.](https://files.readme.io/a8ad2c4-Receive_events.png)
Event names in the receive nodes.
![Screenshot of updating the variable that is used as the input message to bot node.](https://files.readme.io/7b95262-Receive_var_set.png)
Updating the variable that is used as the input message to bot node.
  1. For the app.mo node outcome of the receive nodes in step 6, connect them to the evaluate node for checking PCI compliance and parsing any attachments. This should already be present in your livechat flow. If now, you can use the code and transition actions from the evaluate node in linked sample flow.
  2. For each of the 3 branches, use the Append conversation node for incoming user messages. Select the appropriate message type and response object based on the branch.

![Screenshot of appending conversation with generic template response received from the user.](https://files.readme.io/6ceb303-Append1.png)
Appending conversation with generic template response received from the user.
  1. For the branches with app.postback node outcome of the receive nodes in step 6, use a data parser node extract the payload of the button clicked by the user if you wish to send the payload back to the bot as user message. Update the value of the variable that is being used as an input message to the bot node when exiting these data parser nodes.

![Screenshot of extracting the payload value from the button clicked by user.](https://files.readme.io/3d907bd-Data_parser_0.png)
Extracting the payload value from the button clicked by user.
![Screenshot of updating the variable that is used as the input message to bot node.](https://files.readme.io/264c6a6-Data_parse.png)
Updating the variable that is used as the input message to bot node.
  1. Connect the success outcomes of data parser or append conversation nodes back to the bot node in all branches.


### 
Evaluate node code for data transformation
[](https://help.webexconnect.io/docs/wxcc-using-bot-rich-responses-with-livechat#evaluate-node-code-for-data-transformation)
...
* * *
[Handling various customer intents using flows and Bot Builder](https://help.webexconnect.io/docs/wxcc-handling-various-customer-intents-bot-builder-wxcc)[Best Practices and Guidelines - WXCC](https://help.webexconnect.io/docs/wxcc-best-practices-guidelines)
Did this page help you?
Yes
No
  * [Table of Contents](https://help.webexconnect.io/docs/wxcc-using-bot-rich-responses-with-livechat)
  *     * [Sample flow](https://help.webexconnect.io/docs/wxcc-using-bot-rich-responses-with-livechat#sample-flow)
    * [Steps to use rich responses in a Webex Connect flow](https://help.webexconnect.io/docs/wxcc-using-bot-rich-responses-with-livechat#steps-to-use-rich-responses-in-a--flow)
      * [Evaluate node code for data transformation](https://help.webexconnect.io/docs/wxcc-using-bot-rich-responses-with-livechat#evaluate-node-code-for-data-transformation)
