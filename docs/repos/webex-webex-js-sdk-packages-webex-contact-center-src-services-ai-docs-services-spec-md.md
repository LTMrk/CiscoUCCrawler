---
doc_id: webex-webex-js-sdk-packages-webex-contact-center-src-services-ai-docs-services-spec-md
source_url: https://github.com/webex/webex-js-sdk/blob/next/packages/%40webex/contact-center/src/services/ai-docs/services-spec.md
repo: webex/webex-js-sdk
ruta: packages/@webex/contact-center/src/services/ai-docs/services-spec.md
licencia: NOASSERTION
retrieved_at: 2026-09-21T01:52:50.961018+00:00
---

# webex-js-sdk — packages/@webex/contact-center/src/services/ai-docs/services-spec.md
...
| generated_by / approved_by / updated_at | Codex generator / Agent Wellness Break v0.4 intake audit / 2026-09-07 |
...
| `services.agent-wellness-break` | SDK data / action API | `ApiAIAssistant.requestWellnessBreak` and `respondToWellnessBreak`, invoked through `ContactCenter`. | Submit validated `WellnessBreakAction` custom events through direct HTTP. | Additive semver-public behavior; exact action/event payload is backend-owned. | `src/services/ApiAiAssistant.ts`, `src/types.ts` | `../../../ai-docs/CONTRACTS.md` |
...
| SERVICES-R-008 | ApiAIAssistant wellness requests must accept no public identity or context-mutating input, read effective enablement and non-empty SDK-owned organization/agent/session context from a read-only provider supplied at construction, then POST the exact `WellnessBreakAction` custom-event body directly to `/event`; only HTTP 202 resolves successfully. Public action unions derive from exported `as const` action-value objects. | SDK-owned live context prevents callers from supplying stale/different login identities to action calls, one value/type source prevents wire drift, and direct HTTP matches the AI event endpoint rather than AQM completion semantics. | `src/services/ApiAiAssistant.ts`, `src/types.ts`, `src/index.ts`, `src/cc.ts` | `test/unit/spec/services/ApiAiAssistant.ts`, `test/unit/spec/cc.ts` | None. | PRESENT |
...
- ApiAIAssistant evaluates the current effective wellness flag and identities through its constructor-supplied read-only provider; the public object exposes no context mutator, and the Services singleton itself remains unconditionally composed.
...
---
> Fuente: https://github.com/webex/webex-js-sdk/blob/next/packages/%40webex/contact-center/src/services/ai-docs/services-spec.md (licencia NOASSERTION)
