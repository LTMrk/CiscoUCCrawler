---
doc_id: webex-webex-js-sdk-packages-calling-src-voicemail-ai-docs-voicemail-spec-md
source_url: https://github.com/webex/webex-js-sdk/blob/next/packages/calling/src/Voicemail/ai-docs/voicemail-spec.md
repo: webex/webex-js-sdk
ruta: packages/calling/src/Voicemail/ai-docs/voicemail-spec.md
licencia: NOASSERTION
retrieved_at: 2026-08-24T09:08:13.684504+00:00
---

# webex-js-sdk — packages/calling/src/Voicemail/ai-docs/voicemail-spec.md
...
|---|---|---|---|---|---|---|
| VOICEMAIL-R-001 | Initializes the selected voicemail connector and resolves backend-specific endpoints and authentication: XSI for WXC/BroadWorks and VG Gateway for UCM. | Initialization resolves the backend-specific endpoint and credential path before operations so later calls do not mix WXC, BroadWorks, and UCM contracts. | `src/Voicemail/Voicemail.ts`; `src/Voicemail/WxCallBackendConnector.ts`; `src/Voicemail/BroadworksBackendConnector.ts`; `src/Voicemail/UcmBackendConnector.ts` | `src/Voicemail/Voicemail.test.ts`; `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | none identified | PRESENT |
| VOICEMAIL-R-002 | Retrieves paginated, sorted voicemail lists. WXC/BWRKS use XSI VoiceMessagingMessages API; UCM uses VG Gateway API. | Backend-aware pagination and sorting provide a stable list contract despite XSI and VG Gateway returning data differently. | `src/Voicemail/WxCallBackendConnector.ts`; `src/Voicemail/BroadworksBackendConnector.ts`; `src/Voicemail/UcmBackendConnector.ts` | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | none identified | PRESENT |
| VOICEMAIL-R-003 | Fetches the audio content (media type + base64 content) for a specific voicemail message. | Audio content is fetched separately so list responses stay small and consumers request media only for the selected message. | `src/Voicemail/WxCallBackendConnector.ts`; `src/Voicemail/BroadworksBackendConnector.ts`; `src/Voicemail/UcmBackendConnector.ts` | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | none identified | PRESENT |
| VOICEMAIL-R-004 | Retrieves quantitative summary (new, old, urgent message counts) via XSI `MessageSummary` endpoint. Only supported on WXC; BWRKS and UCM return `null`. | Returning null for unsupported backends distinguishes unavailable summary capability from an empty mailbox. | `src/Voicemail/WxCallBackendConnector.ts`; `src/Voicemail/BroadworksBackendConnector.ts`; `src/Voicemail/UcmBackendConnector.ts` | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | none identified | PRESENT |
| VOICEMAIL-R-005 | Updates the read status of a voicemail message. | Persisting read state remotely keeps message indicators consistent across clients and devices. | `src/Voicemail/WxCallBackendConnector.ts`; `src/Voicemail/BroadworksBackendConnector.ts`; `src/Voicemail/UcmBackendConnector.ts` | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | none identified | PRESENT |
| VOICEMAIL-R-006 | Deletes a voicemail message by its messageId. | Deletion must target the authoritative backend message id so local state cannot diverge from the mailbox. | `src/Voicemail/WxCallBackendConnector.ts`; `src/Voicemail/BroadworksBackendConnector.ts`; `src/Voicemail/UcmBackendConnector.ts` | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | none identified | PRESENT |
| VOICEMAIL-R-007 | Retrieves the text transcript of a voicemail via XSI. Only supported on WXC; BWRKS and UCM return `null`. | Returning null where transcription is unsupported gives callers an explicit capability result rather than a transport-shaped failure. | `src/Voicemail/WxCallBackendConnector.ts`; `src/Voicemail/BroadworksBackendConnector.ts`; `src/Voicemail/UcmBackendConnector.ts` | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | none identified | PRESENT |
| VOICEMAIL-R-008 | Resolves caller identity from `CallingPartyInfo` using `userExternalId` (SCIM query) and `name` (People search API). Only supported on WXC; BWRKS and UCM return `null`. | SCIM/People enrichment converts backend caller identifiers into display information useful to voicemail consumers without changing the stored message. | `src/Voicemail/WxCallBackendConnector.ts`; `src/Voicemail/BroadworksBackendConnector.ts`; `src/Voicemail/UcmBackendConnector.ts` | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | none identified | PRESENT |
| VOICEMAIL-R-009 | Submits success/error metrics for voicemail list, content, read/unread, delete, and non-null summary/transcript responses via MetricManager. | Success/error metrics make backend and operation failures observable across asynchronous voicemail workflows. | `src/Voicemail/Voicemail.ts` | `src/Voicemail/Voicemail.test.ts` | none identified | PRESENT |
| VOICEMAIL-R-010 | Delegates to WXC, BroadWorks, or UCM connectors based on user entitlements. | Entitlement-based connector selection keeps one public interface while honoring materially different WXC, BroadWorks, and UCM capabilities. | `src/Voicemail/Voicemail.ts` | `src/Voicemail/Voicemail.test.ts` | none identified | PRESENT |
...
|---|---|---|
| VOICEMAIL-R-001 | `src/Voicemail/Voicemail.test.ts`; `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | Re-check negative/error edge coverage during independent validation |
| VOICEMAIL-R-002 | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | Re-check negative/error edge coverage during independent validation |
| VOICEMAIL-R-003 | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | Re-check negative/error edge coverage during independent validation |
| VOICEMAIL-R-004 | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | Re-check negative/error edge coverage during independent validation |
| VOICEMAIL-R-005 | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | Re-check negative/error edge coverage during independent validation |
| VOICEMAIL-R-006 | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | Re-check negative/error edge coverage during independent validation |
| VOICEMAIL-R-007 | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | Re-check negative/error edge coverage during independent validation |
| VOICEMAIL-R-008 | `src/Voicemail/WxCallBackendConnector.test.ts`; `src/Voicemail/BroadworksBackendConnector.test.ts`; `src/Voicemail/UcmBackendConnector.test.ts` | Re-check negative/error edge coverage during independent validation |
| VOICEMAIL-R-009 | `src/Voicemail/Voicemail.test.ts` | Re-check negative/error edge coverage during independent validation |
| VOICEMAIL-R-010 | `src/Voicemail/Voicemail.test.ts` | Re-check negative/error edge coverage during independent validation |
...
---
> Fuente: https://github.com/webex/webex-js-sdk/blob/next/packages/calling/src/Voicemail/ai-docs/voicemail-spec.md (licencia NOASSERTION)
