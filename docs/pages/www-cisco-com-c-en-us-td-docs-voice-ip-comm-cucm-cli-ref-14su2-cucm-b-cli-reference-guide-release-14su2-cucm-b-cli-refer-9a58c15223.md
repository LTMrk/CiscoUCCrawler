...
This command overwrites the database on the server on which you run this command with the database from the other server in
                              the Connection cluster. SSH connection from client work station to the server should be kept open until overwrite db procedure completes. Replication restarts after the database is overwritten. This method is used when you restore one server from a backup and
                              must copy the restored data to the other server.
