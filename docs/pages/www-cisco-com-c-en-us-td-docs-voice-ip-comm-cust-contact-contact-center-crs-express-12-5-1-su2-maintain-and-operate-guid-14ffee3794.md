---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-crs-express-12-5-1-su2-maintain-and-operate-guid-14ffee3794
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/crs/express_12_5_1_su2/maintain_and_operate/guide/uccx_b_1251su2_admin-and-operations-guide/uccx_b_12_5_2admin-and-operations-guide_chapter_010110.html
retrieved_at: 2026-08-16T21:37:10.249075+00:00
---
...
The RSA key lengths supported for the certificate signing request (CSR) generation are 1024, 2048, 3072, and 4096 bits. For
                                                                     Elliptic Curve Digital Signature Algorithm (ECDSA), P-384 key size is recommended, which is roughly equivalent to 7680 bits.
...
In environments where group policies are enforced via the Active Directory domain, the root certificate can be added automatically
                                          to each user's trust store. Adding the certificate automatically simplifies user configuration requirements.
...
Sign in as a user on a computer that is part of the domain.
...
When using Chrome, it is recommended you import your certificates to the Chrome trust store to avoid issues with the Cisco
                                                                  Unified CCX Administration interface. Refer to CSCwa89310 for more details.
...
| Step 1 | Generate a CSR. Click Security > Certificate Management > Generate CSR . Note The RSA key lengths supported for the certificate signing request (CSR) generation are 1024, 2048, 3072, and 4096 bits. For
                                                                     Elliptic Curve Digital Signature Algorithm (ECDSA), P-384 key size is recommended, which is roughly equivalent to 7680 bits. From the Certificate Name drop-down list, choose tomcat and click Generate CSR . | Note | The RSA key lengths supported for the certificate signing request (CSR) generation are 1024, 2048, 3072, and 4096 bits. For
                                                                     Elliptic Curve Digital Signature Algorithm (ECDSA), P-384 key size is recommended, which is roughly equivalent to 7680 bits. |
|---|---|---|---|
| Note | The RSA key lengths supported for the certificate signing request (CSR) generation are 1024, 2048, 3072, and 4096 bits. For
                                                                     Elliptic Curve Digital Signature Algorithm (ECDSA), P-384 key size is recommended, which is roughly equivalent to 7680 bits. |
...
| Step 5 | Click Import and browse to the ca_name .cer file. In the Trusted Root Certification Authorities tab, ensure that the new certificate appears in the
                                                   list. |
| Step 6 | Restart the browser for the certificate to install. Note When using Chrome, it is recommended you import your certificates to the Chrome trust store to avoid issues with the Cisco
                                                                  Unified CCX Administration interface. Refer to CSCwa89310 for more details. | Note | When using Chrome, it is recommended you import your certificates to the Chrome trust store to avoid issues with the Cisco
                                                                  Unified CCX Administration interface. Refer to CSCwa89310 for more details. |
| Note | When using Chrome, it is recommended you import your certificates to the Chrome trust store to avoid issues with the Cisco
                                                                  Unified CCX Administration interface. Refer to CSCwa89310 for more details. |
