---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-12-6-2-reference-g-1870692155
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_2/reference/guide/ucce_b_acd-supplement-guide-for-avaya-12_6_2/ucce_b_acd-supplement-guide-for-avaya-12_5_chapter_01.html
retrieved_at: 2026-08-16T19:45:49.440124+00:00
---
...
Application Enablement Services (AES) software runs on an external server that communicates to Avaya Aura Communication Manager
                           ...
                           call control and receive event notifications. The ICM PG uses  TSAPI link, which is a client/service software.
...
Setting Up the  TSAPI Links

Setting Up Agents and Hunt Group

Setting Up Call Routing

### Setting up the  TSAPI Links on AES Server

This section describes how to set up the TSAPI links on an AES server.
...
On the Avaya switch, a hunt group is a group of extensions to which similar calls are routed. A hunt group may include all agents who
                              have a particular skill (for example, the ability to speak Spanish). It also includes all the agents who cover a geographical
                              territory (for example, Boston sales). A hunt group is sometimes referred to as a skill group. By default, Avaya PG (TAESPIM)
                              supports extensions of up to ten digits and this does not require registry configuration. The Agent Login IDs support up to
                              ten digits.
...
```
HKEY_LOCAL_MACHINE\SOFTWARE\GeoTel\ICR\<cus01>\<PG1A>\MDS\CurrentVersion\Clients\pim1
HKEY_LOCAL_MACHINE\SOFTWARE\GeoTel\ICR\<cus01>\<PG1A>\MDS\CurrentVersion\Clients\opc
HKEY_LOCAL_MACHINE\SOFTWARE\GeoTel\ICR\<cus01>\<PG1A>\MDS\CurrentVersion\Process
```

```
HKEY_LOCAL_MACHINE\SOFTWARE\GeoTel\ICR\<cus01>\<PG1A>\MDS\CurrentVersion\Clients\pgag
```
...
PG requires skill groups to be monitored to track agent login and logout events. No agents can log in to that skill group
                                 if a skill group is not monitored. PG uses Monitor request API s to monitor a skill group, using the TSAPI interface.

Avaya currently restricts one application to third-party domain control of a skill group.

For optimal performance, external applications that alter agent state on the Avaya ECS , use the Enterprise CTI interface.
...
| Step 1 | Enter the
                                          			 command add
                                             				hunt-group next and press Return . (You can also enter add
                                             				hunt-group.xxx , where .xxx is the hunt group number.) The first
                                          			 Hunt Group screen appears. Figure 3. Defining
                                                				  Agent Hunt Groups |
|---|---|
| Step 2 | Complete
                                          			 screens 1 through 2 of the hunt group record as described in the Avaya
...
| Step 1 | Enter the
                                          			 command change < agent login ID number >. The Agent
                                             				Login ID screen appears. Figure 4. Modifying Agent Login ID |
|---|---|
| Step 2 | Add the hunt
                                             				group that indicates Unified CCE agents in the SN (Skill Number) field in the table at the bottom
                                          			 of the screen. |
...
| Step 1 | Enter the
                                          			 command add
                                             				VDN.xxxx (where xxxx indicates the VDN). The Vector Directory Number screen appears. Figure 7. GUI
