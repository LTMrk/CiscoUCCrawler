---
doc_id: www-cisco-com-c-en-us-td-docs-solutions-cvd-collaboration-enterprise-14-collbcvd-security-html-7891355f5c
source_url: https://www.cisco.com/c/en/us/td/docs/solutions/CVD/Collaboration/enterprise/14/collbcvd/security.html
retrieved_at: 2026-08-16T18:23:24.712998+00:00
---
...
Webex Cloud-Connected Certificate Management option to monitor certificates

Certificate Monitoring

May 21, 2021

Support of multi-server certificates with self-signed certificates in Unified CM and IM & Presence

Various sections of this chapter

May 21, 2021

SIP OAuth support and recommendation on the 7800/8800 phones

Various sections of this chapter

May 21, 2021

Additional information related to SIP OAuth support on the 7800/8800 Series phones

Various sections of this chapter

May 21, 2021

TFTP configuration file confidentiality and encryption

Various sections of this chapter

May 21, 2021

Certificate deployment procedure updated with CMS, refer to the CMS chapter for example

Cisco Meeting Server

May 21, 2021

Remove Prime Collaboration Provisioning information
...
Scan and spam over Internet telephony
...
Cisco Unified CM calling search space (CSS) and partitions; toll-fraud prevention and access protection; Cisco Collaboration Edge Policy Rules and security
...
Used for secure web connections. Also used for example with services such as LDAP, ILS and LBM, Disaster Recovery System, Jabber, and secure signaling for endpoints configured with SIP OAuth.

Cisco Unified CM

CallManager

Used for secure signaling when not using SIP OAuth and for TFTP files signature. In order to reduce the number of certificates in your Unified CM cluster, it is recommended to re-use the tomcat certificate for the CallManager service.
...
Used for the ITL and CTL files signature.

Cisco Unified CM

ipsec

For IPsec connections. IPsec can be enabled, but it is not covered in this document.
...
Shared certificate used for Web Admin, Call Bridge, Web Bridge, and database server
...
Used for various applications, including administrators and users accessing the web interface and Jabber accessing UDS and logging in. The tomcat certificate is required to be CA-signed when configuring SIP OAuth with the 7800/8800 Series phones.
...
Only if IPsec is used. The IPsec certificates are required to be CA-signed in this case.
...
CallManager-ECDSA certificate is not used in this guide 1 . Use a multi-server self-signed certificate.
...
1. In this guide, we are not using ECDSA with tomcat, CallManager, cup-xmpp-ECDSA, and cup-xmpp-s2s-ECDSA certificates. However, these ECDSA certificates are still present in the servers and have to renewed before they expire in order to avoid certificate expiration alerts. In order to simplify the management of those certificates, generate self-signed multi-server certificates for these ECDSA certificates.
...
With CTL: ITLRecovery and CallManager certificate on publisher
...
The ITL and CTL files are signed by using the ITL recovery key. The TFTP configuration files are signed by using the TFTP servers’ CallManager private keys.
...
Table 7-8 Method to enable media and signaling encryption

Jabber clients

Supported, but use SIP OAuth instead

Recommended method in this guide

7800 / 8800 2 series phones

Supported, but use SIP OAuth instead

Recommended method in this guide

Video endpoints and phones that do not support SIP OAuth
...
Table 7-9 Methods to perform TFTP configuration file confidentiality

Jabber clients

Can be used if Jabber is not configured in encrypted mode based on SIP OAuth (so Jabber clients could be configured in non-encrypted mode or in encrypted mode based on mixed mode). This would require installing an LSC on Jabber.

Not available with Jabber clients.

7800/8800 Series phones (support SIP OAuth)

Can be used if the phones are not configured in encrypted mode based on SIP OAuth.

Automatically performed if media/signaling encryption is configured through SIP OAuth.
...
Table 7-10 Trust Stores Where the CA Certificate is Imported for Unified CM with IM and Presence Service
...
Table 7-11 CSRs for Multi-Server Certificates
...
Table 7-12 Key Usage and Extended Key Usage Requirements
...
Table 7-13 Certificates to import into Unifiec CM Phone-Edge-Trust for 7800 / 8800 Series phones configured with SIP OAuth

Certificate of the root CA that signs the Unified CM tomcat certificate

This is for the 7800/8800 Series phones to verify the Unified CM tomcat certificate when they connect to Unified CM SSO Service Provider (SSOSP) or CallManager service on port 5090. This is required if configuring SIP OAuth on those phones.

Certificate of the root CA that signs the Expressway-E certificate

This is for the 7800/8800 Series phones to verify the Expressway-E certificate when they connect via MRA and are configured with SIP OAuth. This is also required for activation code onboarding via MRA.
...
Table 7-14 SIP Trunk Security Profile Parameters for Secure SIP Trunks
...
- Cisco Meeting Server: cms.ent-pa.com (Cisco Meeting Server domain name)
...
Table 7-15 SIP Trunk Configuration for Secure SIP Trunks
...
Table 7-16 Phone Security Profiles to Configure
...
For Jabber and 7800/8800 Series phones configured with SIP OAuth, on-premises and connecting via MRA.
...
Optional. For on-premises video endpoints configured with TFTP configuration file encryption.
...
For on-premises video endpoints configured with no TFTP configuration file encryption and for MRA video endpoints.
...
Table 7-17 Cisco Expressway Trust Store
...
Table 7-18 Subject Alternate Names (SAN) to be Added to the CSR
...
Unified CM registrations domains 3
...
4. There is no need to add the phone security profile name that is used for SIP OAuth in the SAN in the Expressway-C certificated (see Table 7-16 for the list of phone security profiles). Only add the name of the phone security profile for endpoints that are configured with encryption based on mixed mode, which would correspond to the video endpoints in our guide.
...
Table 7-19 Cisco Expressway Key Usage and Extended Key Usage
...
Table 7-20 Expressway-C Business-to-Business Unified CM Neighbor Zone Configuration
...
Table 7-21 Expressway-C Business-to-Business Traversal Zone Configuration
...
Table 7-22 Expressway-E Business-to-Business Traversal Zone Configuration
...
Table 7-23 Expressway-E Default Zone Configuration
...
Table 7-24 Expressway-E DNZ Zone Configuration
...
|---|---|---|
| Webex Cloud-Connected Certificate Management option to monitor certificates | Certificate Monitoring | May 21, 2021 |
| Support of multi-server certificates with self-signed certificates in Unified CM and IM & Presence | Various sections of this chapter | May 21, 2021 |
| SIP OAuth support and recommendation on the 7800/8800 phones | Various sections of this chapter | May 21, 2021 |
| Additional information related to SIP OAuth support on the 7800/8800 Series phones | Various sections of this chapter | May 21, 2021 |
| TFTP configuration file confidentiality and encryption | Various sections of this chapter | May 21, 2021 |
| Certificate deployment procedure updated with CMS, refer to the CMS chapter for example | Cisco Meeting Server | May 21, 2021 |
| Remove Prime Collaboration Provisioning information | Cisco Prime Collaboration Provisioning | May 21, 2021 |

| Threats | Countermeasures |
|---|---|
| Denial of Service (DoS) | Physical security; network security; firewall and intrusion prevention system (IPS); QoS |
| Scan and spam over Internet telephony | Firewall and advanced malware protection (AMP); Cisco Collaboration Edge security; Cisco Unified Communications Manager (Unified CM) dial-plan |
| Virus | Host-based firewall; IPS; anti-virus software |
| Toll-fraud | Cisco Unified CM calling search space (CSS) and partitions; toll-fraud prevention and access protection; Cisco Collaboration Edge Policy Rules and security |
...
|---|---|---|
| Cisco Unified CM | tomcat | Used for secure web connections. Also used for example with services such as LDAP, ILS and LBM, Disaster Recovery System, Jabber, and secure signaling for endpoints configured with SIP OAuth. |
| Cisco Unified CM | CallManager | Used for secure signaling when not using SIP OAuth and for TFTP files signature. In order to reduce the number of certificates in your Unified CM cluster, it is recommended to re-use the tomcat certificate for the CallManager service. |
...
| Cisco Unified CM | ITLRecovery | Used for the ITL and CTL files signature. |
...
| Cisco Meeting Server | Shared certificate used for Web Admin, Call Bridge, Web Bridge, and database server | For simplicity, except for the database client, we use the same certificate for all Cisco Meeting Server nodes and services. |
...
|---|---|---|
| Cisco Unified CM and IM and Presence Service | tomcat | Used for various applications, including administrators and users accessing the web interface and Jabber accessing UDS and logging in. The tomcat certificate is required to be CA-signed when configuring SIP OAuth with the 7800/8800 Series phones. |
| Cisco Unified CM | CallManager | Used for various applications, including SIP trunks. |
| Cisco Unified CM | ipsec | Only if IPsec is used. The IPsec certificates are required to be CA-signed in this case. |
...
| Unified CM and IM and Presence Service | Tomcat-ECDSA | Tomcat-ECDSA certificate is not used in this guide*. Use a multi-server self-signed certificate. |
| Unified CM | CallManager |  |
| Unified CM | CallManager-ECDSA | CallManager-ECDSA certificate is not used in this guide 1 . Use a multi-server self-signed certificate. |
| Unified CM | TVS | TVS certificates can remained self-signed, but use multi-server certificates to reduce the number of certificates. |
| IM and Presence Service | cup-xmpp |  |
| IM and Presence Service | cup-xmpp-s2s |  |
| IM and Presence Service | cup-xmpp-s2s-ECDSA | cup-xmpp-s2s ECDSA certificate is not used in this guide*. Use a multi-server self-signed signed certificate. |
...
| System Administrator Security Token (SAST) | With CTL: ITLRecovery and CallManager certificate on publisher With ITL: ITLRecovery and CallManager certificates on TFTP servers | To authenticate the SAST, which is the entity that signs the CTL, ITL, or TFTP configuration files. This type of record is included in the ITL and CTL files. The ITL and CTL files are signed by using the ITL recovery key. The TFTP configuration files are signed by using the TFTP servers’ CallManager private keys. |
...
| Endpoint Type | Method to enable media and signaling encryption |
|---|---|
| Unified CM mixed mode with endpoint certificates | SIP OAuth |
| Jabber clients | Supported, but use SIP OAuth instead | Recommended method in this guide |
| 7800 / 8800 2 series phones | Supported, but use SIP OAuth instead | Recommended method in this guide |
| Video endpoints and phones that do not support SIP OAuth | Only option available. LSCs must be installed on the video endpoints. | Not supported |

| 2. Cisco IP Phones 8821 does not support SIP OAuth. See the Cisco phone data sheet for the latest information. Use encryption based on mixed mode for the phones that do not support SIP OAuth. |
|---|

| Endpoint Type | TFTP configuration file confidentiality method |
|---|---|
| File encryption of the TFTP configuration file (with MIC/LSC endpoint certificates) | Mutually authenticated and encrypted connection between endpoints and TFTP servers |
| Jabber clients | Can be used if Jabber is not configured in encrypted mode based on SIP OAuth (so Jabber clients could be configured in non-encrypted mode or in encrypted mode based on mixed mode). This would require installing an LSC on Jabber. | Not available with Jabber clients. |
| 7800/8800 Series phones (support SIP OAuth) | Can be used if the phones are not configured in encrypted mode based on SIP OAuth. | Automatically performed if media/signaling encryption is configured through SIP OAuth. |
...
|---|---|
| Certificate of the root CA that signs the Unified CM tomcat certificate | This is for the 7800/8800 Series phones to verify the Unified CM tomcat certificate when they connect to Unified CM SSO Service Provider (SSOSP) or CallManager service on port 5090. This is required if configuring SIP OAuth on those phones. |
| Certificate of the root CA that signs the Expressway-E certificate | This is for the 7800/8800 Series phones to verify the Expressway-E certificate when they connect via MRA and are configured with SIP OAuth. This is also required for activation code onboarding via MRA. |
...
| Phone Security Profile Name Examples | Authentication Mode (for CAPF Enrollment) | TFTP Encrypted Configuration File | OAuth Authentication | Type of endpoint using this profile |
|---|---|---|---|---|
| UDT-SIPOAuth | By Existing Certificate (precedence to LSC) | N/A | Enabled | For Jabber and 7800/8800 Series phones configured with SIP OAuth, on-premises and connecting via MRA. |
| UDT-Encrypted-LSC.ent-pa.com | By Existing Certificate (precedence to LSC) | Enabled | Disabled | Optional. For on-premises video endpoints configured with TFTP configuration file encryption. |
| UDT-Encrypted-NullString.ent-pa.com | By Null String or By Authentication String | Disabled | Disabled | For on-premises video endpoints configured with no TFTP configuration file encryption and for MRA video endpoints. |
...
| Unified CM registrations domains 3 | Required on Expressway-E only | – | – |
...
| Unified CM phone security profile names (FQDN format) 4 | Required on Expressway-C only | – | – |
