---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-admin-15-admingd-cucm-b-administration-guide-15-cucm-b-test-adminguide--4e34ede433
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/admin/15/adminGd/cucm_b_administration-guide-15/cucm_b_test-adminguide_chapter_010100.html
retrieved_at: 2026-08-17T00:36:58.980647+00:00
---
...
#### Establish TLS Connections with a Syslog Server

Use this procedure to configure a secure TLS connection between Unified Communications Manager and a Syslog server using a
                                    self-signed or a CA-signed certificate. The following authentication modes are supported:

Unidirectional x.509 Authentication—Only the Syslog server authenticates itself to Unified CM.

Bidirectional x.509 Authentication—Both the Unified CM and the Syslog server authenticates with each other.

Step 1

For Self-Signed Certificates:

For unidirectional authentication, to establish a TLS connection using self-signed certificates, a security certificate from
                                                   the syslog server must be uploaded to the tomcat trust store of the Unified Communications Manager Publisher node.

For bidirectional x.509 authentication:

To establish a TLS connection using self-signed certificates, a security certificate from the syslog server must be uploaded
                                                            to the tomcat trust store of the Unified Communications Manager Publisher node.

From the Cisco Unified OS Administration, navigate to Security > Certificate Management , and download the Tomcat certificate from the Unified Communications Manager and IM and Presence Service.

Upload the Tomcat certificate into the Syslog server's certificate directory.

Restart the Syslog server if necessary.

Step 2

For CA-Signed Certificates:

For unidirectional authentication, upload the certificate authority (CA) certificate to the tomcat trust store on the Unified
                                                   Communications Manager Publisher node.

For bidirectional x.509 authentication:

To establish a TLS connection, you must upload the certificate authority (CA) certificate to the tomcat trust store on the
                                                            Unified Communications Manager Publisher node.

From Cisco Unified OS Administration, navigate to Security > Certificate Management and download the Tomcat certificate from Unified Communications Manager and IM and Presence Service.

Ensure that this certificate is signed by the same CA.

Upload the CA-signed Tomcat certificate to the tomcat store on the Unified Communications Manager Publisher node.

Upload the CA certificate to the Syslog server's certificate directory.
...
| Step 1 | For Self-Signed Certificates: For unidirectional authentication, to establish a TLS connection using self-signed certificates, a security certificate from
                                                   the syslog server must be uploaded to the tomcat trust store of the Unified Communications Manager Publisher node. For bidirectional x.509 authentication: To establish a TLS connection using self-signed certificates, a security certificate from the syslog server must be uploaded
                                                            to the tomcat trust store of the Unified Communications Manager Publisher node. From the Cisco Unified OS Administration, navigate to Security > Certificate Management , and download the Tomcat certificate from the Unified Communications Manager and IM and Presence Service. Upload the Tomcat certificate into the Syslog server's certificate directory. Note If the Tomcat certificate is regenerated, it must be reuploaded to the syslog server. Restart the Syslog server if necessary. | Note | If the Tomcat certificate is regenerated, it must be reuploaded to the syslog server. |
|---|---|---|---|
| Note | If the Tomcat certificate is regenerated, it must be reuploaded to the syslog server. |
| Step 2 | For CA-Signed Certificates: For unidirectional authentication, upload the certificate authority (CA) certificate to the tomcat trust store on the Unified
                                                   Communications Manager Publisher node. For bidirectional x.509 authentication: To establish a TLS connection, you must upload the certificate authority (CA) certificate to the tomcat trust store on the
                                                            Unified Communications Manager Publisher node. From Cisco Unified OS Administration, navigate to Security > Certificate Management and download the Tomcat certificate from Unified Communications Manager and IM and Presence Service. Ensure that this certificate is signed by the same CA. Upload the CA-signed Tomcat certificate to the tomcat store on the Unified Communications Manager Publisher node. Upload the CA certificate to the Syslog server's certificate directory. Restart the Syslog server if necessary. |

| Note | If the Tomcat certificate is regenerated, it must be reuploaded to the syslog server. |
|---|---|

| Note | Refer to the Syslog server documentation for more information on the configuration details related to TLS and their supported
                                             formats (for example, PEM, DER, and so on). |
|---|---|

| Note | After successful migration using Cisco Prime Collaboration Deployment (PCD) or Fresh Install with Data Import along with a
                                             change in hostname, you must upload the new tomcat certificate to the syslog server again. |
|---|---|
