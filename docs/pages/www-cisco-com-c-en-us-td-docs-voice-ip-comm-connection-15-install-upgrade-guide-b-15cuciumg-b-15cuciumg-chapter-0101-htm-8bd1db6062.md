---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-connection-15-install-upgrade-guide-b-15cuciumg-b-15cuciumg-chapter-0101-htm-8bd1db6062
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/install_upgrade/guide/b_15cuciumg/b_15cuciumg_chapter_0101.html
retrieved_at: 2026-08-17T02:13:05.975644+00:00
---
...
For more information on the encryption in Cisco Unity Connection, see " "Cisco Unity Connection- Restricted and Unrestricted Version " chapter of the Security Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/security/guide/b_15cucsecx.html .
...
On the Licenses page, select View/Edit link under Transport Settings field. The Smart Transport Settings dialog box appears.

Step 3

Select the applicable deployment option for the Smart Licensing:

- Direct - Cisco Unity Connection connects directly to the Smart Software Manager over the internet. This is the default option. With
                                                   this option, you must deploy DNS on Cisco Unity Connection thato resolve the CSSM server.

(Applicable for releases before 15SU3) Transport Gateway : Unity Connection communicates with CSSM through Smart Software Manager satellite. To use satellite as a transport gateway,
                                                      enter the valid URL of the satellite that is installed in your premises.

Default setting: Check box is not checked.

IP Address : IP address or Hostname of the proxy server.

- Port : The port through which Unity Connection connects to the proxy server.

Username : Username of proxy server.

Password : Password of proxy server.

(Applicable for release 15SU3 and later) Smart Software Manager satellite - Cisco Unity Connection connects to an on-premise Cisco Smart Software Manager satellite for system licensing. In the URL
                                                      text box, enter the address and port of the Smart Software Manager satellite.

Smart Transport is supported in Cisco Smart Software Manager satellite starting from version 7-201907.

Default setting: Check box is not checked.

Step 4

Do not share my hostname or IP address with Cisco - Check this check box to restrict the hostname or IP address information of Cisco Unity Connection being exchanged with CSSM
                                             or satellite.

Step 5

(Applicable for Release 15SU3 and later) Test connectivity with Smart Transport - Allows administrator to test the connectivity of Unity Connection with Smart Transport endpoint through transport setting
                                             configurations.

Step 6

(Applicable for Release 15SU3 and later) Switch - Allows administrator to change the transport mode from Call Home to Smart Transport. Cisco Smart License Manager service
                                             ...
                                             Manager satellite in Call Home Mode. Switch Button is enabled once Test connectivity with Smart Transport is successful with Smart Transport.

Step 7

Select Save.

- If the product is registered with CSSM or satellite, you must deregister the product before changing the transport settings.

(Applicable for Release 15SU3 and later) Smart Transport has been introduced as an enhanced transport mechanism for Smart Licensing, in addition to the existing Call
...
(Applicable for Release 15SU3 and later) Transport Gateway as a transport mode is not supported in Smart Transport. Hence, it continues to use call home post upgrade
...
- Evaluation Mode : The authorization status of fresh installed Unity Connection is Evaluation Mode until it registers with CSSM or satellite.
                                       In this mode, Unity Connection can use licensed feature except SpeechView. The Evaluation Period of 90 days are provided once
                                       in the entire life cycle of the product. The Evaluation Period of Unity Connection begins as soon as it starts consuming licenses.
                                       After successful registration with CSSM, the Evaluation timer stops. You can further use the remaining Evaluation Period when
                                       Unity Connection will go in Unregister or Registration Expired state.
...
For more information on above CLI commands, see Command Line Interface Reference Guide for Cisco Unified Communications Solutions Release 15 available at https://www.cisco.com/c/en/us/support/unified-communications/unified-communications-manager-callmanager/products-command-reference-list.html .
...
Whenever you upgrade Cisco Unity Connection from any earlier releases to 15 and later, all licenses (legacy and PLM-based)
...
| Note | If you upgrade the Unity Connection from any earlier releases to 15, all the licenses(legacy and PLM based) used in Unity
...
| Step 1 | In Cisco Unity
                                             			 Connection Administration, expand System Settings and select Licenses. |
|---|---|
| Step 2 | On the Licenses page, select View/Edit link under Transport Settings field. The Smart Transport Settings dialog box appears. |
| Step 3 | Select the applicable deployment option for the Smart Licensing: Direct - Cisco Unity Connection connects directly to the Smart Software Manager over the internet. This is the default option. With
                                                   this option, you must deploy DNS on Cisco Unity Connection thato resolve the CSSM server. (Applicable for releases before 15SU3) Transport Gateway : Unity Connection communicates with CSSM through Smart Software Manager satellite. To use satellite as a transport gateway,
                                                      enter the valid URL of the satellite that is installed in your premises. HTTP/HTTPS Proxy - Unity Connection communicates with CSSM through HTTP or HTTPS Proxy. To use the proxy server, enter the following settings: Authentication needed on HTTP or HTTPS Proxy : Check this checkbox to enter details of password protected proxy server. Default setting: Check box is not checked. IP Address : IP address or Hostname of the proxy server. Port : The port through which Unity Connection connects to the proxy server. Username : Username of proxy server. Password : Password of proxy server. (Applicable for release 15SU3 and later) Smart Software Manager satellite - Cisco Unity Connection connects to an on-premise Cisco Smart Software Manager satellite for system licensing. In the URL
                                                      text box, enter the address and port of the Smart Software Manager satellite. Smart Transport is supported in Cisco Smart Software Manager satellite starting from version 7-201907. Default setting: Check box is not checked. |
| Step 4 | Do not share my hostname or IP address with Cisco - Check this check box to restrict the hostname or IP address information of Cisco Unity Connection being exchanged with CSSM
                                             or satellite. |
| Step 5 | (Applicable for Release 15SU3 and later) Test connectivity with Smart Transport - Allows administrator to test the connectivity of Unity Connection with Smart Transport endpoint through transport setting
                                             configurations. |
| Step 6 | (Applicable for Release 15SU3 and later) Switch - Allows administrator to change the transport mode from Call Home to Smart Transport. Cisco Smart License Manager service
                                             is restarted during switch process. This option is available only for Direct or HTTP/HTTPS Proxy server or Cisco Smart Software
                                             Manager satellite in Call Home Mode. Switch Button is enabled once Test connectivity with Smart Transport is successful with Smart Transport. Note The Switch operation can be performed only through the GUI. | Note | The Switch operation can be performed only through the GUI. |
| Note | The Switch operation can be performed only through the GUI. |
| Step 7 | Select Save. |

| Note | The Switch operation can be performed only through the GUI. |
|---|---|

| Note | If the product is registered with CSSM or satellite, you must deregister the product before changing the transport settings. (Applicable for Release 15SU3 and later) Smart Transport has been introduced as an enhanced transport mechanism for Smart Licensing, in addition to the existing Call
                                                      Home transport mode. All new Cisco Unity Connection installations will only support Smart Transport for Smart Licensing functionality. (Applicable for Release 15SU3 and later) Transport Gateway as a transport mode is not supported in Smart Transport. Hence, it continues to use call home post upgrade
                                                      or migration. To switch to Smart Transport mode, deregister the product. Also, the system must connect to Smart Transport
                                                      URL using Direct or Cisco Smart Software Manager satellite or HTTP/HTTPS Proxy server. |
|---|---|
...
| Note | After upgrading the Unity Connection from any earlier releases to 15, you must register the product with Cisco Smart Software
...
| CUC_SpeechConnectPort | Total number of speech connect ports. | Specifies the maximum number of simultaneous voice recognition sessions and Text to Speech (TTS) sessions allowed in Unity
                                                Connection. |
| CUC_SpeechConnectGuestUser 1 | Total number of Contacts. | Specifes the maximum number of local contacts, along with VPIM contacts created from Non Unity Connection Server. |
