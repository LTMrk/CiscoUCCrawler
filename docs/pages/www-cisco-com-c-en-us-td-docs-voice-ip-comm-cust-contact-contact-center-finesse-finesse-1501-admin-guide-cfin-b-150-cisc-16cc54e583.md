...
The Cisco Finesse Notification Service provides custom properties that allow you to change the behavior of notification service.

The following admin CLI allows you to modify the Cisco Finesse Notification Service properties:

utils finesse set_property notification <property name> value – This command enables you to set the server property to the specified value.

The Cisco Finesse Notification Service is a critical component of the Cisco Finesse application, and changes to its properties
                                          could affect its functionality and performance. Use this CLI only after ensuring that the changes are necessary and will not
                                          negatively impact your specific deployment.

It is NOT recommended to change any of these properties without consulting Technical Assistance Center (TAC). Some of these
                              properties are performance sensitive and adjusting the value of these properties might affect the load/functionality.

You cannot modify the following properties using the CLI:

aws.default.auth.list

aws.port

admin.authorizedJIDs

finesse.local.rmi.registry.port

httpbind.port.plain

httpbind.port.secure

rovider.auth.className
...
### Enabling Third Party Client Subscription to Agent Notification Feeds

The Cisco Finesse Notification Service has implemented restrictions on third-party client subscriptions to agent notification
                                 feeds, limiting access to the same agent only. These measures are designed to enhance security and scalability, as increased
                                 subscriptions can lead to higher server load.

Below are the updated Cisco Finesse Notification properties that enforce these restrictions on third-party client subscriptions
                                 to agent notification feeds:

finesse.applyrestrictions.usercreation – enables or disables user creation restrictions for third-party clients. The default value is true..

finesse.applyrestrictions.nodemanagement – enables or disables node creation and management restrictions for third-party clients. The default value is true.

finesse.applyrestrictions.nodesubscription – enables or disables node subscription restrictions for third-party clients. The default value is true.

finesse.applyrestrictions.chat – enables or disables restrictions on one-to-one chat. The default value is true.

If these restrictions are enabled, it is the customer's responsibility to ensure that server performance does not degrade
                                             and that subscriptions are appropriately cleaned up when users log out.

To view the status of a property, use the command: utils finesse show_property notification <property name> .

For example, utils finesse show_property notification finesse.applyrestrictions.usercreation .
...
While enabling toaster notification, use the closeTimeout parameter (timeout in seconds) to set the time interval after which toaster automatically closes. If no parameter is specified,
                                    timeout is set to 8 seconds by default. The valid range for timeout activity is between 5-60 seconds. The browser must be refreshed for timeout changes to take effect.
...
Use the following CLI commands to configure the below properties:

unreadNotificationsWaitTime : Unread notifications are those sent via the Notification Popover Service API with the timeout parameter set to 0. The unreadNotificationsWaitTime
                                    refers to the duration between the reception of a new notification on the Finesse desktop and the display of a generic message
                                    indicating the presence of unread messages in the notification center.

notificationPopupWaitTime : When a new notification is sent using the Notification Popover Service API without specifying the timeout parameter, the
                                    notificationPopupWaitTime refers to the duration for which the notification pop-up is displayed on the Finesse desktop.

These properties in the CLI relate to the behavior of notification pop-ups and their interaction with the Notification Popover
                              Service API. It's crucial to understand how timeout settings in the API affect the pop-up display.

utils finesse show_property desktop unreadNotificationsWaitTime : This command displays the notification wait time for unread messages. The default unread notification wait time is 20 seconds.
                                    This property defines the duration after which a generic pop-up appears in the notification center indicating unread messages.

utils finesse set_property desktop unreadNotificationsWaitTime 90 : This command configures the notification wait time for unread messages. In this case, the configured notification wait time
                                    is 90 seconds. This command sets the time after which the generic unread notification pop-up appears in the notification center.

utils finesse show_property desktop notificationPopupWaitTime : This command displays the notification pop-up wait time for new notification messages. The default notification pop-up wait
                                    time is 8 seconds. This property determines how long a notification pop-up is displayed when the timeout parameter is NOT
                                    specified in the Notification Popover Service API call.

utils finesse set_property desktop notificationPopupWaitTime 10 : This command configures the notification pop-up wait time for new notification messages. In this case, the notification
                                    pop-up wait time is 10 seconds. This command sets the duration for which a notification pop-up will be displayed.

For more details, refer to the Notification Popover Service section in the Cisco Finesse Web Services Developer and JavaScript Guide .

## Accessibility

The colors of graphical elements and text on the Finesse desktop appear brighter by default, in accordance with the accessibility
                              WCAG guidelines. If you want to revert to the previous color scheme, use the following commands:

utils finesse show_property enhanceContrast : This command checks if high contrast mode is enabled. The default value is true.

utils finesse set_property enhanceContrast false : This command disables high contrast mode, causing the text and graphical items on the Finesse desktop to display in lighter
                                    colors, similar to the previous color scheme.
...
The default-src directive is part of CSP header that specifies the default sources for all types of content if no other specific
                              directives are defined in the CSP policy.

Use the following CLIs to configure the sources for the default-src directive. This ensures that only trusted sources can
                              load resources on your page.

utils finesse csp default-src add [source1],[source2]—This command adds one or more sources that are allowed to load the content.

Example

```
admin:utils finesse csp default-src add https://cisco.com, https://cuic1.autobot.cvp:8445
Source(s) successfully added.
Ensure source(s) are added in all Finesse nodes in the cluster.

Restart Cisco Finesse Tomcat for the changes to take effect:
utils service restart Cisco Finesse Tomcat
```

utils finesse csp default-src list —This command displays the current list of sources that are allowed to load the content by default.

Example

```
admin:utils finesse csp default-src list
The following are the list of source(s) configured: 
1. https://cuic1.autobot.cvp:8445
2. https://cisco.com
```

utils finesse csp default-src delete —This command deletes one or more sources from which the content must not be loaded.

Example

```
admin:utils finesse csp default-src delete
1: https://cisco.com
a: all
q: quit
Select the index of source(s) to be deleted [1-1 or a, q]: 1
Source deleted successfully.
Ensure the source deleted from all Finesse nodes in the cluster.
Restart Cisco Finesse Tomcat for the changes to take effect: utils service restart Cisco Finesse Tomcat
```

utils finesse csp default-src enable_all —This command loads the content from all sources, which is useful for testing or troubleshooting purpose.

Example

```
admin:utils finesse csp default-src enable_all

The csp default-src directive updated to global or all sources successfully.
Ensure csp default-src directive set to enable all in all Finesse nodes in the cluster.
...
### Enable Trace Logging for Cisco Finesse Desktop Console Logs

The Cisco Finesse Desktop offers an option to enable trace-level logging in the browser's console. This feature helps capture
                              debug information about the desktop's behavior, such as the XMPP heartbeat messages sent to the Cisco Finesse Notification
                              Service every 10 seconds to maintain an active connection. These trace logs are particularly useful for diagnosing issues
                              like slowness or throttling within the browser.

The enableConsoleTraceLogging property controls this functionality. When set to true, it activates trace-level logging, including ping-pong messages, to
                              assist in identifying potential performance concerns. By default, this property is set to false.

Use the following CLI commands to manage the enableConsoleTraceLogging property:

To enable: utils finesse set_property desktop enableConsoleTraceLogging true

To disable: utils finesse set_property desktop enableConsoleTraceLogging false

To check if the enableConsoleTraceLogging property is enabled or disabled, run the command utils finesse show_property desktop enableConsoleTraceLogging .

### Manage Content Security Policy (CSP)

Cisco Finesse enforces a strict CSP across the desktop to ensure that all resources loaded from desktop comply with CSP guidelines.
                              By default, the strict CSP is enabled.

To disable the strict CSP Header, run the following CLI command:

utils finesse set_property desktop useStrictCSPHeader false
...
By default, Cisco Finesse restricts access to /finesse/api/User/<id>/Queues REST API details . Only the agent, the supervisor
                              of the agent’s team, or the administrator can access this information. To control access to the /finesse/api/User/<id>/Queues
                              REST API details of other agents, use the following CLI commands:

utils finesse set_property webservices allowAccessToOtherAgentsData true —This command enables access to the queue details of other agents.

utils finesse set_property webservices allowAccessToOtherAgentsData false —This command disables access to the queue details of other agents and allow the access only for valid supervisors or administrators.
...
| Note | The Cisco Finesse Notification Service is a critical component of the Cisco Finesse application, and changes to its properties
                                          could affect its functionality and performance. Use this CLI only after ensuring that the changes are necessary and will not
                                          negatively impact your specific deployment. |
|---|---|

| Note | If these restrictions are enabled, it is the customer's responsibility to ensure that server performance does not degrade
                                             and that subscriptions are appropriately cleaned up when users log out. |
|---|---|
...
| Note | This feature is available only if you upgrade to Cisco Finesse 15.0(1) SU1. For information on how to upgrade, see the Upgrade chapter of the Cisco Finesse Installation and Upgrade Guide, Release 15.0(1) at https://www.cisco.com/c/en/us/support/customer-collaboration/finesse/products-installation-guides-list.html . |
|---|---|
