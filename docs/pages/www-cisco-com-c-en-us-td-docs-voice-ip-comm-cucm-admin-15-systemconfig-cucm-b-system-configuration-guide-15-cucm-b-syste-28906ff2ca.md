---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-admin-15-systemconfig-cucm-b-system-configuration-guide-15-cucm-b-syste-28906ff2ca
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/admin/15/systemConfig/cucm_b_system-configuration-guide-15/cucm_b_system-configuration-guide-14_chapter_011011.html
retrieved_at: 2026-08-16T16:12:02.034570+00:00
---
...
Configure LDAP Directory Synchronization
...
When configuring the LDAP Directory on Unified Communications Manager Release 15SU2, passwords must consist exclusively of
                                                               ASCII characters and be limited to a maximum of 32 characters to ensure successful LDAP directory integration.

This limitation is addressed and resolved in Release 15SU3 and later versions.
...
| Step 3 | Create an LDAP Filter | Optional . Create an LDAP filter if you want Unified Communications Manager to synchronize only a subset of users from your corporate
                                          LDAP directory. |
| Step 4 | Configure LDAP Directory Synchronization | Configure settings for the LDAP directory sync such as field settings, LDAP server locations, synchronization schedules, and
                                          assignments for access control groups, feature group templates, and primary extensions. |
...
| Step 2 | Perform one of
                                          			 the following steps: Click Find and select an existing LDAP directory. Click Add
                                                				  New to create a new LDAP directory. |
| Step 3 | In the LDAP Directory Configuration window, enter the following: In the LDAP Configuration Name field, assign a unique name to the LDAP directory. In the LDAP Manager Distinguished Name field, enter a user ID with access to the LDAP directory server. Enter and confirm the password details. Note When configuring the LDAP Directory on Unified Communications Manager Release 15SU2, passwords must consist exclusively of
                                                               ASCII characters and be limited to a maximum of 32 characters to ensure successful LDAP directory integration. This limitation is addressed and resolved in Release 15SU3 and later versions. In the LDAP User Search Space field, enter the search space details. In the LDAP Custom Filter for Users Synchronize field, select either Users Only or Users and Groups . (Optional). If you want to limit the import to only a subset of users who meet a specific profile, from the LDAP Custom Filter for Groups drop-down list, select an LDAP filter. | Note | When configuring the LDAP Directory on Unified Communications Manager Release 15SU2, passwords must consist exclusively of
                                                               ASCII characters and be limited to a maximum of 32 characters to ensure successful LDAP directory integration. This limitation is addressed and resolved in Release 15SU3 and later versions. |
| Note | When configuring the LDAP Directory on Unified Communications Manager Release 15SU2, passwords must consist exclusively of
                                                               ASCII characters and be limited to a maximum of 32 characters to ensure successful LDAP directory integration. This limitation is addressed and resolved in Release 15SU3 and later versions. |
...
| Step 14 | If you want to use TLS to create a secure connection to the LDAP server, check the Use TLS check box. Note Sometimes, when we try to synchronize users through the secure port after restarting tomcat, the users will not be synchronized.
                                                      You must restart the Cisco DirSync service for the user synchronization to happen successfully. | Note | Sometimes, when we try to synchronize users through the secure port after restarting tomcat, the users will not be synchronized.
                                                      You must restart the Cisco DirSync service for the user synchronization to happen successfully. |
| Note | Sometimes, when we try to synchronize users through the secure port after restarting tomcat, the users will not be synchronized.
                                                      You must restart the Cisco DirSync service for the user synchronization to happen successfully. |
| Step 15 | Click Save . |
...
| Step 4 | In the Confirm Password field, enter the password for the LDAP manager. Note Ensure that you renter the LDAP password when you are upgrading your Unified Communications Manager from Release 11.5(1)SU2
                                                      to Release 14SU3 and above. Note When configuring the LDAP Directory on Unified Communications Manager Release 15SU2, passwords must consist exclusively of
                                                         ASCII characters and be limited to a maximum of 32 characters to ensure successful LDAP directory integration. This limitation is addressed and resolved in Release 15SU3 and later versions. | Note | Ensure that you renter the LDAP password when you are upgrading your Unified Communications Manager from Release 11.5(1)SU2
                                                      to Release 14SU3 and above. | Note | When configuring the LDAP Directory on Unified Communications Manager Release 15SU2, passwords must consist exclusively of
                                                         ASCII characters and be limited to a maximum of 32 characters to ensure successful LDAP directory integration. This limitation is addressed and resolved in Release 15SU3 and later versions. |
