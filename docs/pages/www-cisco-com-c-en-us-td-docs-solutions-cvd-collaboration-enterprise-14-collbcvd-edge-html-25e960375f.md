---
doc_id: www-cisco-com-c-en-us-td-docs-solutions-cvd-collaboration-enterprise-14-collbcvd-edge-html-25e960375f
source_url: https://www.cisco.com/c/en/us/td/docs/solutions/CVD/Collaboration/enterprise/14/collbcvd/edge.html
retrieved_at: 2026-08-16T18:23:10.611960+00:00
---
...
- Through an IP trunk to a Telecom carrier, usually for voice-only services. This connectivity is provided by the Cisco Unified Border Element (CUBE) on a Cisco Integrated Services Router (ISR), a Cisco Aggregation Services Router (ASR) or a Catalyst Edge platform. Cisco Unified Border Element should be deployed in a central site where the Telecom carrier's network communicates with the enterprise network.
...
As shown in Figure 4-2 , the Cisco Expressway solution encompasses two main components: the Expressway-E peer and the Expressway-C peer. These two components work in combination with Cisco Unified Communications Manager (Unified CM) to enable secure mobile and remote access. The Expressway-E peer provides the secure edge interface to mobile and remote devices.

Expressway-C creates a secure connection with the Expressway-E peer. The Expressway-C peer provides proxy registration to Unified CM for remote secure endpoint registration. The Expressway-C peer includes a back-to-back user agent (B2BUA), which provides media termination capabilities.
...
Prior Cisco architectures included using the Cisco Adaptive Security Appliance (ASA) firewall and allowing inbound ports to be opened through the external firewall to directly access the internal IM and Presence servers. This is still the recommended solution for SIP federation.
...
2. Expressway-C initiates traversal connections outbound through the firewall to specific ports on Expressway-E, with secure login credentials. If the firewall allows outbound connections, as it does in the vast majority of cases, no additional ports are required to be opened in the enterprise firewall. For ports details, refer to the latest version of the document on Cisco Expressway IP Port Usage Configuration Guide .
...
4. When Expressway-E receives a call or other collaboration service request, it issues an incoming request to Expressway-C reusing the same connection that has been created by the Expressway-C outbound to Expressway-E.
...
When an endpoint on the Internet connects to Unified CM or other collaboration application through Expressway, its IP address is first translated to a public IP address by the local customer premises equipment (CPE). On Expressway-E, the endpoint source IP address is replaced by the address of the internal IP LAN interface of Expressway-E. When the packet enters Expressway-C, Expressway-C replaces the source IP address of the packet with its own IP address before forwarding the packet to the collaboration service applications.
...
Figure 4-7 NAT in the Life of a Packet
...
When a Jabber or hardware endpoint user logs in, they specify the discovery domain, user ID and password. Jabber concatenates the User ID and the discovery domain in a fully qualified name (for example, user1@ent-pa.com). The client queries the public DNS server for specific SRV records:
...
As shown in Figure 4-9 , while a forward proxy server provides services information for on-premises clients by hiding client details when connecting to Internet servers, a reverse proxy server provides information for off-premises clients by hiding on-premises server information.
...
Business-to-business communications require the ability to look up the domains of remote organizations for the purpose of URI routing. This is done by creating a DNS zone on Expressway-E. Though security best practice suggst that H.323 should be turned off, as it is an inherently insecure protocol, in some cases this might not be possible for business reasons. In the DNS zone both SIP and H.323 can independently be turned on. This allows Expressway-E to automatically re-initiate a DNS query using the other protocol not used by the initiating call, thereby giving the call the best chance of success. Expressway-C and Expressway-E use the protocol that was used to initiate the call, and they automatically try the other protocol when SIP-to-H.323 interworking is enabled on the Expressway.

If H.323 is enabled on the Expressway, SIP-to-H.323 interworking should be set to On for Expressway-E. If a call is received as an H.323 call, this allows Expressway-E to interwork the call to SIP and use native SIP for the rest of the call legs to Unified CM. Likewise, an outbound call to an H.323 system will remain a SIP call until it reaches Expressway-E, where it will be interworked to H.323.
...
We recommend deploying Expressway-C and Expressway-E in clusters. Each cluster can have up to six Expressway peers and a maximum of N+2 physical redundancy. All peers are active in the cluster. For details about cluster configuration, refer to the latest version of the Cisco Expressway Cluster Creation and Maintenance Deployment Guide .

Expressway clusters provide configuration redundancy.The first peer configured in the cluster is the publisher primary peer or publisher , while all other peers are peers or subscribers . Configuration is done in the primary peer and automatically replicated to the other peers.

Expressway clusters provide call license sharing and resilience. All rich media sessions are shared equally across peers in the cluster. Call licenses are contributed by the licenses configured on each peer.

The following rules apply to Expressway clustering:

- Expressway-C and Expressway-E types cannot be mixed in the same cluster.

- All peers in a cluster must have identical configurations for zones, authentication, and call policy.

- Configuration changes should be made only on the primary peer, and this will overwrite the configuration on the other peers in the cluster when replication occurs.

- If a peer becomes unavailable, the licenses it contributed to the cluster will become unavailable after 2 weeks.

- Deploy an equal number of peers in Expressway-C and Expressway-E clusters.

- Deploy the same OVA template throughout the cluster.

- All peers in a cluster need to be within 80 ms maximum round-trip time to all other cluster peers. This means that each Expressway in the cluster must be within a 40ms hop of all other peers in the cluster.

- H.323 must be enabled on all peers in a cluster for database replication. At the same time, if you also want to block H.323 calls coming from the Internet, you can configure Expressway-E with firewall rules to drop H.323 traffic on the external LAN interface.

- If mobile and remote access and business-to-business communications are enabled on the same Expressway-C and Expressway-E pairs, the SIP port number used on the SIP trunk between Unified CM and Expressway-C used for B2B needs to be changed from the default 5060 or 5061.

- A DNS SRV record must be available for the cluster and must contain A records for each peer of the cluster.

Since Expressway-C is deployed in the internal network and Expressway-E in the DMZ, Expressway-C has to be connected to Expressway-E through a Unified Communications traversal zone for mobile and remote access. Business-to-business calls require a separate traversal zone, which retains the name of traversal client zone for Expressway-C and traversal server zone for Expressway-E. The traversal server, traversal client, and Unified Communications traversal zones include all the peers of Expressway-C and Expressway-E, so that if one of the peers is not reachable, another peer of the cluster will be reached instead.
...
The traversal client zone configured on Expressway-C should contain the fully qualified domain names of all of the cluster peers of the corresponding Expressway-E cluster. Likewise, the traversal server zone on Expressway-E should connect to all Expressway-C cluster peers. This is achieved by including in the subject alternative names of the Expressway-C certificate the FQDNs of the Expressway-C cluster peers and by setting the TLS verify subject name on Expressway-E equal to the FQDN of the Expressway-C cluster. This creates a mesh configuration of cluster peers across the traversal zone and provides continuous and high availability of the traversal zone until the last cluster peer is unavailable.

Expressway-C connects to Unified CM via trunks for routing inbound and outbound business-to-business calls. Unified CM also trunks to Expressway-C. For high availability, the fully qualified domain names of each Expressway-C cluster peer should be listed in the trunk configuration on Unified CM. Conversely, the fully qualified domain name (FQDN) of each member of the Unified CM cluster should be listed in the neighbor zone profile of Expressway-C.
...
DNS SRV records can add to availability of Expressway-E for inbound business-to-business traffic. For high availability all peers in the cluster should be listed with the same priority and same weight in the SRV record. This allows all peers to be returned in the DNS query. A DNS SRV record helps to minimize the time spent by a client on lookups since a DNS response can contain all of the peers listed in the SRV record. The far-end server or far-end endpoint will typically cache the DNS response and will try all peers returned in the DNS query until a response is received. This provides the best chance for a successful call.

In addition, Expressway clusters support rich media license sharing across clusters. If a peer is lost from the cluster, its call licenses will continue to be shared for the next 2 weeks. Any one Expressway cannot process any more rich media licenses than its physical capacity, even though it can carry more licenses than its physical capacity.
...
The connection between Unified CM and Expressway-C may be encrypted and authenticated, depending on the configuration. We recommend end-to-end encryption of media and signaling in all of the call signaling and media legs between the source and the destination. This might require mixed-mode on Unified CM if SIP OAuth is not an option. SIP OAuth is the recommended encryption and authentication method but it is endpoint-dependent. See the Security chapter for more information.
...
CPL are very useful in blocking spam calls and toll fraud. For this reason, due to an increase of these types of fraudulent attempts, we recommend to carefully evaluate the CPL configuration. The rules should be set so that only specific destinations are allowed, while everything else is discarded.

CPL should be used in combination with the Calling Search Space on Unified CM, to prevent unauthenticated calls access from the PSTN by mistake. Check the Call Control chapter for further details.

As an example, consider a company where calls are allowed to a set of devices in the range of +1 408 555 44XX.

Clearly, the administrator has to give access to only those numbers and not to a wider range. Also, if any call enters the Expressway with the corporate domain or Expressway corporate IP in the calling ID, this might indicate that it is a spam call.

The administrator might allow alphanumeric URI dialing. In this case it is also advisable that the smallest range of SIP URIs are allowed, based on the corporate policy. For example a good regular expression to identify those SIP URIs where the right-hand-side might not contain more than 8 characters, cannot start with a digit, cannot contain digits but might end with a digit, could be the following:

^[a-z]{2,7}\w@ent-pa\.com

This is very different from the more common rule:.* @ent-pa\.com, because in the first case only a portion of possible patterns is allowed, where the latter allows all SIP URIs if they contain the corporate domain.

So the administrator allows access to the numeric range or the SIP URIs only if the calling ID does not include Expressway-E IP or the corporate domain, as in this example in Table 4-5 .

Table 4-5 Example of a Deny-Based Policy

From Address:.*ent\-pa\.com.*

.*

Reject

From Address:.*@192\.0\.2\.10
...
Any other destination that is not included in the exposed portion of the internal dial plan will be rejected by the last rule in Table 4-5 ("deny all").

The CPL works together with the Intrusion Protection Rule called “SIP authentication failure”. It is recommended that this rule is turned on, because by default after 10 minutes (detection window) and 5 wrong attempts (trigger level) identified by a rejected CPL the source IP originating the spam call will be automatically jailed by the Expressway firewall for 10 minutes (block duration). The administrator might decide to configure different values for the detection window, trigger level and block duration.
...
If a registration request is made from a client belonging to a Unified CM and IM an Presence cluster that has not been discovered by Expressway-C, that client will not be able to log in. This is the reason why it is important for each Expressway-C to discover all Unified CM and IM and Presence clusters if users are enabled for mobility, as shown in Figure 4-18 . Very large networks might require user segregation in order to avoid every Expressway-C having to discover all internal Unified CM and IM&P servers. This exception is not covered in this document.
...
DNS SRV records are used to determine which Expressway-E clusters are authorized for the SIP and H.323 ent-pa.com domain. SRV records with the same weight and priority are used to balance calls across Expressway-E cluster peers.
...
An alternative solution for B2B calls is designed to return the edge that is closest to the destination endpoint or device. This requires finding or knowing where the destination endpoint is located and then returning the appropriate edge. The benefit of this solution is to minimize the use of bandwidth on the customer network by delivering the shortest internal path to the endpoint.
...
|---|---|---|
| From Address:.*ent\-pa\.com.* | .* | Reject |
| From Address:.*@192\.0\.2\.10 | .* | Reject |
| Zone: Default Zone | \+14085554\d\d\d@ent-pa\.com | Allow |
| Zone: Default Zone | ^[a-z]{2,7}\w@ent-pa\.com | Allow |
