---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-connection-15-install-upgrade-guide-b-15cuciumg-b-15cuciumg-chapter-010-html-d0a7475519
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/install_upgrade/guide/b_15cuciumg/b_15cuciumg_chapter_010.html
retrieved_at: 2026-08-16T18:48:26.681244+00:00
---
...
For 11.5 to 15, you must follow an intermediate upgrade path. Example: 11.5 to 12.5 or later and then 12.5 or later to 15.

If the operating system version of the Unity Connection changes during an upgrade, it is referred to as a Refresh Upgrade
                                             (RU).

- You need the following COP file before performing from 11.5 or later to 15 : ciscocm.CSCwi52160_15-direct-migration_v1.0.k4.cop.sha512
                                                         . 1

Refresh Upgrades from Pre-12.5.x source to Release 15 is not supported.

A fresh installation with data import is an alternative to direct upgrades from releases 11.5 and above to release 15. For
                                                         more information, see Install with Data Import section.

Level 2 (L2)

12.5.1SU3 or earlier to 15
...
- ciscocm.CSCwi52160_15-direct-migration_v1.0.k4.cop.sha512 1
...
12.5.1SU4 or later to 15

14 or later to 15

You need the following COP file before performing this upgrade: ciscocm.CSCwi52160_15-direct-migration_v1.0.k4.cop.sha512 1
...
If you are upgrading Unity Connection to 15 and later, then after completion of successful upgrade, you must reinstall the
                                       set of available locales that are compatible with the upgraded version. To install locale, refer https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/install_upgrade/guide/b_15cuciumg/b_15cuciumg_chapter_0100.html .
...
After successful upgrade to Unity Connection 15, if you need to revert the server to previous software version, you can switch
                                       version the software to older version. After that, you can not upgrade the server to any pre 15 release (for example: 11.5(1)
                                       or 12.0(1)). In addition to this, if the upgrade from any previous releases to Unity Connection 15 fails for any reason, then
                                       also you cannot upgrade the server to pre 15 release. To troubleshoot the issue, contact Cisco TAC.

If administrator wants to upgrade the server to pre 15 release in above scenarios, fresh cluster rebuild is required by performing
                                       DRS backup and restore before upgrade.

If you are upgrading Unity Connection from 11.5(1) or 12.0(1) as base release to 15 and later, then you must rename custom
...
Before upgrading Unity Connection to Release 15, make sure to set Latency Sensitivity to High on the virtual machines for
                                 both the publisher and subscriber servers through VMware. For details on how to configure Latency Sensitivity, refer to the CUC 15 OVA readme
...
If you are performing upgrade with FIPS enabled Unity Connection Release to 15 and later, you must consider the below limitations
...
In Unity Connection Release 15, the IPsec policies with DH group key values 1, 2 or 5 are disabled. If you are upgrading Unity
                                 Connection to Release 15 with FIPS enabled and IPse configured, then you must perform any one of the given procedure for successful
                                 upgrade to Unity Connection 15
...
In FIPS mode, if you have configured Unified Messaging with NTLM web authentication mode then you must select a Basic authentication
                                 mode before upgrading Unity Connection to 15 and later. NTLM web authentication mode is no longer supported.

If you are upgrading from any release of Unity Connection lower than 12.5 SU6 and 14 SU2 in FIPS mode to Unity Connection
                                 15 and later, make sure to install COP File ciscocm.ciscossl7_upgrade_CSCwa48315_CSCwa77974_v1.0.k4.cop.sha512 on both nodes of cluster before upgrade.

For more information on FIPS mode, see " FIPS Compliance in Cisco Unity Connection " chapter of Security Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/security/guide/b_15cucsecx.html .
...
If you are already running the current version on a virtual server, make sure it is compatible with the upgraded version.
                                 See the Cisco Unity Connection 15 Supported Platform List at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/supported_platforms/b_15cucspl.html .
...
- Migrate all the licenses (legacy and PLM based) before you upgrade to Unity Connection 15 server. For more information, see
...
After successful upgrade to Unity Connection 15, the product remains in Evaluation Mode until you register the product with
                                 CSSM or satellite.

If you are performing an upgrade from a FIPS enabled Unity Connection Release to Unity Connection 15, make sure to follow
                                 ...
                                 see the Regenerating Certificates for FIPS section of the "FIPS Compliance in Cisco Unity Connection" chapter in Security Guide for Cisco Unity Connection Release 15, available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/security/guide/b_15cucsecx.html .
...
Cisco Unity Connection supports HAProxy which frontends all the incoming web traffic into Unity Connection offloading Tomcat.
                                 ...
                                 upgrade, see chapter IP Communications Required by Cisco Unity Connection in Security Guide for Cisco Unity Connection Release 15, available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/security/guide/b_15cucsecx.html .
...
If Next Generation Security over HTTPS interface is configured on the system then after successful upgrade to Unity Connection
                                 15, the configured settings of HTTPS ciphers get reset. You must reconfigure the HTTPS ciphers on Enterprise Parameter page
...
If Specific License Reservation(SLR) mode is enabled on the system, then after successful upgrade to Unity Connection Release
                                 15, you must return all reserved licenses to Cisco Smart Software Manager(CSSM) and reconfigure SLR with new version licenses.
                                 For more information on configuration of Specific License Reservation in Unity Connection, see the Configuring Specific License Reservation in Unity Connection section of the "Managing Licenses" chapter in Install, Upgrade, and Maintenance Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/install_upgrade/guide/b_15cuciumg.html .
...
Cisco Unity Connection supports SAML-based Single Logout (SLO). The SLO allows you to log out simultaneously from all sessions
                                 ...
                                 Release 15 you must follow steps mentioned in section SAML-Based Single Logout (SLO) of Quick Start Guide for SAML SSO Access available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/quick_start/guide/b_15cucqssamlsso.html .
...
If you are creating a new Intrasite link or if there is any existing Intrasite link between two nodes of Unity Connection
                                 in FIPS mode with one node on 15 release and other node on any release lower than 15, then only message delivery between two
                                 nodes will work. Object(users, system distribution lists if applicable, partitions, search spaces and Unity Connection locations)
                                 synchronization is not supported. For object synchronization to work, you must upgrade all the Unity Connection nodes in network
                                 to 15 release.

After successful upgrade to Unity Connection 15, if you need to perform rollback of server from 15 to any older release then
                                 you must re-register the product with CSSM or satellite using a registration token for successful functioning of Smart Licensing
                                 as applicable to the release.

If Secure SMTP is enabled on the system, then after successful upgrade to Unity Connection Release 15 you must reconfigure
...
CUNI Subscriptions will be removed from Cisco Unity Connection server database, if you perform a refresh upgrade to Unity
                                 Connection 15. Make sure to perform re-subscription after successful upgrade of the cluster.

If you are performing upgrade to Unity Connection 15 from any of the older release, make sure to reconfigure permissions on
                                 Azure Portal after sucessful upgrade. To learn how to reconfigure the permissions, see Step4g of the section "Task List for Configuring Unified Messaging with Office 365" of the chapter "Configuring Unified Messaging"
                                 of the Unified Messaging Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/unified_messaging/guide/b_15cucumgx.html .

SpeechView with Cisco Webex in-house transcription service is the only available option after upgrade to 15 SU2 or later in
                                 order to use SpeechView feature.
...
In order to use SpeechView feature with Cisco Webex in-house transcription service in Digital Networking, make sure to upgrade
                                 all nodes in the network to release 15 SU2 or later.

If Unity Connection Cluster has been onboarded on Cisco Webex Cloud-Connected UC before upgrade to 15 SU2, then after the
...
After upgrading to Cisco Unity Connection (CUC) Release 15SU4, the Microsoft Graph API is the only supported option for Unified
                                 Messaging with Office 365. For details, refer to the Task List for Configuring Unified Messaging with Office 365 (Graph API) .

Before upgrading, you must update the Azure portal permissions for the Graph API as documented. No changes to UMS settings
                                             are required on the Unity Connection side, as existing configurations will be carried over automatically during the upgrade.
...
- All messages are preserved but in Refresh Upgrades, if a rollback is performed then any messages left for existing users on
                              the new version will be lost. While in Level 2 Upgrades, any messages left for existing users on the new version will be retained,
                              even if a rollback is done.
...
|---|---|---|
| Refresh Upgrade (RU) | For 11.5 to 15, you must follow an intermediate upgrade path. Example: 11.5 to 12.5 or later and then 12.5 or later to 15. | If the operating system version of the Unity Connection changes during an upgrade, it is referred to as a Refresh Upgrade
                                             (RU). Note You need the following COP file before performing from 11.5 or later to 15 : ciscocm.CSCwi52160_15-direct-migration_v1.0.k4.cop.sha512
                                                         . 1 Refresh Upgrades from Pre-12.5.x source to Release 15 is not supported. A fresh installation with data import is an alternative to direct upgrades from releases 11.5 and above to release 15. For
                                                         more information, see Install with Data Import section. | Note | You need the following COP file before performing from 11.5 or later to 15 : ciscocm.CSCwi52160_15-direct-migration_v1.0.k4.cop.sha512
                                                         . 1 Refresh Upgrades from Pre-12.5.x source to Release 15 is not supported. A fresh installation with data import is an alternative to direct upgrades from releases 11.5 and above to release 15. For
                                                         more information, see Install with Data Import section. |
| Note | You need the following COP file before performing from 11.5 or later to 15 : ciscocm.CSCwi52160_15-direct-migration_v1.0.k4.cop.sha512
                                                         . 1 Refresh Upgrades from Pre-12.5.x source to Release 15 is not supported. A fresh installation with data import is an alternative to direct upgrades from releases 11.5 and above to release 15. For
                                                         more information, see Install with Data Import section. |
| Level 2 (L2) | 12.5.1SU3 or earlier to 15 | If the operating system version of the Unity Connection do not change during an upgrade, it is referred to as an Level 2 (L2)
                                             upgrade. You need the following COP file before performing this upgrade: ciscocm.CSCwi52160_15-direct-migration_v1.0.k4.cop.sha512 1 ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn The new version is installed on the inactive partition to which you can switch later on. |
| 12.5.1SU4 or later to 15 14 or later to 15 | You need the following COP file before performing this upgrade: ciscocm.CSCwi52160_15-direct-migration_v1.0.k4.cop.sha512 1 |
...
| Note | If you are upgrading Unity Connection to 15 and later, then after completion of successful upgrade, you must reinstall the
...
| Caution | After successful upgrade to Unity Connection 15, if you need to revert the server to previous software version, you can switch
                                       version the software to older version. After that, you can not upgrade the server to any pre 15 release (for example: 11.5(1)
                                       or 12.0(1)). In addition to this, if the upgrade from any previous releases to Unity Connection 15 fails for any reason, then
                                       also you cannot upgrade the server to pre 15 release. To troubleshoot the issue, contact Cisco TAC. If administrator wants to upgrade the server to pre 15 release in above scenarios, fresh cluster rebuild is required by performing
                                       DRS backup and restore before upgrade. If you are upgrading Unity Connection from 11.5(1) or 12.0(1) as base release to 15 and later, then you must rename custom
                                       role "Read Only Administrator" to different name on base release before upgrade. |
|---|---|
...
| Caution | (Applicable to 12.5SU1, 12.5SU2, 12.5SU3 releases only) For upgrading Unity Connection to release 15, Pre and Post Upgrade COP files should be installed via CLI only. |
|---|---|

| Note | To use SpeechView with Cisco Webex in-house transcription service after upgrade to 15 SU2 or later, refer "SpeechView Cisco
                                          Webex in-house transcription service" chapter available at System Administration Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/administration/guide/b_15cucsag.html . |
|---|---|

| Note | Before upgrading, you must update the Azure portal permissions for the Graph API as documented. No changes to UMS settings
                                             are required on the Unity Connection side, as existing configurations will be carried over automatically during the upgrade. |
|---|---|
...
| Caution | If you revert to the version on the inactive partition in case of RU upgrade rollback from 15 to 14,12.x or 11.x or 10.x versions,
