---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-12-6-2-configurati-6df3ab8759
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_2/configuration/guide/ucce_b_features-guide-1262/ucce_b_features-guide-1261_chapter_01100.html
retrieved_at: 2026-08-16T20:11:10.539484+00:00
---
...
The Enable single sign-on checkbox is enabled only when the global SSO Enabled setting is Hybrid . You cannot change the SSO state of a person who meets either of the following conditions:

The person record is linked to agent records on multiple peripherals. Remove the agent records from all but one peripheral
                                                      before changing the SSO state of this person.

The person is a supervisor. Remove the supervisor status before changing the SSO state of this person.
...
As there's no automatic association of IDS in UCCE, UCCE deployment is manual from CCE Administration for each VM.  Navigate
                                       to CCE Administration > Overview > Inventory .

Step 2

Click the pencil icon for the External HDS for 2000 Agents deployment.
...
If you're working with a Cisco IdS cluster, perform these steps on the Cisco IdS primary publisher node.

For CCE 4000, 12000, and 24000 Agents deployment, ensure that the Principal AW is configured and functional before using the
                                                      Single Sign-On tool in UCCE Administration.

Also, add the SSO-capable machines to the Inventory, and select the default Cisco IdS for each of the SSO-capable machines.
...
| Note | The Enable single sign-on checkbox is enabled only when the global SSO Enabled setting is Hybrid . You cannot change the SSO state of a person who meets either of the following conditions: The person record is linked to agent records on multiple peripherals. Remove the agent records from all but one peripheral
...
| Step 1 | As there's no automatic association of IDS in UCCE, UCCE deployment is manual from CCE Administration for each VM.  Navigate
                                       to CCE Administration > Overview > Inventory . |
|---|---|
...
| Step 5 | Click Save . Note If you're working with a Cisco IdS cluster, perform these steps on the Cisco IdS primary publisher node. For CCE 4000, 12000, and 24000 Agents deployment, ensure that the Principal AW is configured and functional before using the
                                                      Single Sign-On tool in UCCE Administration. Also, add the SSO-capable machines to the Inventory, and select the default Cisco IdS for each of the SSO-capable machines. | Note | If you're working with a Cisco IdS cluster, perform these steps on the Cisco IdS primary publisher node. For CCE 4000, 12000, and 24000 Agents deployment, ensure that the Principal AW is configured and functional before using the
                                                      Single Sign-On tool in UCCE Administration. Also, add the SSO-capable machines to the Inventory, and select the default Cisco IdS for each of the SSO-capable machines. |
| Note | If you're working with a Cisco IdS cluster, perform these steps on the Cisco IdS primary publisher node. For CCE 4000, 12000, and 24000 Agents deployment, ensure that the Principal AW is configured and functional before using the
                                                      Single Sign-On tool in UCCE Administration. Also, add the SSO-capable machines to the Inventory, and select the default Cisco IdS for each of the SSO-capable machines. |
