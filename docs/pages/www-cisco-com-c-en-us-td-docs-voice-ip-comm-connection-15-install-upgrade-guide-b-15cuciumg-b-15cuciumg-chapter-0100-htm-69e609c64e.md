---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-connection-15-install-upgrade-guide-b-15cuciumg-b-15cuciumg-chapter-0100-htm-69e609c64e
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/install_upgrade/guide/b_15cuciumg/b_15cuciumg_chapter_0100.html
retrieved_at: 2026-08-17T02:13:01.833376+00:00
---
...
If third-party certificates are deployed in Unity Connection then after successfully replacing the subscriber server, you
                                                               must reconfigure third party certificates for the newly build subscriber server. For information on how to configure the certificates,
                                                               see Security chapter of Cisco Unified Communications Operating System Administration Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/os_administration/guide/b_15cucosagx.html .

(Applicable to 15 SU2 or later) If you are using SpeechView with Cisco Webex in-house transcription service, onboard the Subscriber Server of the Unity Connection
...
### Important Considerations and Prerequisites before Changing the Hostname or IP Address of a Unity Connection Server

When you change the IP address or hostname of the Unity Connection server, make sure to apply the same changes on all the
                                    associated components that refer the Unity Connection server by IP address or hostname.
...
You can change the IP address and hostname of a Unity Connection server or cluster following the steps mentioned in the Changing the IP Address or Hostname of a Unity Connection Server or Cluster section.
...
Ensure that the cluster status corresponds accurately with the assigned IP address by running the show cuc cluster status CLI command. .

Backup the database using Disaster Recovery System. See the Backing Up and Restoring Cisco Unity Connection Components chapter.

On a DNS server, change the DNS record of the Unity Connection server to the new IP address. Update both the forward (A) and
                                    reverse (PTR) records.

Check the network connectivity and DNS server configuration by running the utils diagnose module validate network CLI command.

Check the RTMT for critical alerts:

Sign in to the standalone server or the publisher server using Real-Time Monitoring Tool .

Expand Tools> Alert and select Alert Central .

In the Systems tab, make sure the ServerDown is black. If ServerDown is red, then resolve all the problems and change it to black.
...
On the Cluster Management page, check whether server status is Primary or Secondary. If there is any other status value then
                                          resolve the problem.
...
Change DHCP/DNS settings from Cisco Unified Operating System Administration> Settings and select the applicable option from
                                          IP, and restart Unity Connection by running the CLI command utils system restart .
...
It is recommended to change the IP address or hostname using CLI.

## Change IP Address or Hostname through CLI

Perform the prerequisites on your deployment before changing IP address or hostname through CLI. In case of cluster prerequisites should be done on
                           both nodes.

About this task : You can use below CLI to change the IP address or hostname for publisher and subscriber nodes that are defined by a hostname
                           in your deployment. Each step in this procedure applies to both publisher and subscriber nodes.

Log in to the CLI of the node you want to change.

Enter set network hostname . This command allows the administrator to set the network host name, change the IP address of the node, and restart the system.
...
## Change IP Address Only using CLI

Log into the CLI of the node that you want to change

Enter set network ip eth0 new-ip_address new_netmask new_gateway to change the IP address of the node. This command sets the IP address for Ethernet interface 0. You cannot configure Ethernet
                                    interface 1.

For more information on CLI usage see, Command Line Interface Reference Guide for Cisco Unified Communications Solutions, Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/cli_ref/15/cucm_b_cli_reference_guide_release_15/cucm_b_cli_reference_guide_release_1401_chapter_0110.html#CUCM_CL_SEB8A06A_00 .

### Before you begin

Perform the prerequisites on your deployment before changing IP address through CLI.

## Change IP Address or Hostname through GUI
...
- On the standalone or publisher server, change the IP address, Hostname, Subnet Mask and default Gateway (if applicable):

- If you change the IP address or Hostname of a standalone server, skip to Step 4. (Applicable only when you change the IP address
                              or Hostname of a publisher server in case of a cluster) On the subscriber server, change the IP address of the publisher server:

- Sign in to Real-Time Monitoring Tool and confirm that the server is available and running. This completes the process of changing the IP address of the standalone
                              server.

- For the cluster, repeat Step 1 to Step 4 on subscriber server also.This completes the process of changing the IP address of
                              a cluster.
...
In the Port Information, enter the value of IP.
...
| Step 5 | Check the cluster status by running the show cuc cluster status
                                          			 CLI command on the subscriber server. Note If third-party certificates are deployed in Unity Connection then after successfully replacing the subscriber server, you
                                                               must reconfigure third party certificates for the newly build subscriber server. For information on how to configure the certificates,
                                                               see Security chapter of Cisco Unified Communications Operating System Administration Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/os_administration/guide/b_15cucosagx.html . (Applicable to 15 SU2 or later) If you are using SpeechView with Cisco Webex in-house transcription service, onboard the Subscriber Server of the Unity Connection
                                                               Cluster on Cisco Webex Cloud-Connected UC. For more details on Onboarding Unity Connection server on Cisco Webex Cloud-Connected
                                                               UC, refer https://help.webex.com/en-us/article/nzt6c0b/Set-up-Webex-Cloud-Connected-UC-for-on-premises-devices . | Note | If third-party certificates are deployed in Unity Connection then after successfully replacing the subscriber server, you
                                                               must reconfigure third party certificates for the newly build subscriber server. For information on how to configure the certificates,
                                                               see Security chapter of Cisco Unified Communications Operating System Administration Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/os_administration/guide/b_15cucosagx.html . (Applicable to 15 SU2 or later) If you are using SpeechView with Cisco Webex in-house transcription service, onboard the Subscriber Server of the Unity Connection
                                                               Cluster on Cisco Webex Cloud-Connected UC. For more details on Onboarding Unity Connection server on Cisco Webex Cloud-Connected
                                                               UC, refer https://help.webex.com/en-us/article/nzt6c0b/Set-up-Webex-Cloud-Connected-UC-for-on-premises-devices . |
| Note | If third-party certificates are deployed in Unity Connection then after successfully replacing the subscriber server, you
                                                               must reconfigure third party certificates for the newly build subscriber server. For information on how to configure the certificates,
                                                               see Security chapter of Cisco Unified Communications Operating System Administration Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/os_administration/guide/b_15cucosagx.html . (Applicable to 15 SU2 or later) If you are using SpeechView with Cisco Webex in-house transcription service, onboard the Subscriber Server of the Unity Connection
                                                               Cluster on Cisco Webex Cloud-Connected UC. For more details on Onboarding Unity Connection server on Cisco Webex Cloud-Connected
                                                               UC, refer https://help.webex.com/en-us/article/nzt6c0b/Set-up-Webex-Cloud-Connected-UC-for-on-premises-devices . |
...
| Note | It is recommended to change the IP address or hostname using CLI. |
|---|---|

| Note | For more information on CLI usage see, Command Line Interface Reference Guide for Cisco Unified Communications Solutions, Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/cli_ref/15/cucm_b_cli_reference_guide_release_15/cucm_b_cli_reference_guide_release_1401_chapter_0110.html#CUCM_CL_SEB8A06A_00 . |
|---|---|

| Step 1 | (Applicable only when you change the IP address or Hostname of a standalone server or a cluster defined by an IP address or
                                       Hostname). Changing the IP addresses or Hostname of a standalone server or the publisher server in Connection Administration: Sign in to Cisco Unity Connection Administration . Note In case of a cluster, you must sign in to the publisher sever and select subscriber server to change the IP address or Hostname
                                                            of a subscriber server. Expand System Settings , and select Cluster . Select Find to display a list of servers in the cluster. Select the name of the standalone server or publisher server. Change the value of the Hostname/IP Address field to the new Hostname/IP address. Select Save . | Note | In case of a cluster, you must sign in to the publisher sever and select subscriber server to change the IP address or Hostname
                                                            of a subscriber server. |
|---|---|---|---|
| Note | In case of a cluster, you must sign in to the publisher sever and select subscriber server to change the IP address or Hostname
                                                            of a subscriber server. |
| Step 2 | On the standalone or publisher server, change the IP address, Hostname, Subnet Mask and default Gateway (if applicable): Sign in to Cisco Unified Operating System Administration . From the Settings menu, select IP > Ethernet . In the Host Information, enter the value of Hostname. Select Save . In the Port Information, enter the value of IP. Select Save . If you want an alternate hostname for the server, run the set web-security CLI command. In the Hostname, change the hostname of the server. For more information on the CLI commands, see the applicable version of the Command Line Interface Reference Guide for Cisco Unified Communications Solutions at http://www.cisco.com/en/US/products/ps6509/prod_maintenance_guides_list.html . Note Enter a certificate signing request. Then download the certificate signing request to the server on which you installed Microsoft
                                                            Certificate Services or another application that issues certificates, or download the request to a server that you can use
...
| Caution | After saving the page, node services will restart automatically. Restarting services ensures the proper update for the changes
                                                      to take effect. Do not perform any action on the server until the services are up and running. To check the status of the
                                                      services, run utils service list CLI command. |
| Step 3 | If you change the IP address or Hostname of a standalone server, skip to Step 4. (Applicable only when you change the IP address
                                       or Hostname of a publisher server in case of a cluster) On the subscriber server, change the IP address of the publisher server: Sign in to Cisco Unified Operating System Administration . From the Settings menu, select IP > Publisher. Change the IP address of the publisher server. Select Save . |
| Step 4 | Sign in to Real-Time Monitoring Tool and confirm that the server is available and running. This completes the process of changing the IP address of the standalone
                                       server. |
| Step 5 | For the cluster, repeat Step 1 to Step 4 on subscriber server also.This completes the process of changing the IP address of
                                       a cluster. |
...
| Step 14 | If you are finished with installing
                                             				languages: Restart the services: Sign in to Cisco Unity Connection
                                                   					 ...
                                                   					 Connection Conversation Manager row, select Start . Wait for the service to
                                                   					 start. In the Critical Services menu, in the Connection Mixer row, select Start . Wait for the service to start. Note To complete the locale update, restart the Cisco Tomcat service across the entire cluster (both publisher and subscriber nodes).
                                                   This ensures that the Cisco Unity Connection GUI is synchronized and that the correct language settings are displayed on both
                                                   the publisher and subscriber nodes. |
