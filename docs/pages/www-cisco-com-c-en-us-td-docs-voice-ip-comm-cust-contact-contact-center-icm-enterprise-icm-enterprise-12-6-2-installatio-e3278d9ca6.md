---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-12-6-2-installatio-e3278d9ca6
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_2/installation/guide/ucce_b_install_upgrade_guide_1262/ucce_b_12_6_1-install_upgrade_guide_chapter_0110.html
retrieved_at: 2026-08-16T19:59:48.437581+00:00
---
...
In 12.6(2), the RAM requirement for Live Data VM has changed. Refer to the Update VM Properties section in Upgrade Overview to update the Live Data VM properties before you upgrade the component.
...
Following an AW technology refresh, perform the following steps for the CUIC-LD-IDS on the destination machine:

Remove and re-add the CUIC-LD-IDS hostname in Unified CCE Administration. Grant symmetric key access to the livedata user. For detailed information, refer
                                             to the Before You Begin section of Configure Live Data with AW .

Execute the set live-datasource command on the CUIC-LD-IDS. For more details, refer to Configure Live Data Unified Intelligence Center Data Sources in this guide.
...
If you are using AppDynamics for performance monitoring on 12.6(1), then before upgrading the Distributor node from 12.6(1)
                                             to 12.6(2), disable AppDynamics performance monitoring on 12.6(1) and re-enable it after upgrading to 12.6(2). If AppDynamics
                                             performance monitoring is not disabled before the Distributor node is upgraded to 12.6(2), then post upgrade, restart the
                                             Distributor node.
...
The supported upgrade paths to Unified CCE 12.6(2) are as follows:

Unified CCE 12.5(1) to Unified CCE 12.6(2). EDMT is not required during this upgrade process. If Windows and SQL platform
                                    upgrade is involved during this upgrade process, see the Technology Refresh Upgrade section for details on using EDMT.

Unified CCE 12.5(2) to Unified CCE 12.6(2). EDMT is not required during this upgrade process.

Unified CCE 12.6(1) to Unified CCE 12.6(2). EDMT is not required during this upgrade process.

Unified CCE 12.0(1) to Unified CCE 12.5(1) with inline upgrade to Unified CCE 12.6(2) using Common Ground or Technology Refresh
                                    upgrade. Use EDMT during this upgrade process:

In case of Common Ground upgrade, use 12.5(x) EDMT to upgrade from Unified CCE 12.0(1) to Unified CCE 12.5(1).

In case of Technology Refresh upgrade, use 12.6(x) EDMT to upgrade from Unified CCE 12.0(1) to Unified CCE 12.5(1).
...
Before you begin the upgrade from Cloud Connect 12.6(1) to Cloud Connect 12.6(2), check if the ucos.keymanagement.v02.cop.sgn is applied on the base version. If not, you must install it; else the upgrade will fail. Restart the Cloud Connect node after
                           installing ucos.keymanagement.v02.cop.sgn .

Before you begin the upgrade from Cloud Connect 12.5(1) to Cloud Connect 12.6(2), check if the ucos.keymanagement.v01.cop.sgn is applied on the base version. If not, you must install it; else the upgrade will fail. Restart the Cloud Connect node after
                           installing ucos.keymanagement.v01.cop.sgn .

Before upgrading Cloud Connect from 12.6(1) to 12.6(2), ensure that you update the VM configuration as below:

Increase vRAM from 10 GB to 16 GB

Expand vDisk from 246 GB to 750 GB

Download the ISO file from the software download page for Cloud Connect https://software.cisco.com/download/home/268439622/type/283914286/release/12.6(2) to the SFTP server that can be accessed from the Cloud Connect system.

After you switch the version of the publisher and subscriber nodes successfully, verify if the nodes are upgraded to Cloud
                                       Connect 12.6(2). The active version is Cloud Connect 12.6(2), while the inactive version is Cloud Connect 12.6(1).
...
Verify if the node is upgraded to Cloud Connect 12.6(2), after you switch the version successfully (where active version is
                                             Cloud Connect 12.6(2) and inactive version is Cloud Connect 12.5(1) or Cloud Connect 12.6(1).
...
Verify if the nodes are upgraded to Cloud Connect 12.6(2), after you switch the version of the publisher node and subscriber
                                             nodes successfully (where active version is Cloud Connect 12.6(2) and inactive version is Cloud Connect 12.5(1) or Cloud Connect
                                             12.6(1).
...
| 4 | Agent/Supervisor Desktop, Central Controller, and Reporting | ECE Cisco Finesse Unified CCE Rogger Admin & Data server (AW/HDS/DDS) CUIC-LD-IDS CUIC Reporting Templates CCMP | In 12.6(2), the RAM requirement for Live Data VM has changed. Refer to the Update VM Properties section in Upgrade Overview to update the Live Data VM properties before you upgrade the component. After you upgrade AW, import the self-signed certificate of all solution components (if applicable) to all AWs. After you upgrade Finesse to Release 15.0(1), to load any gadgets to Finesse, you must first import all self-signed certificates
                                             ...
                                             Unified CCE Administration Portal, and Unified Intelligence Center Dashboard before bringing the upgraded IdS nodes online. Deployments using VPN-less access to Finesse desktop should also upgrade the reverse-proxy to 15.0(1) before Cisco IdS is
                                             ...
                                             identity providers. This process ensures that the upgraded system can properly communicate and authenticate with other services. After you upgrade Live Data (LD), you must enable CORS on the LD box for Finesse and CUIC. For more information, see Installation and Upgrade Guide for Cisco Unified Intelligence Center Guide at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-intelligence-center/products-installation-guides-list.html . After you upgrade LD, you must import the Finesse certificate to LD. Following an AW technology refresh, perform the following steps for the CUIC-LD-IDS on the destination machine: Remove and re-add the CUIC-LD-IDS hostname in Unified CCE Administration. Grant symmetric key access to the livedata user. For detailed information, refer
                                             to the Before You Begin section of Configure Live Data with AW . Execute the set live-datasource command on the CUIC-LD-IDS. For more details, refer to Configure Live Data Unified Intelligence Center Data Sources in this guide. | Note | After upgrading cuic-ld-ids to 15.0(1), run the utils finesse layout updateCuicGadgetUrl command to update the gadget URL. |
...
| 7 | Central Controller | Unified CCE  Router Unified CCE  Logger Admin & Data server (AW/HDS/DDS) Standalone Live Data (if Deployed) CUIC Reporting Templates CCMP Administration Client | In 12.6(2), the RAM requirement for Live Data VM has changed. Refer to the Update VM Properties section in Upgrade Overview to update the Live Data VM properties before you upgrade the component. After you upgrade AW, import the self-signed certificate of all solution components (if applicable) to all AWs. If you are using AppDynamics for performance monitoring on 12.6(1), then before upgrading the Distributor node from 12.6(1)
                                             to 12.6(2), disable AppDynamics performance monitoring on 12.6(1) and re-enable it after upgrading to 12.6(2). If AppDynamics
                                             performance monitoring is not disabled before the Distributor node is upgraded to 12.6(2), then post upgrade, restart the
                                             Distributor node. After you upgrade Live Data (LD), you must enable CORS on the LD box for Finesse and CUIC. For more information, see Installation and Upgrade Guide for Cisco Unified Intelligence Center Guide at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-intelligence-center/products-installation-guides-list.html . After you upgrade LD, you must import the Finesse certificate to LD. After you upgrade LD, you must run the set live-data machine-services and set live-data cuic-datasource commands. For more information, see Configure Live Data Machine Services and Configure Live Data for Unified Intelligence Center Data Sources . |
...
| Note | Before upgrading Cloud Connect from 12.6(1) to 12.6(2), ensure that you update the VM configuration as below: Increase vRAM from 10 GB to 16 GB Expand vDisk from 246 GB to 750 GB |
|---|---|

| Note | After you switch the version of the publisher and subscriber nodes successfully, verify if the nodes are upgraded to Cloud
                                       Connect 12.6(2). The active version is Cloud Connect 12.6(2), while the inactive version is Cloud Connect 12.6(1). |
|---|---|
...
| Note | Verify if the node is upgraded to Cloud Connect 12.6(2), after you switch the version successfully (where active version is
                                             Cloud Connect 12.6(2) and inactive version is Cloud Connect 12.5(1) or Cloud Connect 12.6(1). |
|---|---|

| Note | Verify if the nodes are upgraded to Cloud Connect 12.6(2), after you switch the version of the publisher node and subscriber
                                             nodes successfully (where active version is Cloud Connect 12.6(2) and inactive version is Cloud Connect 12.5(1) or Cloud Connect
                                             12.6(1). |
|---|---|