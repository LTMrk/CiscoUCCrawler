---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-jtapi-dev-12-5-1-cucm-b-cisco-unified-jtapi-developers-guide-1251-cucm--1c5c9705bb
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/jtapi_dev/12_5_1/cucm_b_cisco-unified-jtapi-developers-guide-1251/cucm_b_cisco-unified-jtapi-developers-guide-1251_chapter_010.html
retrieved_at: 2026-08-16T18:11:56.027955+00:00
---
...
As mentioned above, applications that wish to use FIPS compliance must not only explicitly request it, they must also download
                              cryptographic libraries, and alter their classpath variable to include them. As mentioned earlier, applications that need
                              to use FIPS compliance must not only explicitly request it, but also download cryptographic libraries and modify their classpath
                              variables to include them. RSA libraries are applicable for Windows and CiscoJ libraries are applicable for Linux. The RSA
                              libraries are "jcmFIPS.jar" , "cryptojcommon.jar" , "cryptojce.jar" and "sslj.jar" , and are FIPS-compliant libraries from RSA, Inc. The CiscoJ library and its dependent libraries are listed as below:

Up to Release 12.5(1)SU4 :

"CiscoJCEProvider.jar" , "CiscoJUtils.jar" , "log4j-1.2.17.jar" , " slf4j-api-1.7.24.jar" , "slf4j-log4j12-1.7.24.jar" , "slf4j-simple-1.7.24.jar" , "bcpkix-jdk15on-154.jar" , and "bcprov-jdk15on-154.jar" , " libCiscoJCEJNI.so" , "libssl.so" , "libssl.so.1.0.1" , "libciscosafec.so" , "libciscosafec.so.3" , "libciscosafec.so.3.0.1" , "libcrypto.so" , "libcrypto.so.1.0.1"

In Release 12.5(1) SU5 and 12.5(1)SU6 :

"CiscoJCEProvider.jar" , "CiscoJUtils.jar" , "log4j-1.2.17.jar" , "slf4j-api-1.7.24.jar" , "slf4j-log4j12-1.7.24.jar" , "slf4j-simple-1.7.24.jar" , "bcpkix-jdk15on.jar" , and "bcprov-jdk15on.jar" , "libCiscoJCEJNI.so" , "libssl.so" , "libssl.so.1.0.1" , "libciscosafec.so" , "libciscosafec.so.3" , "libciscosafec.so.3.0.1" , "libcrypto.so" , "libcrypto.so.1.0.1"

From Release 12.5(1) SU7 and any upcoming ES or SU on this release train:

CiscoJCEProvider.jar, CiscoJUtils.jar, log4j-api.jar, log4j-core.jar, log4j-1.2-api.jar, slf4j-api-1.7.24.jar, slf4j-log4j12-1.7.24.jar,
                              slf4j-simple-1.7.24.jar, bcpkix-jdk15on-154.jar, and bcprov-jdk15on-154.jar, libCiscoJCEJNI.so, libssl.so, libssl.so.1.0.1,
                              libciscosafec.so, libciscosafec.so.4, libciscosafec.so.4.0.1, libcrypto.so, libcrypto.so.1.0.1

These libraries contain special implementations of several key cryptographic functions and supersede the older implementation
                              in "jtapi.jar" .

If your application contains a lib folder where third-party libraries are stored, the classpath looks as follows:

For Windows (32 and 64 bit): ./libs/jcmFIPS.jar;./libs/cryptojcommon.jar;./libs/cryptojce.jar;./libs/sslj.jar;./libs/jtapi.jar
