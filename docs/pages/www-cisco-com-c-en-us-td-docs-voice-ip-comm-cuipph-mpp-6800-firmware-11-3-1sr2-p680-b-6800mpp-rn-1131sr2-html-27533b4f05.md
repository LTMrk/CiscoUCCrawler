---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cuipph-mpp-6800-firmware-11-3-1sr2-p680-b-6800mpp-rn-1131sr2-html-27533b4f05
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cuipph/MPP/6800/firmware/11-3-1sr2/p680_b_6800mpp-rn-1131sr2.html
retrieved_at: 2026-08-21T23:13:51.650197+00:00
---

Cisco IP Phone 6800 Series Multiplatform Phones Release Notes for Firmware Release 11.3(1)SR2
...
Use these release notes with the Cisco IP Phone 6800 Series Multiplatform Phones running SIP Firmware Release 11.3(1)SR2.
...
Under Latest Release , select 11.3.1 MSR2-6 .

(Optional) Place your mouse pointer on the file name to see the file details and checksum values.

Download the corresponding file.

6821: cmterm-6821.11-3-1MSR2-6_REL.zip

When you upgrade the Cisco IP phone 6821 to the firmware release 11.3.1 and 11.3.1SR1,
                                                   									you encounter an issue and you need to upgrade the phone
                                                   									firmware with a different file. Please see Cisco IP Phone 6821 Upgrades with Different Phone Firmware (CSCvu49174) to register
                                                   									the phone succussfully.

Other phones in 6800 series: cmterm-68xx.11-3-1MSR2-6_REL.zip
...
http://10.73.10.223/firmware/sip6821.11-3-1MSR2-6.loads

https://server.domain.com/firmware/sip6821.11-3-1MSR2-6.loads

Other phones in 6800 series:

http://10.73.10.223/firmware/sip68xx.11-3-1MSR2-6.loads

https://server.domain.com/firmware/sip68xx.11-3-1MSR2-6.loads
...
```
https://10.74.10.225/admin/upgrade?http://10.73.10.223/firmware/sip6821.11-3-1MSR2-6.loads
```

```
https://10.74.10.225/admin/upgrade?https://server.domain.com/firmware/sip6821.11-3-1MSR2-6.loads
```

```
https://10.74.10.225/admin/upgrade?http://10.73.10.223/firmware/sip68xx.11-3-1MSR2-6.loads
```

```
https://10.74.10.225/admin/upgrade?https://server.domain.com/firmware/sip68xx.11-3-1MSR2-6.loads
```
...
### Cisco IP Phone 6821 Upgrades with Different Phone Firmware (CSCvu49174)

The Cisco IP phone 6821 fails to register when you want to upgrade an out-of-box phone to the
                        				firmware release 11.3.1 and 11.3.1SR1. To fix this issue, download the corresponding
                        				file from the following location and upgrade your phone.

https://software.cisco.com/download/home/286322437/type/286320168/release/11.3.1%20MSR2UPG-1

The .UPG load includes all the bug fixes in 11.3.1SR2 and it upgrades to further MPP releases
                        				successfully.
...
The following list contains the severity 1, 2, and 3 defects that are open for the Cisco IP Phone 6800 Series Multiplatform Phones that use Firmware Release 11.3(1)SR2.
...
The following list contains the severity 1, 2, and 3 defects that are resolved for the Cisco IP Phone 6800 Series Multiplatform Phones that use Firmware Release 11.3(1)SR2.
...
CSCvs59424 3pcc-88xx: Phone is not uploading the config when Report To Server is set to On
...
CSCvt25512 MPP phone configuration displays unsupported locales

CSCvt22995 MPP does not show BLF when NOTIFY has different URI

CSCvs32299 6821 attempts to establish TLS for SIP but fails to "unknown CA"

CSCvt52122 Transferor hears busy signal during consultative transfer

CSCvu41656 Skip Valid From date for EDOS and GDS
...
| Step 4 | Under Latest Release , select 11.3.1 MSR2-6 . |
| Step 5 | (Optional) Place your mouse pointer on the file name to see the file details and checksum values. |
| Step 6 | Download the corresponding file. 6821: cmterm-6821.11-3-1MSR2-6_REL.zip Note When you upgrade the Cisco IP phone 6821 to the firmware release 11.3.1 and 11.3.1SR1,
                                                   									you encounter an issue and you need to upgrade the phone
                                                   									firmware with a different file. Please see Cisco IP Phone 6821 Upgrades with Different Phone Firmware (CSCvu49174) to register
                                                   									the phone succussfully. Other phones in 6800 series: cmterm-68xx.11-3-1MSR2-6_REL.zip | Note | When you upgrade the Cisco IP phone 6821 to the firmware release 11.3.1 and 11.3.1SR1,
                                                   									you encounter an issue and you need to upgrade the phone
                                                   									firmware with a different file. Please see Cisco IP Phone 6821 Upgrades with Different Phone Firmware (CSCvu49174) to register
                                                   									the phone succussfully. |
| Note | When you upgrade the Cisco IP phone 6821 to the firmware release 11.3.1 and 11.3.1SR1,
                                                   									you encounter an issue and you need to upgrade the phone
                                                   									firmware with a different file. Please see Cisco IP Phone 6821 Upgrades with Different Phone Firmware (CSCvu49174) to register
                                                   									the phone succussfully. |
...
| Step 8 | Unzip the file and place the files in the appropriate location on your upgrade server. The appropriate location is the TFTP, HTTP, or HTTPS download folder, depending on the protocol that you want to use for the
                                 upgrade. |
| Step 9 | Upgrade the phone firmware with one of these methods. Upgrade the phone firmware from the phone administration web page: On the phone administration web page, go to Admin Login > Advanced , Voice > Provisioning > Firmware Upgrade . In the Upgrade Rule field, enter the load file URL as described below. Load file URL format: <upgrade protocol>://<upgrade server ip address>[:<port>]>/<path>/<file name>.loads Examples: 6821: http://10.73.10.223/firmware/sip6821.11-3-1MSR2-6.loads https://server.domain.com/firmware/sip6821.11-3-1MSR2-6.loads Other phones in 6800 series: http://10.73.10.223/firmware/sip68xx.11-3-1MSR2-6.loads https://server.domain.com/firmware/sip68xx.11-3-1MSR2-6.loads Click Submit All Changes . Upgrade the phone firmware directly from your web browser: In the address bar of your web browser, enter the phone upgrade URL as described below. Phone upgrade URL format: <phone protocol>://<phone ip address[:port]>/admin/upgrade?<load file URL> Load file URL format: <upgrade protocol>://<upgrade server ip address>[:<port>]>/<path>/<file name>.loads Examples: 6821: https://10.74.10.225/admin/upgrade?http://10.73.10.223/firmware/sip6821.11-3-1MSR2-6.loads https://10.74.10.225/admin/upgrade?https://server.domain.com/firmware/sip6821.11-3-1MSR2-6.loads Other phones in 6800 series: https://10.74.10.225/admin/upgrade?http://10.73.10.223/firmware/sip68xx.11-3-1MSR2-6.loads https://10.74.10.225/admin/upgrade?https://server.domain.com/firmware/sip68xx.11-3-1MSR2-6.loads Note Specify the <file name>.loads file in the URL. The <file name>.zip file contains other files. | Note | Specify the <file name>.loads file in the URL. The <file name>.zip file contains other files. |
