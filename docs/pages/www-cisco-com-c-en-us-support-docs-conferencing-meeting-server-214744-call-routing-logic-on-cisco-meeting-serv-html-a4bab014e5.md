...
If the call did not hit any of the rules on the incoming call matching table or there was no match for the call to proceed (e.g user dialed non existing or wrong space URI), it goes through the second table called the call forwarding table. This is also domain based only and allows you to specifically block calls to certain domains or to specifically only allow calls to certain domains. If you want to do this, it is important to have more specific rules with a higher priority, so they are checked first.

This example shows that calls to dummy.com is rejected, while calls to tplab.local is forwarded:

When you leave the call forwarding table blank, this results in a state where CMS is not acting as a gateway between Skype and SIP participants for example as there is not any call forwarding rule. In the assumption that either the domain of the incoming call is no match on the incoming call matching table or the domain matches but there is no match on the spaces, users or IVRs (or Skype meetings), then the call is not forwarded with regards of the outbound call table.

Note : This does happen though with CMA clients (thick clients and WebRTC) as they are able to make outbound calls (*Web App in 3.0 cannot make outbound calls, but rather calls made by CMS space out by Callbridge). Similarly, outbound calls on CMS do work fine as well when made via API for example (in the case of TMS scheduled conferences). In general, calls that are initiated from CMS itself (either CMS directly or via CMA) must not follow the call forwarding logic.
...
2018-10-04  07:02:24.839  Info     SIP trace: To : <sip:stejanss@ newany.com > 2018-10-04  07:02:24.839  Info     SIP trace: From: "EX60 Steven" <sip:1060@steven.lab>;tag=2aa2a49bba231a1b
```
...
2018-10-12  11:25:16.181  Info     DNS trace: resolution of "steven.lab" (SRV "_sips._tcp") for call 822 using 10.48.36.215:5061 2018-10-12  11:25:16.181  Info     SIP trace: connection 45: allocated for outgoing encrypted connection to 10.48.36.215:5061
...
2018-10-12  11:25:16.202  Info     DNS trace: resolution of "steven.lab" (SRV "_sip._tcp") for call 822 using 10.48.36.215:5060 2018-10-12  11:25:16.202  Info     SIP trace: connection 46: allocated for outgoing connection to 10.48.36.215:5060
