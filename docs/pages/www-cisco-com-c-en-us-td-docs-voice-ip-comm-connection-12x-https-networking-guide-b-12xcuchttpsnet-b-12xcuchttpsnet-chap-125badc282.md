...
Do the following procedure to remove an HTTPS link
                              		between two Unity Connection 11.x locations. If either of the location is a
                              		Unity Connection cluster, do the steps for that location only on the publisher
                              		server.
...
Make the change on the Unity Connection location according to
                                    			 the instructions in the “ Maintaining Cisco Unity
                                       				Connection Server ” chapter of the Install,
