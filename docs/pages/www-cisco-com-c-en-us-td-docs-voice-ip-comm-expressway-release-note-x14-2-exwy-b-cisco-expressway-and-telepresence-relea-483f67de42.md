---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-expressway-release-note-x14-2-exwy-b-cisco-expressway-and-telepresence-relea-483f67de42
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/expressway/release_note/X14-2/exwy_b_cisco-expressway-and-telepresence-release-note-x142.html
retrieved_at: 2026-08-16T22:08:19.225068+00:00
---

Cisco Expressway and Cisco TelePresence Video Communication Server Release Note (X14.2)
...
### Notices Relating to Smart Licensing Export Compliance
...
The CAP of  2500 secured/crypto sessions is also applicable to Cisco TelePresence Video Communication Server (VCS) Series.
...
#### (Preview) Hardware Security Module (HSM) Support - Withdrawn from X14.2 release
...
Expressway Series : Enable Export Control (set to True ) to activate FIPS. After enabling this, all commands ( <leave/enter/status> ) become usable.
...
If you are upgrading from a version lower than 14.2 to 14.2 or higher version, note that we have appended "ECDHE-RSA-AES256-GCM-SHA384:" to the default Ciphers List to prefer RSA certificates over ECDSA. If you prefer ECDSA certificates over RSA, then remove "ECDHE-RSA-AES256-GCM-SHA384:" from the cipher string using web UI ( Maintenance > Security > Ciphers ) or CLI command ( xConfiguration Ciphers ).
...
Update the system with the upcoming release of  Cisco Expressway X14.2.1 to have Uncapped access to secured sessions.

The release date of  X14.2.1 is not yet matriculated.
...
| Important | Product Activation Keys (PAK) Licensing (Option Keys) are removed from version X14.2. Smart License is default and the only licensing mode for Expressway-C and Expressway-E. In Smart License mode, the functionality is enabled by default and hence they are not needed or supported and may not be
                                             converted in the License Registration Portal . |
|---|---|
...
| Note | The following aspects is in context of the Cisco Expressway X14.2 release. Cisco Expressway X14.2 release is a Unlimited and Capped version with a CAP of 2500 secured sessions. Note: CAP of 2500 secured sessions is also applicable to Cisco TelePresence Video Communication Server (VCS) Series. Update the system with the upcoming release of  Cisco Expressway X14.2.1 to have Uncapped access to secured sessions. The release date of  X14.2.1 is not yet matriculated. |
|---|---|