---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-pcce-pcce-12-5-1-maintenance-guide-pcce-b-featur-7935cf677d
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/pcce/pcce_12_5_1/maintenance/guide/pcce_b_features_guide_12_5/pcce_m_courtesy-callback.html
retrieved_at: 2026-08-16T19:29:41.026510+00:00
---
...
CallbackEngine: Keeps the VoIP leg of the call alive when the
                                                   					 ...
                                                   					 actually receives the callback. Do not modify this script.
...
queueapp is
                                                      					 the name of the VXML Server queuing application that is executed for this
                                                      					 queue.
...
The Callback_Add element is used to add a callback object to the database after all the callback
...
The Callback_Enter_Queue element is responsible for
...
| Step 2 | Each folder
                                          			 contains a Call Studio project having the same name as the folder. The five
                                          			 individual projects comprise the Courtesy Callback feature. Do not modify the following scripts: CallbackEngine: Keeps the VoIP leg of the call alive when the
                                                   					 caller elects to receive the callback (and hangs up) and when the caller
                                                   					 actually receives the callback. Do not modify this script. CallbackQueue: Handles the keepalive mechanism for the call when
...
| Step 7 | Block #3: Set up the parameters to be passed to CallbackEntry (VXML
                                             			 application). Note This step
                                                				...
                                                				related to these variables are: CallbackEntry is the name of the VXML Server application that is executed. ewt is
                                                      					 calculated in Block
                                                         						#2 . qname is
                                                      					 the name of the VXML Server queue into which the call is placed. There must be
                                                      					 a unique qname for each unique resource pool queue. queueapp is
                                                      					 the name of the VXML Server queuing application that is executed for this
                                                      					 queue. ani is the
