---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-im-presence-configadminguide-12-5-1-su4-cup0-b-config-and-admin-guide-1-353c1c1361
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/im_presence/configAdminGuide/12_5_1_su4/cup0_b_config-and-admin-guide-1251su4/cup0_b_config-and-admin-guide-1251su3_chapter_01111.html
retrieved_at: 2026-08-16T16:45:40.978590+00:00
---
...
While setting up a connection to MSSQL external database, you can enable Windows authentication.

Step 7

Migrate persistent chat rooms from one external database to another

In the IM and Presence Service, migrate all the persistent chat rooms and groups from your existing external database to
                                          another of the same database type or different types. For more information on how to perform the external database migration,
                                          see the "Migrate Persistent Chat Rooms from One External Database to Another" section of the Cisco IM and Presence Database
                                          Setup Guide 12.5(1)SU2 Release.
...
Supported from Release 14SU2 onwards.

To configure external database connection, see Add External Database Connection .

Step 1

From the Database Type drop-down, select the type of external database as Microsoft SQL Server .
...
In the Domain field, enter the Windows domain name.

Step 4

Enter the User Name and Password information of the Windows user.

By using Windows authentication, Windows groups can be created at the domain level, and a login can be created on MSSQL server
...
| Step 6 | Windows Authentication for MSSQL Database for Persistent Chat | While setting up a connection to MSSQL external database, you can enable Windows authentication. |
| Step 7 | Migrate persistent chat rooms from one external database to another | In the IM and Presence Service, migrate all the persistent chat rooms and groups from your existing external database to
                                          another of the same database type or different types. For more information on how to perform the external database migration,
                                          see the "Migrate Persistent Chat Rooms from One External Database to Another" section of the Cisco IM and Presence Database
                                          Setup Guide 12.5(1)SU2 Release. |
...
| Step 4 | Enter the User Name and Password information of the Windows user. | Note By using Windows authentication, Windows groups can be created at the domain level, and a login can be created on MSSQL server
                                                         for the entire group. | Note | By using Windows authentication, Windows groups can be created at the domain level, and a login can be created on MSSQL server
                                                         for the entire group. |
| Note | By using Windows authentication, Windows groups can be created at the domain level, and a login can be created on MSSQL server
                                                         for the entire group. |
