...
For more information on how to run Service Account Manager tool, see the Staging Guide for Cisco Unified Contact Center Enterprise at http://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-guides-list.html .
...
When you bring up Side A Router to service, Side B Router service stops automatically.
...
Use the maintenance mode command to invoke maintainence mode and stop all the Unified CCE services on the Side B Call Router
                                          and Logger. However, before stopping Side B services, also make sure that all non-upgraded AW and data servers are stopped and shutdown
                                          using the maintenance mode command to invoke maintenance mode.

For more information, see the Invoking Maintenance Mode topic in the Administration Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-maintenance-guides-list.html .
...
See Security Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html
...
| Step 1 | Manually start the Unified CCE services on the Side A Call Router and Logger, and the upgraded Administration & Data Server.
...
\CurrentVersion\Recovery\CurrentVersion\Purge\Retain\System\ConfigMessageLog Note When you bring up Side A Router to service, Side B Router service stops automatically. | Category | Operation | General | Setup logs indicate no errors or failure conditions. AD domain has all users. Schema upgrade is successful for all databases (no loss of data integrity or loss of data). All component services start without errors. Calls are successfully processed. | Call Router | The Rtsvr logs indicate that the upgraded Administration & Data Server has connected successfully. | Logger | Recovery process that is not required, no activity other than process start up. Users are in correct domain. Configuration information is passed to Call Router. Replication process begins when HDS comes online. | Administration & Data Server | The updateAW process logs indicate that the Administration & Data Server is waiting for work. Replication process begins with no errors. 1 | Security | Specified users are able to use configuration manager. | Script Editor | Previous settings for users are present when application is opened. Validate All script yields the same results that the preupgrade test yielded. You can open, edit, delete, or create new scripts. | ICMDBA | Import or Export functionality is present. Database space allocation and percent used are correct. | Note | When you bring up Side A Router to service, Side B Router service stops automatically. |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Category | Operation |
...
| Note | When you bring up Side A Router to service, Side B Router service stops automatically. |
| Step 2 | Use Unified CCE Service Control to set the Unified CCE services to Automatic Start on each of the upgraded Unified CCE components. |
| Step 3 | Verify production system operation while running with the upgraded Side A Call Router and Side A Logger. |
| Step 4 | Use the maintenance mode command to invoke maintainence mode and stop all the Unified CCE services on the Side B Call Router
                                          and Logger. However, before stopping Side B services, also make sure that all non-upgraded AW and data servers are stopped and shutdown
                                          using the maintenance mode command to invoke maintenance mode. For more information, see the Invoking Maintenance Mode topic in the Administration Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-maintenance-guides-list.html . |
...
| Step 5 | Verify overall
                                       			 system operation. |
| Step 6 | Upgrade any other Administration & Data Servers or HDSs using the steps that are documented in Migrate HDS Database and Upgrade Unified CCE Administration & Data Server . |
...
| Customer Collaboration Platform | See Security Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html |
