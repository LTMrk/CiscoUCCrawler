---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-dx-series-admin-1022-dx00-bk-c9fcbae4-00-cisco-dx-series-ag1022-dx00-bk-c9fc-f57ef0ffc2
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/dx/series/admin/1022/DX00_BK_C9FCBAE4_00_cisco-dx-series-ag1022/DX00_BK_C9FCBAE4_00_cisco-dx-series-ag1022_chapter_01000.html
retrieved_at: 2026-08-21T21:02:01.602726+00:00
---
...
Each 
					 ...
					 installed certificate (MIC), but for additional security, you can specify in Cisco Unified Communications Manager Administration that a certificate be installed through use of the Certificate Authority Proxy Function (CAPF). Alternatively, you can
...
Video is
		  transmitted as nonsecure. So, even if both 
		  devices are secure, the Encrypted lock icon is not displayed for video
		  calls.
...
Various
		  ...
		  audio conference calls only; video will not be secure.
...
Cisco DX Series devices also provide a proxy EAPOL
		...
		link between the LAN switch and the device is maintained. To maintain  network integrity, the device sends an EAPOL-Logoff message to the
...
The Automatic Lock controls how long the
		  display will stay on before it dims or goes off. If
		  
		  the device is in the Always On
		  mode, the device will dim. If 
		  the device is in the
		  Nightlight mode, it will turn off completely. The Automatic Lock value can be configured to a maximum value of 10 minutes.
		To configure the Automatic Lock value, go to Settings > Security > Automatically lock .

The following table
		  shows the relationship of the Screen Lock Timeout value and Automatic Lock value.
...
- If 
						the device is in
						Nightlight mode, the device locks and turns off when the Automatic Lock value is reached. When the Screen Lock Timeout value is reached, no
...
| File
					 ...
					 memory on the device. The device rejects such files without further processing. |
| File
					 encryption | Encryption prevents disclosure of sensitive information while the file is in transit to the device. In addition, the device validates the
					 signature to make sure that file tampering did not occur after file creation.
					 Files that fail authentication are not written to flash memory on the device.
					 The device rejects such files without further processing. |
