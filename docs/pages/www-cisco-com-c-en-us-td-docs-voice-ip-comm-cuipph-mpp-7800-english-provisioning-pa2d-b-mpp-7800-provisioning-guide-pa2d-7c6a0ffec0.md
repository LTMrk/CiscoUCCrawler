---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cuipph-mpp-7800-english-provisioning-pa2d-b-mpp-7800-provisioning-guide-pa2d-7c6a0ffec0
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cuipph/MPP/7800/english/provisioning/pa2d_b_mpp-7800-provisioning-guide/pa2d_b_mpp-7800-provisioning-guide_chapter_01.html
retrieved_at: 2026-08-21T02:20:38.662412+00:00
---
...
The examples in this document use configuration profiles with an XML format (XML) syntax. Sample profiles can be found in Sample Configuration Profiles .
...
Cisco IP Phone 7800 Series— sip78xx.v1-v2-v3MPP-BN
...
In the software version, v1-v2-v3-v4 can specify different digits and characters, but must start with a numeric digit. When
                                       comparing the software version, v1-v2-v3-v4 is compared in sequence, and the leftmost digits take precedence over the latter
                                       ones.

If v[x] includes only numeric digits, the digits are compared; if v[x] includes numeric digits + alpha characters, digits
                                       are compared first, then characters are compared in alphabetical order.
...
It is important to enclose macro variables in double quotes where a string literal is expected. Do not do so where a number
...
When used in the context of the Profile_Rule* and Upgrade_Rule parameters, conditional expressions must be enclosed within
                                       the syntax “(expr)?” as in this upgrade rule example. Remember that BN means Build Number.
...
The uid and pwd options can be used to specify userID and Password authentication for the specified URL. The bracketed optional arguments
                                 are macro expanded. Special purpose parameters, GPP_SA through GPP_SD, are macro expanded into macro variables, $SA through
                                 $SD, only when they are used as key option arguments. See these examples:
...
```
http://p.tel.com/firmware/sip78xx.11-0-0MPP-BN.loads

where BN==Build Number
```

```
http://p.tel.com/firmware/sip7832.11-0-1MPP-BN.loads
...
```
(“$F” ne “beta-customer”)? http://p.tel.com/firmware/sip78xx.11-0-0MPP-BN.loads
| http://p.tel.com/firmware/sip78xx.11-0-0MPP-BN.loads
...
In the Configuration Profile section, choose Yes from the Provision Enable drop-down list box.
...
In the Firmware Upgrade section, choose Yes from the Upgrade Enable drop-down list box.
...
| Step 1 | Rename the image as follows: cp-x8xx-sip.aa-b-cMPP.cop to cp-x8xx-sip.aa-b-cMPP.tar.gz where: x8xx is the phone series, such as 7811 or 7832. aa-b-c is the release number, such as 10-4-1 |
|---|---|
