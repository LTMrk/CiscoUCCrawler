---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-15-0-1-design-guid-ec7a0e185e
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_15_0_1/design/guide/ucce_b_ucce_soldg-for-unified-cce-1501/rcct_m_ucce_design-consider-integrated_1501.html
retrieved_at: 2026-08-16T19:50:13.142648+00:00
---
...
The Voice Gateway dial peers and Cisco Contact Center SIP Proxy routing policies are used for SIP Dialers to place outbound calls. This enables calls to be placed using gateways that are
...
The SIP Dialer uses the Contact Center SIP Proxy server to place and transfer outbound calls when Sip Server Type is set to CUSP server in the Dialer setup dialog. These calls are placed or transferred to CVP or outbound agents.
...
The following figure shows the deployment model for Contact Center SIP Proxy and eight Voice Gateways. The active Dialer points to the Contact Center SIP Proxy server. The proxy handles load balancing and failover. The SIP Dialer supports Contact Center SIP Proxy on the Cisco 3845 Integrated Services Router.

This figure does not show the optional redundant Campaign Manager.

In a multiple gateway deployment, the SIP Dialer requires Server Group and Route Table configurations on Contact Center SIP Proxy servers to identify the gateways. It also requires numbers so that the gateways can transfer customer calls to CVP or agents
...
For SIP Dialer deployment, a Contact Center SIP Proxy server is installed for one SIP Dialer on each PG side, and the Side A/Side B Dialer is targeting the same set of Voice Gateways
                                    through its own Contact Center SIP Proxy server. Multiple Voice Gateways can be installed locally to customer phones, or each Voice Gateway can be installed locally
...
The Cisco Contact Center SIP Proxy servers are locally redundant at Site 2 to avoid the WAN SIP signaling traffic for transfering live outbound calls.

Each SIP Dialer connects to its own Cisco Contact Center SIP Proxy server at Site 2.

Each Cisco Contact Center SIP Proxy server controls the set of Voice Gateways at Site 3 in the United States.
...
The Unified CM subscriber supports a certain rate of outbound calls per second. To support a larger CPS at the Agent PG, distribute
                                    the Dialer across multiple subscribers using a Contact Center SIP Proxy server.

##### CCCSP Considerations
...
Since four SIP Proxy transactions are required for some outbound call scenarios with Unified CVP, give Unified CVP its own Contact Center SIP Proxy server in large-scale deployments.
...
When the SIP Dialer connects through the CCCSP in the deployment, the port throttle setting on the dialer must not exceed the total gateway capacity under assumption. Calls
                                 are load-balanced through CCCSP and each gateway reaches its maximum available capacity. Limit the port throttle by the CCCSP maximum transaction. Currently, the dialer maximum throttle setting is 60 calls per second. Under general transfer rate,
                                 calls through CCCSP do not exceed maximum CCCSP transaction rate given that CCCSP is exclusively used by outbound deployments.
...
The throttling mechanism in the SIP Dialer process is not aware of which gateway the Contact Center SIP Proxy server selects to place outbound calls. Calculate the appropriate weight for each gateway in the Server Group configuration
                                    of the Contact Center SIP Proxy server for the load balance.
...
The load values in this example are currently undergoing validation and have not yet been finalized.
...
When using Unified CVP deployments for transfer to a VRU campaign, and the Unified CVP servers are separated from the Outbound
                                    Option Dialer servers by a WAN. Provide Unified CVP with its own Cisco Contact Center SIP Proxy Server in the local cluster to reduce the WAN traffic.

When deploying a SIP Dialer solution for transfer to a VRU campaign, and the Cisco Contact Center SIP Proxy Servers for the SIP Dialers are separated from the Outbound Option Dialer servers by a WAN.
...
| Note | The load values in this example are currently undergoing validation and have not yet been finalized. |
|---|---|
