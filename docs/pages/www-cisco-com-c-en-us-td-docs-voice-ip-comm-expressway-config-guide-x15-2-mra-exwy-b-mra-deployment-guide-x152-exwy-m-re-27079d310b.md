---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-expressway-config-guide-x15-2-mra-exwy-b-mra-deployment-guide-x152-exwy-m-re-27079d310b
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/expressway/config_guide/X15-2/mra/exwy_b_mra-deployment-guide-x152/exwy_m_requirements-for-mra.html
retrieved_at: 2026-08-16T15:12:27.707674+00:00
---
...
When a Unified CM node goes down, 78XX / 88XX / 98XX series phones registered over MRA will continue communicating with another
                                             active node. And the phones will re-register after some time.

Jabber registered over MRA and using OAuth token may get de-registered when a Unified CM node goes down and displays a message "Your session is expired. Sign in again to keep using Cisco Jabber" . You can sign in to your Jabber to continue using the service.
...
For Unified CM failover over MRA, the Cisco IP Phone forms two static server groups, not a full mesh of server groups for
                                             devices behind Expressway-E. Therefore, registration will fail if an Expressway-C and Unified CM node goes down and the IP
                                             Phone does not have a valid server group.
...
|---|---|---|
| Subject Alternative Names | The Expressway-C list of Subject Alternative Names must include: Phone Security Profiles used by MRA endpoints Expressway cluster name (for clustered Expressways only) IM and Presence chat node aliases (for Federated group chat) | The Expressway-E list of Subject Alternative Names must include: Unified CM Registration Domains XMPP Federation Domains IM and Presence chat node aliases (for Federated group chat) If the Expressway-E is also known by other FQDNs, all of the aliases must be included in the server certificate SAN. |
...
| Cisco IP Phone 9800 Series | 15.2 |
