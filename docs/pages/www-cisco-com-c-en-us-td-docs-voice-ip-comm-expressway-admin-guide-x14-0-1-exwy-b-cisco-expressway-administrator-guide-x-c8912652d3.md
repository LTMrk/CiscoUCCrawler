---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-expressway-admin-guide-x14-0-1-exwy-b-cisco-expressway-administrator-guide-x-c8912652d3
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/expressway/admin_guide/X14-0-1/exwy_b_cisco-expressway-administrator-guide-x14-0-1/exwy_m_user-accounts.html
retrieved_at: 2026-08-16T22:42:35.408118+00:00
---
...
This section provides information about how to configure administrator and FindMe user accounts, and how to display the details
                        of all active administrator and FindMe sessions.
...
Number of special characters [printable characters from 7-bit ASCII, eg. (space), @, $ etc.)] may be between 0 and 255 (default
                                    2)
...
When TLS is enabled, the LDAP server’s certificate must be signed by an authority within the Expressway’s trusted CA certificates
                                       file.
...
The Expressway provides a root account which can be used to log in to the Expressway operating system. This account has a
                              username of root (all lower case) and a default password of TANDBERG (all upper case). For security reasons you must change the password as soon as possible. An alarm is displayed on the web
                              interface and the CLI if the root account has the default password set.
...
| Note | If you haven’t configured the Forbidden password dictionary, clicking it displays a warning message, This Expressway is not currently configured to use a custom forbidden password dictionary. |
|---|---|
...
| Encryption | Determines whether the connection to the LDAP server is encrypted using Transport Layer Security (TLS). TLS : Uses TLS encryption for the connection to the LDAP server. Off : No encryption is used. The default is TLS . For more information, see Configuring Minimum TLS Version and Cipher Suites . | When TLS is enabled, the LDAP server’s certificate must be signed by an authority within the Expressway’s trusted CA certificates
