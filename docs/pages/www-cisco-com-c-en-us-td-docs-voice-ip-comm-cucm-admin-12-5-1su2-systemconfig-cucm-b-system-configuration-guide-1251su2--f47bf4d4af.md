---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-admin-12-5-1su2-systemconfig-cucm-b-system-configuration-guide-1251su2--f47bf4d4af
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/admin/12_5_1SU2/systemConfig/cucm_b_system-configuration-guide-1251su2/cucm_b_system-configuration-guide-for-cisco-1251su2_chapter_011111.html
retrieved_at: 2026-08-16T17:47:16.786537+00:00
---
...
Here is  a simple setup with a single proxy TFTP server that has two clusters. Endpoints in clusters A and B get all their
                              configuration files from one proxy TFTP server. The MAC address of the endpoint is used to find the cluster of the endpoint.

When there are multiple proxy TFTP servers in a deployment, you need to designate one to act as the primary for all clusters.
                              Peer relationships between the primary proxy TFTP server and all the other TFTP servers that are in the network are used to
                              tell the primary TFTP server where to look for configuration files that it cannot find in its own database.

In this example, two remote clusters (Cluster C and Cluster D) are added. Each cluster could have a proxy TFTP server of its
                              own configured, but only one server is designated as the primary proxy TFTP server. All TFTP servers must have a peer relationship
                              with the primary proxy TFTP server.

When an endpoint sends a request to the primary proxy TFTP server, it first searches its own database for the file. If the
                              file is not found, the primary proxy TFTP server polls the rest of the TFTP servers on the network. When the configuration
                              file is located, it is passed to the primary proxy  TFTP server. The primary proxy TFTP server downloads the file to the endpoint.

You must add any remote proxy TFTP servers to the Trust Verification Lists (TVL) of the endpoints; otherwise, they will not
                              accept the files. Autoregistration is not supported win a proxy TFTP server deployment.

A proxy TFTP server can coexist with a Cisco Unified Communications Manager on the same server. The proxy TFTP server does not need to have the highest Cisco Unified Communications Manager release, but we recommend that you do upgrade to the latest release  whenever you upgrade the clusters.
...
If a device gets no response from the TFTP server that you chose to act as the primary, it tries to connect to the secondary
                                 proxy TFTP server.  If the primary proxy TFTP server is in the middle of rebuilding all configuration files and returns a
                                 delay message to the requesting device, the device does not attempt to use the secondary one; instead, it waits and retries
                                 the primary proxy TFTP server.
...
At initial startup, phones that run SCCP query a DHCP server for network configuration information. The DHCP server responds
                                 with an IP address for the device, a subnet mask, a default gateway, a Domain Name System (DNS) server address, and a TFTP
                                 server name or address. If DHCP is not enabled on the SCCP device, you must assign it an IP address and configure the TFTP
                                 server on the device.

SIP phones get all their configuration files from the TFTP server. The SIP phone contacts the TFTP server to get the configuration
                                 files, then it registers itself to the appropriate Unified Communications Manager node.
...
The following table lists the configuration files for SIP phones. Filenames use information that is in the MAC address and
                                 description fields in the Phone Configuration window of Cisco Unified Communications Manager Administration and the device name field in the Unified Communications Manager database. The MAC address uniquely identifies the phone.
...
| Model | SIP IP Phone | Dial Plan | Softkey Template |
|---|---|---|---|
| 9971, 9951, 8961, 8945, 8941, 7975, 7971, 7970, 7961, 7941, 7911, Cisco Cius | SEP<mac>.cnf.xml | DR<dialplan>.xml | SK<softkey_template>.xml |
| Cisco TP 1000, Cisco TP 1100, Cisco TP 1300-47, Cisco TP 1300-65, Cisco TP 1310-65 |
| 7960, 7940 | SIP<mac>.cnf | <dialplan>.xml | Not configurable |
| 7905 | ld<mac> | Parameter in Id<mac> | Not configurable |
| 7912 | gk<mac> | Parameter in gk<mac> | Not configurable |
| Cisco IP Communicator | <Device Name>.cnf.xml | Parameter in <Device Name>.cnf.xml | Parameter in <Device Name>.cnf.xml |
| Cisco Jabber for Tablet | TAB<Userid in Upper Case>.cnf.xml | Not configurable | SK<softkey_template>.xml |
