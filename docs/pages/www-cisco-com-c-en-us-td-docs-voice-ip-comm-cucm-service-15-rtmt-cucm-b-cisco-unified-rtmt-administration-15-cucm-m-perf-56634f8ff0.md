...
The Cisco CallManager object provides information about calls, applications, and devices that are registered with the Unified
                                 Communications Manager. The following table contains information about Cisco CallManager counters.
...
This represents the number of Cisco Jabber on iPhone devices that are currently registered in the system over Mobile and Remote
                                             Access.
...
This represents the number of Cisco Jabber on Android devices that are currently registered in the system over Mobile and
                                             Remote Access.
...
This represents the number of Cisco Jabber on iPad devices that are currently registered in the system over Mobile and Remote
                                             Access.
...
This represents the number of Cisco Jabber on desktop devices that are currently registered in the system over Mobile and
                                             Remote Access.
...
This represents the number of Webex App on iPhone devices that are currently registered in the system over Mobile and Remote
                                             Access.
...
This represents the number of Webex App on Android devices that are currently registered in the system over Mobile and Remote
                                             Access.
...
This represents the number of Webex App on iPad devices that are currently registered in the system over Mobile and Remote
                                             Access.
...
This represents the number of Webex App on desktop devices that are currently registered in the system over Mobile and Remote
                                             Access.
...
This represents the number of Cisco hardware IP phones (for example, models 7861, 7841, 8845, 8865, and so on.) that are currently
                                             registered in the system over Mobile and Remote Access.

RegisteredDualModeDevices

Important

This counter details the dual-mode devices that are registered over Mobile and Remote Access and on-premise. TCT and BOT are
                                             the only dual-mode devices supported.

RegisteredDevicesMRA

Important

This represents the number of Cisco IP Phones and Dual-mode devices that are currently registered in the system over Mobile
                                             and Remote Access.
...
Total transcription request failures

The total number of transcription request failures since the last restart of the SpeechView Processor service.
...
If you're upgrading your source cluster from Release 12.5.x or Release 14 and SUs to Release 15, and you are using a customized
                                             value for the LowSwapPartitionAvailableDiskSpace alert before upgrade, the system will retain this customized value after
                                             the upgrade to Release 15. In such scenarios, you may receive an alarm stating that swap memory is utilized though it will
                                             not impact the system performance. This behavioral change is because of the underlying memory management changes of the OS
                                             in Release 15.

You can disable the LowSwapPartitionAvailableDiskSpace RTMT alert from Unified RTMT if the system is running without any issues.
...
Trigger alert when the following condition met:

SwapPartition available disk space below configured threshold (1%)
...
### SmartLicenseCommunication

This alert occurs when the auto switch fails from Call Home to Smart Transport Mode in the Licensing Transport Settings page
                                 or post upgrade, migration, and switch failure scenarios.
...
Trigger alert when the following condition is met: SmartLicenseCommunication event generated
...
Post upgrade, migration, or switch failure from the License Management page
...
This alert occurs when Unified Communications Manager running in the Evaluation period is enabled for License Reservation
                                 and pending installation of reserved authorization code.
...
Trigger alert when following condition met: SmartLicense_Reservation_InEval event generated
...
This alert occurs when the Unified Communications Manager license Evaluation period is expired and pending installation of
                                 Specified License Reservation or Permanent License Reservation authorization code.
...
This alert occurs when Unified Communication Manager has mixed-mode and License Reservation is enabled, and also when Unified
                                 Communication Manager is in Evaluation mode, Evaluation period expired, Registered-Specific License Reservation, and Registered-Permanent
                                 License Reservation states.
...
The system generates this alert whenever an XMPP client connection exceeds the inbound traffic limit continuously for 30 seconds.
                                       This alert warns administrators about potential misuse of system resources by an XMPP client. The system protects itself by
                                       throttling inbound traffic from the affected connection. The alert severity depends on the frequency of the alert occurrences
                                       and the number of client connections generating it. The limit is 15,000 bytes per second.
...
### XcpWebCmConnectionsPerIpLimit

The number of incoming web (BOSH) client connections from a single IP address has reached the configured maximum. This limit
                                       is defined by the 'Maximum number of web (BOSH) client connections per IP address' service parameter in the Cisco XCP Web
                                       Connection Manager ( Cisco Unified CM IM and Presence Administration > System > Service Parameters ). The default value is 10. New connection attempts from this IP address might be rejected once this limit is reached.

### XcpWebCmHttpdError

### XcpWebCmPauseSockets

### XcpWebCmReadBytesLimit

A single web (BOSH) client connection has exceeded its inbound read-byte budget during the alarm window. The read-byte budget
                                       is replenished every second, and excess usage carries forward as debt against the next refill. This alarm indicates that a
                                       web (BOSH) client connection might be generating an abnormally high volume of traffic, and the server is throttling the connection.
                                       The default limit is 15,000 bytes.
...
| Counters | Counter Descriptions |
|---|---|
| Rx Bytes | The number of bytes, including framing characters that are received on this interface. Range varies from 0 to 2 64 −1. |
...
| Rx Packets | The number of packets that this sublayer delivered to a higher sublayer. This situation does not include the packets that
                                             are addressed to a multicast or broadcast address at this sublayer. Range varies from 0 to 2 64 −1. |
| Total Bytes | The total number of received (Rx) bytes and transmitted (Tx) bytes. Range varies from 0 to 2 64 −1. |
| Total Packets | The total number of Rx packets and Tx packets. Range varies from 0 to 2 64 −1. |
| Tx Bytes | The total number of octets, including framing characters, that are transmitted out from the interface. Range varies from 0 to 2 64 −1. |
...
| Tx Packets | The total number of packets that the higher-level protocols requested for transmission, including those that are discarded
                                             or not sent. This situation doesn't include packets that are addressed to a multicast or broadcast address at this sublayer. Range varies from 0 to 2 64 −1. |
...
| SpaceFree | This counter represents the amount of free data blocks in the ram-based file system (ramfs). A block is a uniformly sized
                                             unit of data storage for a filesystem. The block size specifies the size that the file system uses to read and write data.
                                             On the Unified Communications Manager system, the block size is 4096 bytes. Range varies from 0 to 2 64 −1. |
...
| RegisteredOtherStationDevices | This counter represents the number of station devices other than Cisco hardware IP phones that are currently registered in
                                             the system (for example, Cisco IP SoftPhone, CTI port, CTI route point, Cisco voicemail port). |
| RegisteredTCTJabberNonMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Cisco Jabber on iPhone devices that are currently registered on-premise in the system. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredTCTJabberMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Cisco Jabber on iPhone devices that are currently registered in the system over Mobile and Remote
                                             Access. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredBOTJabberNonMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Cisco Jabber on Android devices that are currently registered on-premise in the system. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredBOTJabberMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Cisco Jabber on Android devices that are currently registered in the system over Mobile and
                                             Remote Access. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredTABJabberNonMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Cisco Jabber on iPad devices that are currently registered on-premise in the system. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredTABJabberMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Cisco Jabber on iPad devices that are currently registered in the system over Mobile and Remote
                                             Access. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredCSFJabberNonMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Cisco Jabber on desktop devices that are currently registered on-premise in the system. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredCSFJabberMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Cisco Jabber on desktop devices that are currently registered in the system over Mobile and
                                             Remote Access. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredTCTWebexNonMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Webex App on iPhone devices that are currently registered on-premise in the system. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredTCTWebexMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Webex App on iPhone devices that are currently registered in the system over Mobile and Remote
                                             Access. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredBOTWebexNonMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Webex App on Android devices that are currently registered on-premise in the system. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredBOTWebexMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Webex App on Android devices that are currently registered in the system over Mobile and Remote
                                             Access. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredTABWebexNonMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Webex App on iPad devices that are currently registered on-premise in the system. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredTABWebexMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Webex App on iPad devices that are currently registered in the system over Mobile and Remote
                                             Access. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredCSFWebexNonMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Webex App on desktop devices that are currently registered on-premise in the system. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredCSFWebexMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Webex App on desktop devices that are currently registered in the system over Mobile and Remote
                                             Access. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredHardwarePhonesMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Cisco hardware IP phones (for example, models 7861, 7841, 8845, 8865, and so on.) that are currently
                                             registered in the system over Mobile and Remote Access. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredDualModeDevices Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This counter details the dual-mode devices that are registered over Mobile and Remote Access and on-premise. TCT and BOT are
                                             the only dual-mode devices supported. |
| Important | Applicable from Release 14SU1 onwards. |
| RegisteredDevicesMRA Important Applicable from Release 14SU1 onwards. | Important | Applicable from Release 14SU1 onwards. | This represents the number of Cisco IP Phones and Dual-mode devices that are currently registered in the system over Mobile
                                             and Remote Access. |
| Important | Applicable from Release 14SU1 onwards. |
...
| Total transcription request failures | The total number of transcription request failures since the last restart of the SpeechView Processor service. |
...
| Note | If you're upgrading your source cluster from Release 12.5.x or Release 14 and SUs to Release 15, and you are using a customized
                                             value for the LowSwapPartitionAvailableDiskSpace alert before upgrade, the system will retain this customized value after
                                             the upgrade to Release 15. In such scenarios, you may receive an alarm stating that swap memory is utilized though it will
                                             not impact the system performance. This behavioral change is because of the underlying memory management changes of the OS
                                             in Release 15. You can disable the LowSwapPartitionAvailableDiskSpace RTMT alert from Unified RTMT if the system is running without any issues. |
|---|---|
...
| Enable/Disable this alert on the following servers | Enabled on listed servers |
| Threshold | Trigger alert when the following condition met: SwapPartition available disk space below configured threshold (1%) |
| Duration | Trigger alert immediately Default: 24 hours (86400 seconds) |
| Frequency | Trigger up to 3 alerts within 30 minutes |
| Schedule | 24 hours daily |
| Enable Email | Selected |
| Trigger Alert Action | Default |
...
| Severity | Warning |
| Enable or Disable this alert on the following servers | Enabled on listed servers |
| Threshold | Trigger alert when the following condition is met: SmartLicenseCommunication event generated |
| Duration | Trigger alert immediately |
| Frequency | Trigger alert on every poll |
| Schedule | Post upgrade, migration, or switch failure from the License Management page |
| Enable Email | Selected |
| Trigger Alert Action | Default |
