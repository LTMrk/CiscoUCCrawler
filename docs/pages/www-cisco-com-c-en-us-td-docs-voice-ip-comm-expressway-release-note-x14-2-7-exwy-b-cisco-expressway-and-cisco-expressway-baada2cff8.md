---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-expressway-release-note-x14-2-7-exwy-b-cisco-expressway-and-cisco-expressway-baada2cff8
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/expressway/release_note/X14-2-7/exwy_b_cisco-expressway-and-cisco-expressway-select-release-note-x1427.html
retrieved_at: 2026-08-16T15:38:22.122057+00:00
---

Cisco Expressway and Cisco Expressway Select Release Note (X14.2.7)
...
Updated the section "Other Changes in this Release."

June 2023

Republished X14.2.7

Expressway and Expressway Select X14.2.7 release.

Made changes to the section "ESXi Requirements".

Included a new section "Accessibility and Compatibility Features".

June 2023

First publication for Expressway and Expressway Select X14.2.7

Expressway and Expressway Select X14.2.7 release

May 2023

First publication for Expressway Select X14.2.6

Expressway Select X14.2.6 release
...
From X14.2 release and later versions , EXSi 6.5 Update 2a, ESXi 7.0 Update 3, ESXi 7.0 Update 3c, and ESXi 7.0 Update 3d are supported.

From X14.2.6 release and later versions , EXSi 6.5 Update 2a, ESXi 7.0 Update 3c, ESXi 7.0 Update 3d, and ESXi 8.0 are supported.

VMware withdrew the following supported versions: ESXi 7.0 Update 3, 3a, and 3b due to critical issues identified with those
                                          builds. ( Reference: Link ).

## Change Notices

### Smart Licensing - Unrestricted (Capped Version)
...
The CAP of 2500 secured/crypto sessions are also applicable to Cisco TelePresence Video Communication Server (VCS) Series.

Encrypted signaling to endpoints refers to SIP registrations or SIP calls, H.323 registrations or calls, WebRTC calls, and
                        XMPP registrations.

Important

Ensure that the limited number of encrypted signaling is not more than 2500 endpoints per instance of Expressway. A customer that needs to exceed this limit may deploy additional peers/clusters
                                          if entitled, to provide additional capacity.
...
For PID Details and Ordering Guide, see Get More Information .

### Smart Licensing Export Compliance - Restricted (UnCapped Version)
...
Every software release must comply with all relevant Export Control legislation - the US and local country regulations that
                        control the conditions under which certain software and technology may be exported or transferred to other countries and parties.
...
CCO does not perform a "license determination check" . So existing customers will only have access to the Export Unrestricted image. Users must submit a new $0 order (PID L-EXPWY-14.X-K9=)
                                    for an export-restricted image exceeding 2500 encrypted signaling sessions.
...
## Feature and Changes in X14.2.7
...
Cisco Expressway Release X14.2 and later only supports Smart Licensing. Versions prior to X14.2.7 were capped at 2500 encrypted signalling sessions to endpoints. It also
                                       includes changes in the trafficserver behavior (bug ID CSCwc69661 refers) that can lead to MRA failures - see here . For detailed information, see the Cisco Expressway and Cisco TelePresence Video Communication Server Release Note (x14.2) and Cisco Expressway Administrator Guide (X14.2) before upgrading to X14.2 .
...
Cisco Smart License Reservation means to reserve licenses and install them on the device. This process enables you to generate
...
Permanent License Reservation (PLR) is a part of Cisco smart licensing solution. Smart Licensing requires a smart account.
                                 The product must connect to the Cisco Smart Software Management (CSSM) or Smart Software Manage (SSM On-Prem) in order to
                                 reactivate and report the most recent license status. You will use Cisco Permanent License Reservation or PLR license if you
                                 have extremely secured inner networks with restricted internet access.

You can apply Permanent licenses over evaluation licenses, and also apply it incrementally (that is, you can have multiple
                                 permanent licenses, and so on). Its capabilities are designed for highly secure environments, where communication with an
                                 outside environment is impossible.
...
#### X14.2.5, X14.2.6, and X14.2.7 release(s)

#### Software Download Folder Path

The software download folder and path APPLY to both Unrestricted (Capped Version) and Restricted (Uncapped Version).
...
From X14.2.5, Expressway and Expressway Select is not supported on the Cisco TelePresence Video Communication Server (VCS)
                                    series. The end of the software maintenance release date was 29-December 2022.
...
Regardless of the licensing model (Smart Licensing or PAK) a CAP of 2500 secured/crypto sessions are applicable when upgraded
                                    to X14.2 release.

X14.2.5 is not supported on the Cisco TelePresence Video Communication Server (VCS) series. The end of the software maintenance
                                    release date was 29-December 2022.
...
#### get_edge_config response

From X14.1 release, get_edge_config response contains IP addresses instead of Fully Qualified Domain Name (FQDN).
...
For both platforms regardless of the licensing model (Smart Licensing or PAK) a CAP of 2500 secured/crypto sessions are applicable
...
This improvement displays the content of the Extensible Communications Platform (XCP) routing table. This content is a complete
...
Any customer has a fresh install of X14.2 image, ECDSA is being preferred. If you prefer RSA certificates over ECDSA, then
                                          prefix the cipher string with “ECDHE-RSA-AES256-GCM-SHA384:” using either Web User Interface ( Maintenance > Security > Ciphers ) or CLI command ( xConfiguration Ciphers ).
...
Due to improvements in the traffic server service on Expressway, beginning in X14.0.2, the following must be in place for
                        MRA.
...
From the list of bugs that appear, use the Filter drop-down list to filter on either Keyword , Modified Date , Severity , Status , or Technology .
...
## Appendix 1: Get More Information

There are additional resources you can access to get help and find more information.

### PID Details

The list of PIDs in the table below applies to both Unrestricted (Capped Version) and Restricted (Uncapped Version).
...
Restricted, can exceed 2500 signaling sessions

Products > Cisco Products > Unified Communications > Communications Gateways > Cisco Expressway Series > Cisco Expressway Select

A-SW-EXPWY-14XU-K9

Unrestricted has a cap of 2500 signaling sessions

Products > Cisco Products > Unified Communications > Communications Gateways > Cisco Expressway Series > Cisco Expressway

L-EXPWY-14.X-K9=

0$ PID, Restricted, can exceed 2500 signaling sessions for existing customers who need to upgrade to uncapped image
...
On CSSM, in the Create Registration Token page, the Allow export-controlled functionality on the products registered with this token . check box is not applicable to Expressway images.

Ensure the Quantity of 0$ PID should equal the number of nodes.

## Appendix 2: Accessibility and Compatibility Features

A Voluntary Product Accessibility Template (VPAT®) is a document that explains how information and communication technology
                     (ICT) products such as software, hardware, electronic content, and support documentation meet (conform to) the Revised 508
...
| Date | Change | Reason |
|---|---|---|
| July 2024 | Republished X14.2.7 | Updated the section "Other Changes in this Release." |
| June 2023 | Republished X14.2.7 | Expressway and Expressway Select X14.2.7 release. Made changes to the section "ESXi Requirements". Included a new section "Accessibility and Compatibility Features". |
| June 2023 | First publication for Expressway and Expressway Select X14.2.7 | Expressway and Expressway Select X14.2.7 release |
| May 2023 | First publication for Expressway Select X14.2.6 | Expressway Select X14.2.6 release |
| May 2023 | First publication for Expressway Select X14.2.5 | Expressway Select X14.2.5 release |
| April 2023 | First publication for X14.2.6 | X14.2.6 release |
...
| Note | VMware withdrew the following supported versions: ESXi 7.0 Update 3, 3a, and 3b due to critical issues identified with those
                                          builds. ( Reference: Link ). |
|---|---|

| Note | The CAP of 2500 secured/crypto sessions are also applicable to Cisco TelePresence Video Communication Server (VCS) Series. |
|---|---|

| Important | Ensure that the limited number of encrypted signaling is not more than 2500 endpoints per instance of Expressway. A customer that needs to exceed this limit may deploy additional peers/clusters
                                          if entitled, to provide additional capacity. CCO does not perform a "license determination check" . So existing customers will only have access to the limited/capped version. |
|---|---|
...
| Important | CCO does not perform a "license determination check" . So existing customers will only have access to the Export Unrestricted image. Users must submit a new $0 order (PID L-EXPWY-14.X-K9=)
                                    for an export-restricted image exceeding 2500 encrypted signaling sessions. |
|---|---|
...
| Note | Cisco Expressway Release X14.2 and later only supports Smart Licensing. Versions prior to X14.2.7 were capped at 2500 encrypted signalling sessions to endpoints. It also
                                       includes changes in the trafficserver behavior (bug ID CSCwc69661 refers) that can lead to MRA failures - see here . For detailed information, see the Cisco Expressway and Cisco TelePresence Video Communication Server Release Note (x14.2) and Cisco Expressway Administrator Guide (X14.2) before upgrading to X14.2 . |
|---|---|
...
| Note | The software download folder and path APPLY to both Unrestricted (Capped Version) and Restricted (Uncapped Version). |
|---|---|

| Important | Cisco Expressway is available in the software download folder on software.cisco.com Path: From the Downloads Home > Unified Communications > Communications Gateways > Expressway Series > Expressway . Or From the Downloads Home > Unified Communications > Communications Gateways > Expressway Series > Expressway Select . Select a Software Type - Expressway Core and Edge . |
|---|---|
...
|---|---|---|---|
| CAP of 2500 secured/crypto sessions | No | From X14.2.5, Expressway and Expressway Select is not supported on the Cisco TelePresence Video Communication Server (VCS)
                                    series. The end of the software maintenance release date was 29-December 2022. Cisco has announced end-of-sale and end-of-life dates for the Cisco TelePresence Video Communication Server (VCS) product. Details are available at the following link . |  |
...
|  | Cisco Expressway | Cisco TelePresence Video Communication Server (VCS) |
|---|---|---|
| CAP of 2500 secured/crypto sessions | Yes Regardless of the licensing model (Smart Licensing or PAK) a CAP of 2500 secured/crypto sessions are applicable when upgraded
                                    to X14.2 release. | X14.2.5 is not supported on the Cisco TelePresence Video Communication Server (VCS) series. The end of the software maintenance
                                    release date was 29-December 2022. Cisco has announced end-of-sale and end-of-life dates for the Cisco TelePresence Video Communication Server (VCS) product. Details are available at the following link . |
| Support Advanced Account Security (AAS) and FIPS140-2 Cryptographic Mode | No |
| Smart Licensing | Yes |
...
|---|---|---|---|
| CAP of 2500 secured/crypto sessions | Yes | Yes | For both platforms regardless of the licensing model (Smart Licensing or PAK) a CAP of 2500 secured/crypto sessions are applicable
                                    when upgraded to X14.2 release. |
...
| Note | The list of PIDs in the table below applies to both Unrestricted (Capped Version) and Restricted (Uncapped Version). The following PIDs A-SW-EXPWY-14X-K9 and A-SW-EXPWY-14XU-K9 are found under A-FLEX-3 PID. |
|---|---|
...
| A-SW-EXPWY-14XU-K9 | Unrestricted has a cap of 2500 signaling sessions | Products > Cisco Products > Unified Communications > Communications Gateways > Cisco Expressway Series > Cisco Expressway |
| L-EXPWY-14.X-K9= | 0$ PID, Restricted, can exceed 2500 signaling sessions for existing customers who need to upgrade to uncapped image | Products > Cisco Products > Unified Communications > Communications Gateways > Cisco Expressway Series > Cisco Expressway Select |
...
| Note | On CSSM, in the Create Registration Token page, the Allow export-controlled functionality on the products registered with this token . check box is not applicable to Expressway images. Ensure the Quantity of 0$ PID should equal the number of nodes. |
|---|---|