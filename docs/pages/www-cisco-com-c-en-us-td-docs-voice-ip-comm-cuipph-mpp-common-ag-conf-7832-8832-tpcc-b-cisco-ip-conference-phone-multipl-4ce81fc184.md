...
During a recording, your user can see different recording icons on the Calls screen. The icon depends on the recording state.
...
### Enable Automatic Group Call Pickup Notification

To enable automatic group call pickup notification, do the following:

#### Before you begin

Require access to the phone administration web page.
...
Select Voice > Ext n > XSI Line Service .

Step 3

Select Yes for the parameter User ExternalID .
...
| Step 4 | In the Programmable Softkeys section, to enable softkeys, add a string in this format in the Connected Key List and Conferencing Key List fields. crdstart;crdstop;crdpause;crdresume |
| Step 5 | Click the Ext(n) tab that requires call recording. |
| Step 6 | In the SIP Settings section, in the Call Recording Protocol , select SIPREC as the call recording protocol. You can configure this parameter in the phone configuration XML file (cfg.xml) by entering a string in this format: <Call_Recording_Protocol_3_ ua="na">SIPREC</Call_Recording_Protocol_3_> Options: SIPREC and SIPINFO Default: SIPREC |
...
| Step 4 | In the Programmable Softkeys section, to enable softkeys, add a string in this format in the Connected Key List and Conferencing Key List fields. crdstart;crdstop;crdpause;crdresume |
| Step 5 | Click the Ext(n) tab that requires call recording. |
| Step 6 | In the SIP Settings section, for the Call Recording Protocol parameter, select SIPINFO as the call recording protocol. You can configure this parameter in the phone configuration XML file (cfg.xml) by entering a string in this format: <Call_Recording_Protocol_1_ ua="na">SIPINFO</Call_Recording_Protocol_1_> Options: SIPREC and SIPINFO Default: SIPREC |
| Step 7 | Click Submit All Changes . |
