---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-15-0-1-configurati-d91cf8d5e2
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_15_0_1/configuration/guide/ucce_b_security-guide-for-cisco-unified-icm-contact-center-enterprise-release-15-0/certificate_management_for_secured_connections.html
retrieved_at: 2026-08-20T18:55:44.443656+00:00
---
...
The Unified CCE Certificate Monitoring service monitors the self-signed and CA-signed certificates and keys that are used
                                       for certificate management. The service alerts the system administrator about the validity and expiry of these certificates.
                                       For more information, see the Serviceability Guide for Cisco Unified Contact Center Enterprise at http://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html
...
The default host.key file is created during Unified CCE setup. The /generateCSR command reuses the existing key; therefore, you do not need to generate or replace host.key when requesting a CA-signed certificate. Do not modify, move, or replace this file unless the private key is compromised
                                                   and you are performing a planned certificate-and-key rotation.
...
/generateCSR The command creates a Certificate Signing Request (CSR) named host.csr . If host.key already exists, the command reuses that key. The command generates only the CSR; it does not generate, rename, or deploy
                                       the CA-signed application certificate. Submit host.csr to your Certificate Authority. After you receive the CA-signed application certificate:

Rename the CA-signed application certificate to host.pem

Manually copy host.pem to <install_drive>:\icm\ssl\certs .

Install the CA-signed certificate and the applicable root and intermediate CA certificates in the required Windows certificate
                                             stores by using the /install command described in step 5.
...
The Diagnostic Framework Certificate Manager utility is a command-line utility used to manage certificate creation and binding
                                 for the Diagnostic Portico. It is installed at <ICM_Drive>:\icm\serviceability\diagnostics\bin\DiagFwCertMgr.exe .

Step 1

Create a self-signed RSA or ECDSA certificate.

Step 2

Store the certificate in a local computer personal certificate store.
...
Validate the certificate binding to the HTTP service for the Diagnostic Framework service.
...
By default the RSA certificate will be enabled. However, the installer will also generate an ECDSA certificate, import it
                                             the local store, and update the ECDSA certificate hash in HKLM\SOFTWARE\Cisco Systems, Inc.\ICM\Serviceability\ DiagnosticFramework\SelfSignedCertECDSAForDiagFwSvc .
...
(Used by ICM-CCE Install)

BindCertFromStore

Looks up the certificate provided by /certhash argument in the certificate store and binds it with the HTTP service on the
                                                         given port.
...
Removes the certificate binding from the specified port. Also, delete the self-signed certificate created by the CreateAndBindCert option.
...
Removes the certificate binding from the specified port. Also, delete the self-signed ECDSA certificate created by CreateAndBindCertECDS option.

The Diagnostic Framework Certificate Manager utility stores the thumbprints  of the self-signed certificate created by the
                                             utility, and of the certificate used by the Diagnostic Framework service. The thumbprints are stored in the registry at the
                                             following locations:
...
For more information see, Serviceability Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html
...
## Enabling the Validation for Certificate Revocation

To enable the Certificate Revocation List (CRL) validation feature during the TLS handshake for Unified Config API interactions
                              with other solution components, set the RevocationCheckEnabled value of the Computer\HKEY_LOCAL_MACHINE\SOFTWARE\Cisco Systems, Inc.\ICM\Cisco SSL registry tree to true .

The scope of this validation rule is to check if the server's X.509 certificate is revoked or not.

While executing CRL, one of the following possible outcomes of the CRL check may happen:

CRL validation is enabled.

Server's certificate is revoked: TLS connection fails .

Server's certificate is not revoked: TLS connection succeeds .

CRL validation is disabled.

Server's certificate is revoked: TLS connection succeeds .

Server's certificate is not revoked: TLS connection succeeds .

The revocation checks that are supported are as follows:

Certificate Revocation List (CRL)

Online Certificate Status Protocol (OCSP)

During the execution, depending on the server's certificate, validation is either enabled or disabled.

You can enable CRL validation on the following components:
...
To disable CRL, set the RevocationCheckEnabled value of the Computer\HKEY_LOCAL_MACHINE\SOFTWARE\Cisco Systems, Inc.\ICM\Cisco SSL registry tree to false .

The Unified Config API WEB application modifies the registry value immediately Restarting or redeploying of this application
                              is not required.

## Manage Secured PII in Transit

The Contact Center Enterprise (CCE) solution handles customer sensitive Personally Identifiable Information (PII) that include
                              credit card information, PIN, and other sensitive details. Such sensitive information is sent across the system in ECC variables
...
CCE components support secure communication over existing TCP connections using TLS 1.2. When enabled, all inter-component
                              traffic—including MDS messaging, state transfer, data recovery, replication, and real-time distribution—is encrypted.
...
Secured agent desktop communications: To secure the communications with Cisco Finesse Server, enable mixed-mode connection in the CTI server. Next, enable secured
                                          connection in the Cisco Finesse Server, as applicable.
...
Secured CCE Internal Interface Communication:

To secure CCE internal interface communication, enable secured connection between the following components:

Router (Server) and PG (Client)

Router (Server) and Logger (Client)

Router (Server) and Administration and Data Server (Client)

Router (Server) and peer Router (Client)

Logger (Server) and peer Logger (Client)

Logger (Server) and Administration and Data Server (Client)

Administration and Data Server (Server) and Administration Client (Client)
...
For secured connection between Application Gateway Servers and Clients, see Configuration Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html

For secured multichannel connections, see: Configuration Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html .
...
### CCE Internal Interface Secure Connection

By default, TCP connections between CCE components (Router, Logger, Peripheral Gateway, Administration and Data Server, Administration
                              Client, Dialer) carry unencrypted traffic. Enabling secure communication wraps these connections with TLS 1.2, protecting
                              data in transit across both private (intra-data-center) and public (inter-data-center) network interfaces.

For more information about enabling secure connections to CCE components, see the following guides:

Add Components to Unified CCE Instance section in the Installation chapter of Cisco Unified Contact Center Enterprise Installation and Upgrade Guide at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-guides-list.html .

Enable Secure Connection Between CCE Components section in the Security Considerations chapter of Cisco Packaged Contact Center Enterprise Installation and Upgrade Guide at https://www.cisco.com/c/en/us/support/customer-collaboration/packaged-contact-center-enterprise/products-installation-guides-list.html .

To establish secure communication between CCE components, certificates must be exchanged and installed on all participating
                              components.

For more information, see the Secured CCE Internal Interface Communication row in the Secured Connection between Server and Client table of Manage Secured PII in Transit section in this guide.

Perform certificate exchange between the CCE components listed in the Server and Supported Client columns, and ensure the
                              certificates are properly installed on each respective component. Use the CiscoCertUtil utility to install the certificates. For more information detailed instructions, see the CiscoCertUtil Utility section in this guide.
...
Rename the CA-signed application certificate to host.pem . Manually copy host.pem to (<install_drive>:\icm\ssl\certs) and to the applicable algorithm-specific directories: (<install_drive>:\icm\ssl\rsa) or (<install_drive>:\icm\ssl\ecdsa) as applicable.
...
To revert to RSA or to bind with different curve certificate, see SSL Encryption Utility in the Security Guide for Cisco Unified Contact Center at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html

P384 curve is used when curve_name is not specified.

For CA certificate, the administrator can generate CSR and install self-signed certificates to the Windows store. For more
                                                            information, see Secured PII in Transit in the Security Guide for Cisco Unified Contact Center at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_1/configuration/guide/ucce_b_security-guide_12_6_1.html
...
For more information about DiagFwCertMgr utility , see Certificate Management in Serviceability Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html .
...
Enabling ECDSA is applicable for all the secured interfaces of Unified CCE and Packaged CCE. By default, ECDSA is disabled and advised to be enabled during maintenance windows.
...
For CA certificate, the administrators, can generate CSR and install the signed certificates to the Windows store. For more
                                                            information, see Manage Secured PII in Transit in Security Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_1/configuration/guide/ucce_b_security-guide_12_6_1.html
...
utils permitlist admin_ui add utils permitlist admin_ui add
...
## Manage SQL Server Certificate

The SQLServerTLS.ps1 PowerShell script manages certificate binding for Microsoft SQL Server. The script supports both self-signed and CA-signed
                           certificates and automates the manual steps otherwise required to bind a certificate to SQL Server.

Before you begin:

Ensure that you install ICM_15.0.1_ES202607 or a later cumulative ES release. The SQLServerTLS.ps1 PowerShell script is available as part of this installer. This script is invoked on the CCE core components that have SQL
                                 installed, such as the Logger and Administration and Data Server (AW).

Ensure that the required certificate is already available at the following location on the node: <ICMInstall drive>\ssl\certs\host.pem .

Log in with an account that has administrator privileges.

After installation, the system binds the certificate to SQL Server automatically by default. Administrators can later run
                           the script manually  to view, update, or disable the certificate binding.
...
Run the script with the required parameter to manage the SQL Server certificate binding:

To enable certificate binding:

```
.\SQLServerTLS.ps1 -enable
```

The -enable parameter enables certificate binding for SQL Server.

To reimport or rebind TLS certificate:

```
.\SQLServerTLS.ps1 -enable -force
```

The -force parameter rebinds or reimports the TLS certificate to SQL Server.

Ensure that you restart SQL Server after rebinding the TLS certificate.

To restart the SQL Server:

```
.\SQLServerTLS.ps1 -enable -RestartSQL
```

The -RestartSQL parameter automatically restarts the SQL Server service after the certificate binding is enabled.

To disable certificate binding:

```
.\SQLServerTLS.ps1 -disable
```

The -disable parameter disables the SQL Server certificate binding.

Ensure that you restart SQL Server after certificate binding.

To restart the SQL Server

```
.\SQLServerTLS.ps1 -disable -RestartSQL
```

The -RestartSQL parameter automatically restarts the SQL Server service after the certificate binding is disabled.

To view the current certificate binding status:

```
.\SQLServerTLS.ps1 -status
```

The -status parameter displays the certificate currently bound to SQL Server.

### Automatic Certificate Rebinding

If the certificate type changes from RSA to ECDSA or from ECDSA to RSA, the Cisco CertMon service automatically triggers the SQLServerTLS.ps1 script and binds the new certificate to SQL Server on that node.
...
| Note | The Unified CCE Certificate Monitoring service monitors the self-signed and CA-signed certificates and keys that are used
                                       for certificate management. The service alerts the system administrator about the validity and expiry of these certificates.
                                       For more information, see the Serviceability Guide for Cisco Unified Contact Center Enterprise at http://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html |
|---|---|
...
| Note | The default host.key file is created during Unified CCE setup. The /generateCSR command reuses the existing key; therefore, you do not need to generate or replace host.key when requesting a CA-signed certificate. Do not modify, move, or replace this file unless the private key is compromised
                                                   and you are performing a planned certificate-and-key rotation. |
|---|---|
...
| Step 1 | Create a self-signed RSA or ECDSA certificate. |
|---|---|
| Step 2 | Store the certificate in a local computer personal certificate store. |
...
| Step 6 | Validate the certificate binding to the HTTP service for the Diagnostic Framework service. The following section explains the usage of the utility: DiagFwCertMgr /task:<task_name> [/port:<port_number>] [/certhash:<certificate_thumbprint>] [/logpath:<logfile_path>] Where: /task: specifies the task to be performed. /port: specifies the port number used by the service. This is optional as the port number is automatically read from the service
                                                   ...
                                                   certificate store, to a port. /logpath: specifies the path where the log file should be created. By default, it is the current folder. By default the RSA certificate will be enabled. However, the installer will also generate an ECDSA certificate, import it
                                             the local store, and update the ECDSA certificate hash in HKLM\SOFTWARE\Cisco Systems, Inc.\ICM\Serviceability\ DiagnosticFramework\SelfSignedCertECDSAForDiagFwSvc . To enable ECDSA, you must use the BindCertFromStore command with the /certhash argument with the ECDSA hash available in the registry. For more commands, refer to the table below. Table 1. Diagnostic Framework Certificate Manager Utility Tasks Task Description CreateAndBindCert Creates a self-signed certificate in the local computer personal certificate store and binds it with HTTP service on the given
                                                         port. (Used by ICM-CCE Install) BindCertFromStore Looks up the certificate provided by /certhash argument in the certificate store and binds it with the HTTP service on the
                                                         given port. UnbindCert Removes the certificate binding from the specified port, does not modify any certificate in the store. UnbindAndDeleteCert Removes the certificate binding from the specified port. Also, delete the self-signed certificate created by the CreateAndBindCert option. (Used by ICM-CCE Uninstall) ValidateCertBinding Verifies the certificate binding on the specified port and confirms its presence in the local computer certificate store. CreateAndAddToStoreCertECDSA Creates and stores the self-signed ECDSA certificate in the local computer certificate store without binding it to the port. CheckAndCreateStoreCertECDSA Checks if the certificate is present in the store and creates only if it is NOT present. CreateAndBindCertECDSA Creates a self-signed ECDSA certificate in the local computer certificate store and binds it with HTTP service on the given
                                                         port. DeleteCertECDSA Deletes the self-signed ECDSA certificate. UnbindAndDeleteCertECDSA Removes the certificate binding from the specified port. Also, delete the self-signed ECDSA certificate created by CreateAndBindCertECDS option. The Diagnostic Framework Certificate Manager utility stores the thumbprints  of the self-signed certificate created by the
                                             utility, and of the certificate used by the Diagnostic Framework service. The thumbprints are stored in the registry at the
                                             following locations: HKLM\SOFTWARE\Cisco Systems, Inc.\ICM\Serviceability\ DiagnosticFramework\SelfSignedCertCreatedForDiagFwSvc HKLM\SOFTWARE\Cisco Systems, Inc.\ICM\Serviceability\ DiagnosticFramework\CertUsedByDiagFwSvc Note For more information see, Serviceability Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html | Task | Description | CreateAndBindCert | Creates a self-signed certificate in the local computer personal certificate store and binds it with HTTP service on the given
                                                         port. (Used by ICM-CCE Install) | BindCertFromStore | Looks up the certificate provided by /certhash argument in the certificate store and binds it with the HTTP service on the
                                                         given port. | UnbindCert | Removes the certificate binding from the specified port, does not modify any certificate in the store. | UnbindAndDeleteCert | Removes the certificate binding from the specified port. Also, delete the self-signed certificate created by the CreateAndBindCert option. (Used by ICM-CCE Uninstall) | ValidateCertBinding | Verifies the certificate binding on the specified port and confirms its presence in the local computer certificate store. | CreateAndAddToStoreCertECDSA | Creates and stores the self-signed ECDSA certificate in the local computer certificate store without binding it to the port. | CheckAndCreateStoreCertECDSA | Checks if the certificate is present in the store and creates only if it is NOT present. | CreateAndBindCertECDSA | Creates a self-signed ECDSA certificate in the local computer certificate store and binds it with HTTP service on the given
                                                         port. | DeleteCertECDSA | Deletes the self-signed ECDSA certificate. | UnbindAndDeleteCertECDSA | Removes the certificate binding from the specified port. Also, delete the self-signed ECDSA certificate created by CreateAndBindCertECDS option. | Note | For more information see, Serviceability Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html |
| Task | Description |
| CreateAndBindCert | Creates a self-signed certificate in the local computer personal certificate store and binds it with HTTP service on the given
                                                         port. (Used by ICM-CCE Install) |
| BindCertFromStore | Looks up the certificate provided by /certhash argument in the certificate store and binds it with the HTTP service on the
                                                         given port. |
| UnbindCert | Removes the certificate binding from the specified port, does not modify any certificate in the store. |
| UnbindAndDeleteCert | Removes the certificate binding from the specified port. Also, delete the self-signed certificate created by the CreateAndBindCert option. (Used by ICM-CCE Uninstall) |
...
| UnbindAndDeleteCertECDSA | Removes the certificate binding from the specified port. Also, delete the self-signed ECDSA certificate created by CreateAndBindCertECDS option. |
| Note | For more information see, Serviceability Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html |
...
| Secured agent desktop communications: To secure the communications with Cisco Finesse Server, enable mixed-mode connection in the CTI server. Next, enable secured
                                          connection in the Cisco Finesse Server, as applicable. | CTI Server | Cisco Finesse | GED 188 |
|  |
...
| Secured CCE Internal Interface Communication: To secure CCE internal interface communication, enable secured connection between the following components: Router (Server) and PG (Client) Router (Server) and Logger (Client) Router (Server) and Administration and Data Server (Client) Router (Server) and peer Router (Client) Logger (Server) and peer Logger (Client) Logger (Server) and Administration and Data Server (Client) Administration and Data Server (Server) and Administration Client (Client) | Router | PG | TCP |
| Logger |
| Administration Client |
| Administration and Data Server |
| Peer Router (Side A or B) |
| Logger | Peer Logger (Side A or B) |
| Administration and Data Server |
| Administration and Data Server | Administration Client |
| Cisco Finesse | Administration and Data Server |
...
| Step 2 | To verify, in IIS Manager server , go to Site
                                             ...
                                             binded. Note ECDSA certificates can be generated and bound only with the
                                                                  ...
                                                                  manager) to generate and bind to IIS port 443. To revert to RSA or to bind with different curve certificate, see SSL Encryption Utility in the Security Guide for Cisco Unified Contact Center at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html Note P384 curve is used when curve_name is not specified. For CA certificate, the administrator can generate CSR and install self-signed certificates to the Windows store. For more
                                                            information, see Secured PII in Transit in the Security Guide for Cisco Unified Contact Center at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_1/configuration/guide/ucce_b_security-guide_12_6_1.html | Note | ECDSA certificates can be generated and bound only with the
                                                                  command line option. For CA certificate, the administrator can use Windows tools (IIS
                                                                  manager) to generate and bind to IIS port 443. To revert to RSA or to bind with different curve certificate, see SSL Encryption Utility in the Security Guide for Cisco Unified Contact Center at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html | Note | P384 curve is used when curve_name is not specified. For CA certificate, the administrator can generate CSR and install self-signed certificates to the Windows store. For more
                                                            information, see Secured PII in Transit in the Security Guide for Cisco Unified Contact Center at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_1/configuration/guide/ucce_b_security-guide_12_6_1.html |
| Note | ECDSA certificates can be generated and bound only with the
                                                                  command line option. For CA certificate, the administrator can use Windows tools (IIS
                                                                  manager) to generate and bind to IIS port 443. To revert to RSA or to bind with different curve certificate, see SSL Encryption Utility in the Security Guide for Cisco Unified Contact Center at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html |
| Note | P384 curve is used when curve_name is not specified. For CA certificate, the administrator can generate CSR and install self-signed certificates to the Windows store. For more
                                                            information, see Secured PII in Transit in the Security Guide for Cisco Unified Contact Center at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_1/configuration/guide/ucce_b_security-guide_12_6_1.html |
...
| Step 4 | After you bind, validate the command DiagFwCertMgr/task:ValidateCertBinding and confirm from
                                             the output if the certificate binding with the current port is valid. For more information about DiagFwCertMgr utility , see Certificate Management in Serviceability Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html . |
...
| Step 3 | The system will restart the Unified CCE. Once the reboot is complete, ECDSA
                                             mode is enabled. Note ECDSA will be enabled only after the system restart. System restart time is as per RebootWaitSecs (default is 5 mins).
                                                                  This can be modified. Note For CA certificate, the administrators, can generate CSR and install the signed certificates to the Windows store. For more
                                                            information, see Manage Secured PII in Transit in Security Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_1/configuration/guide/ucce_b_security-guide_12_6_1.html To disable ECDSA, set the ECDSAEnabled Registry flag to False . The system will automatically activate the RSA
                                                            certificate, available under <install_Drive>
...
| Note | ECDSA will be enabled only after the system restart. System restart time is as per RebootWaitSecs (default is 5 mins).
                                                                  This can be modified. |
| Note | For CA certificate, the administrators, can generate CSR and install the signed certificates to the Windows store. For more
                                                            information, see Manage Secured PII in Transit in Security Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_1/configuration/guide/ucce_b_security-guide_12_6_1.html To disable ECDSA, set the ECDSAEnabled Registry flag to False . The system will automatically activate the RSA
                                                            certificate, available under <install_Drive>
                                                               :\icm\ssl\rsa . |
...
| Note | Ensure that you restart SQL Server after rebinding the TLS certificate. |
|---|---|

| Note | Ensure that you restart SQL Server after certificate binding. |
|---|---|
