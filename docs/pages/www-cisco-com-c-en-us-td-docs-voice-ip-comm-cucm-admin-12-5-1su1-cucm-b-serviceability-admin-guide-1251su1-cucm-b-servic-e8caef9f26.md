---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-admin-12-5-1su1-cucm-b-serviceability-admin-guide-1251su1-cucm-b-servic-e8caef9f26
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/admin/12_5_1SU1/cucm_b_serviceability-admin-guide-1251su1/cucm_b_serviceability-admin-guide-1251su1_chapter_0111.html
retrieved_at: 2026-08-21T01:14:03.722036+00:00
---
...
Detailed audit logging is an optional feature that logs additional configuration modifications that are not stored in standard
                                 (default) audit logs. In addition to all of the information that is stored in standard audit logs, detailed audit logging
                                 also includes configuration items that were added, updated, and deleted, including the modified values. Detailed audit logging
                                 is disabled by default, but you can enable it in the Audit Log Configuration window.
...
To set a TLS connection, a security certificate has to be uploaded from the syslog server to the tomcat trust store on Unified
...
| Step 3 | If you need to
                                             			 change the protocol on this node, do the following: To configure TCP, run the utils
                                                   				  ...
                                                   				  remotesyslog set protocol udp command. To configure TLS, run the utils remotesyslog set protocol tls command. To set a TLS connection, a security certificate has to be uploaded from the syslog server to the tomcat trust store on Unified
                                                   Communications Manager and IM and Presence service. Note In Common Criteria Mode, strict host name verification is implemented. Hence, it is required to configure the server with
