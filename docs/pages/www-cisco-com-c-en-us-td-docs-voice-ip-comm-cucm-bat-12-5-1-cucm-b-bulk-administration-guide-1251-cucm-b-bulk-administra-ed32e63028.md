...
Problem MAC address values are not allowed in the file if dummy MAC address values are desired.

Possible Cause The CSV file contains MAC addresses. You cannot provide dummy MAC addresses when MAC addresses are present in any row in the CSV file.

Solution If you want to use dummy MAC addresses, create a new CSV file that contains only those records for which you have not specified MAC addresses. Alternatively,
                                 you can specify MAC addresses in the CSV file and not check the Create Dummy MAC Address check box.
...
For Cisco Unified
                                             			 IP phones (models 7905, 7911, 7912, 7941, 7961, 7970, and 7971), the migration
                                             			 activity works correctly, and the phones register as SIP after migration.
...
Solution Enter some dummy value for the last column while creating the CSV file.

Solution Save the CSV file and close it.

Solution Open the saved CSV file using Text Editor/Notepad.

Solution Replace the dummy values with a blank value and save it.
