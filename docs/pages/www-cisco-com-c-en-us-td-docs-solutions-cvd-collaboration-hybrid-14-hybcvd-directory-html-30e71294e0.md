...
Webex Hybrid Services enable Webex customers to connect on-premises collaboration services to Webex. Integrating directory services between the on-premises LDAP directory and the identity service within the customer's Webex organization adds value by simplifying user on-boarding.

## Overview

The Webex Hybrid Directory Service high-level architecture, depicted in Figure 2-1 , allows Webex customers to synchronize their corporate Microsoft Active Directory with the identity store of their organization in Webex. This makes Webex user on-boarding and service provisioning simple and consistent.

Figure 2-1 Webex Hybrid Directory Service High-Level Architecture
...
The core components for Webex Hybrid Directory Service include:
...
- Ensure that the end-user account mail ID field in the Unified CM End User database contains the user's email address. Webex App users correlate to Cisco Unified CM end users by means of email addresses. With LDAP directory integration, the mail ID field for Unified CM end users is typically mapped from the mail field of the LDAP directory during synchronization.
...
- Synchronization of identities, users, resources, and groups from the corporate Microsoft Active Directory to the cloud, and the creation of Webex user accounts from this corporate directory source.
...
Highly available network connectivity to the Internet is also required to ensure that Webex services are reachable from the enterprise. Redundant physical Internet connections, preferably from different providers, are recommended.
...
2. Enable directory synchronization and download Cisco Directory Connector software from the Control Hub.
...
Log into Control Hub at https://admin.webex.com from the web browser on the Windows Server host you deployed in Step 1. Use your Webex organization administrator credentials.
...
Launch Cisco Directory Connector and sign into the Webex organization by entering the email address and password of an administrator account for the organization. Note that this is the same email address and password used to log into the Control Hub management portal. Click to confirm the Webex organization and domain.
...
Enter a name for the Directory Connector (for example, DIRSYNC1). This is the name that will be displayed on the dashboard and on the Control Hub web portal.
...
After the enterprise directory user information has been propagated to Webex, the administrator is able to provision users for cloud services and manage those service features and settings by using Control Hub.

Use administrator credentials for your Webex organization to log into Control Hub at https://admin.webex.com from a web browser.

On Control Hub, begin managing and provisioning user services by navigating to Users and then clicking Manage Users . Once directory synchronization is enabled, there are multiple ways to modify users and the services they use. Users can be modified individually or in bulk.
...
|---|---|---|
| General | Connector Name | Enter a name for the Directory Connector (for example, DIRSYNC1). This is the name that will be displayed on the dashboard and on the Control Hub web portal. |
