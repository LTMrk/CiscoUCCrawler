---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-12-6-1-configurati-aaeee0353b
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_1/configuration/guide/ucce_b_features-guide-1261/rcct_m_vpnless_1261es04_appendix.html
retrieved_at: 2026-08-16T20:13:03.963534+00:00
---
...
Nginx reverse-proxy produces many logs. Configure log rotation to ensure optimum use of disk space, else the logs fill up
                                    the disk. The steps to configure log rotation is as follows:

Copy the configuration file from /usr/local/openresty/nginx/logrotate/saproxy to /etc/logrotate.d/reverseproxy .

Ensure that there’s a file in the /etc/cron.daily/logrotate . This does the log rotation daily based on the configuration in the /etc/logrotate.d/reverseproxy .

If log rotation has to be done more frequently, such as every hour, copy the configuration file from /etc/cron.daily/logrotate to /etc/cron.hourly/logrotate .

Log files under /usr/local/openresty/nginx/logs/ are rotated based on the configuration in /etc/logrotate.d/reverseproxy . That is, rotate the log when the file size exceeds 100 MB and keep no more than 20 most recently rotated files.
...
Reverse-proxy supports maximum of 8000 folders (including subdirectories) in the finesse/3rdpartygadget folder.
...
|---|---|
| Step 2 | Check if the <host> values in both <primaryNode> and <secondaryNode> are valid in reverse-proxy hostnames. It should not be Finesse hostnames. Note If CORS status is "enabled", you must explicitly add the reverse-proxy domain name to the list of CORS trusted domain names. Reverse-proxy supports maximum of 8000 folders (including subdirectories) in the finesse/3rdpartygadget folder. | Note | If CORS status is "enabled", you must explicitly add the reverse-proxy domain name to the list of CORS trusted domain names. Reverse-proxy supports maximum of 8000 folders (including subdirectories) in the finesse/3rdpartygadget folder. |
| Note | If CORS status is "enabled", you must explicitly add the reverse-proxy domain name to the list of CORS trusted domain names. Reverse-proxy supports maximum of 8000 folders (including subdirectories) in the finesse/3rdpartygadget folder. |
