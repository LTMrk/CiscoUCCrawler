---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-expressway-config-guide-x15-0-cert-creation-use-exwy-b-cisco-expressway-cert-b493c997a3
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/expressway/config_guide/X15-0/cert-creation-use/exwy_b_cisco-expressway-certificate-creation-and-use-deployment-guide-x150/exwy_m_using-acme-on-expressway-e.html
retrieved_at: 2026-08-16T15:13:22.135860+00:00
---
...
### Let's Encrypt Verification Process

From June 2024, Let's encrypt uses any of the ECDSA or RSA certificates.

Currently, four intermediates in active rotation is maintained.

Subscriber certificates containing an ECDSA public key will be issued from one of the ECDSA intermediates.

Similarly, Subscriber certificates containing an RSA public key will be issued from one of the RSA intermediates.
...
The virtual host only listens on one unprivileged port on its localhost interface. The virtual host is hardened in the usual
                                 way. It denies: directory browsing, symbolic links, all options, and usage of .htaccess files. Due to this, the HTTP to HTTPS
                                 redirect is only supported if the Web Administration port for Expressway E is configured as the default 443 port.

If the Expressway-E is configured to redirect port 80 to 443:

We add an exception to the 80 to 443 redirect rule for ACME challenge traffic. This exception is added automatically in the
                                       background and cannot be manually configured.
...
Go to https://letsencrypt.org/certificates/#root-cas .
...
Go to https://letsencrypt.org/certificates/#subordinate-intermediate-cas .
...
A generated CSR for a cluster should look like:

Subject Common Name = FQDN of cluster

Subject Alternate Name = FQDN of only this Expressway peer (generating this CSR), FQDN of cluster

Warning

Ensure you do NOT include every cluster node as a Subject Alternate Name (SAN).

### Sign the CSR using ACME Provider

Cluster FQDNs and collab-edge.externaldomain.com records should resolve each Expressway peer's public IP address, and multiple records pointing to different Expressway(s)
                                             is acceptable.
...
| Note | From June 2024, Let's encrypt uses any of the ECDSA or RSA certificates. Currently, four intermediates in active rotation is maintained. Subscriber certificates containing an ECDSA public key will be issued from one of the ECDSA intermediates. Similarly, Subscriber certificates containing an RSA public key will be issued from one of the RSA intermediates. |
|---|---|
...
| Step 1 | Go to https://letsencrypt.org/certificates/#root-cas . |
|---|---|
...
| Step 1 | Go to https://letsencrypt.org/certificates/#subordinate-intermediate-cas . |
|---|---|
...
| Important | A generated CSR for a cluster should look like: Subject Common Name = FQDN of cluster Subject Alternate Name = FQDN of only this Expressway peer (generating this CSR), FQDN of cluster |
|---|---|

| Warning | Ensure you do NOT include every cluster node as a Subject Alternate Name (SAN). |
|---|---|

| Note | Cluster FQDNs and collab-edge.externaldomain.com records should resolve each Expressway peer's public IP address, and multiple records pointing to different Expressway(s)
                                             is acceptable. |
|---|---|
