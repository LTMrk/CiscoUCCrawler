...
You can configure Unified CCE and Enterprise Chat and
                                    						Email so that Unified CCE routes Web Collaboration requests that are processed by Enterprise Chat and
                                    						Email .

### Overview of ECE Web Request Routing Through Unified CCE

With the ECE integrated with Unified CCE , Unified CCE can route chat requests.

The process for routing Web requests through Unified CCE can be divided into 5 parts, as shown in the following image:
...
This script is scheduled to run for web chat requests. For more information about call types and scheduling Web requests routing
                                       scripts, see Example - How Unified CCE Determines the Call Type for Web Request .

The script queues the request to a chat skill group. At this time, Unified CCE attempts to find an available agent who is a member of that skill group. When the agent is found, the system software returns
                                       the agent ID to the Cisco Interaction Manager.
...
The script queues the request to a chat skill group. At this time, Unified CCE attempts to find an available agent who is a member of that skill group. When the agent is found, Unified CCE returns the agent ID to the Enterprise Chat and
                                             						Email .
...
The script queues the request to the appropriate skill group for that type of request. At this time, Unified CCE attempts to find an available agent who is a member of that skill group. When the agent is found, the system software returns
                                       the agent ID to the Cisco Interaction Manager.

## Example Enterprise Chat and
                              						Email - Email Scripts

You can configure Unified CCE and the Enterprise Chat and
                                    						Email so that the system software routes email messages that are processed by Enterprise Chat and
                                    						Email .

For more information about configuring Unified CCE and Enterprise Chat and
                                    						Email in an integrated environment, see the following documents:

Configuration Guide for Cisco Unified Contact Center Enterprise
...
### Overview of ECE E-mail Routing Through Unified CCE

When the ECE is in an integrated environment, the administrator can choose to defer message assignment to Unified CCE . The administrator determines if skill groups can initiate message routing through the system software when the skill groups
                                 are created. The administrator then defines rules to assign messages to those Unified CCE Routing skill groups.

In this situation:

A message enters the ECE .

A rule assigns the message to a Unified CCE Routing skill group.

ECE sends a request for assignment to Unified CCE .
...
Based on the routing script's determination, the Unified CCE instructs the ECE to assign the message to a particular agent or ECE routing skill group.

The message is placed in the agent's or skill group's queue.

If the message is assigned to an agent by the Unified CCE , it is presented to the agent in push mode; if the message is assigned to an ECE routing skill group, it is placed in that skill group's queue.
...
The script is run when a e-mail message is assigned to a Unified CCE Routing skill group. This script is scheduled to run for messages assigned to that skill group. For more information about
                                       call types and scheduling e-mail routing scripts, see Example - How Unified CCE Determines the Call Type for an E-Mail contact .

The script queues the request to the skill group. At this time, the Unified CCE attempts to find an available agent who is a member of that skill group. When the agent is found, the system software instructs
                                       the Enterprise Chat and
                                             						Email to push the e-mail message to that agent, so the agent can respond to the customer.
...
The script is run when a e-mail message is assigned by the Enterprise Chat and
                                             						Email to a Unified CCE Routing skill group. This script is scheduled to run for messages in that skill group. For more information on call types
                                       and scheduling e-mail routing scripts, see Example - How Unified CCE Determines the Call Type for an E-Mail Contact .
...
If the message is not urgent, control passes to the Queue to Skill Group node. The Unified CCE attempts to find an available agent who is a member of that skill group. When the agent is found, the Unified CCE instructs the Enterprise Chat and
...
To configure these ECC variables in Configuration Manager's Expanded Call Variable List Tool, see the Configuration Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html .
...
The script is run when the Enterprise Chat and Email assigns an email message to a Unified CCE Routing Skill Group.
...
If the IF node evaluates to False, control passes to path of Push routing. The script queues the request to the skill group. At this
                                       time, Unified CCE attempts to find an available agent who is a member of that skill group. When an agent is found, the system software instructs
                                       the Enterprise Chat and Email to push the e-mail message to that agent, so the agent can respond to the customer.
...
The script queues the task request to the skill group. At this time, Unified CCE attempts to find an available agent who is a member of that skill group. Unified CCE runs an external script to notify the customer an estimated wait time. When the agent is found, the system software instructs
...
Following are example scripts for use when the Unified CCE is part of an Outbound environment:
...
| Note | In a Unified CCE integrated with the ECE , the allowed responses for an invoked route request are: Return an Agent, Return a Label, or Failure (the script could not
...
| Note | To assign a message directly to an agent, the Unified CCE script must use the Queue to Agent node to route e-mail messages, not the Agent node. |
|---|---|
...
| Note | These Expanded Call Context (ECC) variables are optional. Before you use these ECC variables, configure them in Unified CCE
                                             and ECE. To configure these ECC variables in Configuration Manager's Expanded Call Variable List Tool, see the Configuration Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html . To configure these ECC variables in ECE, see at https://www.cisco.com/c/en/us/support/customer-collaboration/cisco-enterprise-chat-email/products-maintenance-guides-list.html . |
|---|---|
