---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-admin-12-5-1-systemconfig-cucm-b-system-configuration-guide-1251-cucm-b-833bf843cc
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/admin/12_5_1/systemConfig/cucm_b_system-configuration-guide-1251/cucm_b_system-configuration-guide-1251_chapter_01011100.html
retrieved_at: 2026-08-16T17:32:10.467460+00:00
---
...
When you provision with dummy MAC addresses, activation codes are tied to the phone model. You must enter an activation code that matches the phone model
                                          in order to activate the phone.
...
| Note | When you provision with dummy MAC addresses, activation codes are tied to the phone model. You must enter an activation code that matches the phone model
                                          in order to activate the phone. For added security, you can provision the phone with the actual MAC address of the phone. This option involves more configuration
                                          because the administrator must gather and input each phone's MAC address during provisioning, but provides greater security
                                          because users must enter the activation code that matches the actual MAC address on their phone. |
|---|---|
...
|---|---|
| Step 2 | In the Device Defaults Configuration window, select the device type that will use activation codes for registration in the Dual Bank Information section, and change Onboarding Method from Auto Registration to Activation Code . |
...
| Step 8 | Click Add . |
| Step 9 | Check the Requires Activation Code for Onboarding check box. |
...
| Step 5 | Check the Require Activation Code for Onboarding check box. |
...
| Step 5 | Add your new phone details to the spreadsheet. If you are using dummy MAC addresses, leave the MAC Address field empty. |
...
| Re-onboarding phones via autoregistration | You can switch onboarding methods for specific phone models between Activation Codes and Autoregistration via the Onboarding Method field in the Device Defaults Configuration window. Note If you want to re-onboard an existing phone via autoregistration, you must delete the existing record from the database for
