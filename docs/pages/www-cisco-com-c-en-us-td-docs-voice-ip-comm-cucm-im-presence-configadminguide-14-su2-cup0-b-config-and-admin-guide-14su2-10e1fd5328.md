---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-im-presence-configadminguide-14-su2-cup0-b-config-and-admin-guide-14su2-10e1fd5328
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/im_presence/configAdminGuide/14_su2/cup0_b_config-and-admin-guide-14su2/cup0_b_config-and-admin-guide-1401_chapter_010110.html
retrieved_at: 2026-08-16T16:18:20.760683+00:00
---
...
Use this procedure to limit the persistent chat room creation within Cisco Jabber users home cluster.
...
Granting Owner Status to Persistent Chat Rooms

Use this procedure to transfer ownership of the persistent chat rooms belonging to the home cluster to any other existing
...
Use this procedure to view the chat rooms count on the own and peer cluster aliases present in the external database.
...
This feature is applicable from release 14 SU1 onwards.

Use this procedure to limit the persistent chat room creation within Cisco Jabber users home cluster. This feature reduces
                                 the inter-cluster traffic and increases the system bandwidth.

The IM and Presence Service administrator manages all the chat rooms created by the users on home cluster. The maintenance
                                 activity of other clusters does not impact chat rooms created by users in home clusters.
...
Before enabling this feature, check the Alias Report on Group Chat and Persistent Chat Settings window. For more information see, Persistent Chat Alias Report .

Cisco Jabber 14.1 version or higher is needed to support this feature.

Step 1

Log in to Cisco Unified CM IM and Presence Service Administration on the database publisher node.
...
Under Enable Persistent Chat , check the Limit room creation to home cluster check box.
...
Use this procedure for the IM and Presence Service administrators who have the access to the GUI to grant owner status to
                                 persistent chat rooms.

For example, John created a persistent chat room and had added a few members, and later leaves the organization. If John was
                                 the only persistent chat room owner, and the room owner capabilities are still required for the given room, the IM and Presence
                                 Service administrator can select and add one or more current room members to the list of owners.

Consider the following while updating the Owner ID :

You can grant owner status to a chat room only through the IM and Presence Service cluster that hosts the chat room.
...
The administrator cannot set the room creator's ID as the new owner ID of the chat room.

To grant the owner status to a persistent chat room, perform the following steps:
...
Log into Cisco Unified Communications Manager IM and Presence Service Administration on the database publisher node.
...
Under Persistent Chat Database Assignment , click Room Report .

Step 4

Use the filter tools if you want to limit the selection to rooms that meet specific criteria and click Find .

Step 5

(Optional) Click on a Room JID to view the fields of persistent chat rooms such as the list of owners, list of members, and the date of the last message.
                                          See the online help for more information and description about the fields.

Step 6

Select a check box from the Room JID to edit the Owner ID field.

The Owner ID column is editable only for the persistent chat rooms belonging to the home cluster.

Step 7

Enter the Owner ID in E-mail format of the chat room member to whom you want to grant the owner status.

Step 8

Click Update Owner ID .
...
Use this procedure to view the External Database Persistent Chat alias report which lets you view the chat rooms count and
                                 the home and peer cluster aliases present in the external database.

Step 1

Log into Cisco Unified CM IM and Presence Service Administration on the database publisher node.
...
Under Persistent Chat Database Assignment , select External Database from the drop-down list.

Step 4

Click Alias Report button. Refer to the online help for field descriptions.
...
| Step 3 | Limit Persistent Chat Room Creation to Home Cluster | Use this procedure to limit the persistent chat room creation within Cisco Jabber users home cluster. |
| Step 4 | View External Database Text Conferencing Report | Use this procedure to view the External Database Text Conferencing Report that lets you view details on the persistent chat
                                          rooms. |
| Step 5 | Granting Owner Status to Persistent Chat Rooms | Use this procedure to transfer ownership of the persistent chat rooms belonging to the home cluster to any other existing
                                          member of the chat room. |
| Step 6 | Persistent Chat Alias Report | Use this procedure to view the chat rooms count on the own and peer cluster aliases present in the external database. |
| Step 7 | Edit chat room settings. Complete any of the following tasks, in any order, to update chat room settings: | Note If you update any of the Persistent Chat settings, on Cisco Unified IM and Presence Serviceability, choose Tools > Control Center - Feature Services to restart the Cisco XCP Text Conference Manager service. | Note | If you update any of the Persistent Chat settings, on Cisco Unified IM and Presence Serviceability, choose Tools > Control Center - Feature Services to restart the Cisco XCP Text Conference Manager service. |
...
| Step 8 | Reset Chat Rooms to System Defaults | Complete this optional task if you want to reset your chat configuration to the system defaults. Be aware that ad hoc chat
                                          is enabled by default, but persistent chat is disabled by default. Completing this task would disable persistent chat. |
| Step 9 | Manage Chat Node Aliases | Aliases create a unique address for each chat node so that users (in any domain) can search for specific chat rooms on specific
                                          nodes, and join chat in those rooms. Each chat node in a system must have a unique alias. |
| Step 10 | Clean External Database for Persistent Chat | Optional. Use the External Database Cleanup Utility to configure jobs that monitor the external database and delete expired
                                          records. This will ensure that there is always enough disk space for new records. |
...
| Step 3 | Under Persistent Chat Database Assignment , click Room Report . |
| Step 4 | Use the filter tools if you want to limit the selection to rooms that meet specific criteria and click Find . |
| Step 5 | (Optional) Click on a Room JID to view the fields of persistent chat rooms such as the list of owners, list of members, and the date of the last message.
                                          See the online help for more information and description about the fields. |
| Step 6 | Select a check box from the Room JID to edit the Owner ID field. Note The Owner ID column is editable only for the persistent chat rooms belonging to the home cluster. | Note | The Owner ID column is editable only for the persistent chat rooms belonging to the home cluster. |
| Note | The Owner ID column is editable only for the persistent chat rooms belonging to the home cluster. |
| Step 7 | Enter the Owner ID in E-mail format of the chat room member to whom you want to grant the owner status. |
