...
At any point, when you are requested to  insert CD or DVD media, you must mount the ISO file through the vSphere client for
                                       the VMWare server. See "Adding DVD or CD Drives to a Virtual Machine" here for guidance.

The security password on all nodes in a cluster must match. Change the security password on all machines, or the cluster
                                       nodes will not communicate.
