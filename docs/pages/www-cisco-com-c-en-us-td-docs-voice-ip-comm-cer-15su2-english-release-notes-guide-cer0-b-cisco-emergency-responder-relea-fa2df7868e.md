...
Cisco Emergency Responder Release 15SU2 supports the following new features:

## Enhanced Security Considerations

From Cisco Emergency Responder Release 14 onwards, to create a trust between a publisher node and subscriber node, the Tomcat
                  certificate must be exchanged between the publisher and subscriber for any cluster-related operations to work. From Release
                  15SU2 onwards, if you need to perform manual or scheduled DRS backups or if you want to change the server hostname, it is
                  not required to manually exchange any tomcat certificate between the publisher and subscriber nodes. The publisher certificate
...
From this release, Call Home as a transport mode for smart licensing is deprecated. Smart Transport mode, the new transport
                  mode, is introduced for smart licensing. All freshly installed Emergency Responder nodes support only Smart Transport.

Post upgrade or migration, Emergency Responder automatically switches to Smart Transport if connection to the endpoints “smartreceiver.cisco.com”
                  is successful. In case of connection failure, Emergency Responder falls back to the Call Home mode.

## Switch Support

Support switch port based tracking for new Voice LAN Switches: Cisco Catalyst 2960L Series, Cisco 1100 Integrated Services
                  Routers, Cisco 890 Integrated Services Routers, Cisco Meraki Switch 130, and Meraki Managed Catalyst Switch C9300. For more
                  information, see the Supported Voice Ready Lan Switches section.

## Requirements

### Supported Hardware and Software

The information in the following sections discuss Hardware and Software requirements for Cisco Emergency Responder 15SU2.
...
Performance load runs on 15su1 with Ipv6 enabled routes the calls to Default ERL
...
Enter a valid value for Onsite Alert email Address
...
| CSCwj51735 | Performance load runs on 15su1 with Ipv6 enabled routes the calls to Default ERL |
| CSCwk99247 | CER not purging cercallhistory table after it reaches above 10000 |
| CSCwm34950 | Enter a valid value for Onsite Alert email Address |
