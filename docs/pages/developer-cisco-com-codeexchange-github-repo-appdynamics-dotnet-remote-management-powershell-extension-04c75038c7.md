---
doc_id: developer-cisco-com-codeexchange-github-repo-appdynamics-dotnet-remote-management-powershell-extension-04c75038c7
source_url: https://developer.cisco.com/codeexchange/github/repo/Appdynamics/dotnet-remote-management-powershell-extension
retrieved_at: 2026-08-20T20:28:28.971015+00:00
---
...
- Launch Power Shell and import the module: Import-Module .\AppDynamics.psm1

- While loading, the module checks for PowerShell version 3.0 or later and if the current user has administrative permissions. It issues a warning if there are problems.
...
- When you begin the agent installer and template configuration are located on your local machine where your are running the PowerShell module or on a central shared repository location.

- The Install-Agent cmdlet copies agent installation files to the default RemoteShare directory on the remote server(s): c$\Temp\AppDynamics\Install\

- When the script executes on the remote machine, it looks for the installation files at the default RemotePath directory: c:\Temp\AppDynamics\Install\

- Change the overrides using the -RemoteShare and -RemotePath command options.
...
- Get the local agent version.

- Compare the installed agent version to the version from the .msi file.

- If the installed version is the same or newer - do nothing. Otherwise continue.

- If -RestartIIS or -RestartWindowsServices parameters were passed - stop IIS and/or stop windows service(s).

- Uninstall existing agent.

- Check the return code - if there is a problem, stop and report the error code. Otherwise continue.

- Install new agent.

- If -RestartIIS or -RestartWindowsServices parameters were passed - start IIS and/or start windows service(s).

- Start AppDynamics.Agent.Coordinator service.
...
The Uninstall-Agent cmdlet uninstalls the .NET Agent from local or remote server(s) from the command line. It is important to restart the monitored applications and/or services during the restart.
...
- Examine events in the “Application” log around the time of the installation failure.

- There should be multiple entries from the “MsiInstaller” source.

- Find the one with which have error details.

Additionally grab the MSI log from the server, logs are %TEMP\AppDynamicsInstall.log and %TEMP\AppDynamicsUninstall.log If you can’t identify the root cause and resolve the problem then contact AppDynamics support.
