...
Precision queues —In precision queue routing, Webex CCE assigns tasks to agents in order of the precision queue steps. Webex CCE assigns tasks to agents who match the attributes for step one, up to their task limit, until all those agents are busy. Webex CCE then assigns tasks to agents who match attributes for step two, and so on. If you configure agents to handle three concurrent
                                    tasks, Webex CCE assigns three tasks to each agent in the first step. It then moves on to the second step and assigns any remaining tasks
                                    to those agents.

Overflow skill groups —Routing scripts can specify a preferred skill group and an overflow skill group. Webex CCE assigns tasks to all agents in the preferred skill group, up to their task limit, before assigning any tasks in the overflow
                                    skill group. If you configure agents to handle three concurrent tasks, Webex CCE assigns three tasks to each agent in the preferred skill group. It then moves on to the overflow skill group and assigns
