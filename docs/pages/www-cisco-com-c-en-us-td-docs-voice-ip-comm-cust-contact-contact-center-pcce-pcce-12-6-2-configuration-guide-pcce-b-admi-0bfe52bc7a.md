---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-pcce-pcce-12-6-2-configuration-guide-pcce-b-admi-0bfe52bc7a
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/pcce/pcce_12_6_2/configuration/guide/pcce_b_admin_configuration_guide_12_6_2/rcct_m_zero-downtime-upgrade.html
retrieved_at: 2026-08-21T12:16:45.055059+00:00
---
...
To enable graceful shutdown on Unified CCE Agent PG, install the ICM12.6.2_ES69 or later cumulative installer on both Side A and Side B.
...
To enable graceful shutdown on Unified CCE Media Routing PG, install the ICM12.6.2_ES69 or later cumulative installer on both Side A and Side B.
...
To enable graceful shutdown on Unified CCE AW, install the ICM12.6.2_ES70 or later cumulative installer on both Side A and Side B.

### Unified CCE Router Graceful Shutdown

To enable graceful shutdown on Unified CCE Routers, install the 12.6(2) ICM12.6.2_ES68 patch or later cummulative installer on both Side A and Side B. When you initiate a graceful shutdown, the active Router side verifies that the inactive side
...
Either Side A or Side B is not on version 12.6(2) .

When the ICM12.6.2_ES68 patch has not been applied to both Side A and Side B. If the patch is missing on either side, the Router node will reject the transition to maintenance mode.

The inactive side lacks device majority after the active side is shut down.

Limitations

If one side of the router service stops, database and application gateway lookups may fail. This error condition can be managed
                              within the routing script.
...
This section explains the procedure for gracefully shutting down services of Call server or Reporting server from the CLI.
                              Administrator can use the utility by following the instructions provided.
...
Enter the details at the prompt:
...
In case of graceful shutdown of the Reporting server, ensure that the CVP Call server is up and running.

You need to provide the IP-address of Reporting server, if reportingserver service is selected.
...
A PowerShell script on the Unified CCE VM runs an executable to coordinate maintenance mode activities on the PG, Router,
                           Logger, and Rogger. Use the following command to invoke the maintenance mode:
...
When you execute the invoke and status of maintenance mode commands on the script, it displays the output as a return code.
...
The following table shows the return codes for PG, Router, and Logger:
...
Common Return Codes (PG, Router, Logger) : All codes to be prefixed with "MMRequest-".
...
| Note | To enable graceful shutdown on Unified CCE Agent PG, install the ICM12.6.2_ES69 or later cumulative installer on both Side A and Side B. |
|---|---|

| Note | To enable graceful shutdown on Unified CCE Media Routing PG, install the ICM12.6.2_ES69 or later cumulative installer on both Side A and Side B. |
|---|---|

| Note | To enable graceful shutdown on Unified CCE AW, install the ICM12.6.2_ES70 or later cumulative installer on both Side A and Side B. |
|---|---|
...
| Step 4 | Enter the details at the prompt: CALLSERVER-IP-ADDRESS: <IP-Address of the Call Server> CALLSERVER-USERNAME: <Username of the Call Server> CALLSERVER-PASSWORD: <Password of the Call Server> SERVICE-NAME: <Choose the Service name which you need to shutdown gracefully(callserver/reportingserver)>
REPORTINGSERVER-IP-ADDRESS: <IP-Address of the REPORTING SERVER> Note In case of graceful shutdown of the Reporting server, ensure that the CVP Call server is up and running. You need to provide the IP-address of Reporting server, if reportingserver service is selected. | Note | In case of graceful shutdown of the Reporting server, ensure that the CVP Call server is up and running. You need to provide the IP-address of Reporting server, if reportingserver service is selected. |
| Note | In case of graceful shutdown of the Reporting server, ensure that the CVP Call server is up and running. You need to provide the IP-address of Reporting server, if reportingserver service is selected. |
