---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-finesse-finesse-1501-admin-guide-cfin-b-150-cisc-16d7ac5a2b
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/finesse/finesse_1501/admin/guide/cfin_b_150_cisco-finesse-administration-guide/cfin_m_150_perform-routine-maintenance.html
retrieved_at: 2026-08-21T12:05:09.257048+00:00
---
...
https:// FQDN :8443/plugins/CiscoRTMTPlugin.zip
...
DRAPI_REST_OPERATION_JOB_DISCARD

Clearing the Digital Routing API rest job when operation has failed multiple times and the maximum number of retries are over.
...
THIRD_PARTY_GADGET_MONITORING_SERVICE_ERROR

Error starting 3rdPartyGadget monitoring service = {Error Name}

By default, Finesse tomcat supports listening upto 8000 folders including subdirectories in the finesse/3rdpartygadget folder. If you add folders beyond 8000, it will not be reflected nor monitored and the syslog errors are as follows:

While starting Finesse tomcat, if the number of folders and subfolders are more than 8000, the error Error while starting a third-party gadget monitoring service is displayed.

While Finesse tomcat running, when new folders/subfolders are added in the 3rdpartygadget folder, if the count of folders is is more than 8000, the error Error occurred in a third-party gadget monitoring service is displayed.

## Finesse Performance Counters

You can get the JMX counters from Finesse using the following methods of Performance API:

list

read

For more information about these methods, refer to the Cisco FinesseWeb Services Developer and JavaScript Guide on the DevNet site.

For the list of JMX counters that are available for digital channels, see the JMX Counters for Digital Channels table in the JMX Counter Thresholds topic.
...
CloseTasksRequestsInterval

Close task API requests during interval.

This does not include retry request.

com.cisco.ccbu:category=

DRAPITasksStatsBean,component0=

DRAPITasksStats

TotalCloseTasksRequests

Total number of Close task API requests to Digital Routing API from starting of the Finesse service.
...
TransferTasksRequestsInterval

Transfer task API requests during interval.
...
TotalTransferTasksRequests

Total number of Transfer task API requests to Digital Routing API from starting of the Finesse service.
...
DiscardedTasksInterval

Discarded Digital Routing API tasks during interval.
...
Total number of discarded Digital Routing API tasks from starting of the Finesse service.
...
Total number of Digital Routing API failovers from starting of the Finesse service.
...
PendingRequestsInQueue

Total number of Digital Routing API rest requests that are pending in queue.
...
| Step 4 | Under the Logs tab, select Application Logs > CiscoSyslog to view and save the syslog file. Tip When you double-click the CiscoSyslog message, the Show Detail dialog displays the syslog definition and recommended actions in an adjacent pane. For more information, see the Cisco Unified Real-Time Monitoring Tool Administration Guide . Note System log messages generated by Cisco Finesse are also available under SysLog Viewer > System Logs > messages . The following are the different types of messages and corresponding
                                             descriptions that are captured in the SysLog Viewer > System Logs > messages . CTI_SOCKET_ERROR System has encountered an error connecting to the CTI server. CTI_CONNECTION_LOST System has lost contact with the CTI server. CTI_OPEN_FAILURE CTI Server rejected open request. CTI_CONNECTION_RETRIES_EXCEEDED System has failed to connect to the CTI server in the allowed number of retries. CTI_CONNECTION_ESTABLISHED System has successfully connected to the CTI server. SUBSYS_INIT_ERROR Error initializing subsystem. UNABLE_TO_CONNECT_TO_XMPP_SERVER Unable to connect xmpp server. DB_SS_CONNECTION_CHECK There was an error connecting to the database. cfservice_CORE_ERROR_DB_CONNECTION Unable to connect to the Database. AWDB_NOT_ACCESSIBLE Unable to connect to AWDB server. VOS_DB_ADAPTER_ERROR There was an error on the VOS DB Adapter operation. FINESSE_APP_STARTUP_ERROR Error during Finesse Application Startup. OF_STATE_CHANGED OF subsystem state successfully changed. CONNECTED_TO_XMPP_SERVER Successfully connected to xmpp server. SSO_API_ERROR Error processing REST API Request for SSO. API_ERROR_DETAIL Error processing REST API request. AWDB_CONNECTION_ERROR Error while connecting the AWDB server. AWDB_CONNECTION_SWITCH_SUCCESS AWDB
                                                server connection successfully switched. CTI_DISCONNECT Disconnecting primary CTI connection. STANDBY_CTI_DISCONNECT Disconnecting standby CTI connection. RESET_STANDBY_CONNECTION Resetting standby CTI connection. RESET_PRIMARY_CTI_CONNECTION Resetting primary CTI connection. SWAP_CTI_CONNECTIONS Swapping primary and standby CTI Connections. STANDBY_ACMI_CONNECTION_ESTABLISHED Established connection with standby CTI. STANDBY_ACTIVE_EVENT Received standby active event from standby CTI. MAINTENANCE_MODE_REQUEST_TIMEOUT CTI STANDBY_ACTIVE_EVENT isn’t received before the threshold time. MAINTANCE_MODE_CONTINUE_CTI CTI confirms to continue maintenance mode. MAINTANCE_MODE_DUPLICATE_REQUEST Duplicate maintenance mode request received from CTI. FINESSE_MAINTENANCE_MODE_FAILED Finesse current node went out of service. FINESSE_MAINTENANCE_MODE_FAILED Finesse alternate node is in maintenance mode. FINESSE_MAINTENANCE_MODE_FAILED Internal error occurred while performing the maintenance mode
                                                   ...
                                                   node. DRAPI_REST_OPERATION_JOB_DISCARD Clearing the Digital Routing API rest job when operation has failed multiple times and the maximum number of retries are over. DRAPI_HOST_ALERT Failover of Digital Routing API host-pair. Failover isn’t supported when the Digital Routing API host backup isn’t configured. DRAPIAsyncRestClient Failed to create SSL connection to Digital Routing API. THIRD_PARTY_GADGET_MONITORING_SERVICE_ERROR Error starting 3rdPartyGadget monitoring service = {Error Name} Note By default, Finesse tomcat supports listening upto 8000 folders including subdirectories in the finesse/3rdpartygadget folder. If you add folders beyond 8000, it will not be reflected nor monitored and the syslog errors are as follows: While starting Finesse tomcat, if the number of folders and subfolders are more than 8000, the error Error while starting a third-party gadget monitoring service is displayed. While Finesse tomcat running, when new folders/subfolders are added in the 3rdpartygadget folder, if the count of folders is is more than 8000, the error Error occurred in a third-party gadget monitoring service is displayed. | Tip | When you double-click the CiscoSyslog message, the Show Detail dialog displays the syslog definition and recommended actions in an adjacent pane. For more information, see the Cisco Unified Real-Time Monitoring Tool Administration Guide . | Note | System log messages generated by Cisco Finesse are also available under SysLog Viewer > System Logs > messages . | Note | By default, Finesse tomcat supports listening upto 8000 folders including subdirectories in the finesse/3rdpartygadget folder. If you add folders beyond 8000, it will not be reflected nor monitored and the syslog errors are as follows: While starting Finesse tomcat, if the number of folders and subfolders are more than 8000, the error Error while starting a third-party gadget monitoring service is displayed. While Finesse tomcat running, when new folders/subfolders are added in the 3rdpartygadget folder, if the count of folders is is more than 8000, the error Error occurred in a third-party gadget monitoring service is displayed. |
...
| Note | By default, Finesse tomcat supports listening upto 8000 folders including subdirectories in the finesse/3rdpartygadget folder. If you add folders beyond 8000, it will not be reflected nor monitored and the syslog errors are as follows: While starting Finesse tomcat, if the number of folders and subfolders are more than 8000, the error Error while starting a third-party gadget monitoring service is displayed. While Finesse tomcat running, when new folders/subfolders are added in the 3rdpartygadget folder, if the count of folders is is more than 8000, the error Error occurred in a third-party gadget monitoring service is displayed. |
...
| Counter Name | Description | JMX Object Name |
|---|---|---|
| CloseTasksRequestsInterval | Close task API requests during interval. Note This does not include retry request. | Note | This does not include retry request. | com.cisco.ccbu:category= DRAPITasksStatsBean,component0= DRAPITasksStats |
| Note | This does not include retry request. |
| TotalCloseTasksRequests | Total number of Close task API requests to Digital Routing API from starting of the Finesse service. Note This does not include retry request. | Note | This does not include retry request. | com.cisco.ccbu:category= DRAPITasksStatsBean,component0= DRAPITasksStats |
| Note | This does not include retry request. |
| TransferTasksRequestsInterval | Transfer task API requests during interval. Note This does not include retry request. | Note | This does not include retry request. | com.cisco.ccbu:category= DRAPITasksStatsBean,component0= DRAPITasksStats |
| Note | This does not include retry request. |
| TotalTransferTasksRequests | Total number of Transfer task API requests to Digital Routing API from starting of the Finesse service. Note This does not include retry request. | Note | This does not include retry request. | com.cisco.ccbu:category= DRAPITasksStatsBean,component0= DRAPITasksStats |
| Note | This does not include retry request. |
| DiscardedTasksInterval | Discarded Digital Routing API tasks during interval. | com.cisco.ccbu:category= DRAPITasksStatsBean,component0= DRAPITasksStats |
| TotalDiscardedTasks | Total number of discarded Digital Routing API tasks from starting of the Finesse service. | com.cisco.ccbu:category= DRAPITasksStatsBean,component0= DRAPITasksStats |
| FailoversInInterval | Failovers during intervals. | com.cisco.ccbu:category= DRAPITasksStatsBean,component0= DRAPITasksStats |
| TotalFailovers | Total number of Digital Routing API failovers from starting of the Finesse service. | com.cisco.ccbu:category= DRAPITasksStatsBean,component0= DRAPITasksStats |
| PendingRequestsInQueue | Total number of Digital Routing API rest requests that are pending in queue. | com.cisco.ccbu:category= DRAPITasksStatsBean,component0= DRAPITasksStats |
