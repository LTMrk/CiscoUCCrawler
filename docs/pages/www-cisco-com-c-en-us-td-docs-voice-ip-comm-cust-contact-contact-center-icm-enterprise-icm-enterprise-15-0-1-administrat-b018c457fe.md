---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-15-0-1-administrat-b018c457fe
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_15_0_1/administration/guide/ucce_b_150_administration-guide-for-cisco-unified-contact-center-enterprise/ucce_m_150_agent-administration.html
retrieved_at: 2026-08-16T20:43:56.781908+00:00
---
...
Enable the Agent event detail option by selecting ICM Configuration Manager > Tools > Explorer Tools > PG Explorer , and then selecting the Unified CCE peripheral.
...
General tab: Supervisors can edit the password for agents who do not have IDS or Webex Common Identity single sign-on enabled. Other fields are read-only.
...
Define a CTI Route Point, for example "9999" , in the Unified CCE . Associate it with the JTAPI User that is connected to the Unified CCE PIM in the system software.
...
In the Administration Client or Administration & Data Server, define a Dialed Number for the Unified CCE PIM and a call type for that dialed number. You can then associate this call type with a Unified CCE script; for example, "NetXfer2."

Do not define the labels of agents for the Unified CCE PG. Instead, define the labels for the VRU PIM so that the route result is returned to VRU instead of a Unified CCE PG. If you do define the agent labels for the Unified CCE PG, the Router returns the route result to the VRU PIM, if "Network Transfer Preferred" is enabled on the Unified CCE PG and VRU PIM and returns the route result to the Unified CCE PG if "Network Transfer Preferred" " is disabled on the Unified CCE PG and VRU PIM.

Step 3

When the call is delivered to Agent 1 using the Unified CCE Script "NetXfer1," the agent can dial the number 9999 to send the call to another script, "NetXfer2."
...
| Step 1 | Select ICM Configuration
                                             				  Manager > Tools > List
                                             				  Tools > Reason Code List . Example: Note If you are using the agent desktop, make sure the Reason Codes match the codes on the desktop. Unified CCE Reason Codes appear in the Agent Not Ready reports, but the agent actually selects the desktop code, so these codes must
                                                   match to avoid confusion. Configure predefined Not Ready Reason Codes so their text appears in the reports. | Note | If you are using the agent desktop, make sure the Reason Codes match the codes on the desktop. Unified CCE Reason Codes appear in the Agent Not Ready reports, but the agent actually selects the desktop code, so these codes must
                                                   match to avoid confusion. Configure predefined Not Ready Reason Codes so their text appears in the reports. |
|---|---|---|---|
| Note | If you are using the agent desktop, make sure the Reason Codes match the codes on the desktop. Unified CCE Reason Codes appear in the Agent Not Ready reports, but the agent actually selects the desktop code, so these codes must
                                                   match to avoid confusion. Configure predefined Not Ready Reason Codes so their text appears in the reports. |
| Step 2 | Enable the Agent event detail option by selecting ICM Configuration Manager > Tools > Explorer Tools > PG Explorer , and then selecting the Unified CCE peripheral. |
...
| Step 1 | Define a CTI Route Point, for example "9999" , in the Unified CCE . Associate it with the JTAPI User that is connected to the Unified CCE PIM in the system software. Note You cannot use the DN for a CTI Route Point on a different CTI Route Point in another partition. Ensure that DNs are unique
...
| Note | You cannot use the DN for a CTI Route Point on a different CTI Route Point in another partition. Ensure that DNs are unique
                                                         across all CTI Route Points on all partitions. |
| Step 2 | In the Administration Client or Administration & Data Server, define a Dialed Number for the Unified CCE PIM and a call type for that dialed number. You can then associate this call type with a Unified CCE script; for example, "NetXfer2." Note Do not define the labels of agents for the Unified CCE PG. Instead, define the labels for the VRU PIM so that the route result is returned to VRU instead of a Unified CCE PG. If you do define the agent labels for the Unified CCE PG, the Router returns the route result to the VRU PIM, if "Network Transfer Preferred" is enabled on the Unified CCE PG and VRU PIM and returns the route result to the Unified CCE PG if "Network Transfer Preferred" " is disabled on the Unified CCE PG and VRU PIM. | Note | Do not define the labels of agents for the Unified CCE PG. Instead, define the labels for the VRU PIM so that the route result is returned to VRU instead of a Unified CCE PG. If you do define the agent labels for the Unified CCE PG, the Router returns the route result to the VRU PIM, if "Network Transfer Preferred" is enabled on the Unified CCE PG and VRU PIM and returns the route result to the Unified CCE PG if "Network Transfer Preferred" " is disabled on the Unified CCE PG and VRU PIM. |
| Note | Do not define the labels of agents for the Unified CCE PG. Instead, define the labels for the VRU PIM so that the route result is returned to VRU instead of a Unified CCE PG. If you do define the agent labels for the Unified CCE PG, the Router returns the route result to the VRU PIM, if "Network Transfer Preferred" is enabled on the Unified CCE PG and VRU PIM and returns the route result to the Unified CCE PG if "Network Transfer Preferred" " is disabled on the Unified CCE PG and VRU PIM. |
| Step 3 | When the call is delivered to Agent 1 using the Unified CCE Script "NetXfer1," the agent can dial the number 9999 to send the call to another script, "NetXfer2." |
