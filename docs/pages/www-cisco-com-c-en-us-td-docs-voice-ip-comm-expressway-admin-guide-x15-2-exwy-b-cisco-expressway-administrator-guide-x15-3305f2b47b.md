---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-expressway-admin-guide-x15-2-exwy-b-cisco-expressway-administrator-guide-x15-3305f2b47b
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/expressway/admin_guide/X15-2/exwy_b_cisco-expressway-administrator-guide-x152/exwy_m_expressway-interfaces.html
retrieved_at: 2026-08-16T22:29:47.252581+00:00
---
...
Disabling or Enabling the Cross-Site Request Forgery Protection : Starting X15.0.3 and later  releases, the CSRF Protection is Enabled by default. You can execute the following commands to enable or disable the custom header.
...
All hosted API resources (web services) that accept client application connections MUST use authentication and authorization
                                             to protect API functionality and access data commensurate with the API's data sensitivity.
...
| Note | You need not add any changes to the scripts while using any RAML API other than the above. All hosted API resources (web services) that accept client application connections MUST use authentication and authorization
                                             to protect API functionality and access data commensurate with the API's data sensitivity. Earlier API access was 'Enabled' by default, and now it is 'Disabled'. |
|---|---|
