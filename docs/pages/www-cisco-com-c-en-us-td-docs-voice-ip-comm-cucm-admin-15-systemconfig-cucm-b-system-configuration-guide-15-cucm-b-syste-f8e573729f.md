...
Direct connection to Cisco Smart Software Manager on cisco.com—Unified Communications Manager connects directly to the Cisco
                                    Smart Software Manager on cisco.com. With this option, you must configure DNS on Unified Communications Manager that resolves tools.cisco.com for Call Home and smartreceiver.cisco.com for Smart Transport .
...
Connection to on-premise Cisco Smart Software Manager satellite—Unified Communications Manager connects to an on-premise Smart
                                    Software Manager satellite. DNS is not required on Unified Communications Manager. DNS that can resolve tools.cisco.com for Call Home and smartreceiver.cisco.com for Smart Transport is required on the satellite server if you are deploying Connected mode. DNS is not required on the satellite server if you
...
Authentication needed on HTTP or HTTPS proxy—Enable the check box if you want to register to Cisco Smart Software Manager
                                                      using an authentication based proxy server.
...
From the Smart Software Licensing section, click the View/Edit the Licensing Transport settings link.
...
Direct —Unified Communications Manager connects directly to the Smart Software Manager on cisco.com. This is the default option.
                                                With this option, you must deploy DNS on Unified Communications Manager that can resolve tools.cisco.com for Call Home and smartreceiver.cisco.com for Smart Transport .

Cisco Smart Software Manager satellite —Unified Communications Manager connects to an on-premise Cisco Smart Software Manager satellite for system licensing. In
                                                the URL text box, enter the address and port of the Smart Software Manager satellite. For example, fqdn_of_smart_software_manager:port_number . For HTTPS, use port 443. Cisco Smart Software Manager satellite supports Smart Transport from Satellite Version 7-201907
                                                onwards.

HTTP/HTTPS Proxy —Unified Communications Manager connects to a proxy server, which connects to the Cisco Smart Software Manager service on
...
Click Save first, after selecting the method for connecting Unified Communications Manager to the Smart Licensing service.

Step 6

Click Test connectivity with Smart Transport to test the Unified Communications Manager connection to Smart Transport endpoint through transport setting configurations.
...
For an offline deployment that uses Specific License Reservation (SLR) or Permanent License Reservation (PLR), the License
                                       ...
                                       Unified Communications Manager enforces the reserved license limits locally and does not communicate license usage to Cisco
                                       cloud licensing servers.
...
The following table describes the license and authorization statuses that can appear for offline license reservation deployments.
...
NOT AUTHORIZED - RESERVED

Authorization status

The product instance is operating offline with Specific License Reservation (SLR) or Permanent License Reservation (PLR).
                                             The reserved license entitlement is insufficient to cover the current license consumption.

AUTHORIZED - RESERVED

Authorization status

The product instance is operating offline with SLR or PLR. The reserved license entitlement is sufficient to cover the current
                                             license consumption.
...
The product instance has completed SLR and operates offline with a specific set of reserved licenses without connectivity
                                             to Cisco Smart Software Manager (CSSM). Depending on current license consumption, the authorization status is AUTHORIZED - RESERVED or NOT AUTHORIZED - RESERVED .

REGISTERED - UNIVERSAL LICENSE RESERVATION

License status

The product instance has completed PLR and operates independently of CSSM. All license types are reserved, and the reservation
                                             covers all active license consumption.

RESERVATION IN PROGRESS

Authorization status

The reservation request has been initiated, but the authorization code generated by CSSM has not yet been installed on the
                                             product instance. The product instance remains in Evaluation Mode until the reservation is completed.
...
### Upgrading Specific License Reservation Enabled System to Version 14

If you are upgrading the 12.5 Unified Communications Manager system which is enabled for license reservation to version 14
                                 following are the scenarios need to be considered:

Before upgrade to version 14, return the 12.x licenses using "license smart reservation return" command (Recommended).

OR

After upgrade to version 14, return the 12.x licenses using "license smart reservation return" command.
...
Install the authorization code using "license smart reservation install <auth-code>" command in Cisco Unified Communications
                                       Manager.

### Upgrading Permanent License Reservation Enabled System to Version 14SU3

If you are upgrading your Unified Communications Manager system from Release 14 SU2 to Release 14SU3 which is enabled for
                                 Permanent License Reservation (PLR), consider the following scenarios:

Before upgrade to version 14SU3, return the license using the "license smart reservation return" command.

After upgrade, create the request code using the "license smart reservation request" command. Generate an authorization code
                                       with PLR license in Cisco Smart Software Manager.

Install the authorization code using the "license smart reservation install <auth-code>" command in Unified Communications
                                       Manager.

## Upgrading Permanent License Reservation Enabled System to Version 15

If you are upgrading the 14 SU2 and above Unified Communications Manager systems which is enabled for Permanent License Reservation
                              (PLR) to version 15, consider the following scenarios:
...
Install the authorization code using "license smart reservation install <auth-code>" command in Unified Communications Manager.
...
| Important | From Release 14SU4 and 15SU2 onwards, Call Home as a transport mode for smart licensing is deprecated. Smart Transport mode,
                                       the new transport mode is introduced for smart licensing. |
|---|---|

| Note | Smart Transport is not supported for Transport gateway and is not applicable from Release 14SU4 and 15SU2 onwards. |
|---|---|
...
| Step 3 | Configure Connection to Smart Software Licensing (Applicable from Release 14SU4 and 15SU2 Onwards) | Select transport settings through which Unified Communications Manager connects to the Smart Software Licensing service. The Direct option is selected by default where the product communicates directly with Cisco licensing servers. |
| Step 4 | Register with Cisco Smart Software Manager . | Perform this step to register Unified Communications Manager with Cisco Smart Software Manager or Cisco Smart Software Manager satellite. |
...
| Step 3 | Select the method of connecting Unified Communications Manager to the Smart Licensing service: Direct —Unified Communications Manager connects directly to the Smart Software Manager on cisco.com. This is the default option.
                                                ...
                                                with Transport Gateway also along with satellite on cisco.com. Enter the IP address or hostname of the proxy server along
                                                with the port: Authentication needed on HTTP or HTTPS proxy—Enable the check box if you want to register to Cisco Smart Software Manager
                                                      using an authentication based proxy server. IP Address/Host Name Port—For HTTPS, use port 443. User Name Password |
...
|---|---|
| Step 2 | From the Smart Software Licensing section, click the View/Edit the Licensing Transport settings link. The Transport Settings dialog box appears. |
| Step 3 | Select the method of connecting Unified Communications Manager to the Smart Licensing service: Direct —Unified Communications Manager connects directly to the Smart Software Manager on cisco.com. This is the default option.
                                                With this option, you must deploy DNS on Unified Communications Manager that can resolve tools.cisco.com for Call Home and smartreceiver.cisco.com for Smart Transport . Cisco Smart Software Manager satellite —Unified Communications Manager connects to an on-premise Cisco Smart Software Manager satellite for system licensing. In
                                                the URL text box, enter the address and port of the Smart Software Manager satellite. For example, fqdn_of_smart_software_manager:port_number . For HTTPS, use port 443. Cisco Smart Software Manager satellite supports Smart Transport from Satellite Version 7-201907
                                                onwards. HTTP/HTTPS Proxy —Unified Communications Manager connects to a proxy server, which connects to the Cisco Smart Software Manager service on
                                                cisco.com. Enter the IP address or hostname of the proxy server along with the port: Authentication needed on HTTP or HTTPS proxy—Enable the check box if you want to register to Cisco Smart Software Manager
                                                      using an authentication based proxy server. IP Address/Host Name Port—For HTTPS, use port 443. User Name Password Note Transport Gateway as a transport mode is not supported in Smart Transport. Hence, it continues to use call home post upgrade
                                                         or migration. To switch to Smart Transport mode, deregister the product. Also, the system must connect to Smart Transport
                                                         URL using Direct or Cisco Smart Software Manager satellite or HTTP/HTTPS Proxy server. Important From Release 15SU5 onwards, Transport Gateway requires version 4.2.2 or later, which includes support for compatible cipher
                                                            suites. | Note | Transport Gateway as a transport mode is not supported in Smart Transport. Hence, it continues to use call home post upgrade
                                                         or migration. To switch to Smart Transport mode, deregister the product. Also, the system must connect to Smart Transport
                                                         URL using Direct or Cisco Smart Software Manager satellite or HTTP/HTTPS Proxy server. | Important | From Release 15SU5 onwards, Transport Gateway requires version 4.2.2 or later, which includes support for compatible cipher
                                                            suites. |
| Note | Transport Gateway as a transport mode is not supported in Smart Transport. Hence, it continues to use call home post upgrade
                                                         or migration. To switch to Smart Transport mode, deregister the product. Also, the system must connect to Smart Transport
                                                         URL using Direct or Cisco Smart Software Manager satellite or HTTP/HTTPS Proxy server. |
| Important | From Release 15SU5 onwards, Transport Gateway requires version 4.2.2 or later, which includes support for compatible cipher
                                                            suites. |
| Step 4 | Check the Do not share my hostname or IP address with Cisco check box to restrict Unified Communications Manager from sharing its IP address and hostname during the Smart Licensing
                                          registration. |
| Step 5 | Click Save first, after selecting the method for connecting Unified Communications Manager to the Smart Licensing service. |
| Step 6 | Click Test connectivity with Smart Transport to test the Unified Communications Manager connection to Smart Transport endpoint through transport setting configurations. |
| Step 7 | Click Switch to change the transport mode from Call Home to Smart Transport. Cisco Smart License Manager service is restarted during the
                                          switch process. This option is available only for Direct or HTTP/HTTPS Proxy server or Cisco Smart Software Manager satellite
                                          in Call Home Mode. Note The Switch Button is enabled once Test connectivity with Smart Transport is successful with Smart Transport. | Note | The Switch Button is enabled once Test connectivity with Smart Transport is successful with Smart Transport. |
| Note | The Switch Button is enabled once Test connectivity with Smart Transport is successful with Smart Transport. |
| Step 8 | Click Close . |
...
| Note | For an offline deployment that uses Specific License Reservation (SLR) or Permanent License Reservation (PLR), the License
                                       Authorization Status displays NOT AUTHRORIZED - RESERVED. This status is expected and does not indicate an authorization failure.
                                       Unified Communications Manager enforces the reserved license limits locally and does not communicate license usage to Cisco
                                       cloud licensing servers. |
|---|---|
...
|---|---|---|
| NOT AUTHORIZED - RESERVED | Authorization status | The product instance is operating offline with Specific License Reservation (SLR) or Permanent License Reservation (PLR).
                                             The reserved license entitlement is insufficient to cover the current license consumption. |
| AUTHORIZED - RESERVED | Authorization status | The product instance is operating offline with SLR or PLR. The reserved license entitlement is sufficient to cover the current
                                             license consumption. |
| REGISTERED - SPECIFIC LICENSE RESERVATION | License status | The product instance has completed SLR and operates offline with a specific set of reserved licenses without connectivity
                                             to Cisco Smart Software Manager (CSSM). Depending on current license consumption, the authorization status is AUTHORIZED - RESERVED or NOT AUTHORIZED - RESERVED . |
| REGISTERED - UNIVERSAL LICENSE RESERVATION | License status | The product instance has completed PLR and operates independently of CSSM. All license types are reserved, and the reservation
                                             covers all active license consumption. |
| RESERVATION IN PROGRESS | Authorization status | The reservation request has been initiated, but the authorization code generated by CSSM has not yet been installed on the
                                             product instance. The product instance remains in Evaluation Mode until the reservation is completed. |
