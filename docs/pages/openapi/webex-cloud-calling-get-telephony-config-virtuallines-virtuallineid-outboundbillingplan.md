---
doc_id: webex-cloud-calling-get-telephony-config-virtuallines-virtuallineid-outboundbillingplan
source: webex-openapi-specs/public-spec/webex-cloud-calling.json
api: Webex Cloud Calling
api_version: 1.0.0
method: GET
path: /telephony/config/virtualLines/{virtualLineId}/outboundBillingPlan
operation_id: getVirtualLineOutboundBillingPlan
tags: Virtual Line Call Settings
deprecated: false
scopes: 
license: CC-BY-4.0
retrieved_at: 2026-09-01T07:56:51.793793+00:00
---

# GET /telephony/config/virtualLines/{virtualLineId}/outboundBillingPlan

**API:** Webex Cloud Calling
**Área:** Virtual Line Call Settings
**operationId:** `getVirtualLineOutboundBillingPlan`
...
The response returns `enabled` as `true` when Cisco Calling Plan outbound billing is enabled for the virtual line. Otherwise, the response returns `enabled` as `false`.

Retrieving a virtual line's outbound billing plan requires a full, user, read-only administrator, or location administrator auth token with a scope of `spark-admin:telephony_config_read`.
...
```bash
curl -X GET '/telephony/config/virtualLines/<virtualLineId>/outboundBillingPlan' \
