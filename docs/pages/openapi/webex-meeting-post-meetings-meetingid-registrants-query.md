---
doc_id: webex-meeting-post-meetings-meetingid-registrants-query
source: webex-openapi-specs/public-spec/webex-meeting.json
api: Webex Meetings
api_version: 1.0.0
method: POST
path: /meetings/{meetingId}/registrants/query
operation_id: queryMeetingRegistrants
tags: Meetings
deprecated: false
scopes: 
license: CC-BY-4.0
retrieved_at: 2026-08-18T23:45:44.486526+00:00
---

# POST /meetings/{meetingId}/registrants/query

**API:** Webex Meetings
**Área:** Meetings
**operationId:** `queryMeetingRegistrants`
...
- `status` (string): Registrant's status.  * `approved` - Registrant has been approved.  * `pending` - Registrant is in a pending list waiting for host or cohost approval.  * `rejected` - Registrant has been rejected by the host or cohost. Valores: approved, pending, rejected.
- `orderType` (string): Sort order for the registrants. Valores: DESC, ASC.
- `orderBy` (string): Registrant ordering field. Ordered by `registrationTime` by default.  * `firstName` - Registrant's first name.  * `lastName` - Registrant's last name.  * `status` - Registrant's status.  * `email` - registrant's email. Valores: firstName, lastName, status, email.
- `emails` (array) (**requerido**): List of registrant email addresses.
...
```bash
curl -X POST '/meetings/<meetingId>/registrants/query' \
