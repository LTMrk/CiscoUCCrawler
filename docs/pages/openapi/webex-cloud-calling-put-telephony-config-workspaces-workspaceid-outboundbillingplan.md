---
doc_id: webex-cloud-calling-put-telephony-config-workspaces-workspaceid-outboundbillingplan
source: webex-openapi-specs/public-spec/webex-cloud-calling.json
api: Webex Cloud Calling
api_version: 1.0.0
method: PUT
path: /telephony/config/workspaces/{workspaceId}/outboundBillingPlan
operation_id: updateWorkspaceOutboundBillingPlan
tags: Workspace Call Settings (2/2)
deprecated: false
scopes: 
license: CC-BY-4.0
retrieved_at: 2026-09-01T07:56:51.815436+00:00
---

# PUT /telephony/config/workspaces/{workspaceId}/outboundBillingPlan

**API:** Webex Cloud Calling
**Área:** Workspace Call Settings (2/2)
**operationId:** `updateWorkspaceOutboundBillingPlan`
...
Set `enabled` to `true` to enable Cisco Calling Plan outbound billing. Setting `enabled` to `true` is supported only for a workspace in a Cisco Calling Plan location. Set `enabled` to `false` to disable it.

Updating a workspace's outbound billing plan requires a full, device, or location administrator auth token with a scope of `spark-admin:telephony_config_write`.
...
```bash
curl -X PUT '/telephony/config/workspaces/<workspaceId>/outboundBillingPlan' \
