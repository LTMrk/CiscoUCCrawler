...
license: CC-BY-4.0
retrieved_at: 2026-08-16T11:30:32.974674+00:00
---
...
## Respuestas
- **200**: Returns Recording and Transcription details. If a provided taskId is not found for the specified organization, the API will return a 200 OK response with empty recording and transcription fields for that task. This behavior is intentional to allow partial results when multiple taskIds are provided; unmatched taskIds do not result in a 4xx error.
  ...
  - `data` (array) **(requerido)**:
    ...
    - `screenRecordings` (array):
      ...
      - `attributes` (object) **(requerido)**:
        ...
        - `startTime` (integer) **(requerido)**: Begin time of screen recording (epoch timestamp)
        - `stopTime` (integer) **(requerido)**: End time of screen recording (epoch timestamp).
