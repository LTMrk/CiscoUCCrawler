...
path: /events
license: CC-BY-4.0
retrieved_at: 2026-08-16T11:30:33.510455+00:00
---
...
## Parámetros
- `resource` [query] (string): List events with a specific resource type.
- `type` [query] (string): List events with a specific event type.
...
- `max` [query] (number): Limit the maximum number of events in the response. Value must be between 1 and 1000, inclusive.
- `serviceType` [query] (string): List events for a specific service type. This parameter is only applicable and mandatory when resource is set to `convergedRecordings`.
