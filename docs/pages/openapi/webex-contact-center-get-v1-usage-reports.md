---
doc_id: webex-contact-center-get-v1-usage-reports
...
scopes: 
license: CC-BY-4.0
retrieved_at: 2026-09-17T19:59:29.926556+00:00
---
...
## Descripción
Retrieves a list of usage reports for the organization. Supports optional filtering by resource type and creation date range.

Each report has a `reportId` and may contain one or more generated files in `reportFiles[]`. For long date ranges, the service can split results by calendar month to avoid creating a single very large download. For example, a 12-month report can return up to 12 report files, and a 36-month report can return up to 36 report files.

Use `reportId` to retrieve the overall report status, list the generated files, or delete the report. Use each file's `fileId` or `downloadUrl` to download that specific file. The download API does not accept `reportId`.

## Parámetros
- `resourceType` [query] (string): Filter by resource type
- `createdAfter` [query] (string/date-time): Filter reports created after this timestamp (ISO 8601 format)
- `createdBefore` [query] (string/date-time): Filter reports created before this timestamp (ISO 8601 format)

## Ejemplo de invocación
```bash
curl -X GET '/v1/usage-reports' \
