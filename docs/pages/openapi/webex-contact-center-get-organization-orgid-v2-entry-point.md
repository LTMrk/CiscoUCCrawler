...
operation_id: getAllFilteredConfigWithMetaData_1
tags: Entry Point
deprecated: false
scopes: 
license: CC-BY-4.0
retrieved_at: 2026-08-31T18:15:55.095489+00:00
---
...
- `provisioningView` [query] (boolean): If set to true, the API will only return data that user has access to, according to User Profile. If set to false and desktopProfileFilter query parameter is not specified, the API will add user associated data, based on desktop. Por defecto: False.
- `includeCount` [query] (boolean): Enable the flag to get the count of DN-EP Mapping Por defecto: False.
...
```bash
curl -X GET '/organization/<orgid>/v2/entry-point' \
