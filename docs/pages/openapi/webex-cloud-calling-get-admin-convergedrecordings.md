...
license: CC-BY-4.0
retrieved_at: 2026-08-16T11:30:32.580126+00:00
---
...
- `status` [query] (string): Recording's status. If not specified or `available`, retrieves recordings that are available. if specified as `purged`, retrieves recordings those are deleted but available to a compliance officer. Otherwise, if specified as `deleted`, retrieves recordings that have been moved into the recycle bin.
