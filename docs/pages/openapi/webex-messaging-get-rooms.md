...
path: /rooms
license: CC-BY-4.0
retrieved_at: 2026-08-16T11:30:33.514818+00:00
---
...
- `type` [query] (string): List rooms by type. Cannot be set in combination with `orgPublicSpaces`.
...
- `sortBy` [query] (string): Sort results. Cannot be set in combination with `orgPublicSpaces`.
- `max` [query] (number): Limit the maximum number of rooms in the response. Value must be between 1 and 1000, inclusive.
