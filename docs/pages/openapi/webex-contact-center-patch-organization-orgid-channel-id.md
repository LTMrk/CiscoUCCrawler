...
- `logoType` (string): Type of logo. Only MOMENTUM is accepted via PATCH. Setting logoType to LOGO_URL is rejected with 400 — use PUT with a logoFile part to upload a logo image and switch to LOGO_URL. Valores: MOMENTUM.
- `logoIconName` (string): Momentum icon name — required when logoType is MOMENTUM; ignored / null when logoType is LOGO_URL. Long. max: 100.

## Ejemplo de invocación
```bash
curl -X PATCH '/organization/<orgid>/channel/<id>' \
