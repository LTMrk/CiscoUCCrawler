---
doc_id: webex-cloud-calling-post-telephony-config-operatingmodes
source: webex-openapi-specs/public-spec/webex-cloud-calling.json
api: Webex Cloud Calling
api_version: 1.0.0
method: POST
path: /telephony/config/operatingModes/
operation_id: Create an Operating Mode
tags: Features: Operating Modes
deprecated: false
scopes: 
license: CC-BY-4.0
retrieved_at: 2026-08-18T23:45:43.322731+00:00
---
...
## Parámetros
- `orgId` [query] (string): Create the `operating mode` for this organization.
...
- `locationId` (string): Unique identifier of the location. Mandatory if level is `LOCATION`.
...
- `differentHoursDaily` (object):
  ...
  - `saturday` (object) (**requerido**):
    ...
    - `startTime` (string) (**requerido**): Start time for the `operating mode`.
    - `endTime` (string) (**requerido**): End time for the `operating mode`.
- `holidays` (array): `Operating mode` holidays. Mandatory if type is `HOLIDAY`.
...
```bash
curl -X POST '/telephony/config/operatingModes/' \
  -H 'Authorization: Bearer <TOKEN>' \
  -H 'Content-Type: application/json' \
  -d '{"name": "<name>", "type": "<type>", "level": "<level>", "callForwarding": {}}'
```

## Respuestas correctas
**201**: Created
- `id` (string) (**requerido**): ID of the newly created `operating mode`.
