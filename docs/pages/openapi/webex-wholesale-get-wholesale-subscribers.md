...
- `externalCustomerId` [query] (string): Customer external ID.
- `email` [query] (string): The email address of the subscriber.
- `status` [query] (string): The provisioning status of the subscriber.
...
- `sortBy` [query] (string): Supported `sortBy` attributes are `created` and `lastStatusChange`. Default is `created`.
