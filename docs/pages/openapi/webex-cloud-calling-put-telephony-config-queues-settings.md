...
scopes: 
license: CC-BY-4.0
retrieved_at: 2026-08-25T10:28:32.470867+00:00
---
...
**operationId:** `updateCallQueueSettings`
...
- `maintainQueuePositionForSimRingEnabled` (boolean): Indicates whether callers keep their queue position when simultaneous ringing routes a call to multiple agents.
- `forceAgentUnavailableOnBouncedEnabled` (boolean): Indicates whether Customer Assist agents are changed to unavailable after bounced calls.
- `playToneToAgentForBargeInEnabled` (boolean): Organization-wide default that plays a tone to agents when a supervisor joins an active call using barge in.
