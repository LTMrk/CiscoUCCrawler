---
doc_id: webex-cloud-calling-get-workspaces-workspaceid-features-incomingpermission
source: webex-openapi-specs/public-spec/webex-cloud-calling.json
api: Webex Cloud Calling
api_version: 1.0.0
method: GET
path: /workspaces/{workspaceId}/features/incomingPermission
operation_id: Retrieve Incoming Permission Settings for a Workspace
tags: Workspace Call Settings (1/2)
deprecated: false
scopes: 
license: CC-BY-4.0
retrieved_at: 2026-08-31T10:47:27.373706+00:00
---

# GET /workspaces/{workspaceId}/features/incomingPermission

**API:** Webex Cloud Calling
**Área:** Workspace Call Settings (1/2)
**operationId:** `Retrieve Incoming Permission Settings for a Workspace`
...
```bash
curl -X GET '/workspaces/<workspaceId>/features/incomingPermission' \
  -H 'Authorization: Bearer <TOKEN>'
```

## Respuestas correctas
**200**: OK
- `useCustomEnabled` (boolean) (**requerido**): Incoming Permission state. If disabled, the default settings are used.
- `externalTransfer` (string) (**requerido**): Call transfer setting.  * `ALLOW_ALL_EXTERNAL` - All external calls are allowed.  * `ALLOW_ONLY_TRANSFERRED_EXTERNAL` - Only externally transferred external calls are allowed.  * `BLOCK_ALL_EXTERNAL` - All external calls are blocked. Valores: ALLOW_ALL_EXTERNAL, ALLOW_ONLY_TRANSFERRED_EXTERNAL, BLOCK_ALL_EXTERNAL.
- `internalCallsEnabled` (boolean) (**requerido**): Flag to indicate if workspace can receive internal calls.
- `collectCallsEnabled` (boolean) (**requerido**): Flag to indicate if workspace can receive collect calls.
