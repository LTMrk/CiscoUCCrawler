---
doc_id: webex-cloud-calling-post-telephony-config-workspaces-workspaceid-simultaneousring-criteria
source: webex-openapi-specs/public-spec/webex-cloud-calling.json
api: Webex Cloud Calling
api_version: 1.0.0
method: POST
path: /telephony/config/workspaces/{workspaceId}/simultaneousRing/criteria
operation_id: Create Simultaneous Ring Criteria for a Workspace
tags: Workspace Call Settings (2/2)
deprecated: false
scopes: 
license: CC-BY-4.0
retrieved_at: 2026-08-31T10:47:27.387147+00:00
---

# POST /telephony/config/workspaces/{workspaceId}/simultaneousRing/criteria

**API:** Webex Cloud Calling
**Área:** Workspace Call Settings (2/2)
**operationId:** `Create Simultaneous Ring Criteria for a Workspace`
...
- `scheduleName` (string) (**requerido**): Name of the location's schedule which determines when the simultaneous ring is in effect.
- `scheduleType` (string) (**requerido**): * `businessHours` - The Schedule type that specifies the business or working hours during the day.  * `holidays` - The Schedule type that specifies the day when your organization is not open. Valores: businessHours, holidays.
- `scheduleLevel` (string) (**requerido**): * `GROUP` - Indicates the schedule level that specifies that criteria is of Group level. Valores: GROUP.
...
- `ringEnabled` (boolean) (**requerido**): When set to `true` simultaneous ringing criteria is enabled for calls that meet the current criteria. Criteria with `ringEnabled` set to `false` take priority.
...
```bash
curl -X POST '/telephony/config/workspaces/<workspaceId>/simultaneousRing/criteria' \
  -H 'Authorization: Bearer <TOKEN>' \
  -H 'Content-Type: application/json' \
  -d '{"scheduleName": "<scheduleName>", "scheduleType": "<scheduleType>", "scheduleLevel": "<scheduleLevel>", "callsFrom": "<callsFrom>", "ringEnabled": true}'
```
