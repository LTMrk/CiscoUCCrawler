---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-15-0-1-design-guid-f4de7de6d6
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_15_0_1/design/guide/ucce_b_ucce_soldg-for-unified-cce-1501/rcct_m_ucce-solution-design-considerations_15_0.html
retrieved_at: 2026-08-16T19:50:07.459402+00:00
---
...
Chapter: Unified Contact Center Enterprise Solution Design Considerations
...
##### Detailed 24000 Agent Reference Design

For the 24000 Agent Reference Design, the logical connections under general operating conditions follow the pattern of the
                                    12000 Agent Reference Design. However, there are a minimum of 12 CVPs, PGs, and Finesse servers.

### Ingress, Egress, and Cisco VVB Design Considerations
...
Configure CUBE in media pass-through mode, the default mode on the dial-peer, in the Unified CVP deployment. Media flow-around
                                          mode is supported only in Webex Experience Management.
...
The ASR 1000 Series gateways do not support VXML. So, route the VRU leg of the call to Cisco VVB . Do not use the Send To Originator setting on the CVP Call Server to route the VRU leg of the callback to the originating ASR CUBE Gateway. Route standalone
                                          CVP calls to Cisco VVB .
...
#### Cisco VVB Design Considerations

##### Cisco VVB with DTMF or ASR/TTS

The Cisco VVB allows customers to interact with the VXML browser through DTMF tones or ASR/TTS. Because the gateway does not have PSTN
                                    interfaces, voice traffic is sent using Real-Time Transport Protocol (RTP) to the Cisco VVB . The RFC 2833 uses in-band signaling in RTP packets to transmit DTMF tones. A VXML with DTMF or ASR and TTS allows you to
                                    increase the scale of the deployment and support hundreds of VXML sessions.

In a branch office topology, you can deploy a separate PSTN Gateway and a Cisco VVB to provide an extra layer of redundancy In addition, provide support for Survivable Remote Site Telephony (SRST) at the branch
                                    office.
...
##### Ingress or Cisco VVB at the Branch

Other voice services that run at the branch can affect the Ingress or Cisco VVBs . For example, if the branch is a remote Unified CM site, Unified CM can support both ACD agent lines and nonagent lines.
                                    This deployment uses the PSTN gateway for new contacts and traffic from the nonagent lines. When a branch has the VXML and
                                    Voice Gateway functions on separate devices, ensure that the dial plan sends the VRU leg to the local VXML resource. This
                                    is because the Unified CVP Call Server settransferlabel label applies only to coresident VXML and Voice Gateway configurations.

##### Colocated VXML Servers and Cisco VVBs

Your solution can either have all gateways and servers centralized or have a set of colocated Unified CVP VXML Servers and Cisco VVBs at each site.
...
CVP uses this process to formulate the destination SIP URI for the outbound calls from Unified CVP. This description covers
                                    CONNECT messages that include labels from the Unified CCE (for example, Cisco VVB , and Unified CM). It also applies for calls to the ringtone service, recording servers, and error message playback service.

This process only describes calls using the SIP subsystem, which includes audio only
                                                and basic video SIP calls.

CVP doesn't support  the SendToOriginator algorithm only for a colocated Cisco VVB and Ingress Voice Gateway. Cisco Virtualized Voice Browser (VVB) does not support this algorithm because the gateways cannot
                                                be colocated when you use Cisco VVB.
...
The setting for SendtoOriginator only applies to callers on Cisco Ingress Voice Gateways (the SIP UserAgent header is selected). Non-Cisco IOS Gateways do not have the CVP bootstrap service that the Cisco VVB uses.
...
#### Custom Remote Server Design Considerations

Custom elements in a Call Studio application can be run outside the VXML server by deploying them on a custom remote server.
                                 Refer to Configuration Guide for Cisco Unified Customer Voice Portal for details on the deployment and configuration of remote server.
...
Cisco VVB —Prompts are deployed on flash memory.

A Cisco VVB is  a PSTN Gateway, which has the Ingress Voice Gateway colocated. Store only critical prompts here, such as error messages
                                                or messages for when the WAN is down.

When recorded in G.711 mu-law format, typical prompts are about 10 to 15 KB in size. For these gateways, size the flash memory
                                                by factoring in the number of prompts and their sizes. Also leave space for storing the Cisco IOS image.
...
An example is the transfer of files from the CVP Operations Console to the Ingress Cisco VVB using Bulk Administration File Transfer (BAFT).
...
Cisco VVB does not support the Media Streaming feature.

###### Design Considerations in using VVB Media Streaming

Streaming service provides continuous audio streaming to callers.

In Cisco VVB, the support for audio streaming started in release 12.5 via HTTP(S).
...
The Cisco Wide Area Application Services (WAAS) system is a set of devices called Wide Area Application Engines (WAEs). The
                                       WAEs work together to optimize TCP traffic over your network. Cisco WAAS uses TCP optimization techniques and application
                                       acceleration features to overcome the most common challenges in transporting traffic over a WAN. When deployed at the periphery
                                       of the network on the Cisco VVB , Cisco WAAS performs these functions:
...
In nonstreaming mode, the Cisco VVB downloads the entire audio file from the HTTP server before the Media Player can start playing the prompt. This can cause
                                          a delay for the caller. For small files, the delay is only a few milliseconds. You can avoid the delay for larger files by
                                          using either caching or streaming mode.
...
For the Cisco VVB , you can assign a FreshTime value to the files with the http client cache refresh command. But, that value only applies if the previous sequence fails to set a value.
...
The number of Virtualized Voice Browsers that your solution requires depends on the VRU ports that your solution needs on
                              the Cisco VVB . Install Cisco VVB depending on the number of SIP sessions required for your solution. This table lists feature support by
                              Cisco VVB:
...
Shared lines are supported on the ACD line for up to five devices, enabled via Agent Desk Settings.
...
Cisco Finesse provides a REST API for client applications to access the supported server features. The REST API transports
                                 XML payloads over HTTP or HTTPS.
...
In Unified CCE, the identity is not the same as the username or peripheralID configured in Unified CCE.
...
Desktop Chat is supported with the unrestricted versions of IM&P only if Finesse is accessed via HTTP. To access Finesse using
                                                         HTTP, use the utils finesse application_https_redirect disable CLI.

Desktop Chat is not supported with the unrestricted versions of IM&P.

##### Finesse OVA for large deployments

Deploy the " Large " Cisco Finesse OVA with 8 CPUs, if you plan for any one of the following:

Deployments with 2000 active users and configuring more than eight XML-gadgets per user on average

More than six JSP-based gadgets per user on average

This configuration helps to achieve a quick desktop failover experience.
...
The client updates Live Data reports from a Live Data event stream from the Live Data
                                 service. For more information on Live Data control and data flows, see the Serviceability Guide for Cisco Unified Contact Center Enterprise at http://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html .
...
In Unified CCE, the Live Data Service resides on its own
                                    VM.
...
To allow client's IP addresses, see the commands provided in the Task Routing section of the Cisco Unified Contact Center Enterprise Features Guide available at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-feature-guides-list.html
...
### Cisco Contact Center SIP Proxy Design Considerations

Consider these points when adding CCCSP into your solution:

CCCSP is a VM that can reside on UCS B, C, and E series servers.

A standard CCCSP topology consists of 2 redundant, geographically separated gateways. The gateways have one proxy module each. They use SRV
                                    priority for redundancy of proxies. They do not use HSRP.

CCCSP can coreside with VXML or TDM gateways.

Configure TDM gateways with SRV or with Dial Peer Preferences to use the primary and secondary CCCSP proxies.

Set up CCCSP with Server Groups to find the primary and back up Unified CVP, Unified CM, and Cisco VVBs .

Set up Unified CVP with a Server Group to use the primary and secondary CCCSP proxies.

Set up Unified CM with a Route Group with multiple SIP Trunks, to use the primary and secondary CCCSP proxies

#### Performance Matrix for CCCSP Deployment

The load values in this section are currently undergoing validation and have not yet been finalized.

CCCSP baseline tests in isolation on the proxy give a maximum capacity of 450 TCP transactions per second. Use this as the highest
                                    benchmark and most stressed condition allowable. From the proxy server perspective, a CVP call entails an average of four
                                    separate SIP calls:
...
Record Route is turned off by default on CCCSP .
...
If you have a SIP Proxy, use different VRU labels for the Unified CM routing client and the CVP routing clients. The Unified
                                    ...
                                    client uses its VRU label to send a call to the Cisco VVB for treatment. When a call comes to CVP, CVP transfers to the CVP routing client's VRU label. It then delivers the call to
                                    the Cisco VVB for queuing treatment.
...
[CVP routing client VRU label + correlation-id]: pointing to Cisco VVBs
```
...
You can use load balancers with the Unified CVP solution components to provide load distribution and high availability for
                                 HTTP, HTTPS, and MRCP traffic. Load balancers can spread the rendering of the VXML pages between Cisco VVBs and VXML Server. Load balancers can also spread the fetching of the media files for VRU scripts run from media servers.

If your solution is MRCPv2, use CCCSP for load balancing.
...
Unified CVP sends the call to the Cisco VVB . The caller hears the VRU. However, the call is not recorded.
...
SBC configured as a SIP proxy (instead of CCCSP ) for messages between Cisco components
...
| Note | This process only describes calls using the SIP subsystem, which includes audio only
                                                and basic video SIP calls. CVP doesn't support  the SendToOriginator algorithm only for a colocated Cisco VVB and Ingress Voice Gateway. Cisco Virtualized Voice Browser (VVB) does not support this algorithm because the gateways cannot
                                                be colocated when you use Cisco VVB. |
|---|---|
...
| Number of configured lines on phone | No limit described (only monitoring one line) | Do not configure more than four lines. If you do, the agent cannot sign in on any of the lines. This generates a configuration
                                                alert. |
| Shared ACD Line | Shared lines are supported on the ACD line for up to five devices, enabled via Agent Desk Settings. | Shared lines are supported on the ACD line for up to five devices, enabled via Agent Desk Settings. |
...
| Note | Desktop Chat is supported with the unrestricted versions of IM&P only if Finesse is accessed via HTTP. To access Finesse using
                                                         HTTP, use the utils finesse application_https_redirect disable CLI. |
|---|---|
...
| Note | The load values in this section are currently undergoing validation and have not yet been finalized. |
|---|---|
...
| Note | If your solution is MRCPv2, use CCCSP for load balancing. |
|---|---|
