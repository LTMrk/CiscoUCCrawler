---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-admin-12-5-1-systemconfig-cucm-b-system-configuration-guide-1251-cucm-b-d9ba814ee0
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/admin/12_5_1/systemConfig/cucm_b_system-configuration-guide-1251/cucm_b_system-configuration-guide-1251_chapter_01011010.html
retrieved_at: 2026-08-16T17:32:14.365382+00:00
---
...
Optional. Configure Region-specific settings for your MRA endpoints. For example, if you expect MRA endpoints to use video,
                                          you may want to increase the Maximum Session Bit Rate for Video Calls setting as the default setting of 384 kbps may be too low for some video endpoints.

Step 3

Configure a Device Pool for Mobile and Remote Access

Assign your Date/Time Group and Region configuration to the device pool that your MRA endpoints will use.

Step 4

Configure ICE

Optional. ICE is an optional deployment that uses STUN and TURN services to analyze the available media paths for an MRA call
                                          and then to select the best path. ICE adds potentially to the call setup time, but increases the reliability of MRA calls.

Step 5

Configure Phone Security Profile for Mobile and Remote Access

Use this procedure to set up a phone security profile to be used by MRA endpoints.

Step 6

Configure MRA Access Policy for Cisco Jabber Users

Cisco Jabber only.  Set up an MRA Access Policy for Cisco Jabber users. Cisco Jabber users must be enabled with MRA access
                                          within their user profiles in order to use the MRA feature.
...
Configure and provision endpoints that will use the MRA feature.
...
### Configure MRA Access
                           	 Policy for Cisco Jabber Users

Use this procedure
                                 		  to set up an MRA Access Policy for Cisco Jabber users. Cisco Jabber users must be
                                 		  enabled with MRA access within their user profiles in order to use the MRA
                                 		  feature. 
                                 		The minimum Expressway release for MRA Access Policy support with Cisco Jabber  is X8.10.

The MRA Access
                                             			 Policy is not required for non-Jabber users.

For more details on user profiles, see User Profile Overview .
...
For Cisco Jabber users, the MRA access policy that you configured must be associated to your Cisco Jabber users during the
                                 LDAP sync. For information on how to provision end users, see the End User Configuration .

### Configure Endpoints for MRA

Provision and configure endpoints for Mobile and Remote Access:

For Cisco Jabber clients, refer to Cisco Jabber Configuration Task Flow

For other endpoints, refer to Endpoint Device Configuration
...
| Step 2 | Configure Maximum Session BitRate for Video | Optional. Configure Region-specific settings for your MRA endpoints. For example, if you expect MRA endpoints to use video,
                                          you may want to increase the Maximum Session Bit Rate for Video Calls setting as the default setting of 384 kbps may be too low for some video endpoints. |
| Step 3 | Configure a Device Pool for Mobile and Remote Access | Assign your Date/Time Group and Region configuration to the device pool that your MRA endpoints will use. |
| Step 4 | Configure ICE | Optional. ICE is an optional deployment that uses STUN and TURN services to analyze the available media paths for an MRA call
                                          and then to select the best path. ICE adds potentially to the call setup time, but increases the reliability of MRA calls. |
| Step 5 | Configure Phone Security Profile for Mobile and Remote Access | Use this procedure to set up a phone security profile to be used by MRA endpoints. |
| Step 6 | Configure MRA Access Policy for Cisco Jabber Users | Cisco Jabber only.  Set up an MRA Access Policy for Cisco Jabber users. Cisco Jabber users must be enabled with MRA access
                                          within their user profiles in order to use the MRA feature. |
| Step 7 | Configure Users for MRA | For Cisco Jabber users, the User Policy that you set up must be applied to their End User Configurations. |
| Step 8 | Configure Endpoints for MRA | Configure and provision endpoints that will use the MRA feature. |
...
| Note | For more details on user profiles, see User Profile Overview . |
|---|---|
