---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-15-0-1-ucce-b-1501-6ed45ec9bc
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_15_0_1/ucce_b_1501_features-guide/rcct_m_1501_task-routing.html
retrieved_at: 2026-08-16T20:09:43.957046+00:00
---
...
Unified CCE— Unified CCE Administration and Configuration Manager Tools
...
See https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-implementation-design-guides-list.html for the maximum number of tasks supported within an MRD and across MRDs for a single agent.
...
By default, access to Customer Collaboration Platform administration user interface is restricted. Administrator can provide
                              access by allowing clients IP addresses and revoke by removing the client's IP from the allowed list. For any modification
                              to the allowed list to take effect, Cisco Tomcat must be restarted.
...
See Configure Network VRU and Network VRU Scripts in Unified CCE .

3

Configure the MR PG and PIM.

See the Configure the Media Routing PG and PIM section.
...
Finesse connects to Customer Collaboration Platform to transfer Task Routing tasks and resubmit tasks for RONA. The Finesse
                                 AWDB user requires special database permissions to access Customer Collaboration Platform connection information. Map the
                                 user to the Side A, AWDB, and primary databases. In these databases, give the user the db_datareader and public roles.
...
See https://www.cisco.com/c/en/us/support/customer-collaboration/finesse/products-user-guide-list.html .
...
- Execute the following SQL queries as the SQL administrative user "sa" or as a user with sysadmin privileges.
...
The Network VRU is used to queue nonvoice tasks if an agent is not available to handle them. The Network VRU Script is used
                                 ...
                                 see https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-user-guide-list.html .
...
In the Peripheral ID field, enter the Peripheral ID that you recorded when you configured the Media Routing PG and PIM (applicable to Unified
                                                CCE).
...
These settings include the hostnames of the MR PGs and the Application Connection Port you specified when setting up the MR
                                       PG and PIM (applicable to Unified CCE).
...
Select the main site or remove site and in the External Machines section, click the + icon.
...
Enter the Application Port you specified when setting up the MR PG and PIM. The default value is 38001.
...
Change the registry key on either the Side A or B Router in Unified CCE.
...
For complete multichannel scripting information in Unified CCE, see https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-user-guide-list.html .
...
Enterprise Skill Group Real Time in Unified CCE
...
| 2 | Configure a Network VRU and Network VRU scripts. See Configure Network VRU and Network VRU Scripts in Unified CCE . |  |
| 3 | Configure the MR PG and PIM. See the Configure the Media Routing PG and PIM section. |  |
...
| Step 5 | Select the User Mapping page, and perform the following: Verify that the databases associated with Side A and AWdb are checked. Check the master database. Select the Side A database. In the Database role membership for section, check the db_datareader and public roles. Repeat this step for the AWdb and master databases. Click OK . |
| Step 6 | Repeat these steps on the Side B Logger. |
| Step 7 | Execute the following SQL queries as the SQL administrative user "sa" or as a user with sysadmin privileges. For <user>, enter the Finesse AWDB username. If the Finesse AWDB user is a domain user, rather than a SQL user, use the <domain\user>
                                             format. USE master
GO
GRANT CONTROL ON CERTIFICATE :: UCCESymmetricKeyCertificate TO " <user> "
GRANT VIEW DEFINITION ON SYMMETRIC KEY :: UCCESymmetricKey TO " <user> " |
...
|---|---|
| Step 2 | Click Add. |
| Step 3 | Select the main site or remove site and in the External Machines section, click the + icon. |
| Step 4 | Click the Main or RemoteSite tab |
| Step 5 | Click the New button that appears on top of the grid. The Add Machine dialog box appears. |
| Step 6 | Select Customer Collaboration Platform from the drop-down list. |
| Step 7 | Enter the fully qualified domain name (FQDN), hostname or IP address in the Hostname field. Note The system attempts to convert the value you enter to FQDN. | Note | The system attempts to convert the value you enter to FQDN. |
| Note | The system attempts to convert the value you enter to FQDN. |
| Step 8 | Enter the Customer Collaboration Platform Administration username and password. |
| Step 9 | Select the Side A and Side B Media Routing PGs. |
| Step 10 | Enter the Application Port you specified when setting up the MR PG and PIM. The default value is 38001. |
| Step 11 | Click Save . |
...
|---|---|
| Step 2 | From the Manage menu, configure the following: Item to Configure Details Media Routing Domains Create an MRD for each type of task that the third-party multichannel application submits to CCE (email, chat, and so on). Precision Queues Configure either skill groups or precision queues. If you configure precision queues: For Media Routing Domain , select one of the Task Routing MRDs you created. Associate agents with attributes that are part of the precision queue steps. | Item to Configure | Details | Media Routing Domains | Create an MRD for each type of task that the third-party multichannel application submits to CCE (email, chat, and so on). | Precision Queues | Configure either skill groups or precision queues. If you configure precision queues: For Media Routing Domain , select one of the Task Routing MRDs you created. Associate agents with attributes that are part of the precision queue steps. |
...
| Step 4 | Configure the following: Item to Configure Details Call Types Create call types for Task Routing. Dialed Numbers Create dialed numbers for Task Routing. Add the numbers or strings that the custom application will use when submitting task
                                                         requests. On the Attributes tab, select a Task Routing MRD from the Media routing domain drop-down list box. On the Dialed Number Mapping tab, map the script selector to a call type you created for Task Routing. Important Each dialed number must be associated with a call type. Default call type is not supported for tasks submitted with Task Routing
                                                                           APIs. Skill Groups Configure either skill groups or precision queues. If you configure skill groups: For Media Routing Domain , select one of the Task Routing MRDs you created. Assign agents to the skill group. Expanded Call Variable You can use an existing Expanded Call Variable, or you can create an Expanded Call Variable for Task Routing, depending on
