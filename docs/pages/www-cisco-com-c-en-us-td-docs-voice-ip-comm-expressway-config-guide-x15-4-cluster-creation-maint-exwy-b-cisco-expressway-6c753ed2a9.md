---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-expressway-config-guide-x15-4-cluster-creation-maint-exwy-b-cisco-expressway-6c753ed2a9
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/expressway/config_guide/X15-4/cluster_creation_maint/exwy_b_cisco-expressway-cluster-creation-and-maintenance-deployment-guide-x154/exwy_m_optional-use-fully-qualified-domain.html
retrieved_at: 2026-08-16T15:12:01.724561+00:00
---
...
To change an Expressway-E cluster to use FQDNs, you use the addresses that are entered in the mapping table (see Cluster Address Mapping for Expressway-E Clusters ).
...
If the cluster is an Expressway-E cluster, and you are aiming to enforce TLS verification between the peers, then the peer
                                          address fields should match the identities presented in the certificates. Check that both Clustering and Certificate status messages are green.
...
| Step 3 | On every peer in the cluster: Change the chosen peer address field from the IP address to the corresponding FQDN (if you did mappings, they should be replicated
                                                in all peers at this stage). Click Save . Caution Make sure you only change one peer address on each box. | Caution | Make sure you only change one peer address on each box. |
