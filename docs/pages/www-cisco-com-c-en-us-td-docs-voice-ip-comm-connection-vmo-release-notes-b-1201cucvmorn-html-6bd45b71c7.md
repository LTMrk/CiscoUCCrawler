---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-connection-vmo-release-notes-b-1201cucvmorn-html-6bd45b71c7
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/vmo/release/notes/b_1201cucvmorn.html
retrieved_at: 2026-08-16T18:51:21.563050+00:00
---
...
# Release Notes for
	 Cisco ViewMail for Microsoft Outlook Release 12.0(1)
...
These release notes
		...
		information for Cisco ViewMail for Microsoft Outlook Release 12.0(1).

ViewMail for Outlook
		12.0(1) is available only from the Cisco Download Software website.

## Introduction

Cisco ViewMail for Microsoft
		Outlook provides a visual interface with which users can send, listen to, and
		manage their Cisco Unity Connection voice messages from Outlook.

Note the following considerations with ViewMail for Outlook:

- Users get voice
						messages in the same Inbox as their email.

- Voice messages
						sent from Outlook do not appear in the Sent Items folder.

- ViewMail for
						Outlook is not required with the single-inbox feature. Without it, however,
						users can only play voice messages; they cannot compose, reply to, or forward
						them.

- ViewMail for
						Outlook is required for users to play secure messages in the Exchange mailbox.

- Users get voice
						messages in a separate mail folder in Outlook.

- New voice messages
						get identified by a separate voice message icon.

- ViewMail for
						Outlook is required for users to play secure messages in the Unity Connection
						mailbox.

- On Windows operating
				system with ViewMail for Outlook, the text of the voice messages appears in
				black color and the ViewMail category appears in blue color. However, on MAC
...
- The user workstation must
		  have at least 10 MB of hard-disk space available for ViewMail for Outlook. (If
...
- For a list of all
		  ...
		  Cisco Unity Connection Components” section of System Requirements for Cisco Unity Connection Release 12.x at https:/​/​www.cisco.com/​c/​en/​us/​td/​docs/​voice_ip_comm/​connection/​12x/​requirements/​b_​12xcucsysreqs.html .
...
## New and Changed
	 Requirements and Support—Release 12.0(1)

### Additional
	 Languages for ViewMail for Outlook

All locales will be released soon for ViewMail for Outlook 12.0(1)
		release.
...
(For information on
		new and changed requirements and support in earlier versions of Cisco Unity
		Connection ViewMail for Microsoft Outlook, see the applicable release notes at http:/​/​www.cisco.com/​en/​US/​products/​ps6509/​prod_​release_​notes_​list.html .)

### Support for TLS
	 Version 1.2

With release 12.0(1) and later,
		  Viewmail for Outlook supports TLS 1.2 to communicate with Cisco Unity
...
### Task List for
	 Installing ViewMail for Outlook 12.0(1) for the First Time
...
- Download ViewMail for
		  Outlook from the Cisco Download Software website. See the “ Downloading ViewMail for Outlook 12.0(1) ”
		  section.
...
- Install ViewMail for
		  Outlook. Provide users with instructions, if applicable. See the “ Installing or Upgrading to ViewMail for Outlook 12.0(1) ”
		  section.
...
- Using the ViewMail Options
				...
				instructions, if applicable. See the “ Installing ViewMail for Outlook 12.0(1) Using Command Line Switches ”
				section.

### Task List for
	 Upgrading to ViewMail for Outlook 12.0(1)
...
(See the “ Users ” chapter of the System
				Administration Guide for Cisco Unity Connection Release 12.x at http:/​/​www.cisco.com/​c/​en/​us/​td/​docs/​voice_ip_comm/​connection/​11x/​administration/​guide/​11xcucsagx.html .)
...
(See the Unified
			 Messaging Guide for Cisco Unity Connection Release 12.x at http:/​/​www.cisco.com/​c/​en/​us/​td/​docs/​voice_ip_comm/​connection/​11x/​unified_messaging/​guide/​11xcucumgx.html .)

(See the
			 Security Guide for Cisco Unity Connection Release 12.x at http:/​/​www.cisco.com/​c/​en/​us/​td/​docs/​voice_ip_comm/​connection/​11x/​security/​guide/​11xcucsecx.html .)
...
Use
							 ...
							 12.0(1) It installs prerequisite software, as needed; places the
...
Use
							 this file for quick minor upgrades for ViewMail 32-bit with versions 12.0(1)
							 and later.

Use
							 this file for quick minor upgrades for ViewMail 64-bit with versions 12.0(1)
							 and later.

### Installing
	 ViewMail for Outlook 12.0(1) Using Command Line Switches

To install ViewMail for
			 Outlook 12.0(1) through command line switches :
...
### Incompatibility
	 with Exchange System Management Tools

ViewMail for Outlook
		version 12.0(1) does not work correctly when it is installed on a computer that
		also has Microsoft Exchange System Management Tools installed.
...
### ViewMail for
	 Outlook 12.0(1) Limitations Regarding Reply and Forward Cisco Unity Voice
	 Messages

In Unity Connection,
		users cannot reply to, or forward Cisco Unity voice messages or .wav files by
		using ViewMail for Outlook version 12.0(1).

### ViewMail for
	 Outlook 12.0(1) Limitations Regarding Icon Update when a Voice Message is read
	 or unread

When a voice mail is
		marked read or unread using ViewMail for Outlook 12.0(1), the state of voice
		mail Icon is not updated regardless of the Microsoft Outlook version.

### ViewMail for
	 Outlook 12.0(1) Limitations Regarding Cisco Unity 5 Message Playback
...
### ViewMail for
	 Outlook Limitation Regarding Secured Voice Message Play with OpenAM SSO

If OpenAM Single
		Sign-On for Cisco PCA is enabled in ViewMail for Outlook, we recommend not to
		play the secure messages. If you still play the secure messages after enabling
		OpenAM Single Sign-On, VMO displays the “Loading Audio” prompt and never plays
		the audio message.

- In Cisco Unity Connection
		  OS Administration, disable SSO for REST API.

- Create a rule that puts
		  "/ciscopca/unitymessageserver/ums.do?*" in the "Not Enforced URI" rule section
		  of OpenAM.
...
You can find the
		...
		Outlook version 12.0(1) by using Bug Toolkit, an online tool available for
...
### Open
	 Caveats—Release 12.0(1)

There is no open caveat in ViewMail for Outlook version 12.0(1)
		  release.
...
This section lists the resolved caveats for VMO 12.0(1) release. Click
		  a link in the Caveat Number column to view the latest information on the caveat
		  in Bug Toolkit.

(Caveats are listed in order by severity, then by component, then by
		  caveat number.)
...
Viewmail 12.x msi & exe contain no digital(provider)
						signature
...
Support Of Exchange Cache Mode while configuring ViewMail
						with Outlook using SIB
...
| Single-inbox users | Users get voice
						messages in the same Inbox as their email. Voice messages
						sent from Outlook do not appear in the Sent Items folder. ViewMail for
						Outlook is not required with the single-inbox feature. Without it, however,
						users can only play voice messages; they cannot compose, reply to, or forward
						them. ViewMail for
						Outlook is required for users to play secure messages in the Exchange mailbox. |
|---|---|
| IMAP users | Users get voice
						messages in a separate mail folder in Outlook. New voice messages
						get identified by a separate voice message icon. ViewMail for
						Outlook is required for users to play secure messages in the Unity Connection
						mailbox. |
...
| Note | If you are
		  upgrading from a ViewMail for Outlook version earlier than 12.0(1), see the
		  “ Task List for Installing ViewMail for Outlook 12.0(1) for the First Time ”
		  section. The older version is uninstalled automatically, and installing 12.0(1)
...
| Step 2 | In the tree
			 control on the download page, expand Products > Unified
				...
				Connection ViewMail for Microsoft Outlook Version 12.0 . |
| Step 3 | In the Latest
			 Releases folder, select VMO 12.0 . |
| Step 4 | On the right
...
| Step 2 | In the
			 ViewMail folder, double-click the applicable file: Setup.exe Use
							 ...
							 this file for quick minor upgrades for ViewMail 64-bit with versions 12.0(1)
							 and later. Note For successful installation and upgrade of ViewMail for Outlook
				12.0(1), you must download and run the setup.exe. | Setup.exe | Use
							 this file for new installations and for upgrades from versions earlier than
							 12.0(1) It installs prerequisite software, as needed; places the
							 AdminConfig.xml file in the correct location; and uninstalls older versions
							 during upgrades. This
							 ...
							 this file for quick minor upgrades for ViewMail 64-bit with versions 12.0(1)
							 and later. | Note | For successful installation and upgrade of ViewMail for Outlook
				12.0(1), you must download and run the setup.exe. |
| Setup.exe | Use
							 this file for new installations and for upgrades from versions earlier than
							 12.0(1) It installs prerequisite software, as needed; places the
							 AdminConfig.xml file in the correct location; and uninstalls older versions
...
| ViewMail_64.msi | Use
							 this file for quick minor upgrades for ViewMail 64-bit with versions 12.0(1)
							 and later. |
| Note | For successful installation and upgrade of ViewMail for Outlook
				12.0(1), you must download and run the setup.exe. |
| Step 3 | Follow the
...
| Step 1 | In Outlook,
			 open the ViewMail Settings or Options dialog: Outlook 2010 and above On the ViewMail tab, select Settings. Outlook 2007 From the Tools menu, select Options, then the ViewMail tab. | Outlook 2010 and above | On the ViewMail tab, select Settings. | Outlook 2007 | From the Tools menu, select Options, then the ViewMail tab. |
|---|---|---|---|---|---|
| Outlook 2010 and above | On the ViewMail tab, select Settings. |
| Outlook 2007 | From the Tools menu, select Options, then the ViewMail tab. |
...
| Note | The limitation of
		secured voice message play with OpenAM Single Sign-On is not applicable for
...
|---|---|---|---|
| CSCvg40211 | VMO | 4 | Viewmail 12.x msi & exe contain no digital(provider)
						signature |
| CSCvf75449 | VMO | 4 | Support Of Exchange Cache Mode while configuring ViewMail
						with Outlook using SIB |