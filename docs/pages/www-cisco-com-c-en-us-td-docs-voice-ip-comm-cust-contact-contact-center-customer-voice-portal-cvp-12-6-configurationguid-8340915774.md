---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-customer-voice-portal-cvp-12-6-configurationguid-8340915774
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/customer_voice_portal/cvp_12_6/ConfigurationGuideCVP12_6/guide/ccvp_b_1261-configuration-guide-for-cisco-unified-customer-voice-portal/ccvp_m_1252-unified-cvp-security.html
retrieved_at: 2026-08-21T06:53:25.008348+00:00
---
...
com.sun.management.jmxremote.ssl= true com.sun.management.jmxremote.rmi.port=3000 javax.net.ssl.keyStore= C:\Cisco\CVP\conf\security\.keystore
javax.net.ssl.keyStorePassword= <keystore_password> |
| Step 11 | Run the regedit command. Append the following to the file at: HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Apache Software Foundation\Procrun 2.0\Webservices Manager\Parameters\Java Djavax.net.ssl.trustStore=C:\Cisco\CVP\conf\security\.keystore 
Djavax.net.ssl.trustStorePassword=<keystore_password> 
Djavax.net.ssl.trustStoreType=JCEKS |
...
com.sun.management.jmxremote.ssl= true com.sun.management.jmxremote.rmi.port=9697 javax.net.ssl.keyStore=C:\Cisco\CVP\conf\security\.keystore
javax.net.ssl.keyStorePassword=<keystore_password> |
| Step 14 | Run the regedit command. Append the following to the file at: HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Apache Software Foundation\Procrun 2.0\VXML\Parameters\Java Djavax.net.ssl.trustStore=C:\Cisco\CVP\conf\security\.keystore 
Djavax.net.ssl.trustStorePassword=<keystore_password> 
Djavax.net.ssl.trustStoreType=JCEKS |
...
Djavax.net.ssl.trustStoreType=JCEKS Note After securing the ports for JMX, JConsole can be accessed only after performing the defined steps for JConsole listed in
                                                         ...
                                                         the Oracle docs. | Note | After securing the ports for JMX, JConsole can be accessed only after performing the defined steps for JConsole listed in
                                                         the OpenJDK docs. | Note | After securing the ports for JMX, JConsole can be accessed only after performing the defined steps for JConsole listed in
                                                         the Oracle docs. |
...
| Step 4 | Enter the following command in the keystore to import the Cloud Connect certificate to the call server. c:\Cisco\CVP\jre\bin\keytool.exe -import -keystore .keystore -storetype JCEKS -trustcacerts 
   -alias <cloud connect publisher or cloud connect subscriber> –file <filepath> |
| Step 5 | Enter the keystore password when prompted. To retrieve the keystore password, do the following: At the command prompt, enter %CVP_HOME%\conf\security.properties . Command prompt returns keystore password in the following format: Security.keystorePW = <keystore password> |
...
| Step 1 | Log in to the Call server, retrieve the keystore password from the security.properties file: At the command prompt, enter more %CVP_HOME%\conf\security.properties . Security.keystorePW = <Returns the keystore password> |
|---|---|
...
| Step 1 | Log into the OAMP server, retrieve the keystore password from the security.properties file: At the command prompt, enter more %CVP_HOME%\conf\security.properties . Security.keystorePW = <Returns the keystore password> |
|---|---|
...
| Step 1 | Log into the Reporting server, retrieve the keystore password from the security.properties file: At the command prompt, enter more %CVP_HOME%\conf\security.properties . Security.keystorePW = <Returns the keystore password> |
|---|---|
| Step 2 | Install OpenSSL (64 bit) on your machine. |
| Step 3 | Add OpenSSL bin path to the Windows environment path variable. Example: path=C:\Program Files\OpenSSL-Win64\bin |
| Step 4 | Go to C:\Cisco\CVP\conf\security . |
...
| Step 1 | Log in to the OAMP server, retrieve the keystore password from the security.properties file: At the command prompt, enter more %CVP_HOME%\conf\security.properties . Security.keystorePW = <Returns the keystore password> |
|---|---|
...
| Step 1 | Log in to the Reporting Server, retrieve the keystore password from the security.properties file: At the command prompt, enter more %CVP_HOME%\conf\security.properties . Security.keystorePW = <Returns the keystore password> |
|---|---|
| Step 2 | Download OpenSSL (64 bit) and install on your machine. |
| Step 3 | Add OpenSSL bin path to the Windows environment path variable. Example: path=C:\Program Files\OpenSSL-Win64\bin |
| Step 4 | Go to C:\Cisco\CVP\conf\security . |
