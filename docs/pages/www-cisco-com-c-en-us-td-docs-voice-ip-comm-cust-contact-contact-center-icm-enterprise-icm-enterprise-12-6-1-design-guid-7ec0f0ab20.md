---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-12-6-1-design-guid-7ec0f0ab20
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_1/design/guide/ucce_b_ucce_soldg-for-unified-cce-1261/design_considerations_for_integrated_features.html
retrieved_at: 2026-08-16T19:52:16.874721+00:00
---
...
Unified CCE solution leverages Artificial Intelligence (AI) and Natural Language Understanding (NLU) to provide Contact Center AI Services that assist agents via the Agent Answers and Call Transcript gadgets on the Cisco Finesse desktop. Unified CCE solution integrates
...
In CCE, you associate the Contact Center AI Services with all or specific call types and then provision these services for a specific agent or multiple agents. In Cisco Finesse,
                           you configure the Agent Answers and Call Transcript gadgets for the required teams. During an incoming call, if the CCE and
                           Agent Desktop configurations are present for an agent and if the call has associations to the AI services, the gadgets are
                           displayed to the agents. When the agent speaks with the caller:
...
When an agent is available, CCE sends the GED CONNECT message to the CVP. The GED CONNECT message indicates which Contact Center AI Services are enabled for that agent.

CVP transfers the call to the agent through Unified CM. CCE sends the call information (which includes details on which Contact Center AI Services are enabled for that call) to the Finesse server.
...
If the GED CONNECT message (received at step 3) indicated that at least one Contact Center AI Feature is enabled for the agent, CVP requests CUBE to start forking the media streams of the agent and the caller. The forking request
...
For details on how to configure Contact Center AI Services , see the Agent Answers chapter and the Call Transcription chapter in the Cisco Unified Contact Center Enterprise Features Guide at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-feature-guides-list.html .
...
Following are the concepts that are to be handled in a VAV application. For more information on these concepts, see Google's Dialogflow documentation at https://cloud.google.com/dialogflow/docs/ .
...
The VAV feature is supported only on VVB. It is not supported with the VXML Gateway.
...
#### DialogflowCX Element Call Flow
...
| Note | The VAV feature is supported only on VVB. It is not supported with the VXML Gateway. |
|---|---|
