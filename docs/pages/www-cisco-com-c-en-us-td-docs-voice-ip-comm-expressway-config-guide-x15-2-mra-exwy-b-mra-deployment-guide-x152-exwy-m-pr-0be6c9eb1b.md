---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-expressway-config-guide-x15-2-mra-exwy-b-mra-deployment-guide-x152-exwy-m-pr-0be6c9eb1b
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/expressway/config_guide/X15-2/mra/exwy_b_mra-deployment-guide-x152/exwy_m_provisioning-mra-devices.html
retrieved_at: 2026-08-16T15:12:44.079862+00:00
---
...
Activation Codes provide a simple and secure way to onboard remote endpoints for Mobile and Remote Access (MRA). This feature
                              eliminates the need for an MRA user to be on-premises the first time they use their phones. Remote users can plug in the phone,
...
If you want to use your own Expressway-E certificate, you can use the cloud to distribute certificates to MRA phones so that
                              they can establish trust with Expressway-E. With this option, you must upload your certificate first to Expressway-E, and
                              then upload the root CA certificate to the Phone-Edge-Trust store on Cisco Unified Communications Manager. The certificates are uploaded to the Cisco cloud so that the phone can download
                              them during the device activation process. For more information, see the Cisco Expressway Certificate Creation and Use Deployment Guide .

Rebooting services are not required.
...
When you start Device Activation Service on UCM publisher to on-board clients over Mobile and Remote Access, you need to start
                                             the UDS and CCM services as well. Moreover, delete and rediscover the UCM cluster in Unified Communications configuration
                                             in Expressway-C, as doing a refresh of servers will not work.
...
As of release X14.0, if you are onboarding supported Cisco IP Phone 78xx Series and 88xx Series phones for Mobile and Remote
                                          Access, the phones switch to MRA mode only if the Allow Activation Code via MRA checkbox is checked within the Phone Configuration window of Cisco Unified Communications Manager .

Using this approach, you must configure Activation Code onboarding for MRA phones. In addition, the MRA phone user must enter
                                          the correct activation code to activate and use the phone.
...
TCP port 443 network requirement for the Cisco Cloud onboarding —Connectivity must be enabled from Unified Communications Manager and IM and Presence Service/publisher over TCP port 443
                                    for the following URLs/connections to the Cisco Cloud.
...
The TCP port 443 must be open from the Cisco Unified CM publisher node for outbound HTTPS requests (Cisco Cloud onboarding).
...
Enable OAuth Authentication in Cisco Unified Communication Manager and Expressway:

Enable OAuth on Cisco Unified CM:
...
Onboard Cisco Unified Communication Manager to the cloud for MRA activation code onboarding.
...
Check Trusted Cisco Manufacturing Installed Certificates (MICs) installed. They are required to access the activation code
                                          onboarding functionality:

Cisco Manufacturing Root certificates must be present in the CallManager-trust store to perform onboarding activity.

On Expressway-E, choose Maintenance > Security > Trusted CA certificate .

Click Activation code onboarding trusted CA certificates .
...
With this method, the administrator activates the phone in on-premises mode before resetting the activation code requirement
                              and shipping to the MRA user, who will activate the phone in MRA mode.
...
| Note | Rebooting services are not required. |
|---|---|

| Note | When you start Device Activation Service on UCM publisher to on-board clients over Mobile and Remote Access, you need to start
                                             the UDS and CCM services as well. Moreover, delete and rediscover the UCM cluster in Unified Communications configuration
                                             in Expressway-C, as doing a refresh of servers will not work. |
|---|---|
...
| Note | As of release X14.0, if you are onboarding supported Cisco IP Phone 78xx Series and 88xx Series phones for Mobile and Remote
                                          Access, the phones switch to MRA mode only if the Allow Activation Code via MRA checkbox is checked within the Phone Configuration window of Cisco Unified Communications Manager . Using this approach, you must configure Activation Code onboarding for MRA phones. In addition, the MRA phone user must enter
                                          the correct activation code to activate and use the phone. For details on configuring Activation Code Onboarding, see the "Device Onboarding via Activation Codes" chapter of Feature Configuration Guide for Cisco Unified Communications Manager . |
|---|---|
...
| Steps | Procedures |
|---|---|
| Step 1 | Enable OAuth Authentication in Cisco Unified Communication Manager and Expressway: Enable OAuth on Cisco Unified CM: From Cisco Unified CM Administration, go to System > Enterprise Parameters . Set the OAuth Refresh Login Flow parameter to Enabled . Click Save . Enable OAuth Refresh authentication on Expressway: Go to Configuration > Unified Communications > Configuration > MRA Access Control . Set Authorize by OAuth token with refresh to On . Click Save . |
| Step 2 | Onboard Cisco Unified Communication Manager to the cloud for MRA activation code onboarding. From Cisco Unified CM Administration, choose Advanced Features > Cisco Cloud Onboarding . Click the Generate Voucher button. Check the Enable Activation Code Onboarding with Cisco Cloud check box. Specify the MRA Activation Domain . Click Save . Note Collab-edge DNS records must exist for the MRA Activation domain. There is a limit of one MRA Activation Domain per cluster. The MRA Activation is added automatically to the list of MRA Service
...
| Step 6 | Check Trusted Cisco Manufacturing Installed Certificates (MICs) installed. They are required to access the activation code
                                          onboarding functionality: Note Cisco Manufacturing Root certificates must be present in the CallManager-trust store to perform onboarding activity. On Expressway-E, choose Maintenance > Security > Trusted CA certificate . Click Activation code onboarding trusted CA certificates . | Note | Cisco Manufacturing Root certificates must be present in the CallManager-trust store to perform onboarding activity. |
