---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-jabber-12-1-cjab-b-parameters-reference-guide-for-cisco-12-1-cjab-b-paramete-6a094ac51f
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/jabber/12_1/cjab_b_parameters-reference-guide-for-cisco_12_1/cjab_b_parameters-reference-guide-for-cisco_12_1_chapter_011.html
retrieved_at: 2026-08-21T05:11:55.143504+00:00
---

Parameters Reference Guide for Cisco Jabber 12.1
...
Specifies whether the client retains the chat history after participants close the chat window. The client retains the chat
                              history until the participants reset Jabber.

If you set the key to false and the participants reopen the chat window, the client displays only the last 200 messages.

For persistent chat users, you must set this key as false (default value). If you disable the Disable_IM_History parameter, then it affects the @mention feature in persistent chat rooms.
...
If this parameter is true, it takes precedence (the client saves no history) over the following settings:
...
enable—Displays the call control strip during calls. On the top bar of the Cisco Jabber call window, users have a new option
                                       to
                                       				see or hide the call control strip.

disable
                                       				(default)—Hides the call control strip during calls. Users
                                       				see a video-only window without the call control strip.
...
| Note | For persistent chat users, you must set this key as false (default value). If you disable the Disable_IM_History parameter, then it affects the @mention feature in persistent chat rooms. |
|---|---|

| Note | This parameter is not available for IM-only deployments. |
|---|---|

| Note | If chat history isn’t enabled, then the restored chat windows are empty. |
|---|---|
