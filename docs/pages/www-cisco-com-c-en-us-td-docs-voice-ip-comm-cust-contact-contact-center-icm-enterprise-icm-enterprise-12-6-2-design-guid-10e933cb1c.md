---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-12-6-2-design-guid-10e933cb1c
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_12_6_2/design/guide/ucce_b_ucce_soldg-for-unified-cce-1262/rcct_b_ucce_soldg-for-unified-cce-1261_chapter_01000.html
retrieved_at: 2026-08-16T19:51:10.695688+00:00
---
...
Agent Answers : Assists the agents via Agent Answers gadget on the Cisco Finesse desktop by providing relevant suggestions and recommendations
                                 in real time.

Call Transcription : Assists the agents via Call Transcript gadget on the Cisco Finesse desktop.

Unified CCE solution integrates with the following to provide the Contact Center AI Services :

An AI provider such as Google's CCAI is an extension of services that  assist human agents in conversations with end users
                                 by providing real-time documents and response suggestions. For more details, see https://cloud.google.com/solutions/contact-center .
...
Webex CCAI API service receives the AI service provider's response from the Webex CCAI Orchestrator service. These responses
                                       are provided to the Agent Answers and Call Transcript gadgets.

Webex Cloud is a multi-tenant cloud solution which manages the tenant and its configuration.
...
VVB interfaces with the Orchestrator Service for providing VAV capability. While the call is in the IVR state, VVB acts as
                                 a gateway to the Orchestration Service via the GRPC protocol.

In CCE, the customer associates the Contact Center AI Services with all or specific call types and then provisions these services for a specific agent or multiple agents. In Cisco Finesse,
                           the customer configures the Agent Answers and Call Transcript gadgets for the required teams. During an incoming call, if
                           the CCE and Agent Desktop configurations are present for an agent and if the call has associations to the AI services, the
                           gadgets are displayed to the agents. When the agent speaks with the caller:
...
Agent Answers (AA) Flow: Both CVP and CUBE utilize this variable to route the connection to the nearest available CCAI data
                                    center.
...
For the bandwidth and latency considerations specific to the regional media, refer to the existing bandwidth and latency guidelines
                                 for Agent Answers or Virtual Agent (depending on the service being used) in the Bandwidth, Latency, and QoS Considerations section.
...
An incoming call arrives at CUBE.

CUBE directs the call to CVP.

CVP requests for call treatment instruction from CCE. CCE instructs CVP to subject the call to the standard VAV/IVR treatment.

VVB fetches the Auth token from Cloud Connect, calls the InsightInfer API over GRPC, and sends the media to the Webex CCAI
                                    Orchestrator.

Webex CCAI connects with Google CCAI, calls the Analyzecontent API, and relays the media.

Google CCAI sends back the response to be played back to the caller.

VVB processes the response and plays the prompt to the caller further to collect the inputs.

The conversation goes on with voice and DTMF with the caller and CCAI service.

CCE instructs CVP to queue the call until an agent becomes available. When an agent is available, CCE sends the GED CONNECT
                                    message to the CVP. The GED CONNECT message indicates which Contact Center AI Services are enabled for that agent. CVP transfers
                                    the call to the agent through Unified CM.
...
The call arrives at the Agent Desktop from Finesse. If the Contact Center AI Services are configured for the agent and if the Agent Answers and Call Transcript gadgets are configured on the Agent Desktop, the
                                    gadgets are displayed to the agent in the Agent Desktop.

If the GED CONNECT message (received at step 7) indicated that at least one Contact Center AI Feature is enabled for the agent, CVP requests CUBE to start forking the media streams of the agent and the caller. The forking request
                                    informs CUBE about which AI services are enabled for that call and where the forked media streams should be sent to.
...
The Agent Answers and Call Transcript gadgets start displaying the response received from the Webex CCAI API Service to the
                                    agent in real-time. If the call has gone through the IVR/VAV phase, the call transcript of that conversation is also displayed
                                    in the gadget.

For details on how to configure Contact Center AI Services , see the Virtual Agent–Voice , Agent Answers , and the Call Transcription for Agent/IVR/Virtual Agent-Voice chapters in the Cisco Unified Contact Center Enterprise Features Guide at Cisco Unified Contact Center Enterprise Features Guide at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-feature-guides-list.html .
...
Webex Connect then injects the new task request into the Digital Routing service that runs as a container on the Cisco Cloud
                                    ...
                                    more information about how to configure reverse proxy servers, see the Reverse Proxy Configuration for Digital Channel Interaction section in the Cisco Unified Contact Center Enterprise Features Guide .
...
The Webex Engage sends an asynchronous response request to the DataConn service through the load balancer, which is the network
                                    entry point for your deployment. For instructions about how to configure the host for network entry points, see the Configure network entry point for agent synchronization section in the Cisco Unified Contact Center Enterprise Features Guide .
...
The DataConn service maps the agent configuration details from the AW database with the configurations available in Webex
                                    Engage to synchronize the details. For details about the mapping, see the Field mapping between Webex Engage and CCE section in the Cisco Unified Contact Center Enterprise Features Guide .

Only agents configured for digital channels are synchronized to Webex Engage. For instructions about how to enable agents
                                    for digital channels, see the Enable agents for Digital Channels section in the Cisco Unified Contact Center Enterprise Features Guide .
...
The Webex Contact Center AI Services (CCAI) platform is used for providing VAV capability with speech-based services from different vendors using a cloud-based
                                          connector.
