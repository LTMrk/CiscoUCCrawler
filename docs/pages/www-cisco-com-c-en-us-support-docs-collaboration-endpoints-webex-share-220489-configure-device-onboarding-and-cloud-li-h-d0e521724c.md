...
Even though media does not pass through Webex cloud (unless Webex optimized experience is enabled), you need to enable the same firewall configurations that apply for fully cloud-registered devices. Read more about media and network requirements in the Network Requirements for Webex Services article. Limitations When you enable a Hybrid Calendar on a device linked via Webex Edge for Devices, it disables the TMS calendar on that device, as only one calendar source is supported at a time.

When you first link devices to Webex Edge for Devices, they’re registered as shared mode devices. After you link the devices, it is possible to associate a device with a user.
