---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-15-0-1-user-guide--8543c31a2d
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_15_0_1/user/guide/ucce_b_scripting-and-media-routing-guide-for-cisco-unified-icm-contact-center-enterprise-release-15_0/selection_of_routing_targets.html
retrieved_at: 2026-08-16T20:36:27.295426+00:00
---
...
When Unified CCE routes a call to a translation route, it sends a message to the PG. In this message the PG receives the call context from
                              the Router, including the ultimate target and further instructions for the PG. This enables the PG to seamlessly pass information
                              set or retrieved in the Routing script to the PIM/peripheral as well as to CTI clients. For example, the PG might be instructed
                              to coordinate with a host computer so that the caller's account number appears on the teleset of the agent who picks up the
                              call.
...
A skill target is an entity at a peripheral or in the enterprise to which Unified CCE can route a contact. There are two types of skill targets: Peripheral-level skill targets and Enterprise-level skill targets .
...
When executing the Announcement Node, Unified CCE returns the label associated with the announcement to the routing client. The Announcement Node terminates the script.
...
Unified CCE keeps track of the schedule and the number of calls sent to the target. The routing client informs Unified CCE when a call at the target ends, so it always knows how many calls are currently in progress at the target and whether it
                              can handle an additional call.

When Unified CCE runs the Scheduled Select Node, it searches the list of selected targets for one that is capable of handling the contact,
                              based on its current schedule. If a target is found, the routing script ends and returns a label associated with the target
                              to the routing client. The routing client then translates the label to a peripheral target.
...
Start with first target to have Unified CCE always start the search from the first target in the list.

Start with next target to have Unified CCE start the search from the first target after the last chosen target.
...
When Unified CCE runs a Ring node, it returns the first Ring label associated with the routing client; the routing client then plays an unanswered
                              ring for the caller.

Note: You must define a Ring label using Unified CCE Configuration Manager. For more information, see the Configuration Guide for Cisco Unified Contact Center Enterprise .
...
When Unified CCE runs a Busy node, it returns the first Busy label associated with the routing client; the routing client then plays a busy
                              signal for the caller.

Note: You must define a Busy label using Unified CCE Configuration Manager. For more information, see the Configuration Guide for Cisco Unified Contact Center Enterprise .
...
When Unified CCE runs a Label node, it returns the first valid label for the routing client.

This differs from the Divert Label node, which returns all the values in the Selected Labels list to the routing client.

If Unified CCE finds no valid label, it returns the default label for the Dialed Number. In either case, the Label node terminates execution
                              of the script
...
When you define a Label Node, you can select Configured Labels and Dynamic Labels. Configured labels are static, defined through
                              the Unified CCE Configuration Manager. Dynamic Labels are expressions the CallRouter processes in real time, converting an expression into
...
When Unified CCE runs a Divert Label node, it returns all the values in the Selected Labels list to the routing client.

This differs from the Label node, which returns the first valid label for the current routing client.

Unified CCE then tries each label until it finds one that does not produce a Busy or Ring Tone No Response.

In most cases, you can specify up to ten labels in the Divert Label node.
...
When you define a Divert Label node, you can select Configured Labels and Dynamic Labels. Configured labels are static, defined
                              through the Unified CCE Configuration Manager. Dynamic Labels are expressions the CallRouter processes in real time, converting an expression into
                              a character string that is then returned to the routing client as a label. You use formulas to create a Dynamic Label.
...
- With the Divert Label Node, you can only select labels for routing clients with client types that support DivertOnBusy. Unified
                                 Contact Center client type does not support DivertOnBusy. The following client types do support DivertOnBusy: BT INAP NIC,
                                 DEUTSCHE TELEKOM, ENERGIS INAP NIC, AUCS INAP NIC, SS7IN NIC, NTL NIC and MCI NIC.
...
Select Start with first target or Start with next target to indicate how Unified CCE looks for targets. For more information, see the section Types of Target Searching .
...
## Send Contact to a Different Unified CCE System

You send a contact to a different system by using the ICM Gateway node in the Routing tab of the Palette.

The ICM Gateway node passes a routing request to the selected Unified CCE system.

Step 1

In the Send tab:

Select the gateway to Unified CCE system (and hence the specific Unified CCE instance) from the ICM Gateways list to which you want to send the request.

Check Validate returned labels to have Unified CCE validate the returned labels.

Specify whether Calling Line ID masking instructions should be applied before the request is passed to the other Unified CCE system. The Calling Line ID masking refers to when the caller's phone number is modified so that Unified CCE application does not display all of the digits; this is used in a Client ICM environment, where Client ICM sends the call to a customer Unified CCE . Choose one of the following:
...
In the Available Labels list, select one default label for each routing client to be used by the targeted Unified CCE system.
...
If the script reaches the End node, it has failed to find a target for the contact. Unified CCE then uses the default route for the Dialed Number.
...
Default Label - Unified CCE uses the default label configured for the Dialed Number.
...
You can use the Call Tracer utility from within Script Editor to test and debug a routing script and to confirm that the Unified CCE is selecting targets as you expect. Call Tracer simulates a contact to the CallRouter and generates a text-based description
                              of how the contact was handled. The test contact is processed by the active CallRouter, using all the real-time data of Unified CCE as it exists at that moment, but interactions with any peripheral is simulated.
...
To use Network Transfer Call, a feature that integrates Unified CCE Post-Routing function with a carrier network's call control ability so that a call can be transferred anywhere in the network
                                       without the use of transfer/connect services or inter-site tie lines:
...
Unified CCE runs only installed scripts in response to requests from the Call Tracer. Any uninstalled changes in scripts are not reflected
                                       in the Call Tracer results.
...
In the following example results from Call Tracer, Unified CCE invokes a script called sales. It begins with the Start node and then runs a Percent Allocation node that has three branches
                              allocated for 42%, 16%, and 42%, respectively. The asterisk indicates that for this call, Unified CCE chooses the first branch. This branch leads to a target that maps to the service Scranton.Sales. Had this been an actual
                              call, the call would be delivered to that service.

As you continue to send calls to the Unified CCE , the numbers for each node change to indicate how calls have been distributed. Each time a call arrives at a Percent Allocation
                              node, the Unified CCE calculates the percentage of calls previously sent to each branch:
...
In the next example, for each branch the new call is counted in the denominator. Therefore, the numbers add up to less than
                              1.0 and gradually, as you send more calls, the total increases up to 1.0. Unified CCE picks the branch that is below its allocation value. For the example allocation, Unified CCE picked the third branch as 0.375 is below the allocation of 42% or 0.42 and sent the call to the Gary.Sales service.
...
- Fail. Unified CCE treats the External Script nodes as though it had failed.

- No Response. Unified CCE assumes it received no response from the Run External Script node.

- Response. You can specify a CED value and values for Variable1 through Variable10. Unified CCE assumes these values were returned by the External Script node.
...
| Note | You create routes through Unified CCE Configuration Manager before writing routing scripts. For more information, see the Configuration Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html . |
|---|---|

| Note | You create translation routes through Unified CCE Configuration Manager before writing routing scripts. For more information, see the Configuration Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html . |
|---|---|
...
| Note | You must configure Announcements and associate them with labels using Unified CCE Configuration Manager. For more information, see the Configuration Guide for Cisco Unified Contact Center Enterprise . |
|---|---|
...
| Note | You must configure Scheduled targets and associated them with labels using Unified CCE Configuration Manager. For more information, see the Configuration Guide for Cisco Unified Contact Center Enterprise . |
|---|---|

| Step 1 | In the Evaluation Order field select: Start with first target to have Unified CCE always start the search from the first target in the list. Start with next target to have Unified CCE start the search from the first target after the last chosen target. |
|---|---|
...
| Note | Before you can successfully use an ICM Gateway node in a script, you must use the Unified CCE Configuration Manager to configure a gateway to the Unified CCE system to which you send the request. For more information, see the Configuration Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-guides-list.html . |
|---|---|

| Step 1 | In the Send tab: Select the gateway to Unified CCE system (and hence the specific Unified CCE instance) from the ICM Gateways list to which you want to send the request. Check Validate returned labels to have Unified CCE validate the returned labels. Specify whether Calling Line ID masking instructions should be applied before the request is passed to the other Unified CCE system. The Calling Line ID masking refers to when the caller's phone number is modified so that Unified CCE application does not display all of the digits; this is used in a Client ICM environment, where Client ICM sends the call to a customer Unified CCE . Choose one of the following: 1) Do not apply masking rule - If selected, masking instructions are ignored. 2) Apply masking rule if call is presentation restricted - If selected, applies masking instructions if the call variable CLIDRestricted is set to 1. 3) Always apply masking rule - If selected, masking instructions are always applied. Note The Calling line ID masking rule is set through the Unified CCE Configuration Manager's System Information dialog box. | Note | The Calling line ID masking rule is set through the Unified CCE Configuration Manager's System Information dialog box. |
|---|---|---|---|
| Note | The Calling line ID masking rule is set through the Unified CCE Configuration Manager's System Information dialog box. |
| Step 2 | In the Default Label tab: Figure 34. ICM Gateway Properties In the Available Labels list, select one default label for each routing client to be used by the targeted Unified CCE system. Click Add to move the selected label to
                                             the Selected labels list. |
| Step 3 | Optionally, add comments and connection labels. |
...
| Step 3 | If you want to test a response from a VRU routing script,
                                       enter values in Caller-Entered-Digits (CED) and VRU Responses
                                       for External Script. |
| Step 4 | To use Network Transfer Call, a feature that integrates Unified CCE Post-Routing function with a carrier network's call control ability so that a call can be transferred anywhere in the network
                                       without the use of transfer/connect services or inter-site tie lines: Select Use Network Transfer. Select a routing client and a Dialed Number value. |
| Step 5 | Click Send Call to submit the request. The
                                       results appear in the Call Trace Results field. |
| Step 6 | To send additional calls, optionally change any of the call
                                       parameters and then click Send Call again. The
                                       Call Trace Results field is updated. |
| Step 7 | Unified CCE runs only installed scripts in response to requests from the Call Tracer. Any uninstalled changes in scripts are not reflected
                                       in the Call Tracer results. |
...
| Step 6 | Unified CCE runs only installed scripts in response to requests from the Call Tracer. Any uninstalled changes in scripts are not reflected
                                       in the Call Tracer results. |
