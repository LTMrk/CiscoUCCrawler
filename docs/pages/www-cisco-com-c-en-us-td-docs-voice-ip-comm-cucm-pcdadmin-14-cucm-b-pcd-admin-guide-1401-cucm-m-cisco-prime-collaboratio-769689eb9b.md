---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-pcdadmin-14-cucm-b-pcd-admin-guide-1401-cucm-m-cisco-prime-collaboratio-769689eb9b
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/pcdadmin/14/cucm_b_pcd-admin-guide-1401/cucm_m_cisco-prime-collaboration-deployment-features.html
retrieved_at: 2026-08-17T00:35:39.317599+00:00
---
...
You can only use the 14SU2 or later versions of Cisco Prime Collaboration Deployment for all 14SU2 UC clusters.

For Direct Standard Upgrade or Migration of UC clusters in FIPS mode with the Pre-14SU2 releases, install the COP file ciscocm.ciscossl7_upgrade_CSCwa48315_CSCwa77974_v1.0.k4.cop
                                                on the UC clusters.
...
Cisco Prime Collaboration Deployment supports the destination version 11.5 and above for an upgrade or a migration. The application
                                          versions 10.x and above support virtualization. If the source version is 11.5 and above, the upgrade task is supported. However,
                                          if the source version is prior to 11.5, the upgrade task is not supported.

A migrate cluster task can migrate to any of releases listed in the tables, by having source version as 10.x and destination
                                          version should be 11.5 or higher on virtual machine.
...
Cisco Prime Collaboration Deployment GUI—Define the new
                                          				installation cluster (click the open and close navigation button and choose Inventory > Clusters )
                                          				to define the nodes to be installed, and their associated virtual machines.
                                          				(See Add New Cluster for Fresh Install .)
...
| Note | You can only use the 14SU2 or later versions of Cisco Prime Collaboration Deployment for all 14SU2 UC clusters. For Direct Standard Upgrade or Migration of UC clusters in FIPS mode with the Pre-14SU2 releases, install the COP file ciscocm.ciscossl7_upgrade_CSCwa48315_CSCwa77974_v1.0.k4.cop
                                                on the UC clusters. |
|---|---|
...
| Note | If your 14SU2 version of Cisco Prime Collaboration Deployment is in FIPS mode and you are using any of the Pre-12.5 UC clusters
                                       ...
                                       in the non-FIPS mode before proceeding with any of these tasks. |
|---|---|
...
| Note | Cisco Prime Collaboration Deployment supports the destination version 11.5 and above for an upgrade or a migration. The application
                                          versions 10.x and above support virtualization. If the source version is 11.5 and above, the upgrade task is supported. However,
                                          if the source version is prior to 11.5, the upgrade task is not supported. A migrate cluster task can migrate to any of releases listed in the tables, by having source version as 10.x and destination
                                          version should be 11.5 or higher on virtual machine. |
|---|---|
...
| Migrate Cluster (Install Application and Import Data from Old System) | From 10.x, 11.x, 12.x and 14 To 11.5, 12.x and 14 Note Prime Collaboration Deployment Migration 11x (till 11.5 SU5)/12.0 to 11x (till 11.5 SU5)/12.0 isn't supported, if “11x (till
...
| Note | Prime Collaboration Deployment Migration 11x (till 11.5 SU5)/12.0 to 11x (till 11.5 SU5)/12.0 isn't supported, if “11x (till
                                                   11.5 SU5)/12.0” is an identical version as in same major, same minor, same MR, same SU/ES between the source and destination. |
| Upgrade Cluster (Upgrade Application Version or Install COP Files) | From 11.5, 12.x and 14 To 11.5, 12.x and 14 |
| Restart | 11.5, 12.x and 14 |
| Switch Version | 11.5, 12.x and 14 |
| Fresh Install New Cluster or Edit or Expand an Existing Cluster | 11.5, 12.x and 14 |
| Readdress (Change Hostname or IP Addresses for One or More Nodes in a Cluster) | 11.5, 12.x and 14 |
...
| Readdress (Change Hostname or IP Addresses for One or More Nodes in a Cluster) | 11.5, 11.6, 12.x |
...
| Migrate Cluster (Install Application and Import Data from Old System) | Not Supported |
| Upgrade Cluster (Upgrade Application Version or Install COP Files) | 11.5, 12.x and 14 To 11.5, 12.x and 14 |
| Restart | 11.5, 12.x and 14 |
| Switch Version | 11.5, 12.x and 14 |
| Fresh Install New Cluster or Edit or Expand an Existing Cluster | 11.5, 12.x and 14 |
| Readdress (Change Hostname or IP Addresses for One or More Nodes in a Cluster) | 11.5, 12.x and 14 |
...
| Migrate Cluster (Install Application and Import Data from Old System) | Not Supported |
| Upgrade Cluster (Upgrade Application Version or Install COP Files) | 11.5(x), 12.x , 14 To 11.5(x), 12.x , 14 |
...
| Readdress (Change Hostname or IP Addresses for One or More Nodes in a Cluster) | 11.5(x), 12.x , 14 |
...
| Add a not-yet-discovered cluster to the PCD Inventory | Discover a Cluster Modify and View a Cluster Perform the various tasks mentioned in the application. See Supported Tasks for Applications and Versions . |
...
| Note | If your 14SU2 version of Cisco Prime Collaboration Deployment is in FIPS mode and you are using any of the Pre-12.5 UC clusters
                                          to perform cluster discovery, you must first switch your Cisco Prime Collaboration Deployment to work in the non-FIPS mode
                                          before proceeding with cluster discovery. |
|---|---|
...
| 10.x | Network migration | Secure | When
...
| 10.x | Network migration | Nonsecure | When the
...
| 11.x , and 12.x | Network migration | Secure | When the migration task reaches the Forced Paused step, perform the following steps: CTL Update Bulk Certificate Management Resume the task on Cisco Prime Collaboration Deployment GUI. |
| 11.x , and 12.x | Network migration | Nonsecure | When the migration task reaches the Forced Paused step, perform the following steps: Bulk Certificate Management Resume the task on Cisco Prime Collaboration Deployment GUI. |
...
| 11.x , and 12.x | Network Migration | Secure | Change TFTP Server IP Address . Verify Phone Registration. |
| Network Migration | Nonsecure | Change TFTP Server IP Address . Verify Phone Registration. |
...
| Note | If your 14SU2 version of Cisco Prime Collaboration Deployment is in FIPS mode and you are using any of the Pre-12.5 UC clusters
                                       to perform an upgrade, you must first switch your Cisco Prime Collaboration Deployment to work in the non-FIPS mode before
...
| Note | Readdress tasks cannot be performed for any 14SU2 UC clusters in Cisco Prime Collaboration Deployment due to known caveat
                                             CSCwb95747-Hostname and IP address change task failed during Network change verification in PCD. This issue is fixed from
                                             Release 14SU3 onwards. |
|---|---|
