...
- On all cores that are connected (not co-located with a database) to the database cluster, run database cluster certs dbclient.key dbclient.crt dbca.crt .
...
- In order to apply certificates to Webbridge, run the next commands (replaced with webbridge3 in CMS 3.x with new config requirements):
...
- In order to apply certificates to XMPP, run the next commands (Not available in CMS 3.x):
...
database cluster certs <server_key> <server_certificate> <client_key> <client_certificate> <Root ca_crt> database cluster initialize     (only on primary node) database cluster join <FQDN or IP of primary>    (only on slave node) database cluster connect <FQDN or IP of primary> (only on nodes that are not part of the database cluster)
```
...
Formatting for recertification.
