---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-15-0-1-user-ucce-b-ed6fed2b29
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_15_0_1/user/ucce_b_150_outbound-option-guide-for-unified/architectural_overview.html
retrieved_at: 2026-08-16T20:35:14.101407+00:00
---
...
Use the voice gateway or Cisco Unified Border Element (CUBE) dial peers and Cisco Contact Center SIP Proxy routing policies for outbound call routing.
...
Multipurpose placeholder that sends data from the Agent desktop Finesse desktop to the Outbound Option Dialer. This variable responds to the reservation call to schedule and cancel callbacks, and to changes
...
Queue depth is below 5000 messages
...
Queue depth increases to 5000 messages

25%

Level 1 decreases to Level 0 when queue depth decreases to 3500 messages

2 - Moderately congested

Queue depth increases to 7000 messages

50%

Level 2 decreases to Level 1 when queue depth decreases to 4500 messages

3 - Heavily congested

Queue depth increases to 9500 messages

90%

Level 3 decreases to Level 2 when queue depth decreases to 6500 messages

To achieve optimal campaign performance during outbound dialing at 240 CPS across 1500 campaigns, set the SQL compatibility
                                       level to 120 using the following SQL command:

ALTER DATABASE "database_name"

SET COMPATIBILITY_LEVEL = 120

Run this command on both logger sides, replacing "database_name" with the actual outbound database name.
...
| BAResponse | Multipurpose placeholder that sends data from the Agent desktop Finesse desktop to the Outbound Option Dialer. This variable responds to the reservation call to schedule and cancel callbacks, and to changes
                                             to the callback phone number. See the following rows for more information about BAResponse. |
...
|---|---|---|---|
| 0 - Normal operation | Queue depth is below 5000 messages | No throttling | N/A |
| 1 - Slightly congested | Queue depth increases to 5000 messages | 25% | Level 1 decreases to Level 0 when queue depth decreases to 3500 messages |
| 2 - Moderately congested | Queue depth increases to 7000 messages | 50% | Level 2 decreases to Level 1 when queue depth decreases to 4500 messages |
| 3 - Heavily congested | Queue depth increases to 9500 messages | 90% | Level 3 decreases to Level 2 when queue depth decreases to 6500 messages |

| Note | To achieve optimal campaign performance during outbound dialing at 240 CPS across 1500 campaigns, set the SQL compatibility
                                       level to 120 using the following SQL command: ALTER DATABASE "database_name" SET COMPATIBILITY_LEVEL = 120 Run this command on both logger sides, replacing "database_name" with the actual outbound database name. |
|---|---|