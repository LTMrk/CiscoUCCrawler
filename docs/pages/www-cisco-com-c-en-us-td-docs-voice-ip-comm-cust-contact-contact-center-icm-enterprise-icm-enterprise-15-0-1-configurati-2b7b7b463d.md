...
For Cisco Unified Intelligence Center, log into your Cisco Unified Intelligence Center Administration page through your browser.
...
For Cloud Connect, enter the following URL in you browser and press Enter . https://<FQDN>:8443/plugins/CiscoRTMTPlugin.zip . In the URL, FQDN is the Fully Qualified Domain Name of the Cloud Connect Primary or Secondary node.

Unzip the CiscoRTMTPlugin.zip file and locate the run.bat file.
...
In the HOST IP Address field, enter or modify the IP address for the node you want to monitor.
...
In the Authentication Required dialog box, enter the user name and password for a superuser. (Only a superuser can install RTMT) .
...
|---|---|
| Step 2 | To download RTMT: For Cloud Connect, enter the following URL in you browser and press Enter . https://<FQDN>:8443/plugins/CiscoRTMTPlugin.zip . In the URL, FQDN is the Fully Qualified Domain Name of the Cloud Connect Primary or Secondary node. The CiscoRTMTPlugin.zip file is downloaded to the client computer. Unzip the CiscoRTMTPlugin.zip file and locate the run.bat file. |
| Step 3 | To launch RTMT: Double-click the run.bat file. The Real-Time Monitoring Tool login dialog-box appears with the HOST IP Address field. In the HOST IP Address field, enter or modify the IP address for the node you want to monitor. Accept the default port (8443). Check Secure Connection. You see an error if the Host IP Address is not found or there is no network connection. Click Yes to accept the certificate. In the Authentication Required dialog box, enter the user name and password for a superuser. (Only a superuser can install RTMT) . If a message appears indicating that a time zone mismatch exists, click No to launch RTMT in your current time zone. Click OK to accept the default configuration. Note The performance counters are documented in the Administration Console User Guide for Cisco Unified Intelligence Center at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-maintenance-guides-list.html . The performance counters are not documented in the Online help. | Note | The performance counters are documented in the Administration Console User Guide for Cisco Unified Intelligence Center at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-maintenance-guides-list.html . The performance counters are not documented in the Online help. |
| Note | The performance counters are documented in the Administration Console User Guide for Cisco Unified Intelligence Center at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-maintenance-guides-list.html . The performance counters are not documented in the Online help. |
