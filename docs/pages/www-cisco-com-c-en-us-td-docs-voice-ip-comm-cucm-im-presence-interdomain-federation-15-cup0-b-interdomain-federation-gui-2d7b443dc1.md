---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-im-presence-interdomain-federation-15-cup0-b-interdomain-federation-gui-2d7b443dc1
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/im_presence/interdomain_federation/15/cup0_b_interdomain-federation-guide-15/cup0_b_interdomain-federation-1251su3_chapter_0100.html
retrieved_at: 2026-08-16T16:13:40.194367+00:00
---
...
AOL requires that the Cisco Adaptive Security Appliance certificate is signed by a trusted Certificate Authority (CA). AOL has an established trust list of CAs such as those commonly
                              used in Windows or those in libraries distributed with the major browsers. If you wish to use a CA that is not on the AOL
                              trust list, work with your Cisco representative to provide this information to AOL.

A sample configuration workflow that describes in detail how to configure certificate exchange between the Cisco Adaptive Security Appliance and an external domain (Microsoft Access Edge) using the Verisign CA is provided in the appendix of this guide. Use this
                              procedure as a reference to configure certificate exchange between Cisco Adaptive Security Appliance and the AOL SIP Access Gateway using the Verisign CA. A high-level overview of the configuration steps is provided below.
...
Download the AOL root certificate from http://www.entrust.com/root-certificates/entrust_2048_ca.cer

Download the AOL leaf certificate from http://www.entrust.com/root-certificates/entrust_l1c.cer

Delete any old intermediate and signed certificate, and the trustpoint for the root certificate on the Cisco Adaptive Security Appliance .

Create a new trustpoint on the Cisco Adaptive Security Appliance for the AOL root certificate, see section Import an IM and Presence Service Certificate into the Cisco Adaptive Security Appliance (steps 1-3).

Create a new trustpoint on the Cisco Adaptive Security Appliance for the AOL leaf certificate.

Sign ASA certificate using Public Certificate Authority (Verisign):

Create a new trustpoint for the Verisign CA on the Cisco Adaptive Security Appliance .

On the Cisco Adaptive Security Appliance , import the root certificate, and then generate a Certificate Signing Request (CSR). See section Configure a Certificate on the Cisco Adaptive Security Appliance Using Manual Enrollment for a similar procedure.
...
On the Cisco Adaptive Security Appliance , delete the temporary root certificate used to generate the Certificate Signing Request.

Import the Verisign subordinate intermediate root certificate to the Cisco Adaptive Security Appliance .

Create a trustpoint for the Verisign root CA certificate on the Cisco Adaptive Security Appliance .

Import the Verisign root CA certificate to the Cisco Adaptive Security Appliance , and then import the Verisign signed certificate to the Cisco Adaptive Security Appliance .
...
Configuration a Certificate on the Cisco Adaptive Security Appliance Using Manual Enrollment
...
| Note | The IM and Presence Service node certificate subject CN must match FQDN of the IM and Presence Service node. The public Certificate on the Cisco Adaptive Security Appliance for the IM and Presence Service and the CN must be the same as the Federation Routing IM and Presence FQDN service parameter value. |
|---|---|
