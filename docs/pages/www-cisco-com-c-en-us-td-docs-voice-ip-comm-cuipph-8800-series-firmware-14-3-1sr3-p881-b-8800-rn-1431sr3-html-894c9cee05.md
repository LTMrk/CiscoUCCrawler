---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cuipph-8800-series-firmware-14-3-1sr3-p881-b-8800-rn-1431sr3-html-894c9cee05
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cuipph/8800-series/firmware/14-3-1SR3/p881_b_8800-rn-1431sr3.html
retrieved_at: 2026-08-17T01:12:03.791666+00:00
---

Cisco IP Phone 8800 Release Notes for Firmware Release 14.3(1)SR3
...
This release contains the following features for EU RED DA (Radio Equipment Directive Delegated Act) compliance:

WPA backward compatibility

Min TLS version (default is 1.2) for server and client on phone

Transition of PRT log uploads over HTTPS for CUCM

Special user events logging for privacy assets, including the following:
...
Create/Update/Delete on phone UI or web

Call history

Delete on phone UI or web
...
You can enable the backward compatibility for the oldest version of Wi-Fi Protected Access (WPA). If enabled, the phone can
                        connect to the wireless networks or access points that only support WPA. By default, the feature is disabled.
...
Now the phone that acts as a TLS client can require the minimum TLS version to establish a TLS connection. By default, the
                        TLS min version for the phone is TLS 1.2.

To use this feature, configure the field TLS Client Min Version under the section Product Specific Configuration Layout on Unified CM.

In addition, the default value of Disable TLS 1.0 and TLS 1.1 for Web Access is changed from Disabled to Enabled .
...
In the Latest Release folder, choose 14.3(1)SR3 .

Step 5

Select the firmware file, click the Download or Add to cart button, and follow the prompts:

The firmware filename is cmterm-8845_65-sip.14-3-1-0301-349.k4.cop.sha512 or cmterm-88xx-sip.14-3-1-0301-349.k4.cop.sha512,
...
If a Cisco Unified Communications Manager is not available to load the installer program, the following .zip files are available
                        to load the firmware: cmterm-8845_65.14-3-1-0301-349.zip or cmterm-88xx.14-3-1-0301-349.zip.
...
During a firmware upgrade, the file size of each firmware version is cross-verified with the size documented in the loads
                                    file to ensure integrity. If a new firmware file with an identical filename overwrites an existing file, subsequent attempts
                                    to revert to the prior firmware version will fail due to a file size discrepancy. In scenarios requiring switch between Firmware
                                    1 and Firmware 2, the endpoint will not initiate a new file download, but will instead switch partitions.

To deploy a new firmware version, such as Firmware 3, the corresponding firmware package must be installed into the Call Manager
                                    to ensure compatibility and successful upgrade.
...
In the Latest Releases folder, choose 14.3(1)SR3 .

Step 5

Select the firmware file, click the Download or Add to Cart button, and follow the prompts.

The firmware file name is cmterm-8845_65.14-3-1-0301-349.zip or cmterm-88xx.14-3-1-0301-349.zip, depending on the model you
                                    choose.
...
### Customized Ringtone Settings

After you upgrade the firmware to one of the following releases, the previously customized ringtone settings will be lost:
...
Use this URL for all caveats: https://bst.cloudapps.cisco.com/bugsearch/search?kw=&pf=prdNm&pfVal=284729655&rls=14.3(1)SR2&sb=anfr&svr=3nH&bt=custV

Use this URL for all open caveats: https://bst.cloudapps.cisco.com/bugsearch/search?kw=&pf=prdNm&pfVal=284729655&rls=14.3(1)SR2&sb=afr&sts=open&svr=3nH&bt=custV

Use this URL for all resolved caveats: https://bst.cloudapps.cisco.com/bugsearch/search?kw=*&pf=prdNm&pfVal=284729655&rls=14.3(1)SR2&sb=fr&sts=fd&svr=3nH&bt=custV
...
This release doesn't resolve any issues.
...
| SSH | Successful login/logout Rejected login attempt Service on/off |
|---|---|
| Web | Service on /off |
| Personal directory | Create/Update/Delete on phone UI or web |
| Call history | Delete on phone UI or web |
| PIN on phone UI | Successful unlock Rejected unlock attempt |
...
| Step 4 | In the Latest Release folder, choose 14.3(1)SR3 . |
| Step 5 | Select the firmware file, click the Download or Add to cart button, and follow the prompts: The firmware filename is cmterm-8845_65-sip.14-3-1-0301-349.k4.cop.sha512 or cmterm-88xx-sip.14-3-1-0301-349.k4.cop.sha512,
...
| Note | During a firmware upgrade, the file size of each firmware version is cross-verified with the size documented in the loads
                                    file to ensure integrity. If a new firmware file with an identical filename overwrites an existing file, subsequent attempts
                                    to revert to the prior firmware version will fail due to a file size discrepancy. In scenarios requiring switch between Firmware
                                    1 and Firmware 2, the endpoint will not initiate a new file download, but will instead switch partitions. To deploy a new firmware version, such as Firmware 3, the corresponding firmware package must be installed into the Call Manager
                                    to ensure compatibility and successful upgrade. |
|---|---|
...
| Step 2 | Choose your phone model. |
| Step 3 | Choose Session Initiation Protocol (SIP) Software . |
| Step 4 | In the Latest Releases folder, choose 14.3(1)SR3 . |
| Step 5 | Select the firmware file, click the Download or Add to Cart button, and follow the prompts. The firmware file name is cmterm-8845_65.14-3-1-0301-349.zip or cmterm-88xx.14-3-1-0301-349.zip, depending on the model you
                                    choose. |
...
| Step 1 | Perform one of the following actions: Use this URL for all caveats: https://bst.cloudapps.cisco.com/bugsearch/search?kw=&pf=prdNm&pfVal=284729655&rls=14.3(1)SR2&sb=anfr&svr=3nH&bt=custV Use this URL for all open caveats: https://bst.cloudapps.cisco.com/bugsearch/search?kw=&pf=prdNm&pfVal=284729655&rls=14.3(1)SR2&sb=afr&sts=open&svr=3nH&bt=custV Use this URL for all resolved caveats: https://bst.cloudapps.cisco.com/bugsearch/search?kw=*&pf=prdNm&pfVal=284729655&rls=14.3(1)SR2&sb=fr&sts=fd&svr=3nH&bt=custV |
|---|---|
