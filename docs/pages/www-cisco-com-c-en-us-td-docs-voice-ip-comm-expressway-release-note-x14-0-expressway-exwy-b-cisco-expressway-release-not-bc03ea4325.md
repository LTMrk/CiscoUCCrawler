---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-expressway-release-note-x14-0-expressway-exwy-b-cisco-expressway-release-not-bc03ea4325
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/expressway/release_note/X14-0/expressway/exwy_b_cisco-expressway-release-note-x14-0.html
retrieved_at: 2026-08-16T22:08:56.065817+00:00
---

Cisco Expressway Release Note (X14.0)
...
Cisco has now announced end-of-sale and end-of-life dates for the Cisco TelePresence Video Communication Server (VCS) product. Details are available at the following link . This notice does not affect the Cisco Expressway Series product.
...
As of 13 November 2018, you cannot order the CE1100 appliance from Cisco. Cisco will withdraw hardware support services for
                     the appliance in a future release. See the End-of-sale announcement for other important dates in the lifecycle of this platform.
...
Information about compatible products for MRA specifically, is provided in version tables for infrastructure products and
                        for endpoints in the Mobile and Remote Access Through Cisco Expressway Deployment Guide .

### Which Expressway Services Can Run Together?

The Cisco Expressway Administrator Guide details which Expressway services can coexist on the same Expressway system or cluster. See the table " Services That Can be Hosted Together " in the Introduction section. For example, if you want to know if MRA can coexist with CMR Cloud (it can) the table will tell
                     you.

## Feature Summary for X14.0
...
From X14.0 release, Expressway has a toggle to enable/disable the SSO Redirect URI for Webex feature. This feature enhances
                        the security of Cisco Jabber/Webex Client embedded browser support with following benefits:
...
From X14.0 release, Expressway supports the negotiation and passthrough of AV1 codecs to establish media. The codecs are supported
                        in SIP traversal calls (that is, calls where the Expressway is handling the media streams).

### Escalation from P2P to Meeting

As a Webex user, while in a 1:1 SIP call - you now have the ability to call a Webex meeting and invoke meeting features such
                        as:
...
During the escalation process, you will have the option to invite additional participants or simply transition the 1:1 call
                        to a 1:1 meeting. It also gives you the option to keep the existing audio channel (mobile or desk phone) while augmenting
                        video and share via separate data channel.

### Expressway Cluster Load Balancing Not Applicable to SIP Federation

In an Expressway deployment used for SIP Federation, there is always a high load of SIP volume on the Expressway and multiple
                        peers are needed to handle the SIP Volume. Expressway was unable to distribute this traffic equally across the Expressway
                        Edge traversal Servers.

From X14.0 release, Expressway can load balance the traffic correctly across Traversal Zone connections.

### XCP Support for Jabber Zero Downtime

From X14.0 release, Expressway supports dual connection with Jabber clients. When enabled on the client side, this type of
                        connection ensures zero service downtime during High Availability failover events.
...
Expressway X14.0 builds on existing failover capabilities for clustered Expressways with a number of MRA failover updates
                        that improve substantially the failover time for Cisco Jabber clients that connect over MRA. Among the updates include adaptive
                        routing, STUN keepalive support, and improved error reporting.
...
To enhance usability and consistency we are removing discontinued functions and features from the user interface. Details
                        per release are in Withdrawn or Deprecated Features and Software .

There are no changes in this respect for the X14.0 release.

### Other Changes in this Release

The layout and content of the Mobile and Remote Access Through Expressway Deployment Guide has been enhanced.

From Expressway X14.0 release, the following fields have been added to the Diagnostic Logging section in the UI page:

Filter tcpdump by IP address

Filter tcpdump by port

These fields are enabled when you select take tcpdump while logging option and help filter tcpdump by the IP address or port.

From Expressway X14.0 release, a message indicating withdrawal of PAK-based support appears on the PAK licensing page -

"PAK-based licensing support will be withdrawn in a future release of Expressway. Smart Licensing is recommended. Refer to Cisco Expressway Administrator Guide for more information."

Smart licensing is recommended over PAK-based licensing with options to convert existing PAK-based Licenses to Smart Licenses.

From Expressway X14.0 release, an alarm has been introduced to inform the client of the configuration errors that may appear
                              during upgrade, misconfiguration after Expressway has been installed, difficulty in detecting the availability of necessary
                              external resources, and other issues. The alarm will help identify the configuration error and makes it easier to diagnose
                              and fix the issues that can occur.
...
1 Includes LIC-EXP-RMS-CPW, LIC-EXP-RMS-HCS, LIC-EXP-RMS-MIG, LIC-EXP-RMS-PMP, LIC-EXP-RMS-EA & LIC-EXP-RMS=
...
1. SIP UPDATE for session refresh support over MRA has some limitations. For example, the following features that rely on the
...
2. Unity Voicemail over Mobile and Remote Access (MRA) does not work with X14.0 release. This issue will be fixed in our future
                     release(bug ID CSCvy29217 refers.
...
If you are upgrading a system which is running software earlier than version X8.11.4, you must first upgrade to an intermediate release before you install X14.0 software (this requirement applies to all upgrades X8.11.x and later versions). Depending on the existing system version,
                        the upgrade will fail. We recommend upgrading to X8.11.4 as the intermediate release.

#### Is a release key needed?

A release key is not required to upgrade an Expressway on X8.6.x or later software to this release (from X8.11.4 to X14.0 for example). This change was introduced in X12.5.4. (Release keys are still used for Cisco VCS systems.)
