---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-expressway-admin-guide-x14-0-1-exwy-b-cisco-expressway-administrator-guide-x-1dde51d629
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/expressway/admin_guide/X14-0-1/exwy_b_cisco-expressway-administrator-guide-x14-0-1/exwy_m_network-and-system-settings.html
retrieved_at: 2026-08-16T22:41:46.965846+00:00
---
...
This section describes network services and settings related options that appear under the System menu of the web interface. These options enable you to configure the Expressway in relation to the network in which it is
                        located, for example its IP settings, firewall rules, intrusion protection and the external services used by the Expressway
                        (for example DNS, NTP and SNMP).
...
Mobile and Remote Access (MRA) is not tested or supported in IPv6 mode. For MRA, the primary call control agent is Unified
                                       CM which does not support IPv6.
...
The Rate limits overview page ( System > Protection > Rate limits > Configuration ) is used to limit SIP traffic rate under which Expressway can perform without any issues like crash, high CPU usage, high
                                 memory usage etc.

From X14.0 release, rate limit is enabled by default for SIP traffic.

By default, 100 connections per second are allowed with a burst limit of 20 which come on the SIP ports 5060, 5061, & 5062.

You can enable/disable or change number of connections per second and burst limit.

Connections per second range value is 1 to 150 and default value is 100.

Burst limit range value is 15 to 30 and default value is 20.

The bar graph shows number of connections established over the time and number of connections dropped.
...
We strongly recommend using a non-default port for web administration on the Expressway-E if you enable any features that
                                             need TCP 443, eg. Meeting Server Web Proxy.
...
The algorithm used to hash authentication credentials. From X12.5.7, SHA (Secure Hash Algorithm) is the only supported option.
                                             MD5 (Message-Digest algorithm 5) passwords are not supported.
...
For new installations all services are off by default. For existing systems your current service settings are preserved and
                              remain unchanged after upgrading.
...
| Web administrator port | Sets the https listening port for administrators to access the Expressway web interface. We strongly recommend using a non-default port for web administration on the Expressway-E if you enable any features that
                                             need TCP 443, eg. Meeting Server Web Proxy. Restart the Expressway to make this change effective. | If you use a non-default port, and you prepend the https:// protocol to the address, you must append the port. For example, you would put the address https://vcse.example.com:7443 into your browser; if you try https://vcse.example.com , the browser assumes port 443 and the Expressway denies access. Web access to the Expressway could be lost if a network element blocks traffic to the web admin port - you can use SSH or
                                             the console to change the port if necessary. |
...
| Type | The algorithm used to hash authentication credentials. From X12.5.7, SHA (Secure Hash Algorithm) is the only supported option.
                                             MD5 (Message-Digest algorithm 5) passwords are not supported. |  |
