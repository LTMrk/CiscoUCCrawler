...
Obtain CA Signed Certificate of PG
...
```
keytool.exe -import -file <certificate with fully qualified path> -alias <alias name> -keystore <%CCE_JAVA_HOME%\lib\security\cacerts <ICM install directory>\ssl\cacerts
```
...
| Step 6 | At the Logger/Rogger machine terminal, run the following command: cd % CCE_JAVA_HOME %\bin keytool.exe -import -file <certificate with fully qualified path> -alias <alias name> -keystore <%CCE_JAVA_HOME%\lib\security\cacerts <ICM install directory>\ssl\cacerts |
| Step 7 | Enter the truststore password when prompted. |
