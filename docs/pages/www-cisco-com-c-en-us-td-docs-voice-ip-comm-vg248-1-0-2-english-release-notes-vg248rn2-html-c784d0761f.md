---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-vg248-1-0-2-english-release-notes-vg248rn2-html-c784d0761f
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/vg248/1_0_2/english/release/notes/vg248rn2.html
retrieved_at: 2026-08-21T12:51:32.848631+00:00
---

Cisco VG248 Analog Phone Gateway Version 1.0(2) Release Notes
...
These release notes are for use with the Cisco VG248 Analog Phone Gateway with software version 1.0(2). The VG248 enables you to integrate analog telephony devices with Cisco CallManager IP telephony systems.
...
Table 2 includes the list of caveats from the VG248 1.0(1) that have been resolved with this software update (1.0(2)).

Table 2 VG248 Resolved Caveats
...
Premature re-establishment of audio path in one direction.

If a VG248 port is currently on a call and another call is made to that port's directory number, the VG248 user hears a call waiting tone, inviting them to hook flash and talk to the new caller.

If the user switches to the other call and the new caller later hangs up, the VG248 user hears an error tone and can hook flash to return to the original call.

However, when the VG248 user hears the error tone, the remote party of the original call can hear the VG248 user, even before even before hook flash is used h to re-establish bidirectional audio.

A code change to the VG248 software corrects this behavior.

CSCuk28448

Outgoing call failure leaves VG248 port in inconsistent state.

A VG248 user makes an outgoing call that does not complete because the user did not dial the correct digits, generating an error tone.

After the error tone is heard, the port should be able to receive calls. However, once the VG248 port is in this state it is impossible for a new incoming call to be answered, even if the phone attached to it goes back on hook.

The caller, however, hears the ringback tone and might be misled into thinking that the VG248 user is able to answer.

A code change to the VG248 software corrects this behavior.

CSCuk28824

Enabling disconnect supervision for a port disables call pickup.

A code change to the VG248 software corrects this behavior.

CSCuk28873

The VG248 might occasionally lock-up, requiring a power-cycle to resume normal operation.

Under certain conditions, particularly with use of SNMP, the VG248 might appear to lock up, failing to respond to input and causing loss of service to the attached telephone handsets.

This was caused by a defect in the heap management code which can corrupt the memory on the VG248.

A code change to the VG248 software corrects this behavior.

CSCuk28899

Inconsistent behavior with two active calls when one is dropped.

If a second call is made into the VG248 port, the VG248 user experiences different behavior if the first call or the second call is dropped.

If the current call drops, the user might hear an error tone and be able to connect back to the other call by hook flashing. Or, the user might be automatically switched over to the other call.

A code change to the VG248 software corrects this behavior.

CSCuk28958

Inconsistency between POTS and SCCP key press trace messages.

Using the event log, you can set different logging levels for the sub-systems on the VG248. The most verbose level is trace.

If you enable trace for the SCCP module, messages are generated each time a key is pressed on a user's telephone. For security, because these key presses could correspond to users' PIN or passwords, the event log replaces the numbers with asterisks (*).

However, this security measure is not implemented by the POTS module. If trace is enabled for this sub-system, the event log captures and displays the precise numbers entered.

A code change to the VG248 software corrects this behavior.

CSCuk28989

Telephone sometimes rings back when hanging up.

Under certain circumstances, when replacing the receiver to finish a call, the handset will ring again and, if answered, connect back to that call (assuming the remote party has not yet replaced their receiver).

A code change to the VG248 software corrects this behavior.

CSCuk29385

Large numbers of fax calls causes the VG248 to lock up.

After making a large number of fax calls, the VG248 can enter a state in which many of its internal buffers become unavailable. This stops the VG248 processing incoming network traffic, preventing Telnet access, ping, and causing loss of connectivity to Cisco CallManager. The VG248 appears to be locked up.

A code change to the VG248 software corrects this behavior.

## Open Caveats

Open caveats are unexpected behaviors or defects in the software releases for a product. Table 3 contains information on known problems for the VG248.
...
The VG248 should be able to connect to Cisco CallManager using auto-registration or manual registration. However, auto-registration might not work with some versions of Cisco CallManager 3.1(2c).

To avoid this problem, configure the VG248 ports in Cisco CallManager before enabling them on the VG248 itself.

For more information, refer to the "Configuring Analog Phones Using Cisco CallManager" section in the Software Configuration Guide :
...
You cannot perform some functions using phones with a shared directroy number.
...
http://www.cisco.com/univercd/cc/td/doc/product/voice/c_access/apg/vg248/v1_0/soft_con/vg248swv.htm#xtocid1596413
...
| Cisco VG248 Analog Phone Gateway Version 1.0(2) Release Notes | Describes known caveats and any documentation errata for the VG248. | Online at Cisco.com—HTML and PDF versions of this document are available from Cisco.com: http://www.cisco.com/univercd/cc/td/doc/product/voice/c_access/apg/vg248/v1_0/rel_note/index.htm |

| Bug ID | Summary | Explanation |
|---|---|---|
| CSCuk28059 | Premature re-establishment of audio path in one direction. | If a VG248 port is currently on a call and another call is made to that port's directory number, the VG248 user hears a call waiting tone, inviting them to hook flash and talk to the new caller. If the user switches to the other call and the new caller later hangs up, the VG248 user hears an error tone and can hook flash to return to the original call. However, when the VG248 user hears the error tone, the remote party of the original call can hear the VG248 user, even before even before hook flash is used h to re-establish bidirectional audio. A code change to the VG248 software corrects this behavior. |
| CSCuk28448 | Outgoing call failure leaves VG248 port in inconsistent state. | A VG248 user makes an outgoing call that does not complete because the user did not dial the correct digits, generating an error tone. After the error tone is heard, the port should be able to receive calls. However, once the VG248 port is in this state it is impossible for a new incoming call to be answered, even if the phone attached to it goes back on hook. The caller, however, hears the ringback tone and might be misled into thinking that the VG248 user is able to answer. A code change to the VG248 software corrects this behavior. |
| CSCuk28824 | Enabling disconnect supervision for a port disables call pickup. | A code change to the VG248 software corrects this behavior. |
| CSCuk28873 | The VG248 might occasionally lock-up, requiring a power-cycle to resume normal operation. | Under certain conditions, particularly with use of SNMP, the VG248 might appear to lock up, failing to respond to input and causing loss of service to the attached telephone handsets. This was caused by a defect in the heap management code which can corrupt the memory on the VG248. A code change to the VG248 software corrects this behavior. |
| CSCuk28899 | Inconsistent behavior with two active calls when one is dropped. | If a second call is made into the VG248 port, the VG248 user experiences different behavior if the first call or the second call is dropped. If the current call drops, the user might hear an error tone and be able to connect back to the other call by hook flashing. Or, the user might be automatically switched over to the other call. A code change to the VG248 software corrects this behavior. |
| CSCuk28958 | Inconsistency between POTS and SCCP key press trace messages. | Using the event log, you can set different logging levels for the sub-systems on the VG248. The most verbose level is trace. If you enable trace for the SCCP module, messages are generated each time a key is pressed on a user's telephone. For security, because these key presses could correspond to users' PIN or passwords, the event log replaces the numbers with asterisks (*). However, this security measure is not implemented by the POTS module. If trace is enabled for this sub-system, the event log captures and displays the precise numbers entered. A code change to the VG248 software corrects this behavior. |
| CSCuk28989 | Telephone sometimes rings back when hanging up. | Under certain circumstances, when replacing the receiver to finish a call, the handset will ring again and, if answered, connect back to that call (assuming the remote party has not yet replaced their receiver). A code change to the VG248 software corrects this behavior. |
| CSCuk29385 | Large numbers of fax calls causes the VG248 to lock up. | After making a large number of fax calls, the VG248 can enter a state in which many of its internal buffers become unavailable. This stops the VG248 processing incoming network traffic, preventing Telnet access, ping, and causing loss of connectivity to Cisco CallManager. The VG248 appears to be locked up. A code change to the VG248 software corrects this behavior. |
...
| CSCdv87437 | Failure to connect with Cisco CallManager in auto registration mode | The VG248 should be able to connect to Cisco CallManager using auto-registration or manual registration. However, auto-registration might not work with some versions of Cisco CallManager 3.1(2c). To avoid this problem, configure the VG248 ports in Cisco CallManager before enabling them on the VG248 itself. For more information, refer to the "Configuring Analog Phones Using Cisco CallManager" section in the Software Configuration Guide : http://www.cisco.com/univercd/cc/td/doc/product/voice/c_access/apg/vg248/v1_0/soft_con/vg248swp.htm |
