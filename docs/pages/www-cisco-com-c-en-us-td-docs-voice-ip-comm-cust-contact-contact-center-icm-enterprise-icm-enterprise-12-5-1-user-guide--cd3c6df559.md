...
When attempting to access the remote database, the call router authenticates using external database lookup SQLLogin registry
                              value. For more information, see Configure External DBLookUp Registry Value using CCEDataProtect Tool procedure in the Administration Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-maintenance-guides-list.html

You must configure the database paths to Side A and Side B in the DB Lookup Explorer and the CallRouter Registry for Database
