---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-crs-express-15-0-maintain-and-operate-guide-uccx-cd27ab7902
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/crs/express_15_0/maintain_and_operate/guide/uccx_b_1501_admin-and-operations-guide/uccx_m_1501_cisco-finesse.html
retrieved_at: 2026-08-16T21:28:47.459042+00:00
---
...
After 30 minutes of inactivity, Cisco Finesse automatically signs you out
                                                            of the administration console and you must sign in again.
...
f you have uploaded a CA certificate, you can skip this procedure.
...
Shindig proxies requests from the Finesse desktop to external servers, which introduces the possibility of server side request
                                    ...
                                    by adding URLs to the allowed list. Note that this functionality is enabled by default in Cisco Finesse. For more information on the CLIs used on Gadget Source Allowed List, see Cisco Finesse CLIs .

By default, Finesse includes all its domains such as publisher, subscriber, and reverse-proxy (if configured). The gadget
                                                URLs in the desktop layouts are added in the allowed list automatically.
...
When the Finesse desktop loses network connectivity to the  Notification
                                          Service.
...
When the Finesse loses connection to
...
If SystemInfo API reports Finesse is IN_SERVICE, check the  Notification
                                          Service.
...
The last CTI heartbeat status is checked to ensure that the subscriber is
                                                      ...
                                                      OUT_OF_SERVICE after the client has failed over.  Depending on the
                                                      network topology the subscriber might be slower to sense a network
                                                      disconnect.

If XMPP is disconnected, make the  Notification Service request.
...
Finesse takes up to 120 seconds to detect when an agent closes the browser. If the browser crashes, Finesse waits 60 seconds
                                                before sending a forced logout request to the  server. Under these conditions, Finesse can take up to 180 seconds to sign
                                                out the agent.

The following table lists the conditions under which Finesse sends this code to the  server.
...
Finesse receives a presence notification of Unavailable from the client. Finesse waits 60 seconds, and then sends a forced logout request to the  server.
...
Finesse receives a presence notification of Unavailable from the client. Finesse waits 60 seconds before sending a forced logout request to the  server to allow the browser to reconnect
                                                after the refresh.
...
Finesse desktop sends a forced logout request to the  server.

The Finesse server forwards the forced logout request to the  server.
...
The  Notification Service transitions to OUT_OF_SERVICE.

The Finesse connection to the  server transitions to OUT_OF_SERVICE.

If Finesse IPPA detects a server failure before Finesse fails over to the , then Finesse IPPA declares the Finesse server OUT_OF_SERVICE.
...
Finesse IPPA receives notification that the  Notification Service is disconnected.

Finesse IPPA tries every 5 seconds to reconnect with the XMPP server. After three attempts, if the  Notification Service cannot
                                          be reestablished, Finesse IPPA displays a server unavailable message to the agent.
...
Clear the browser cookies before logging in to the Finesse desktop. This avoids unexpected expiry of the Refresh Token in
                                                the Single Sign-On mode for Unified .
...
Switching to XML-based gadgets reduces latency and improves performance.
...
Available bandwidth from the  to the Finesse server .

Round Trip Time (RTT) between the  server and the Finesse server in case of WAN deployments.
...
RTT between the client and the .
...
For more information about backup and restore, see at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-express/products-maintenance-guides-list.html
...
| Step 2 | The first time when you access the administration console using HTTPS, you are
                                                   ...
                                                   following table describes the steps for each supported browser. Note If you are using HTTPS but have installed a CA Certificate, you can skip
                                                                  this step. For more information about installing a CA Certificate, see Cisco Finesse Installation and Upgrade Guide at https://www.cisco.com/c/en/us/support/customer-collaboration/finesse/products-installation-guides-list.html . Option Description Internet Explorer: A page appears that states this site is untrusted. Click More information > Go on to the webpage . Microsoft Edge: A page appears that states this site is untrusted. Click Details and click Go
                                                                        on to the webpage . Firefox: A page appears that states this connection is untrusted. Click I Understand the Risks , and then
                                                                     click Add Exception . In the Add Security Exception dialog box, ensure that the
                                                                     check box is Permanently store this
                                                                        exception checked. Click Confirm Security Exception . Chrome: A page appears that states this connection is not private. Click Advanced > Proceed to
                                                                        <Hostname> (unsafe) | Note | If you are using HTTPS but have installed a CA Certificate, you can skip
                                                                  this step. For more information about installing a CA Certificate, see Cisco Finesse Installation and Upgrade Guide at https://www.cisco.com/c/en/us/support/customer-collaboration/finesse/products-installation-guides-list.html . | Option | Description | Internet Explorer: | A page appears that states this site is untrusted. Click More information > Go on to the webpage . | Microsoft Edge: | A page appears that states this site is untrusted. Click Details and click Go
                                                                        on to the webpage . | Firefox: | A page appears that states this connection is untrusted. Click I Understand the Risks , and then
                                                                     click Add Exception . In the Add Security Exception dialog box, ensure that the
                                                                     check box is Permanently store this
...
| Microsoft Edge: | A page appears that states this site is untrusted. Click Details and click Go
                                                                        on to the webpage . |
...
| Chrome: | A page appears that states this connection is not private. Click Advanced > Proceed to
                                                                        <Hostname> (unsafe) |
...
| Note | After 30 minutes of inactivity, Cisco Finesse automatically signs you out
                                                            of the administration console and you must sign in again. When your administrator privileges are revoked by another Unified CCX
                                                            administrator, you are automatically signed out (within 5 minutes) from
                                                            the Cisco Finesse administration console. For more information on
                                                            administrators privileges, see Administrator Capability View Menu . |
|---|---|
...
| Note | Cisco Finesse supports a maximum of 100 global and 100 team Wrap-Up reasons. |
|---|---|
...
| Note | By default, Finesse includes all its domains such as publisher, subscriber, and reverse-proxy (if configured). The gadget
                                                URLs in the desktop layouts are added in the allowed list automatically. |
|---|---|
...
| The agent closes the browser, the browser crashes, or the agent clicks the Back button on the browser. | Finesse desktop makes a best-effort attempt to notify the server. | Finesse receives a presence notification of Unavailable from the client. Finesse waits 60 seconds, and then sends a forced logout request to the  server. | Race Conditions The agent closes the browser window. Finesse receives a presence notification of Unavailable for the user. Finesse tries to sign the agent out; however, that agent is already signed out. If the browser crashes, it can take the Finesse server up to 120 seconds to detect that the client is gone and send a presence notification to Finesse. A situation can occur where
                                                      ...
                                                      preceding condition. |
| The client refreshes the browser | — | Finesse receives a presence notification of Unavailable from the client. Finesse waits 60 seconds before sending a forced logout request to the  server to allow the browser to reconnect
                                                after the refresh. | — |
| The Refresh Token has expired. For more information on tokens, see https://developer.cisco.com/docs/finesse/#single-sign-on-apis . | Finesse desktop sends a forced logout request to the  server. | The Finesse server forwards the forced logout request to the  server. | The session expiry warning appears 10 minutes and 5 minutes before the Refresh Token expires. In the last minute, a countdown
                                                timer appears till the Refresh Token expires. The agent is forcefully logged out when the timer reaches zero and must log
                                                in again. |
...
| Note | During failover, agents are redirected to the subscriber and are signed in automatically, and desktop is reloaded. |
|---|---|