---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-admin-14-systemconfig-cucm-b-system-configuration-guide-14su2-cucm-b-sy-d34a242339
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/admin/14/systemConfig/cucm_b_system-configuration-guide-14su2/cucm_b_system-configuration-guide-14_chapter_0100000.html
retrieved_at: 2026-08-16T16:33:54.030055+00:00
---
...
Due to technical limitations device onboarding via Activation Codes is not supported in Proxy TFTP deployments
...
When the on-premise phones are registered and the security profile is configured as OAuth, the phone fetches the access token
                                          implicitly on reset or restart.

| Note | TFTP Proxy setup doesn't support the endpoint registration using activation code Onboarding and MRA. |
|---|---|
...
| Note | When you provision with BAT MAC addresses, activation codes are tied to the phone model. BAT MAC is a reference to the device name that starts with 'BAT' and is followed by a random 12 hexidecimal digits that look
                                          ...
                                          because the administrator must gather and input each phone's MAC address during provisioning, but provides greater security
                                          because users must enter the activation code that matches the actual MAC address on their phone. Due to technical limitations device onboarding via Activation Codes is not supported in Proxy TFTP deployments |
|---|---|
...
| Onboarding On-Premise phones for Use in Mobile and Remote Access mode | You can onboard the phones on-premise, and then mark the phone for onboarding again in Mobile and Remote Access mode to leverage
                                          ...
                                          register, but is ready to contact the Expressway when it is powered up off-premise. Note The off-premise phones that are unregistered cannot update their firmware load.This scenario is useful with out-of-the-box
                                                      phones that need to be on premise to download the latest firmware and use the Activation Code feature. The phone switches to MRA mode when Allow Activation Code via MRA checkbox is checked and has MRA Service Domain and OAuth token. | Note | The off-premise phones that are unregistered cannot update their firmware load.This scenario is useful with out-of-the-box
                                                      phones that need to be on premise to download the latest firmware and use the Activation Code feature. The phone switches to MRA mode when Allow Activation Code via MRA checkbox is checked and has MRA Service Domain and OAuth token. |
| Note | The off-premise phones that are unregistered cannot update their firmware load.This scenario is useful with out-of-the-box
                                                      phones that need to be on premise to download the latest firmware and use the Activation Code feature. The phone switches to MRA mode when Allow Activation Code via MRA checkbox is checked and has MRA Service Domain and OAuth token. |
| Onboarding On-Premise Phones Through Zero Touch Onboarding | When the on-premise phones are registered and the security profile is configured as OAuth, the phone fetches the access token
                                          implicitly on reset or restart. |
