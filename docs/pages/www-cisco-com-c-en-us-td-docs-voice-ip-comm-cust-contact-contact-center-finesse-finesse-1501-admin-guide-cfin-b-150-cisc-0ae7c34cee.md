---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-finesse-finesse-1501-admin-guide-cfin-b-150-cisc-0ae7c34cee
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/finesse/finesse_1501/admin/guide/cfin_b_150_cisco-finesse-administration-guide/cfin_m_150_manage-desktop-layout.html
retrieved_at: 2026-08-21T04:32:38.352975+00:00
---
...
All Finesse gadgets are recommended to comply with the CSP guidelines specified in the SEC-WEB-XSS-FR12: Use Content Security Policy section of the Security Insights page at https://wwwin-si.cisco.com/psb-requirements#SEC-WEB-XSS-4 .

Partners and developers looking to integrate third-party gadgets into Finesse are recommended to ensure that their gadget
                                 codes comply with these CSP guidelines.

To ensure CSP compliance for custom third-party gadgets:

Remove all inline scripts, inline styles, inline event handlers, eval(), new Function(), and string-based setTimeout() from
                                       third-party gadget's code.

Avoid using HTML elements containing inline style= attributes and <script> tags via:
...
Verify that third-party libraries used within third-party gadgets are CSP compliant.

Cisco-owned third-party gadgets hosted on the desktop will be CSP-compliant in upcoming releases. It is recommended that custom
                                             third-party gadgets also adhere to CSP guidelines.
...
Disable historical Transcript : You can configure the CCAI gadgets so agents receive real-time transcripts only. When fetchHistoricalTranscripts=false is set, all agents on the desktop see real time transcription from the point the call is active on the Agent's desktop. Agents
                                                do not receive historical customer transcripts including the Virtual Agent (VA) transcripts from earlier in this conversation.

For example, In transfer and conference scenario, Agent A is the original agent interacting with the caller, and Agent B is
                                                   the receiving agent.

When fetchHistoricalTranscripts=false is configured in the Desktop Layout XML (team or global level), agents do not receive historical transcript data, including
                                                   VA-to-caller content from earlier in the conversation. Only real-time transcripts are shown from the point the call is active
                                                   on the agent's desktop.

Add the fetchHistoricalTranscripts=false query parameter to the gadget URLs within your Desktop Layout XML (at either the Team or Global level) as shown below:

Set fetchHistoricalTranscripts=false on both the Agent Answers and Transcript gadgets. Historical transcript retrieval is disabled only when the same value is
                                                               configured on both gadgets. If the Agent Answers gadget is disabled, you do not need to configure fetchHistoricalTranscripts=false for the Agent Answers gadget.
...
This defines the maximum number of retries attempted by the gadget to load the conversations of the selected tasks (Email,
                                             Live Chat, SMS, Facebook Messenger, WhatsApp, or any digital channel).
...
The administrator can also use the CLI utils finesse set_property webservices enableDropParticipantFor and set permission to allow an agent or a supervisor, who is the participant in a conference call to drop other agents, supervisors,
                                                ...
                                                changes. For more information, see Security Properties .
...
## Call Transcript Gadget

Call Transcript is an Enterprise Voice Assistant that provides advanced speech recognition and transcription capabilities
                              to the Contact Center solution. When Call Transcript is integrated with the Contact Center solution, agents can view the live
                              transcript of the call on their desktop. This transcript enables the agent to wrap-up the call quickly and ensures that the
                              wrap-up notes are accurate and readily available for future reference.

This also enhances agent experience, reduces agent workload, and reduces the cost of Contact Center operations. The reduced
                              wrap-up time increases agent productivity and makes tangible impact to the business. For more information, see the Call Transcript chapter in Cisco Unified Contact Center Enterprise Features Guide at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-feature-guides-list.html .

Cloud Connect is a component that hosts services that allow customers to use cloud capabilities such as Call Transcript . The administrator can configure the Cloud Connect server settings in the Finesse administration console to contact the Cisco
                                          cloud services. For more information, see Cloud Connect Server Settings .

The Call Transcript gadget is displayed on the Home tab of the Finesse desktop. This gadget displays the transcript of the call between an agent and a caller.

Step 1

Sign in to the administration console on the primary Finesse server using the URL: https://FQDN of Finesse server/cfadmin .
...
For upgraded layouts, the Call Transcript gadget will not appear by default. The XML must be copied from the default layout and added to the respective custom layouts.

Step 4

Add the Call Transcript gadget to the Home tab of the agent desktop layout under the <role>Agent</role> tag:
...
```
/3rdpartygadget/files/calltranscript/CallTranscriptGadget.xml
```

Repeat Step 4 to add the gadget for the supervisor desktop layout under the <role>Supervisor</role> tag.
...
To enable Live Data gadgets to use a list of selected agents as a filter parameter, ensure the gadget URL includes the parameter isDynamicFilterEnabled , which must be set to "false".

Example:

```
<gadget>https://<cuic.fqdn>:8444/cuicui/gadget/LiveData/LiveDataGadget.xml?
gadgetHeight=310& vie wId_1=99E6C8E210000141000000D80A0006C4&
filterId_1=agent.id=5002,5011&
isDynamicFilterEnabled=false
</gadget>
```

For the isDynamicFilterEnabled parameter to function, the agent or supervisor must have access to the "Agents" value list. Grant read permission to the
                                             "All Users" group for the "Agents" value list.
...
| Note | Cisco-owned third-party gadgets hosted on the desktop will be CSP-compliant in upcoming releases. It is recommended that custom
                                             third-party gadgets also adhere to CSP guidelines. |
|---|---|
...
| Step 4 | Uncomment the following code snippet within the gadgets tag for the required role. For example, if you want the Transcript gadget to appear on the home page, uncomment (remove the comment (<!-- and -->)) the following code snippet in the gadgets
  ...
  <!-- <gadget managedBy="supervisorMultiTabGadgetContainer" hidden="true">/3rdpartygadget/files/ccaiGadgets/transcriptGadget.xml</gadget> -->
                                                               ...
                                                               add the gadget. The gadget has been optimized to work when deployed within the multi-tab gadget. It is highly recommended to configure the
                                                               gadget using a similar layout. The gadget will be hidden if it is disabled in CCE Admin. This would take effect in Finesse after re-login. Disable historical Transcript : You can configure the CCAI gadgets so agents receive real-time transcripts only. When fetchHistoricalTranscripts=false is set, all agents on the desktop see real time transcription from the point the call is active on the Agent's desktop. Agents
                                                do not receive historical customer transcripts including the Virtual Agent (VA) transcripts from earlier in this conversation. For example, In transfer and conference scenario, Agent A is the original agent interacting with the caller, and Agent B is
                                                   the receiving agent. When fetchHistoricalTranscripts=false is configured in the Desktop Layout XML (team or global level), agents do not receive historical transcript data, including
                                                   VA-to-caller content from earlier in the conversation. Only real-time transcripts are shown from the point the call is active
                                                   on the agent's desktop. Add the fetchHistoricalTranscripts=false query parameter to the gadget URLs within your Desktop Layout XML (at either the Team or Global level) as shown below: <role>Agent</role> : <role>Supervisor</role> : Note Set fetchHistoricalTranscripts=false on both the Agent Answers and Transcript gadgets. Historical transcript retrieval is disabled only when the same value is
                                                               configured on both gadgets. If the Agent Answers gadget is disabled, you do not need to configure fetchHistoricalTranscripts=false for the Agent Answers gadget. | Note | For customized Cisco Finesse desktop layouts or during Cisco Finesse upgrade to Release 12.6(1), the administrator must manually
                                                               add the gadget. The gadget has been optimized to work when deployed within the multi-tab gadget. It is highly recommended to configure the
                                                               gadget using a similar layout. The gadget will be hidden if it is disabled in CCE Admin. This would take effect in Finesse after re-login. | Note | Set fetchHistoricalTranscripts=false on both the Agent Answers and Transcript gadgets. Historical transcript retrieval is disabled only when the same value is
                                                               configured on both gadgets. If the Agent Answers gadget is disabled, you do not need to configure fetchHistoricalTranscripts=false for the Agent Answers gadget. |
| Note | For customized Cisco Finesse desktop layouts or during Cisco Finesse upgrade to Release 12.6(1), the administrator must manually
                                                               add the gadget. The gadget has been optimized to work when deployed within the multi-tab gadget. It is highly recommended to configure the
                                                               gadget using a similar layout. The gadget will be hidden if it is disabled in CCE Admin. This would take effect in Finesse after re-login. |
| Note | Set fetchHistoricalTranscripts=false on both the Agent Answers and Transcript gadgets. Historical transcript retrieval is disabled only when the same value is
                                                               configured on both gadgets. If the Agent Answers gadget is disabled, you do not need to configure fetchHistoricalTranscripts=false for the Agent Answers gadget. |
...
| loadingConversationFailureMaxRetries | Due to initialization delays, digital tasks loaded from the internet hosted service could at times fail. The gadget is expected
                                             ...
                                             Live Chat, SMS, Facebook Messenger, WhatsApp, or any digital channel). The default value is 4. |
...
| Note | The administrator can also use the CLI utils finesse set_property webservices enableDropParticipantFor and set permission to allow an agent or a supervisor, who is the participant in a conference call to drop other agents, supervisors,
                                                or non-agents from the conference call. Enabling this capability at a an API level is a prerequisite for the desktop property
                                                changes. For more information, see Security Properties . The Dialog—Drop Participant from Conference API allows an agent or supervisor to make a request to drop other agents, supervisors, or non-agents from a conference based
...
| Note | Cloud Connect is a component that hosts services that allow customers to use cloud capabilities such as Call Transcript . The administrator can configure the Cloud Connect server settings in the Finesse administration console to contact the Cisco
                                          cloud services. For more information, see Cloud Connect Server Settings . |
|---|---|

| Step 1 | Sign in to the administration console on the primary Finesse server using the URL: https://FQDN of Finesse server/cfadmin . |
|---|---|
| Step 2 | Click Desktop Layout . |
| Step 3 | Select from the following editors: Text Editor XML Editor Note For upgraded layouts, the Call Transcript gadget will not appear by default. The XML must be copied from the default layout and added to the respective custom layouts. | Note | For upgraded layouts, the Call Transcript gadget will not appear by default. The XML must be copied from the default layout and added to the respective custom layouts. |
| Note | For upgraded layouts, the Call Transcript gadget will not appear by default. The XML must be copied from the default layout and added to the respective custom layouts. |
| Step 4 | Add the Call Transcript gadget to the Home tab of the agent desktop layout under the <role>Agent</role> tag: Text Editor —Remove the comment (<!– and -->) from the layout to add the gadget to the desktop layout. Example: <gadgets>
    <gadget>/3rdpartygadget/files/calltranscript/CallTranscriptGadget.xml</gadget>
</gadgets> XML Editor —To add a gadget, copy and paste the XML within the gadgets tag: Example: /3rdpartygadget/files/calltranscript/CallTranscriptGadget.xml Note Repeat Step 4 to add the gadget for the supervisor desktop layout under the <role>Supervisor</role> tag. | Note | Repeat Step 4 to add the gadget for the supervisor desktop layout under the <role>Supervisor</role> tag. |
| Note | Repeat Step 4 to add the gadget for the supervisor desktop layout under the <role>Supervisor</role> tag. |
| Step 5 | Click Save . |
...
| Note | For the isDynamicFilterEnabled parameter to function, the agent or supervisor must have access to the "Agents" value list. Grant read permission to the
                                             "All Users" group for the "Agents" value list. |
|---|---|
