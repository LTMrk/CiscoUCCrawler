---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-connection-15-security-guide-b-15cucsecx-b-15cucsecx-chapter-0100-html-5de5c45314
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/security/guide/b_15cucsecx/b_15cucsecx_chapter_0100.html
retrieved_at: 2026-08-16T18:33:42.742625+00:00
---
...
If you are upgrading from any release of Unity Connection 12.5 in FIPS mode to Unity Connection 15 and later, make sure to
                                       install COP File ciscocm.ciscossl7_upgrade_CSCwa48315_CSCwa77974_v1.0.k4.cop.sha512 on both nodes of cluster before upgrade. For information on Unity Connection upgrades, see Upgrade Considerations with FIPS Mode section of the "Upgrading Cisco Unity Connection" chapter of the Install, Upgrade and Maintenance Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/install_upgrade/guide/b_15cuciumg.html
...
When upgrading from any lower version to 15 SU2 or later, it is mandatory to upgrade all nodes in the networking to ensure
...
Signature Algorithms (Applicable for Release 15 SU2 and later)

In FIPS mode, Networking supports only rsa-sha2-256 (SHA256withRSA) as signature algorithm.

In non-FIPS mode, Networking supports both rsa-sha2-256 and ssh-rsa (SHA1withRSA) as signature algorithms. After the upgrade, ssh-rsa will be enabled by default. It is recommended to use rsa-sha2-256 which is more secure. After upgrade to 15 SU2 or later, to enable rsa-sha2-256 , execute the CLI command given below on Publisher Node of all the Clusters connected in Networking.
...
| Note | If you are upgrading from any release of Unity Connection 12.5 in FIPS mode to Unity Connection 15 and later, make sure to
                                       install COP File ciscocm.ciscossl7_upgrade_CSCwa48315_CSCwa77974_v1.0.k4.cop.sha512 on both nodes of cluster before upgrade. For information on Unity Connection upgrades, see Upgrade Considerations with FIPS Mode section of the "Upgrading Cisco Unity Connection" chapter of the Install, Upgrade and Maintenance Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/install_upgrade/guide/b_15cuciumg.html |
|---|---|

| Caution | (Applicable for Releases before 15 SU2) After enabling or disabling the FIPS mode, the Cisco Unity Connection server restarts automatically. |
|---|---|
...
| Note | Secure Messages are not sent in a FIPS compliant manner unless an IPsec Policy is configured. When upgrading from any lower version to 15 SU2 or later, it is mandatory to upgrade all nodes in the networking to ensure
                                                smooth operation and avoid replication issues. |
|---|---|
...
| Signature Algorithms (Applicable for Release 15 SU2 and later) | In FIPS mode, Networking supports only rsa-sha2-256 (SHA256withRSA) as signature algorithm. Note It is mandatory to upgrade all nodes in the Networking to Release 15 SU2 or later to ensure smooth operation and avoid replication
                                                      issues. In non-FIPS mode, Networking supports both rsa-sha2-256 and ssh-rsa (SHA1withRSA) as signature algorithms. After the upgrade, ssh-rsa will be enabled by default. It is recommended to use rsa-sha2-256 which is more secure. After upgrade to 15 SU2 or later, to enable rsa-sha2-256 , execute the CLI command given below on Publisher Node of all the Clusters connected in Networking. run cuc dbquery unitydirdb update tbl_configuration set valuebool = 1 where 
fullname = 'System.Security.EnableSha256' Note It is recommended to upgrade all nodes in the Networking to Release 15 SU2 or later to ensure smooth operation and avoid replication
                                                      issues. | Note | It is mandatory to upgrade all nodes in the Networking to Release 15 SU2 or later to ensure smooth operation and avoid replication
                                                      issues. | Note | It is recommended to upgrade all nodes in the Networking to Release 15 SU2 or later to ensure smooth operation and avoid replication
                                                      issues. |
| Note | It is mandatory to upgrade all nodes in the Networking to Release 15 SU2 or later to ensure smooth operation and avoid replication
                                                      issues. |
