---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-connection-14-administration-guide-b-14cucsag-b-14cucsag-chapter-01010-html-a964ddefa9
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/14/administration/guide/b_14cucsag/b_14cucsag_chapter_01010.html
retrieved_at: 2026-08-17T02:26:12.729550+00:00
---
...
If Cisco Unity Connection is unable to play a termination warning prompt after reaching the configured termination recording
                                                            warning time while callers record their messages, disable the Recording Termination Warning by setting the value to zero.
...
##### Configuring SMTP client communication over STARTTLS

Cisco Unity Connection 12.5(1) Service Update 3 and later, supports secure SMTP client communication. You can configure Unity
                                       Connection to support secure SMTP client interface using STARTTLS.

Step 1

To enable Secure SMTP client feature, execute the following CLI command only on the Publisher node with the Primary role:
...
After enabling secure SMTP client communication via the CLI, you must restart the Connection SMTP Server service. If you are running a cluster, ensure that you restart this service on both servers to apply the changes successfully.
...
| Step 2 | On the Telephony Configuration page, enter
                                             			 the values of the following fields: Minimum Recording Duration in
                                                      					 ...
                                                      					 Milliseconds Select Save to apply the changes. Note If Cisco Unity Connection is unable to play a termination warning prompt after reaching the configured termination recording
                                                            warning time while callers record their messages, disable the Recording Termination Warning by setting the value to zero. | Note | If Cisco Unity Connection is unable to play a termination warning prompt after reaching the configured termination recording
                                                            warning time while callers record their messages, disable the Recording Termination Warning by setting the value to zero. |
| Note | If Cisco Unity Connection is unable to play a termination warning prompt after reaching the configured termination recording
                                                            warning time while callers record their messages, disable the Recording Termination Warning by setting the value to zero. |
...
| Step 1 | To enable Secure SMTP client feature, execute the following CLI command only on the Publisher node with the Primary role: run cuc dbquery unitydirdb update tbl_configuration set valuebool='1' 
where name ='SmtpSecureClientEnabled' By default this feature is disabled. Note After enabling secure SMTP client communication via the CLI, you must restart the Connection SMTP Server service. If you are running a cluster, ensure that you restart this service on both servers to apply the changes successfully. | Note | After enabling secure SMTP client communication via the CLI, you must restart the Connection SMTP Server service. If you are running a cluster, ensure that you restart this service on both servers to apply the changes successfully. |
|---|---|---|---|
| Note | After enabling secure SMTP client communication via the CLI, you must restart the Connection SMTP Server service. If you are running a cluster, ensure that you restart this service on both servers to apply the changes successfully. |
...
| Step 2 | Client side configuration: In Cisco Unity Connection Administration, expand System Settings> SMTP Configuration and select Smart Host . To enable Secure SMTP client feature, check option Enable Secure Client . You can choose secure port between 25 and 587 using Host Port field if Enable Secure Client option is checked. To enable authentication support, check the Use Authentication option and enter the details of password protected Smart Host. You must enter username and password if this option is checked. Select Save to apply the changes. (For more information on each field, see Help> This Page ). For changes to take effect, you must restart the Connection SMTP Server service in Cisco Unity Connection Serviceability.
                                                      If a Connection cluster is configured, restart the service on both nodes. Note After enabling secure SMTP client communication via the CLI, you must restart the Connection SMTP Server service. If you are running a cluster, ensure that you restart this service on both servers to apply the changes successfully. | Note | After enabling secure SMTP client communication via the CLI, you must restart the Connection SMTP Server service. If you are running a cluster, ensure that you restart this service on both servers to apply the changes successfully. |
| Note | After enabling secure SMTP client communication via the CLI, you must restart the Connection SMTP Server service. If you are running a cluster, ensure that you restart this service on both servers to apply the changes successfully. |
