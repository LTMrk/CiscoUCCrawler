...
Cisco Emergency Responder Release 14 supports the following new features:

## Compliance to New Federal Regulations
...
Cisco Emergency Responder can be configured to provide E911 support to remote teleworker and off-premises users. These users
                  when registering over Virtual Private Network (VPN) or Mobile and Remote Access, can update their location through the phone
                  display or through the Cisco Emergency Responder Off-premises User Page.

## Simplified Administration

Support for configuration APIs for Emergency Response Location (ERL) Management,  User Management, and associated Cisco Unified
                  Communications Manager settings.

Role based Access Control: Default Standard User Groups like System Administrator, Network Administrator, ERL Administrator,
                  Serviceability, Administrator Utility, and End User. Additional User Groups can be created with defined roles and access.

For more information on the APIs and their configuration, see the Cisco Emergency Responder (CER) API Documentation .

## Infrastructure Refresh

Cisco Emergency Responder provides Switch Refresh Utility to simplify Voice LAN Switch replacement, minimizing administrator
                  and logistics overhead, while ensuring E911 support.

## Enhanced Security Considerations

Cipher Management provides the administrators the control to configure the set of security ciphers  allowed for every TLS
                        and SSH connection. This feature allows you to configure the recommended ciphers without breaking the compatibility with other
                        components such as older model phones, or any entity that is securely tracked through Emergency Responder.

From Cisco Emergency Responder Release 14, all the certificate related operations are moved from Ipsec to Tomcat certificates.
                        ...
                        and subscriber for any cluster related operations to work.
...
Emergency Responder supports Version Independent User Licenses. The Licenses are annuity-style and issued for the subscription
...
Emergency Responder 12.x continues to use the version 12.X License.

The licenses are managed on CSSM (Cisco Smart Software Manager). For more information, see the 'Cisco Smart Software Licensing'
                  section in the Cisco Emergency Responder Administration Guide .

## Smart Licensing Registration through Authentication based Proxy

Proxy server is updated to include extra authentication measures to register to Cisco Smart Software Manager using authentication
                  based proxy server (HTTP or HTTPS).

## Switch Support

Support switch port based tracking for new Voice LAN Switches with Simple Network Management Protocol (SNMP): Includes Catalyst
                  9300L, Cisco Catalyst 1000 Series,  Cisco Industrial Ethernet 2000, 3000, 4000, and 5000 Series Switches, and Cisco Meraki
                  Switches MS120, MS125, and MS355. For more information, see the Supported Voice Ready Lan Switches section.

## Simplifying Release Number Scheme

From Release 14 onwards, Cisco Emergency Responder has adopted the single number release plan. There will be no (dot) releases
                  like (dot five) in the past release versions. Service Upgrade releases will be published on top of the main major release
                  14 through the regular Software Maintenance cycle.

## Requirements

### Supported Hardware and Software

The information in the following sections discuss Hardware and Software requirements for Cisco Emergency Responder 14. Read
...
Cisco Webex client (registered native to Unified Communications Manager) is represented through the registered Spark RD (Remote
                                       Destination) and needs to be manually defined in Emergency Responder for tracking.
...
### Cisco Emergency Responder 14 Supported Upgrades
...
Direct upgrades to Cisco Emergency Responder 14 are supported only for the following releases:
...
Cisco Emergency Responder 11.0(1), or Cisco Emergency Responder 11.5(1) or Cisco Emergency Responder 11.5(2) or Cisco Emergency
                              Responder 11.5(2a) or Cisco Emergency Responder 11.5(3) or Cisco Emergency Responder 11.5(4) or Cisco Emergency Responder
                              11.5(4) SU1 or Cisco Emergency Responder 11.5(4) SU2 or Cisco Emergency Responder 11.5(4) SU3 or Cisco Emergency Responder
                              11.5(4) SU4 or Cisco Emergency Responder 11.5(4) SU9 or Cisco Emergency Responder 11.5(4) SU10
...
Cisco Emergency Responder 12.5(1) or Cisco Emergency Responder 12.5(1a) or Cisco Emergency Responder 12.5(1) SU1 or Cisco
                              ...
                              Emergency Responder 12.5(1) SU5

#### COP Files Required for Upgrades to Release 14
...
You must use the following COP file to upgrade from release 10.5(1) and 10.5(1a) to release 14 directly when you select the
                                             Switch Version as No : ciscocm.cer_10x_RU_12.5su3.cop.sgn .
...
11.5(1), 11.5(2), 11.5(2a), 11.5(3), 11.5(4), 11.5(4) SU1, 11.5(4) SU2, 11.5(4) SU3, 11.5(4) SU4, 11.5(4) SU9, 11.5(4) SU10
...
If you want to upgrade Emergency Responder from Release 11.5.x (except for Release 11.5(1)SU11) to 14SU2, use the following
                                             COP file: ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn.

Apply the required Upgrade Readiness COP Files (post-upgrade): ciscocm.cer_postUpgradeCheck-X.k4.cop.sha512 .

12.0(1), 12.0(1) SU1, and 12.0(1) SU2
...
12.5(1), 12.5(1a), 12.5(1) SU1, 12.5(1) SU2, 12.5(1) SU3, 12.5(1) SU4, 12.5(1) SU5,
...
If you want to upgrade Emergency Responder from Release 12.5.x (except for Release 12.5(1)SU6) to 14SU2, use the following
                                             COP file: ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn.
...
CER sends randomly picked line to the Intrado/Redsky when the Offpremise Phone having multiple lines

CSCvx40261

Redsky support for CER 12.5
...
Text truncation in CER ALI Formatting Tool

CSCvw96322

CER not sending email after backup CER becomes active

CSCvw80131

CER disregards proxy settings for licensing

CSCvu48074

CER SLM Connection Fails When Using Proxy

CSCvt97450

Incremental discovery not working due to ipv6 address searched in an incorrect OID

CSCvt73209

Scheduled backups executed one hour early or later when DST takes effect

CSCvs36001

CER: During Refresh Upgrade, VM console showing blue screen for over 30 minutes
...
|---|---|---|
| Cisco Unified Communications Manager | Cisco Unified Communications Manager 14 Cisco Unified Communications Manager 12.5(x) Cisco Unified Communications Manager 12.0(x) Cisco Unified Communications Manager 11.5(x) Cisco Unified Communications Manager 11.0(x) Cisco Unified Communications Manager 10.5(x) Cisco Unified Communications Manager 10.0(x) | The software that runs the telephony network. |
| Web browser | Firefox with Windows 10 (64 bit) Chrome with Windows 10 (64 bit) Internet Explorer 11 with Windows 10 (64 bit) Internet Explorer 11 with Windows 7 (64 bit) Safari with MacOS (10.x) Microsoft Edge browser with Windows 10 (32 bit/64 bit) | We recommend that you use the latest version for all the web browsers supported. |
...
| Phones that you can manually define or track using an IP subnet Phones that are connected to analog line gateways such as Cisco VG350 or VG224 series or ATA 180 series or ATA 190 series
                                             or ATA 191 series. Any H.323 endpoints Cisco Webex App (registered natively to Unified Communications Manager) | These phones are supported only if their calls are routed by Cisco Unified Communications Manager. Note Any IP endpoint can be tracked at call time using the IP subnet provided that the Use IP Address From Call Signaling Telephony
                                                setting is enabled. Cisco Webex client (registered native to Unified Communications Manager) is represented through the registered Spark RD (Remote
                                       Destination) and needs to be manually defined in Emergency Responder for tracking. | Note | Any IP endpoint can be tracked at call time using the IP subnet provided that the Use IP Address From Call Signaling Telephony
                                                setting is enabled. |
| Note | Any IP endpoint can be tracked at call time using the IP subnet provided that the Use IP Address From Call Signaling Telephony
                                                setting is enabled. |
| Phones supported for off-premises location confirmation and update with the Remote Worker Emergency Calling feature in Unified
...
|---|---|---|
| 10.0(1), 10.0(2), 10.5(1), and 10.5(1a) | 14 | Apply the required Upgrade Readiness COP File (pre-upgrade): ciscocm.cer_preUpgradeCheck-X.k4.cop.sha512 . Refresh upgrade. Required COP files: ciscocm.version3-keys.cop.sgn ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn Note You must use the following COP file to upgrade from release 10.5(1) and 10.5(1a) to release 14 directly when you select the
                                             Switch Version as No : ciscocm.cer_10x_RU_12.5su3.cop.sgn . Apply the required Upgrade Readiness COP Files (post-upgrade): ciscocm.cer_postUpgradeCheck-X.k4.cop.sha512 . | Note | You must use the following COP file to upgrade from release 10.5(1) and 10.5(1a) to release 14 directly when you select the
                                             Switch Version as No : ciscocm.cer_10x_RU_12.5su3.cop.sgn . |
| Note | You must use the following COP file to upgrade from release 10.5(1) and 10.5(1a) to release 14 directly when you select the
                                             Switch Version as No : ciscocm.cer_10x_RU_12.5su3.cop.sgn . |
| 11.0(1) | 14 | Apply the required Upgrade Readiness COP File (pre-upgrade): ciscocm.cer_preUpgradeCheck-X.k4.cop.sha512 . Refresh upgrade. Required COP file: ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn Apply the required Upgrade Readiness COP Files (post-upgrade): ciscocm.cer_postUpgradeCheck-X.k4.cop.sha512 . |
| 11.5(1), 11.5(2), 11.5(2a), 11.5(3), 11.5(4), 11.5(4) SU1, 11.5(4) SU2, 11.5(4) SU3, 11.5(4) SU4, 11.5(4) SU9, 11.5(4) SU10 | 14 | Apply the required Upgrade Readiness COP File (pre-upgrade): ciscocm.cer_preUpgradeCheck-X.k4.cop.sha512 . Refresh upgrade. Required COP file: ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn Note If you want to upgrade Emergency Responder from Release 11.5.x (except for Release 11.5(1)SU11) to 14SU2, use the following
                                             COP file: ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn. Apply the required Upgrade Readiness COP Files (post-upgrade): ciscocm.cer_postUpgradeCheck-X.k4.cop.sha512 . | Note | If you want to upgrade Emergency Responder from Release 11.5.x (except for Release 11.5(1)SU11) to 14SU2, use the following
                                             COP file: ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn. |
| Note | If you want to upgrade Emergency Responder from Release 11.5.x (except for Release 11.5(1)SU11) to 14SU2, use the following
                                             COP file: ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn. |
| 12.0(1), 12.0(1) SU1, and 12.0(1) SU2 | 14 | Apply the required Upgrade Readiness COP File (pre-upgrade): ciscocm.cer_preUpgradeCheck-X.k4.cop.sha512 . Refresh upgrade. Required COP files: ciscocm.cer_resetudi.cop.sg ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn Apply the required Upgrade Readiness COP Files (post-upgrade): ciscocm.cer_postUpgradeCheck-X.k4.cop.sha512 . |
| 12.5(1), 12.5(1a), 12.5(1) SU1, 12.5(1) SU2, 12.5(1) SU3, 12.5(1) SU4, 12.5(1) SU5, | 14 | Apply the required Upgrade Readiness COP File (pre-upgrade): ciscocm.cer_preUpgradeCheck-X.k4.cop.sha512 . Required COP file: ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn Note If you want to upgrade Emergency Responder from Release 12.5.x (except for Release 12.5(1)SU6) to 14SU2, use the following
                                             COP file: ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn. Apply the required Upgrade Readiness COP Files (post-upgrade): ciscocm.cer_postUpgradeCheck-X.k4.cop.sha512 . | Note | If you want to upgrade Emergency Responder from Release 12.5.x (except for Release 12.5(1)SU6) to 14SU2, use the following
                                             COP file: ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn. |
| Note | If you want to upgrade Emergency Responder from Release 12.5.x (except for Release 12.5(1)SU6) to 14SU2, use the following
                                             COP file: ciscocm.enable-sha512sum-2021-signing-key-v1.0.cop.sgn. |
...
| CSCvx40261 | Redsky support for CER 12.5 |

| Identifier | Headline |
|---|---|
| CSCvx08115 | Text truncation in CER ALI Formatting Tool |
| CSCvw96322 | CER not sending email after backup CER becomes active |
