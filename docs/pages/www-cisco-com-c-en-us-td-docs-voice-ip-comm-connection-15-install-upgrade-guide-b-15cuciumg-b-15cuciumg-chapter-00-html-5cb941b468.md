---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-connection-15-install-upgrade-guide-b-15cuciumg-b-15cuciumg-chapter-00-html-5cb941b468
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/install_upgrade/guide/b_15cuciumg/b_15cuciumg_chapter_00.html
retrieved_at: 2026-08-16T18:28:13.573381+00:00
---
...
Standard Installation, Unattended Installation, and Install with Data Import — are applicable to all supported virtualization
                              platforms i.e. VMware vSphere ESXi, Nutanix AHV, and Cisco NFVIS for UC from 15SU4 and onwards
...
Unattended Installation : Allows you to install Unity Connection using an installation disk and a pre-configured answer file CDROM drive. The answer
                                 file has all the information required for unattended installation. Unattended installation is a seamless process of installation
                                 that allows you to start installation on both the publisher and subscriber servers simultaneously. The subscriber installation
                                 continues when the publisher is successfully installed. This type of unattended installation is Touchless Installation. For
                                 more information on Touchless Installation, see the Touchless Installation for Virtual Machine .

You can also perform fresh installation of Unity Connection 15 and later using Cisco Prime Collaboration Deployment. For more
...
As part of the transition to 64-bit architecture, the Virtual Floppy Drive is replaced with a virtual CDROM for touchless
                                                   installation configuration files.

Install with Data Import : Cisco Unity Connection supports installation of Unity Connection along with the data import from the previous releases.
                                 It involves migration of data by exporting source release data to SFTP server, and installing a new machine with import of
                                 that data. Examples of data that you can export and import are component specific configurations files, voicemails, DB related
                                 files, platform provision data and platform files like certificates. For more details, see Install with Data Import section.

Automated Installation using vApp properties and VMware OVF Tool : Cisco Unity Connection supports automated installation of Unity Connection via VMware Open Virtualization Format(OVF) Tool.
                                 The VMware OVF Tool is used to deploy and inject the Unity Connection configuration parameters into the virtual machines using
                                 skip-install OVA and vApp properties without using Answer File Generator. For more details, see Automated Installation using vApp properties and VMware OVF Tool section.
...
Cisco Unity Connection Release 15SU4 introduces support for additional virtualization options. In addition to VMware vSphere
                                 ESXi, Release 15SU4 supports Cisco NFVIS-for-UC and Nutanix AHV
...
Verify the system requirements, such as licensing and phone integration requirements necessary for the Unity Connection server
                                 in the System Requirements for Cisco Unity Connection guide at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/requirements/b_15cucsysreqs.html... .
...
For Cisco Unity Connection nodes, the files are available here: https://software.cisco.com/download/home/283062758/type/282074348/release/OVA-15
...
with pre-aligned disk partitions.
...
Data Export must be performed simultaneously on both Publisher and Subscriber during a maintenance window; separate exports
                                             will result in message loss for any voicemails handled by the Subscriber alone.
...
### Direct Subscriber Installation with Data Import

Once the data is exported from publisher node, import data on destination publisher node and fresh install destination subscriber
                                    node.

With above way of data import, in case of Simple Migration , perform the following steps:

Fresh Install Publisher node with Data import .

Fresh Install Subscriber node directly to the above publisher with data import .

Import data on new virtual machines, using Import option available in Installation wizard. For more information see Installing the Publisher Server section.

Caution

In case of a cluster network, modifications on cluster page settings on the publisher server is not required.

Security password must be the same as from the previous publisher server .

With the above way of data import, incase of Network Migration , perform the following steps:

Fresh Install Publisher node with Data import .

In case of a cluster defined by IP address or hostname on publisher server:
...
It is adviced to fresh install subscriber node directly to the publisher . Do not delete the old subscriber entry from publisher
                                                                  or reboot the publisher .

Security password must be the same as from the previous publisher server .

Make sure to install ciscocm.cuc_preUpgradeCheck-005.cop.sha512 COP on both nodes of the cluster and verify that the cluster is ready for migration before Data Export. Download the COP
                                                files from https://software.cisco.com/download/home/286313379/type/286319537/release/COP-Files .

Make sure that the server on which Import is to be performed is created using recommended OVA. For more information, see https://www.cisco.com/c/dam/en/us/td/docs/voice_ip_comm/uc_system/virtualization/virtualization-cisco-unity-connection.html .

Once migrated, if for any reason you decide to roll back Data Export COP file on Unity Connection 15 Release, then install ciscocm.cuc_DataExport_rollback_v1.1.k4.cop.sha512 COP file.
...
Get base OVA. Example OVA file names:

NFVIS and VMware vSphere ESXi - CUC_15_all_esxi_vmv17_or_nfvis_v1.0.sha512.ova

Nutanix AHV - CUC_15_large_2x500_ahv_v1.0.sha512.ova

For more information, see the Creating a Virtual Machine section.

You must manually set the Latency Sensitivity of the virtual machine to "High". For more details, refer to the OVA README
                                                      file available at https://software.cisco.com/download/home/283062758/type/282074348/release/OVA-15
...
To download the OVA template for creating virtual machines, open the following link, select Unity Connection Software, and
                                 then select the appropriate release number:

https://software.cisco.com/download/type.html?mdfid=283062758&flowid=45673 .

One base OVA is used for both VMware vSphere ESXi and Cisco NFVIS for UC, while a set of base OVAs is used for Nutanix AHV.

#### Creating a Virtual Machine on VMware vSphere ESXi

#### Creating a Virtual Machine on NFVIS-for-UC

To deploy the OVA template in a supported Cisco NFVIS Version.
...
Upload the OVA as a profile and the ISO as an image together. This process creates a separate profile for each VM configuration
                                                         and a single image for the bootable ISO. For more information, refer to Cisco Enterprise Network Function Virtualization Infrastructure Software Configuration Guide .

An example OVA file name is: CUC_15_all_esxi_vmv17_or_nfvis_v1.0.sha512.ova
...
An example OVA file name for a large size deployment is: CUC_15_large_2x500_ahv_v1.0.sha512.ova.

- Download the Unity Connection .ova files based on the required configuration from the Software Download page.

- Upload the Unity Connection .ova files (downloaded) onto the Nutanix cluster using Prism Central.

- Download the desired Unity Connection bootable .iso image version from the Software Download page and upload the image to
                                                the Nutanix cluster.

- Creating Unity Connection Publisher and Subscriber VMs using the .ova files.

- Update Unity Connection VMs to mount CD-ROM with the Unity Connection image.
...
If you want to perform an unattended installation, select Skip . Connect the answer file image on a CDROM  drive 2. Create the config files with .iso extension and attach the .iso to this
                                                   drive and select Continue . The installation wizard reads the configuration information during the installation process and then follow the steps mentioned
...
If Import option is selected only for publisher node, while installing Unity Connection 15  release.This is the case network migration
                                                               in which data is exported and imported on publisher node only and subscriber node is freshly installed.
...
You can generate answer files using Cisco Unified Communications Answer File Generator web application. To use the answer
                                 file during installation, you need to save the answer file to the root directory of CDROM drive, browse to the file during
                                 installation, and leave the installation to complete.
...
After generating the answer files,attach the .iso to this drive.
...
Select the CDROM drive 1  > Connect to ISO image on local disk option from the toolbar and select CDROM drive 1  > Connect
                                                to ISO image on a datastore, navigate to the data store to select the ISO image, and click OK. The ISO image is attached and
                                                the installation starts.

Step 3

For Touchless Installation Generate ISO File from the answer file.

Step 4

Select the CDROM drive 2  > Connect to ISO image generated  from Step 3 on local disk option from the toolbar and select CDROM drive 2  > Connect to ISO image on a datastore, navigate to the data
                                                store to select the ISO image, and click OK. The ISO image is attached and the installation starts.

Step 5

(Optional) If you want to test the media before the installation, click OK in the Disc Found message box, or select Skip to
                                                skip testing the media before the installation. The installation proceeds without any manual intervention. The publisher is
                                                installed and the subscribers is added to the publisher.
...
After generating the answer files, create the config files with .iso extension and attach the .iso to CDROM drive 2.
...
You must have already planned your network topology, including addresses for your Unity Connection cluster nodes.

Step 1

Log in to the Cisco Unity Connection Answer File Generator application at https://www.cisco.com/c/en/us/applicat/content/cuc-afg/index.html
...
This check box must be checked if you want to install your full cluster in a single process. Otherwise, you will need to either
...
Repeat this procedure to generate answer files for Cisco Unity Connection.
...
To upload the ISO image, click Upload and select the ISO file from your local file system. For example: Bootable_UCSInstall_CUC_15.0.1.iso .

To configure the Image Properties, enter a pre-populated name in the Image Name to a descriptive name. For example, cuc-15.0.1 . Also, select a VM Type from the drop-down list. If your application type is not listed, select OTHER .

To upload an OVA Metadata, select the Metadata checkbox, click Upload OVA , and select the OVA file. For example, CUC_15_all_esxi_vmv17_or_nfvis_v1.0.sha512.ova .
...
To configure the Image Details, enter a descriptive name in the Image Name field. For example, cuc-15.0.1 .
...
Image File Path : Enter the directory path to the ISO file with the image name. For example: /home/user/images/Bootable_UCSInstall_CUC_15.0.1.14900-45.sha512.iso
...
To upload an OVA Metadata, select the Metadata checkbox, and enter the OVA File Path name. For example /home/user/images/CUC_15_all_esxi_vmv17_or_nfvis_v1.0.sha512.ova
...
Choose the required image from the drop-down list. Images are populated based on the VM type selected. For example, cuc-15.0.1.
...
Download the desired version of the CUC bootable .iso image from the Cisco.com to your local machine.
...
Check the status of the image upload on the Tasks page to confirm the successful upload of the .iso image to the Nutanix cluster. For example, a URL is the following:
...
This procedure is only supported on VMware vSphere ESXi.
...
This feature uses a skip-install Open Virtual Archive (OVA) file containing an application that is installed up to the “skip” configuration point, where the application is ready to
                              ...
                              and vApp properties without using Answer File Generator.
...
If Unity Connection on source release has Secure SIP call configured using SIP Integration then after successful installation,
                                       ...
                                       see section Settings for RSA Key Based certificates of Cisco Unified Communications Manager SIP Integration Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/integration/cucm_sip/b_15cucintcucmsip.html .
...
If Unity Connection on source release uses tomcat-ECDSA certificates (self signed and third party) for next generation security
                                       ...
                                       to regenerate certificates, see section Settings for EC Key Based certificates of Cisco Unified Communications Manager SIP Integration Guide for Cisco Unity Connection Release 15 available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/integration/cucm_sip/b_15cucintcucmsip.html .
...
For more information, see Quick Start Guide for SAML SSO Access available at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/quick_start/guide/b_15cucqssamlsso.html .
...
Default Partition Name does not change during Migration, hence if you want to change the Partition Name to new Name then follow
                                 the steps mentioned in https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/administration/guide/b_15cucsag/b_15cucsag_chapter_0111.html#ID-2420-0000020c .
...
Before Migration, if the Unity Connection Cluster was onboarded to Cisco Webex Cloud-Connected UC, then post migration, first
                                 ...
                                 UC. For more information, refer "SpeechView Cisco Webex in-house transcription service" chapter of the System Administration Guide for Cisco Unity Connection Release 15 , at https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/15/administration/guide/b_15cucsag.html .
...
| Note | You can also perform fresh installation of Unity Connection 15 and later using Cisco Prime Collaboration Deployment. For more
                                                   information on Cisco PCD, see http://www.cisco.com/c/en/us/products/cloud-systems-management/prime-collaboration/index.html The answer file supports only fresh installs and does not support upgrades. As part of the transition to 64-bit architecture, the Virtual Floppy Drive is replaced with a virtual CDROM for touchless
                                                   installation configuration files. |
|---|---|

| Compatible Hypervisor Major/Minor Releases with Minimum Maintenance/Patch Release |
|---|
| Cisco Unity Connection Release | VMware vSphere ESXi | Cisco NFVIS-for-UC | Nutanix AHV |
| Release 15SU4 and later | 8.0 U1 7.0 U3 | 4.18.2a | AHV 10.0 + AOS/PC 7.0 |
| Release 15SU4 and earlier | 8.0 U1 7.0 U3 | Not supported | Not supported |

| Component and Capacity Point | vCPU | Physical CPU Required Minimum Base Frequency | vRAM | vDisk | vNIC |
|---|---|---|---|---|---|
| Extra Small (1000 users) | 2 | 1.80+ GHz | 10 GB | 1x 160 GB | 1 (1GbE+) |
| Small (5000 users) | 2 | 2.50+ GHz | 12 GB | 1x 200 GB | 1 (1GbE+) |
| Medium (10000 users) | 4 | 2.50+ GHz | 12 GB | 2x 146 GB or 2x 300 GB or 2x 500 GB with pre-aligned disk partitions. | 1 (1GbE+) |
| Large (20000 users) | 7 | 2.50+ GHz | 16 GB | 2x 300 GB or 2x 500 GB with pre-aligned disk partitions. | 1 (1GbE+) |
...
| Note | Data exported from Publisher Node cannot be imported on the Subscriber Node. Data Export must be performed simultaneously on both Publisher and Subscriber during a maintenance window; separate exports
                                             will result in message loss for any voicemails handled by the Subscriber alone. |
|---|---|
...
| Note | Import data on new virtual machines, using Import option available in Installation wizard. For more information see Installing the Publisher Server section. |
|---|---|

| Caution | In case of a cluster network, modifications on cluster page settings on the publisher server is not required. Security password must be the same as from the previous publisher server . |
|---|---|

| Note | Import data on new virtual machines, using Import option available in Installation wizard. For more information see Installing the Publisher Server section. |
|---|---|

| Caution | It is adviced to fresh install subscriber node directly to the publisher . Do not delete the old subscriber entry from publisher
                                                                  or reboot the publisher . Security password must be the same as from the previous publisher server . |
|---|---|

| Note | Make sure to install ciscocm.cuc_preUpgradeCheck-005.cop.sha512 COP on both nodes of the cluster and verify that the cluster is ready for migration before Data Export. Download the COP
                                                files from https://software.cisco.com/download/home/286313379/type/286319537/release/COP-Files . Make sure that the server on which Import is to be performed is created using recommended OVA. For more information, see https://www.cisco.com/c/dam/en/us/td/docs/voice_ip_comm/uc_system/virtualization/virtualization-cisco-unity-connection.html . Once migrated, if for any reason you decide to roll back Data Export COP file on Unity Connection 15 Release, then install ciscocm.cuc_DataExport_rollback_v1.1.k4.cop.sha512 COP file. |
|---|---|
...
| Step 1 | Ensure that your servers are listed as supported hardware and
                                          					 ...
                                          					 link http://www.cisco.com/c/dam/en/us/td/docs/voice_ip_comm/uc_system/virtualization/virtualization-cisco-unity-connection.html . |
| Step 2 | Create the virtual machine using the correct OVA template. | Get base OVA. Example OVA file names: NFVIS and VMware vSphere ESXi - CUC_15_all_esxi_vmv17_or_nfvis_v1.0.sha512.ova Nutanix AHV - CUC_15_large_2x500_ahv_v1.0.sha512.ova For more information, see the Creating a Virtual Machine section. Note You must manually set the Latency Sensitivity of the virtual machine to "High". For more details, refer to the OVA README
                                                      file available at https://software.cisco.com/download/home/283062758/type/282074348/release/OVA-15 | Note | You must manually set the Latency Sensitivity of the virtual machine to "High". For more details, refer to the OVA README
                                                      file available at https://software.cisco.com/download/home/283062758/type/282074348/release/OVA-15 |
| Note | You must manually set the Latency Sensitivity of the virtual machine to "High". For more details, refer to the OVA README
                                                      file available at https://software.cisco.com/download/home/283062758/type/282074348/release/OVA-15 |
...
| Step 1 | To deploy the OVA template in a supported Cisco NFVIS Version. Upload the OVA as a profile and the ISO as an image together. This process creates a separate profile for each VM configuration
                                                         and a single image for the bootable ISO. For more information, refer to Cisco Enterprise Network Function Virtualization Infrastructure Software Configuration Guide . An example OVA file name is: CUC_15_all_esxi_vmv17_or_nfvis_v1.0.sha512.ova |
|---|---|
| Step 2 | Deploy a VM Ensure that an OVA and ISO image is already uploaded. Navigate to Configuration > Deploy . Select OTHER as the option when creating virtual machines for your application type. Connect to the virtual network and assign the Image/Profile (depending on the deployment size that you prefer) and click Deploy . Open the VM console via Configuration > Virtual Machine > Manage . Use the console to complete the ISO installation process. |
| Step 3 | Follow on-screen instructions to create the virtual machine. |

| Step 1 | Create virtual machines in Nutanix AHV and select an OVA per each VM configuration from the Base OVA and deploy using the
                                             OVA. An example OVA file name for a large size deployment is: CUC_15_large_2x500_ahv_v1.0.sha512.ova. |
|---|---|
| Step 2 | Download the Unity Connection .ova files based on the required configuration from the Software Download page. Upload the Unity Connection .ova files (downloaded) onto the Nutanix cluster using Prism Central. Download the desired Unity Connection bootable .iso image version from the Software Download page and upload the image to
                                                the Nutanix cluster. Creating Unity Connection Publisher and Subscriber VMs using the .ova files. Update Unity Connection VMs to mount CD-ROM with the Unity Connection image. |
| Step 3 | Follow on-screen instructions to create the virtual machine. |
...
| Step 4 | In the Platform Installation Wizard window, select the
                                          			 applicable option: If you want to perform a standard installation, select Proceed , and continue with this procedure. If you want to Import data from SFTP server during fresh install, select Import and continue. If you want to perform an unattended installation, select Skip . Connect the answer file image on a CDROM  drive 2. Create the config files with .iso extension and attach the .iso to this
                                                   drive and select Continue . The installation wizard reads the configuration information during the installation process and then follow the steps mentioned
                                                   in the Post-Installation Tasks section. |
...
| Step 5 | (Optional) In
                                          			 the MAC Address field, enter the MAC address of the second server. |
| Step 6 | In the Description field, enter a description for the second server and select Save . Note Above mentioned steps are applicable to: Unity Connection 15 release. If Import option is not selected, while installation for Unity Connection 15 release. If Import option is selected only for publisher node, while installing Unity Connection 15  release.This is the case network migration
                                                               in which data is exported and imported on publisher node only and subscriber node is freshly installed. | Note | Above mentioned steps are applicable to: Unity Connection 15 release. If Import option is not selected, while installation for Unity Connection 15 release. If Import option is selected only for publisher node, while installing Unity Connection 15  release.This is the case network migration
                                                               in which data is exported and imported on publisher node only and subscriber node is freshly installed. |
| Note | Above mentioned steps are applicable to: Unity Connection 15 release. If Import option is not selected, while installation for Unity Connection 15 release. If Import option is selected only for publisher node, while installing Unity Connection 15  release.This is the case network migration
                                                               in which data is exported and imported on publisher node only and subscriber node is freshly installed. |
...
|---|---|
| Step 2 | Select the CDROM drive 1  > Connect to ISO image on local disk option from the toolbar and select CDROM drive 1  > Connect
                                                to ISO image on a datastore, navigate to the data store to select the ISO image, and click OK. The ISO image is attached and
                                                the installation starts. |
| Step 3 | For Touchless Installation Generate ISO File from the answer file. |
| Step 4 | Select the CDROM drive 2  > Connect to ISO image generated  from Step 3 on local disk option from the toolbar and select CDROM drive 2  > Connect to ISO image on a datastore, navigate to the data
                                                store to select the ISO image, and click OK. The ISO image is attached and the installation starts. |
| Step 5 | (Optional) If you want to test the media before the installation, click OK in the Disc Found message box, or select Skip to
                                                skip testing the media before the installation. The installation proceeds without any manual intervention. The publisher is
                                                installed and the subscribers is added to the publisher. |
...
| Step 1 | Log in to the Cisco Unity Connection Answer File Generator application at https://www.cisco.com/c/en/us/applicat/content/cuc-afg/index.html |
|---|---|
...
| Step 12 | Repeat this procedure to generate answer files for Cisco Unity Connection. |
...
| Step 3 | Choose the required image from the drop-down list. Images are populated based on the VM type selected. For example, cuc-15.0.1. |
...
| Step 1 | Download the desired version of the CUC bootable .iso image from the Cisco.com to your local machine. |
|---|---|
...
| Step 8 | Check the status of the image upload on the Tasks page to confirm the successful upload of the .iso image to the Nutanix cluster. For example, a URL is the following: nfs://<Host IP>/<path>/Bootable_UCSInstall_CUC_15.0.1.xxxxx-xx.sha512.iso |

| Note | This procedure is only supported on VMware vSphere ESXi. |
|---|---|
