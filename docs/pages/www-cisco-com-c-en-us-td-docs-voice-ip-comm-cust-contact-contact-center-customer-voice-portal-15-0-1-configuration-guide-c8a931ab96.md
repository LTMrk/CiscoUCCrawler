...
-Djavax.net.ssl.trustStoreType=JCEKS Note To ensure that the device status appears correctly in OAMP, renew/upload the certificates on the Reporting Server and its
                                                            associated Call Server during the same maintenance window. | Note | The following registry entries are added by default. However, if a custom Java application uses a different keystore, ensure
                                                            that the required trust certificates are imported into the appropriate keystore | Note | To ensure that the device status appears correctly in OAMP, renew/upload the certificates on the Reporting Server and its
                                                            associated Call Server during the same maintenance window. |
| Note | The following registry entries are added by default. However, if a custom Java application uses a different keystore, ensure
                                                            that the required trust certificates are imported into the appropriate keystore |
| Note | To ensure that the device status appears correctly in OAMP, renew/upload the certificates on the Reporting Server and its
...
| Step 9 | Run the regedit command. Navigate to HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Apache Software Foundation\Procrun 2.0\OPSConsoleServer\Parameters\Java . Append the following to the file sand save it: Note The following registry entries are added by default. However, if a custom Java application uses a different keystore, ensure
                                                               that the required trust certificates are imported into the appropriate keystore Djdk.rmi.ssl.client.enableEndpointIdentification=false
Djavax.net.ssl.trustStore=C:\Cisco\CVP\conf\security\.keystore
Djavax.net.ssl.trustStorePassword=<keystore_password>
