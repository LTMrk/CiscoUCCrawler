...
retrieved_at: 2026-08-21T16:30:41.256923+00:00
---
...
This command shows the proxy host of the speechserver.
...
## show vvb httpsProxy host

This command shows the proxy host on the VVB for both Cisco Speech Server and Cisco Media Gateway.
...
```
admin:show vvb httpsProxy host
abc.cisco.com
Command successful.
```
...
This command shows the proxy port of the speechserver.
...
## show vvb httpsProxy port

This command shows the proxy port on the VVB for both Cisco Speech Server and Cisco Media Gateway.
...
```
admin:show vvb httpsProxy port
80
Command successful.
```

## show vvb httpsProxy nonProxyHosts

This command shows the non proxy host on the VVB for both Cisco Speech Server and Cisco Media Gateway.
...
```
admin:show vvb httpsProxy nonProxyHosts
abc.cisco.com
Command successful.
```
...
This command shows the strict_hostname_verifier for VVB. Hostname verifier is used while establishing the TLS connection.
...
The default value of strict_hostname_verifier is true . If you are upgrading, it is enabled by default.
...
## show speechserver audioPacketSizeInBytes

This command displays the size of the audio packet in bytes which is transmitted to CCAI.
...
```
admin:show speechserver audioPacketSizeInBytes
1000
Command successful.
```

## show speechserver logJVMStats

This command displays the whether the JVM resource logs are enabled or disabled (returns true/false).
...
```
admin:show speechserver logJVMStats
true
Command successful.
```

## show vvb http pna-status

This command shows the status of the Private Network Access (PNA) filter which handles Chrome PNA preflight request.
...
```
admin:show vvb http pna-status
true
Command successful.
```

## show vvb call blindTransferContinueOnerror

This command displays the status of whether to continue the call flow on error during Blind Transfer:

Cisco VVB has been enhanced to provide specific exit state information to the VXML Server during a Blind Transfer, enabling
                                       the VXML application to dynamically adjust the call flows.
...
```
admin:show vvb call blindTransferContinueOnerror
false
 Command successful.
```
...
```
admin:set vvb trace defaults Engine Default traces restored successfully for the module.
```
...
This command sets the proxy host for the speechserver. It also asks for credentials, if required.
...
You must stop and start speechserver for the values to be reflected.

Syntax for stopping speechserver : utils service stop Cisco speechserver

Syntax for starting speechserver : utils service start Cisco speechserver

### set speechserver httpsProxy nonProxyHosts

This command sets the nonProxyHosts for the speechserver. The traffic will not go via proxy to these hosts.
...
This command sets the proxy port for the speechserver.
...
### set vvb httpsProxy Host

This command sets the proxy host on the VVB for both Cisco Speech Server and Cisco Media Gateway. It also asks for credentials,
                                 if required.

Command Syntax :

```
set vvb httpsProxy host <hostname/ip>
Does proxy require Crendentials? [Y/N] y
Enter UserName: username
Enter Password: ****
```
...
```
admin:set vvb httpsProxy host abc.com
Does proxy require Crendentials? [Y/N] y
Enter UserName: username
Enter Password: ****
Command successful.
...
You must stop and start Cisco Speech Server or Cisco Media Gateway by using a GUI or by entering commands in the CLI, depending
                                             on the configured mode, for the values to be reflected.

To stop and start Cisco Speech Server, use the following:

Syntax for stopping Cisco Speech Server : utils service stop Cisco Speech Server

Syntax for starting Cisco Speech Server : utils service start Cisco Speech Server

To stop and start Media Gateway, use the following

Syntax for stopping Cisco Media Gateway : utils service stop Cisco Media Gateway

Syntax for starting Cisco Media Gateway : utils service start Cisco Media Gateway

To apply proxy changes, restart both Cisco Speech Server and Cisco Media Gateway.

### set vvb httpsProxy port

This command sets the proxy port on the VVB for both Cisco Speech Server and Cisco Media Gateway.
...
```
admin:set vvb httpsProxy port 80
 Command successful.
```
...
To stop and start Cisco Media Gateway, use the following
...
### set vvb httpsProxy nonProxyHosts

This command sets the nonProxyHosts on the VVB for both Cisco Speech Server and Cisco Media Gateway. The traffic will not
                                 go via proxy to these hosts.
...
```
admin:set vvb httpsProxy nonProxyHosts <list of nonProxyHosts separated by commas>
 Command successful.
```
...
### set vvb https strict_hostname_verifier

This command sets the https strict_hostname_verifier for VVB. Hostname verifier is used while establishing the TLS connection.
...
```
admin:set vvb https strict_hostname_verifier true
 Command successful.
```
...
### set speechserver audioPacketSizeInBytes

This command sets the audio packets size to the given bytes (1000/4000).
...
```
admin:set speechserver audioPacketSizeInBytes 4000
 Command successful.
```

Restart the Speech Server for the set value to get reflected.

### set speechserver audioPacketSizeInBytes default

This command sets the audio packets size to the default size (1000 bytes).
...
```
admin:set speechserver audioPacketSizeInBytes default
 Command successful.
```

Restart the Speech Server for the set value to get reflected.

### set speechserver logJVMStats

This command enables/disables the JVM resource logs.
...
```
admin:set speechserver logJVMStats true
 Command successful.
```

The default value of logJVMStats is set as true.

Restart the Speech Server for the set value to get reflected.

### set vvb http pna

This command enables or disables the Private Network Access (PNA) filter which handles Chrome PNA preflight request.
...
```
admin:set vvb http pna-enable
 Command successful.
```

The default value of the PNA filter is set to true.

### set vvb call blindTransferContinueOnerror

This command allows you to enable or disable the call flow to continue on error during Blind Transfer:
...
set vvb call blindTransferContinueOnerror enable

Example

```
admin:set vvb call blindTransferContinueOnerror enable
Command successful
```
...
set vvb call blindTransferContinueOnerror disable

Example

```
admin:set vvb call blindTransferContinueOnerror disable
Command successful
```

For the changes to take effect, ensure that you restart the engine by executing the command: utils service restart Engine .

### utils service restart Engine

Use the platform utils command: utils service restart to restart the service engine. Use the utils service restart Engine command to restart the engine during blind transfer error.
...
```
admin:utils service restart Engine
Command successful
```
...
This command unsets the proxy host for the speechserver.
...
Syntax for stopping Speech Server : utils service stop Cisco speechserver

Syntax for starting Speech Server : utils service start Cisco speechserver

### unset speechserver httpsProxy port

This command unsets the proxy port for the speechserver.
...
This command unsets the nonProxy host for the speechserver.
...
### unset vvb httpsProxy host

This command unsets the proxy host on the VVB for both Cisco Speech Server and Cisco Media Gateway.
...
```
admin:unset vvb httpsProxy host
Command successful.
```
...
### unset vvb httpsProxy port

This command unsets the proxy port on the VVB for both Cisco Speech Server and Cisco Media Gateway.
...
```
admin:unset vvb httpsProxy port
Command successful.
```
...
### unset vvb httpsProxy nonProxyHosts

This command unsets the nonProxy host on the VVB for both Cisco Speech Server and Cisco Media Gateway.
...
```
admin:unset vvb httpsProxy nonProxyHosts
Command successful.
```
...
## Media Service Log Commands

### View Media Service Log Files

#### View All Media Service Log Files

Run the following command  to view the media service log files.

file view activelog mediaservice/logs/MediaService/*.log

#### View a Specific Media Service Log File

Run the following command to view a specific media service log file.

file view activelog mediaservice/logs/MediaService/Mediaservice.log

#### View Specific Number of Lines of Media Service Log Files

Run the following command to view specific number of lines of media service log files.

file view activelog mediaservice/logs/MediaService/Mediaservice.log count <number of log file lines>

### Get Media Service Log Files

This section lists the log files for the Media Service directory.

#### Get All Media Service Log Files

Run the following command to get the media service log files.

file get activelog mediaservice/logs/MediaService/*

#### Get a Specific Log File

Run the following command to get a specific media service  log file.

file get activelog mediaservice/logs/MediaService/Mediaservice.log

#### Get Logs with Date Filter

Run the following command to get log files for a specific date.

file get activelog mediaservice/logs/MediaService/Mediaservice.<yyyy-mm-dd>*.log

### List Media Service Log Files

This section lists the log files for media service directory.

#### List All Media Service Log Files

Run the following command to list the media service log files.

file list activelog mediaservice/logs/MediaService/

#### List All Media Service Log Files with Details

Run the following command to get a list of media service log files with details.

file list activelog mediaservice/logs/MediaService/ detail

### List Media Service Real-Time Tail Logs

Run the following command to get tail media log in real-time.

file tail activelog mediaservice/logs/MediaService/Mediaservice.log

| Note | The pound "#"
                                          			 sign, that is prefixed for size, indicates the entry is staled. |
|---|---|

| Note | The default value of strict_hostname_verifier is true . If you are upgrading, it is enabled by default. |
|---|---|

| Note | Cisco VVB has been enhanced to provide specific exit state information to the VXML Server during a Blind Transfer, enabling
                                       the VXML application to dynamically adjust the call flows. |
|---|---|
...
| Note | You must stop and start speechserver for the values to be reflected. Syntax for stopping speechserver : utils service stop Cisco speechserver Syntax for starting speechserver : utils service start Cisco speechserver |
|---|---|

| Note | The parameter can be a single host or mutiple hosts separated by commas. You must stop and start speechserver for the values to be reflected. Syntax for stopping speechserver : utils service stop Cisco speechserver Syntax for starting speechserver : utils service start Cisco speechserver |
|---|---|

| Note | You must stop and start speechserver for the values to be reflected. Syntax for stopping speechserver : utils service stop Cisco speechserver Syntax for starting speechserver : utils service start Cisco speechserver |
|---|---|

| Note | You must stop and start Cisco Speech Server or Cisco Media Gateway by using a GUI or by entering commands in the CLI, depending
                                             on the configured mode, for the values to be reflected. To stop and start Cisco Speech Server, use the following: Syntax for stopping Cisco Speech Server : utils service stop Cisco Speech Server Syntax for starting Cisco Speech Server : utils service start Cisco Speech Server To stop and start Media Gateway, use the following Syntax for stopping Cisco Media Gateway : utils service stop Cisco Media Gateway Syntax for starting Cisco Media Gateway : utils service start Cisco Media Gateway |
|---|---|

| Note | To apply proxy changes, restart both Cisco Speech Server and Cisco Media Gateway. |
|---|---|

| Note | You must stop and start Cisco Speech Server or Cisco Media Gateway by using a GUI or by entering commands in the CLI, depending
                                             on the configured mode, for the values to be reflected. To apply proxy changes, restart both Cisco Speech Server and Cisco Media Gateway. To stop and start Cisco Speech Server, use the following: Syntax for stopping Cisco Speech Server : utils service stop Cisco Speech Server Syntax for starting Cisco Speech Server : utils service start Cisco Speech Server To stop and start Cisco Media Gateway, use the following Syntax for stopping Cisco Media Gateway : utils service stop Cisco Media Gateway Syntax for starting Cisco Media Gateway : utils service start Cisco Media Gateway To apply proxy changes, restart both Cisco Speech Server and Cisco Media Gateway. |
|---|---|
...
| Note | Restart the Speech Server for the set value to get reflected. |
|---|---|

| Note | Restart the Speech Server for the set value to get reflected. |
|---|---|

| Note | The default value of logJVMStats is set as true. Restart the Speech Server for the set value to get reflected. |
|---|---|

| Note | The default value of the PNA filter is set to true. |
|---|---|
...
| Note | For the changes to take effect, ensure that you restart the engine by executing the command: utils service restart Engine . |
|---|---|

| Note | You must stop and start speechserver for the values to be reflected. Syntax for stopping Speech Server : utils service stop Cisco speechserver Syntax for starting Speech Server : utils service start Cisco speechserver |
|---|---|
