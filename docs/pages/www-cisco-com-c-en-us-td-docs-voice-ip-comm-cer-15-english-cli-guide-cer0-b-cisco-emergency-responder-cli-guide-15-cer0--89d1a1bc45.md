...
### license smart transport ssm-satellite (Release 15SU2 Onwards)
...
license smart transport ssm-satellite
...
Use this command to configure or update the license count for the system to operate within, when Permanent License Reservation
                                 ...
                                 The license count set by admin using the CLI will be displayed on the Emergency Responder License Management UI screen.
...
This command sets trace activities for the specified CERAdmin logs. It specifies the file count and size that will be calculated
                                 for the projected usage. This command will be executed only if the projected usage is less than 80% of the overall common
                                 partition size. It restarts the Cisco Tomcat service.

#### Command Syntax

set certrace Admin [ filecount ]  [ filesize ]
...
Number of log files (100-1000)

Default value: 100

(Optional) filesize

Size of each log file in MB (2-5)

Default value: 2 MB

#### Example

This example runs a set certrace Admin command:

```
admin:set certrace Admin 200 3

This command restarts Cisco Tomcat and delete existing logs of CERAdmin

Do you want to continue [Y/N]: y
Current common partition statistics are

Total : 49486 Mb
Used : 3326 Mb
Available : 43616 Mb

Projected common partition utilization after new log level is 8%

We recommended to revert the new log level to original values after troubleshooting the issue

Logs purged successfully for CERAdmin
Changes saved successfully for service: CERAdmin

Cisco Tomcat restarted after new configurations are updated
```
...
### set certrace EmergencyResponder

This command sets trace activities for the specified CERServer logs and its detailed logs. It specifies the file count and
                                 size that will be calculated for the projected usage. This command will be executed only if the projected usage is less than
                                 80% of the overall common partition size. It restarts the Cisco Emergency Responder service.

#### Command Syntax

set certrace EmergencyResponder [ filecount ]  [ filesize ]
...
This example runs a set certrace EmergencyResponder command:

```
admin:set certrace EmergencyResponder 200 2

This command restarts Cisco Emergency Responder and delete existing logs of CERServer & CER_Server_Detailed
...
Projected common partition utilization after new log level is 9%

We recommended to revert the new log level to original values after troubleshooting the issue

Logs purged successfully for CERServer & CER_Server_Detailed
Changes saved successfully for service: CERServer & CER_Server_Detailed

Cisco Emergency Responder restarted after new configurations are updated
```
...
### set certrace PhoneTrackingEngine

This command sets trace activities for the specified CERPhoneTracking logs and its detailed logs. It specifies the file count
                                 and size that will be calculated for the projected usage. This command will be executed only if the projected usage is less
                                 than 80% of the overall common partition size. It restarts the Cisco Phone Tracking Engine service.

#### Command Syntax

set certrace PhoneTrackingEngine [ filecount ] [ filesize ]
...
This example runs a set certrace PhoneTrackingEngine command:

```
admin:set certrace PhoneTrackingEngine 200 3

This command restarts Cisco Phone Tracking Engine and delete existing logs of CERPhoneTracker
...
Logs purged successfully for CERPhoneTracker
Changes saved successfully for service: CERPhoneTracker

Cisco Phone Tracking Engine restarted after new configurations are updated
```
...
Security Warning : This operation will regenerate
       all CER Certificates including any third party
...
====================================================================
 Note: Please verify that the new IP address is unique
       across the cluster.
       Changing the IP address will automatically restart
       system services cluster wide.
       This command should not be used during normal operating hours.
=====================================================================
...
Currently the set tls trace command is supported only for the HAProxy service. Cisco Emergency Responder does not support the HAProxy service and hence
...
```
admin:set tls trace disable
CER doesnt support HAProxy service.Blocking user input
```
...
show open files [ all ]  [ regexp reg_exp ]
...
This command shows the version of the currently running
                                 		  VMware Tools.
...
This
                                 		  command updates the currently installed VMware Tools to the latest version
                                 		  required by the ESXi host for that VM.
...
| Note | This command is executed to configure the Transport Gateway URL. This command cannot be executed on Emergency Responder using
                                          the Smart Transport mode for communication as Transport Gateway is not supported. Use the ‘license smart transport ssm-satellite’
                                          CLI command to configure the satellite URL. Smart Transport gateway is not supported for freshly deployed Emergency Responder
                                          systems. |
|---|---|
...
| Parameters | Description |
|---|---|
| name | The username for the new account. Enter a name comprised of only alphabets or characters comprised of alphanumeric characters (a-z, A-D, 0-9). Note Umlaut characters are not supported. | Note | Umlaut characters are not supported. |
...
| Parameters | Description |
|---|---|
| filecount | Number of log files (100-1000) Default value: 100 |
| (Optional) filesize | Size of each log file in MB (2-5) Default value: 2 MB |
...
| Note | From Release 14SU2 onwards, tomcat and tomcat-ecdsa certificate must be exchanged between the publisher and subscriber nodes
                                          before and after changing the ipaddress/hostname. |
|---|---|

| Parameters | Description |
|---|---|
| hostname | Represents the new network hostname of the system. Note The host name must follow the rules for ARPANET host names. It must start with an alphabetic character, end with an alphanumeric
                                                      character, and consist of alphanumeric characters and hyphens. The host name can have a maximum length of 63 characters. | Note | The host name must follow the rules for ARPANET host names. It must start with an alphabetic character, end with an alphanumeric
                                                      character, and consist of alphanumeric characters and hyphens. The host name can have a maximum length of 63 characters. |
| Note | The host name must follow the rules for ARPANET host names. It must start with an alphabetic character, end with an alphanumeric
                                                      character, and consist of alphanumeric characters and hyphens. The host name can have a maximum length of 63 characters. |
...
| Command | Result |
|---|---|
| show open files [ all ]  [ regexp reg_exp ] | This command shows open files on the system. |
