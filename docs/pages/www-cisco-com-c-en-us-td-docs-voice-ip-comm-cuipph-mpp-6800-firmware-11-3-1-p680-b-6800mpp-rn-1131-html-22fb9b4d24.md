---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cuipph-mpp-6800-firmware-11-3-1-p680-b-6800mpp-rn-1131-html-22fb9b4d24
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cuipph/MPP/6800/firmware/11-3-1/p680_b_6800mpp_rn-1131.html
retrieved_at: 2026-08-21T23:14:04.370770+00:00
---

Cisco IP Phone 6800 Series Multiplatform Phones Release Notes for Firmware Release 11.3(1)
...
Use these release notes with the following Cisco IP Phone 6800 Series Multiplatform Phones running SIP Firmware Release 11.3(1).
...
Cisco IP Phone 6800 Series Multiplatform Phones Quick Start Guide

Cisco IP Phone 6800 Series Multiplatform Phones Release Notes
...
You can enable the phones to support an overload point of 9dB for G.722 codec. The audio overload point is the signal level
                        at which the audio codec is overloaded. For a phone that supports 9dB overload point for G.722 codec, you can configure the
                        phone with the Audio_Overload_Point_9dB parameter. Locate the parameter in the phone configuration file. The default setting is No . If your phones are using ETSI standards and are required to support an overload point of 9dB for G.722 codec, set this parameter
                        to Yes . Otherwise, keep the default setting. When set to No , the audio overload point is 3.17dB across the network for consistent power for both the narrow band and wide band codecs.
...
You can control the behavior of the line key LED on your multiplatform phone with these options.
...
You select the option in the phone web page. If you configure simultaneous LED behavior for phone line keys and key expansion
                        module line keys, then changes are made in the Att Console tab. To view the key expansion module line key behavior, select Voice > Att Console . Look in the Att Console Key LED Pattern section.
...
### One-Button Call Park

Users can park an active call to a specific user with a line key that monitors the user. Users can also retrieve a parked
                        call from any phone or any extension. To unpark a call, do one of three things:
...
Use the existing Busy Lamp Field (BLF) list to configure monitored user accounts on the Broadsoft server. Enable Call Park
                        notification on the server. In the phone web page the parameter BLF List Feature Options supports a drop-down list with two options, prk and blf+sd+cp . If you select prk , the line keys monitoring the BLF list support only the call park feature. If you select blf+sd+cp , the line keys monitoring the BLF list support the blf, speed dial, and call pickup features. The default value of the BLF List Feature Options parameter is blf+sd+cp .

To configure a line key to monitor a single user's line and assign extended functions, enter scripts in the phone web page.
                        Go to Voice > Phone > Line Key <#> > Extended Function field.

If you combine prk with another function (for example: prk+blf ), only prk takes effect.
...
Your phone provides enhanced serviceability messages immediately after it boots up and has no Ethernet connection or has invalid provisioning settings. The enhancement provides valuable information that would otherwise be difficult to obtain.

If your phone supports Wi-Fi, and it can't connect to the internet, it displays the message Verify your internet settings or contact your service provider. You can use the softkey to scan for a Wi-Fi network. This message also displays anytime the phone loses connection to the
                        network.
...
### Wi-Fi Scan When Not Connected to the Network

If your phone supports Wi-Fi, it searches for a network when it boots up. It functions in one of the following ways:

Auto Scan—If the phone isn’t connected to an Ethernet cable, it will auto scan for a Wi-Fi network the first time it boots
                              up, or after a factory reset. A list of networks displays when auto scan completes. Select a network and follow the prompts
                              to connect your phone to the network.

Wi-Fi Scan softkey—The softkey displays on the phone if:

the phone isn’t connected to an Ethernet cable.

the phone isn’t connected to a Wi-Fi network.

the phone Wi-Fi type isn’t WPS (applies only to Cisco IP Phone 6861 Multiplatform Phones ).

it's not the first-time bootup out of the box or a factory reset.

The user presses Wi-Fi Scan to get a list of networks. The user selects a network and follows the prompts to connect the phone to the network.

Only the Cisco IP Phone 6861 Multiplatform Phones support Wi-Fi.
...
To view the UID information of the key expansion module, do one of the following:

On your phone, select Settings > Status > Accessories > Attendant console unit (n) .

In the phone web interface, select Info > Attendant Console Status .
...
6821: cmterm-6821.11-3-1MPP-697_REL.zip

Other phones in 6800 series: cmterm-68xx.11-3-1MPP-697_REL.zip
...
6821: https://10.73.10.223/firmware/sip6821.11-3-1MPP-697.loads

Other phones in 6800 series: https://10.73.10.223/firmware/sip68xx.11-3-1MPP-697.loads
...
6821: https://10.74.10.225/admin/upgrade?https://10.73.10.223/firmware/sip6821.11-3-1MPP-697.loads

Other phones in 6800 series: https://10.74.10.225/admin/upgrade?https://10.73.10.223/firmware/sip68xx.11-3-1MPP-697.loads
...
The following list contains the severity 1, 2, and 3 defects that are open for the Cisco IP Phone 6800 Series Multiplatform Phones that use Firmware Release 11.3(1).
...
CSCvq91660 [6861 Only] OOB Auto WIFI Scan Spinner And System Booting Spinner Don't Have The Same Position

CSCvr94049 Extra char is displayed at the end of cached DNS result in syslog

### Resolved Caveats

The following list contains the severity 1, 2, and 3 defects that are resolved for the Cisco IP Phone 6800 Series Multiplatform Phones that use Firmware Release 11.3(1).
...
CSCvq23768 Evaluation of 68xx for TCP SACK vulnerabilities

CSCvq22023 Phone warm reboot repeatedly when gets DHCP time offset in wired connection and switches to wireless

CSCvr02258 Linux Kernel userspace Parameters Buffer Overflow Vulnerability

CSCvq20186 Toast of call recording is of full lcd sized.

CSCvr01724 LCD display abnormal even though phone connected to AP successfully with IP address.

CSCvq23762 Evaluation of 6821 for TCP SACK vulnerabilities
...
|---|---|
| Cisco IP Phone 6800 Series Multiplatform Phones | BroadSoft BroadWorks 22.0 MetaSphere CFS version 9.5 Asterisk 11.0 |
...
| Step 6 | Download the corresponding file. 6821: cmterm-6821.11-3-1MPP-697_REL.zip Other phones in 6800 series: cmterm-68xx.11-3-1MPP-697_REL.zip |
