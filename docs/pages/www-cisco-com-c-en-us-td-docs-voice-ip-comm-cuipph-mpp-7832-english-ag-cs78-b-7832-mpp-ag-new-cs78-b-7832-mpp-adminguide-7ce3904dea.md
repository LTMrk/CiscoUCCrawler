---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cuipph-mpp-7832-english-ag-cs78-b-7832-mpp-ag-new-cs78-b-7832-mpp-adminguide-7ce3904dea
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cuipph/MPP/7832/english/AG/cs78_b_7832-mpp-ag_new/cs78_b_7832-mpp-adminguide_chapter_01010.html
retrieved_at: 2026-08-21T23:25:22.757719+00:00
---
...
The logo must be a .jpg or a .png file. The phone has a fixed display area. So, if the original logo size doesn't fit into
                                          the display area, you need to scale it to fit the screen. The display area size of the Cisco IP Phone 7832 is 48x48.
...
You can set the phone to display a custom logo as the background on the phone screen.
...
URL locating the (.png) file to display on the phone screen background. The image can display as the the screensaver, or at
                                             bootup depending on the settings of the Screen Saver Type , or Boot Display field.
...
| Step 3 | To display a picture or a logo, enter the path in the Picture Download URL or Logo URL field. For example: http://10.64.84.147/pictures/image04.png When you enter an incorrect URL to download the image, the phone fails to upgrade to the new image and displays the existing
                                          ...
                                          the display area, you need to scale it to fit the screen. The display area size of the Cisco IP Phone 7832 is 48x48. You can also configure this parameter in the configuration file (cfg.xml) by entering a string in this format: <Picture_Download_URL ua="na">http://10.64.84.147/pictures/bootimage1.jpg</Picture_Download_URL> <Logo_URL ua="na">http://10.64.84.147/pictures/logo_image.jpg</Logo_URL> |
...
|---|---|
| Step 2 | In the Screen section, choose one of the options for the Phone Background field: Default —Keeps the system default background. Logo —Displays a logo downloaded from a TFTP, FTP, or HTTPS server. When select this option, enter the URL for the logo image in
...
| Step 3 | Upload the logo image to a TFTP, HTTP, or HTTPS server. The logo must be a .jpg or a .png file. The phone has a fixed display area. So, if the original logo size doesn't fit into
                                          the display area, you need to scale it to fit the screen. The display area size of the Cisco IP Phone 7832 is 48x48. |
| Step 4 | In the Logo URL field, enter the path where the logo image has been uploaded. The URL must include the TFTP, HTTP, or HTTPS server name (or IP address), directory, and file name. Don't exceed 255 characters
...
| Step 5 | Click Submit All Changes . The phone reboots after you change the background image URL. |
...
| Picture Download URL | URL locating the (.png) file to display on the phone screen background. The image can display as the the screensaver, or at
                                             bootup depending on the settings of the Screen Saver Type , or Boot Display field. When you enter an incorrect URL to download a new image, the phone fails to update to the new image and displays the existing
