---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-pcce-pcce-15-0-1-installation-guide-pcce-b-150-c-c967e20d55
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/pcce/pcce_15_0_1/installation/guide/pcce_b_150_cisco_pcce_installationandupgrade_guide/reference.html
retrieved_at: 2026-08-21T12:09:51.019078+00:00
---
...
For each datastore, the following tables describe the RAID group, the ESXi Host, and the virtual machines for the Cisco UCS C240 M5SX, and Cisco UCS C240 M6SX servers.
...
When you configure the DNS server and then add the hostname/IP address entries by running the utils vvb add host-to-ip command, the DNS server takes precedence over the hostname/IP address entries for hostname-to-IP resolution.
...
For more information on security guidelines, refer the General Antivirus Guidelines section in the Security Guide for Cisco Unified Contact Center Enterprise Guide at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html .
...
Install the Unified Contact Center Release 
                                 				
                                 				 15.0(1) software on your Unified CCE virtual machines.
...
Ensure that before you initiate the primary node upgrade, the secondary node's Cisco Identity Service is stopped.

Ensure that before you start the upgrade, logged-in agents log out. If the agent is not logged out, the system will force
                                       the logout.
...
After the upgrade, the Identity Service (IdS) and Identity Provider (IdP) trust must be reestablished.
...
Cisco Virtualized Voice Browser (Cisco VVB) provides a platform for interpreting VXML documents. Cisco VVB serves as an alternative
                              to the use of IOS Voice Browsers . When an incoming call arrives at the contact center, Cisco VVB allocates a VXML port that
                              represents the VoIP endpoint. Cisco VVB sends HTTP requests to the Unified CVP VXML server. The Unified CVP VXML server runs
                              the request and sends back a dynamically generated VXML document.
...
In the Cisco Unified Contact Center Enterprise Enhanced Database Migration Tool that appears, click Next .
...
You cannot upgrade CVP Reporting Server from 12.6 to 15.0
                                 because the version of IBM Informix database server has changed. You need to uninstall CVP Reporting Server 12.6 and install
                                 CVP Reporting Server 15.0
                                 . For more details, see the Upgrade Unified CVP Reporting Server section in the Installation and Upgrade Guide for Cisco Unified Customer Voice Portal at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-customer-voice-portal/products-installation-guides-list.html .
...
If you upgrade Unified Communications Manager (Unified CM) in the contact center, also upgrade the JTAPI client that resides
                                 on the PG. To upgrade the JTAPI client, uninstall the old version of the client, restart the server, and reinstall a new version.
                                 You install the JTAPI client using the Unified Communications Manager Administration application.
...
If the TempDB performance enhancement was done in previous versions, then do not repeat the same procedure upon upgrade.
...
See the Serviceability Guide for Cisco Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-installation-and-configuration-guides-list.html for information about configuring SNMP for Unified CCE.
...
| Note | When you configure the DNS server and then add the hostname/IP address entries by running the utils vvb add host-to-ip command, the DNS server takes precedence over the hostname/IP address entries for hostname-to-IP resolution. |
|---|---|
...
| Step 1 | To install or upgrade VOS for Cisco Finesse, Cisco Unified Intelligence Center, and Cisco Unified Communications Manager: Ensure that your virtual machine is powered on. Right-click the VM in the virtual machine menu. Select Guest > Install / Upgrade VMware tools Choose the automatic tools update and press OK . The process takes a few minutes. When the process is complete, the tools are listed as Running (Current) on the VM's Summary
                                             tab in vSphere. |
|---|---|
| Step 2 | To install or upgrade VMs with Windows guest operating system: Ensure that your Windows virtual machine is powered on. Right click the VM in the virtual machine menu. Select Guest > Install / Upgrade VMware tools . Click OK on the popup window. Log in to the VM as a user with administrative privileges. Run VMware tools from the DVD drive. The installation wizard starts. Follow the prompts in the wizard to complete the VMware Tools installation. Choose the Typical installation option. When the VMware Tools installation has finished, restart the virtual machine for the changes to take effect. When the process is complete, the tools are listed as Running (Current) on the VM's Summary tab in vSphere. |
...
|---|---|
| Step 2 | In the Cisco Unified Contact Center Enterprise Enhanced Database Migration Tool that appears, click Next . |
...
| Note | If the TempDB performance enhancement was done in previous versions, then do not repeat the same procedure upon upgrade. |
|---|---|
