...
Cisco Emergency Responder Release 12.5(1)SU6 supports the following new feature:
...
Remote or Off-premises users which are using Cisco Jabber or Webex App client for calling can update their emergency location
                  ...
                  supports for routing emergency calls from such users to the National E911 Service Provider directly.
...
The information in the following sections discuss Hardware and Software requirements for Cisco Emergency Responder 12.5(1)SU6.
...
ptstacktrace.log file is not rotating
...
CER Receiving 500 Internal Server Error from CUCM after sending an SQL with empty PKID
...
Remove SLM Component in Disaster and Recovery

CSCwa59812

Configuring privacy transport setting on licensing UI does not require SLM service restart

CSCwa77818

Branding file upload is failing in CER

CSCwa18371

Unable to add cucm in cer

CSCvz80450

License smart reservation return failed in CER after upgrade
...
| 11.5(1)— 11.5(4) SU(11) | 12.5.x | Refresh upgrade; no COP file required. |
...
| Identifier | Headline |
|---|---|
| CSCvz87962 | ptstacktrace.log file is not rotating |
...
| CSCvz13019 | CER Receiving 500 Internal Server Error from CUCM after sending an SQL with empty PKID |
| CSCvx38439 | CER sends randomly picked line to the Intrado/Redsky when the Offpremise Phone having multiple lines |
| CSCwa49483 | Remove SLM Component in Disaster and Recovery |
| CSCwa59812 | Configuring privacy transport setting on licensing UI does not require SLM service restart |
