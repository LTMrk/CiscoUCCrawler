---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-15-0-1-design-guid-e04c79d5bb
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_15_0_1/design/guide/ucce_b_ucce_soldg-for-unified-cce-1501/rcct_m_high-availability-and-network-design_15_0.html
retrieved_at: 2026-08-16T19:50:17.906397+00:00
---
...
Ethernet interfaces must be 10 GB and connected to Gigabit Ethernet switches.
...
Ethernet interfaces must be at least 10 GB speed and connected to Gigabit Ethernet switches.
...
In some areas, the PSTN does not provide multiple SIP trunks to a single site. In that case, you can connect the SIP trunk
                              to a CCCSP . Then, you could connect multiple CUBEs to the CCCSP to provide some redundancy.
...
Although Unified CCMP allow sign in to the portal, access to the reporting page fails.
...
## Ingress, Egress, and Cisco VVB High Availability Considerations
...
In some areas, the PSTN does not provide multiple SIP trunks to a single site. In that case, you can connect the SIP trunk
                           to a CCCSP . Following which you could connect multiple CUBEs to the CCCSP to provide some redundancy.
...
### High Availability for Cisco VVBs

The Cisco VVB parses and renders VXML documents from the Unified CVP VXML Servers or an external VXML source. Rendering a VXML document
...
You cannot have a load balancer on the path between the Cisco VVB and the Unified CVP Call Server.

In topologies that separate the ingress gateway from the Unified CVP Call Server, colocate the Cisco VVB at the Call Server site. This arrangement keeps the media stream from using bandwidth across the WAN.

If the Cisco VVB fails, calls are affected as follows:

Calls in progress —The
                                    			 ingress gateway's survivability features route calls in progress to an
                                    			 alternate location by default.

Incoming calls —Incoming calls find an alternate Cisco VVB .
...
CCCSP servers that are used with CVP can select voice browsers and associate them with specific dialed numbers. When a call comes
                                 into the network, the VG queries the Contact Center SIP Proxy to determine where to send the call based on the dialed number.
...
The Voice Browser parses and renders VXML documents obtained from one or several sources. If the Cisco VVB fails, the following happens:
...
Incoming calls — Contact Center SIP Proxy directs incoming calls to an alternate Unified CVP Call Server. If no Call Servers are available, the call is default-routed
                                             to an alternate location by survivability.

#### CVP Media Server

Store the audio files locally in flash memory on the Cisco VVB or on an HTTP or TFTP file server. Audio files stored locally are highly available. However, HTTP or TFTP file servers provide
...
You can locate the Media Server across a WAN from the Cisco VVB . If the WAN connection fails, the gateway continues to use prompts from the gateway cache until the requested prompt expires.
...
#### Custom Remote Code Server

Multiple instances of custom remote code servers can be deployed and load balancing of remote servers can be done using the
                                 VXML servers via internal load balancing, or external load balancers, or via a proxy. Individual RPC's can be run in the micro
                                 service level or the whole session can be run on the same server in a round robin fashion using the Call Studio application
...
#### Cisco Contact Center SIP Proxy Support

CCCSP is our implementation of a SIP Proxy Server. CCCSP is a dedicated SIP Proxy Server that runs on a virtual machine.

#### CCCSP Deployment Options

These sections describe your options for deploying CCCSP in your contact center enterprise solution.
...
You can configure TDM Gateways with SRV or with Dial Peer Preferences to use the primary and secondary CCCSP proxies.

CCCSP is set with Server Groups to find the primary and back up Unified CVP, Unified CM, and Voice Browsers.

Unified CVP is set up with a Server Group to use the primary and secondary CCCSP proxies.

Unified CM is set up with a Route Group with multiple SIP Trunks to use the primary and secondary CCCSP proxies.
...
Due to platform validation restrictions on CCCSP , the ISR is dedicated to the proxy blade function. The ISR is not collocated as a Voice Browser nor as a TDM Gateway.
...
#### CCCSP Design for High Availability

The following points affect high availability for Cisco Contact Center SIP Proxy :
...
When the RecordRoute header is not populated, the signaling bypasses CCCSP once the inbound call reaches the Unified CVP Call Server. From that point in the routing, the signaling runs directly from
                                       the originating device to the CVP Call Server.

Upstream Element Routing with SIP Heartbeats — CCCSP treats any response to an INVITE or OPTIONS as a good response. So, CCCSP does not mark an element as down when it receives a response. If the response is configured in the failover response code
                                       list for the server group, then CCCSP fails over to the next element in the group. Otherwise, CCCSP sends the response downstream as the final response.
...
When agents sign out, all their reporting statistics stop. When the agents next signin,
                                 ...
                                 to sign out agents. For more information, see the Reporting Concepts for Cisco Unified Contact Center Enterprise at http://www.cisco.com/c/en/us/support/customer-collaboration/unified-contact-center-enterprise/products-user-guide-list.html .
...
In some deployments, the Administration and Data Server also hosts an interface for the Unified CCMP . In those deployments, when the Administration and Data Server is down, any configuration changes that are made through either
...
For more information on bandwidth measurements, see Finesse Bandwidth Calculator for Unified Contact Center Enterprise at https://www.cisco.com/c/en/us/support/customer-collaboration/finesse/products-technical-reference-list.html .
...
Cisco Unified Intelligence Center supports data source failover for:

Executing the reports.

Loading the value list entries to filter and control the data displayed in reports.

Authenticating the agent's login to Finesse Desktop.

If the data source connection fails with the following error codes for any of the above mentioned tasks, the task is then
                           attempted to run on the failover data source.
...
The data source failover time depends on the type of error encountered in the network. For example, in case of network failure,
                                       the failover might take more time as the network failures are detected after the timeouts occur.
...
Cisco SocialMiner (CCP) does not support high availability. You do not deploy redundant SocialMiner servers. SocialMiner does not integrate directly with the other components in a Contact Center Enterprise solution.
...
## Cisco Contact Center SIP Proxy High Availability Considerations

With RecordRoute disabled, CCCSP can handle the failover of active calls. In an active call during failover, the backup CCCSP server handles new transactions.
...
Unified CCE solution supports redundant ASR/TTS servers. In a basic configuration, the Cisco VVB first passes all incoming requests to the primary ASR/TTS server. If the primary server is unreachable, the gateway then
...
Cisco VVB does not use active and backup ASR/TTS servers for
                           		redundancy. VVB has a built-in load-balancing mechanism that uses a round-robin
                           		technique to select from a list of servers.

If the selected speech server rejects the MRCP session setup
                           		request, then VVB tries to setup the MRCP session one more time with another
                           		server.
...
To improve high availability in the Cisco Outbound Option, you can also use a redundant CCCSP pair to connect to multiple voice gateways. The redundant gateways ensure that the Dialers have enough trunks available to
...
The active SIP Dialer fails over if it loses connection to the CTI Server, Agent PG, or SIP server. The SIP server can be
                              a voice gateway or CCCSP . Connect each dialer in a redundant pair to a different SIP server.
...
The CCCSP server provides weighted load balancing and redundancy in a multiple-gateway deployment by configuring each gateway as part
                              of the Server group configuration. If a gateway is overloaded or loses its WAN link to the PSTN network, CCCSP can resend an outbound call to the next available gateway.
...
The Campaign Manager and SIP Dialer already include warm-standby functionality. Because of this, do not use the Hot Swappable
                              Router Protocol (HSRP) feature for CCCSP servers that are dedicated for Outbound Option.
...
| Note | You can locate the Media Server across a WAN from the Cisco VVB . If the WAN connection fails, the gateway continues to use prompts from the gateway cache until the requested prompt expires.
                                             The gateway then attempts to reacquire the media, and the call fails if survivability is not enabled. If survivability is
                                             enabled, the calls are default-routed. |
|---|---|
...
| Note | The data source failover time depends on the type of error encountered in the network. For example, in case of network failure,
                                       the failover might take more time as the network failures are detected after the timeouts occur. |
|---|---|
