...
A private message can be forwarded and can be saved locally as a
			 ...
			 the Message Security Options for IMAP Client Acess to learn
			 how to prohibit users from playing and forwarding private messages and from
			 saving private messages as WAV files.)
...
Secure messages are stored only on the Unity Connection server,
			 ...
			 automatically disabled on the Options menu on the Media Master in Cisco Unity
			 Connection ViewMail for Microsoft Outlook (version 8.0), and Cisco Unity
			 Connection ViewMail for IBM Lotus Notes.
...
Cisco Unity Connection ViewMail for Microsoft Outlook (version
				  8.0)
