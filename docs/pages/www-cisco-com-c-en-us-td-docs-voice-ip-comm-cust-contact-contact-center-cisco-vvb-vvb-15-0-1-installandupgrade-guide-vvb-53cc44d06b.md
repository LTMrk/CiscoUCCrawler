...
In the Reverse Lookup Zones , navigate to your deployment's domain name.
...
Add the host IP address, the FQDN, and the hostname for VOS components .
...
For Virtual Agent Voice (VAV) feature, ensure that Cloud Connect is installed and registered with VVB and InService. For more
...
Select the mode, and then click Next .

You can choose to activate any one of the following modes:

Cisco Virtualized Voice Browser (VVB) Service Only

Media Gateway (MGW) Service Only

Mixed Mode (Cisco Virtualized Voice Browser (VVB) & Media Gateway (MGW)

By default, Cisco Virtualized Voice Browser (VVB) is selected.

The new Cisco VVB modes require separate OVAs and have the following VM requirements:

Cisco VVB service only : Small or medium profile

Media Gateway service only : Medium profile

Mixed mode (Cisco VVB and Media Gateway) : Medium profile with increased vRAM from 10 GB to 16 GB

For more details, see the Virtualization page for Cisco Virtualized Voice Browser at https://www.cisco.com/c/dam/en/us/td/docs/voice_ip_comm/uc_system/virtualization/virtualization-cisco-virtualized-voice-browser.html
...
Replace < servername > with the hostname or FQDN (Fully Qualified Domain Name) of the required Cisco VVB server.

A Security Alert dialog box is displayed.

Ensure to enter the complete URL to access the VVB Appadmin page.
...
| Step 5 | In the Reverse Lookup Zones , navigate to your deployment's domain name. |
| Step 6 | Select new ptr record . |
| Step 7 | Add the host IP address, the FQDN, and the hostname for VOS components . |
...
|---|---|
| Step 2 | Select the mode, and then click Next . You can choose to activate any one of the following modes: Cisco Virtualized Voice Browser (VVB) Service Only Media Gateway (MGW) Service Only Mixed Mode (Cisco Virtualized Voice Browser (VVB) & Media Gateway (MGW) By default, Cisco Virtualized Voice Browser (VVB) is selected. Note The new Cisco VVB modes require separate OVAs and have the following VM requirements: Cisco VVB service only : Small or medium profile Media Gateway service only : Medium profile Mixed mode (Cisco VVB and Media Gateway) : Medium profile with increased vRAM from 10 GB to 16 GB For more details, see the Virtualization page for Cisco Virtualized Voice Browser at https://www.cisco.com/c/dam/en/us/td/docs/voice_ip_comm/uc_system/virtualization/virtualization-cisco-virtualized-voice-browser.html | Note | The new Cisco VVB modes require separate OVAs and have the following VM requirements: Cisco VVB service only : Small or medium profile Media Gateway service only : Medium profile Mixed mode (Cisco VVB and Media Gateway) : Medium profile with increased vRAM from 10 GB to 16 GB For more details, see the Virtualization page for Cisco Virtualized Voice Browser at https://www.cisco.com/c/dam/en/us/td/docs/voice_ip_comm/uc_system/virtualization/virtualization-cisco-virtualized-voice-browser.html |
| Note | The new Cisco VVB modes require separate OVAs and have the following VM requirements: Cisco VVB service only : Small or medium profile Media Gateway service only : Medium profile Mixed mode (Cisco VVB and Media Gateway) : Medium profile with increased vRAM from 10 GB to 16 GB For more details, see the Virtualization page for Cisco Virtualized Voice Browser at https://www.cisco.com/c/dam/en/us/td/docs/voice_ip_comm/uc_system/virtualization/virtualization-cisco-virtualized-voice-browser.html |
| Step 3 | Wait for all the components to be activated, and then click Next . |
| Step 4 | Configure system parameters, and then click Next . You can view the status of the VVB setup. |
| Step 5 | Close the Web browser and log in again to use VVB. |
...
| Step 2 | Select the
                                       			 following options: Primary Node Installed
                                                					 On as Virtual Machine Product as Cisco Virtualized Voice Browser Version as 15.0.1 |
| Step 3 | Enter the
                                       			 remaining information on Answer File Generator web page and click to generate a
...
| Step 1 | Open the Cisco VVB Administration Authentication page from a web browser on any computer on your network and enter the case-sensitive
                                       URL in the following format: https:// <servername> /appadmin /main Replace < servername > with the hostname or FQDN (Fully Qualified Domain Name) of the required Cisco VVB server. A Security Alert dialog box is displayed. Note Ensure to enter the complete URL to access the VVB Appadmin page. | Note | Ensure to enter the complete URL to access the VVB Appadmin page. |
|---|---|---|---|
