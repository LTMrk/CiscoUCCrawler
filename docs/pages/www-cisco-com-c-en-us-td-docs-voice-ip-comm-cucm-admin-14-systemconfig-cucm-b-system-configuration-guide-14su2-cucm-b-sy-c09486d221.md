---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cucm-admin-14-systemconfig-cucm-b-system-configuration-guide-14su2-cucm-b-sy-c09486d221
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cucm/admin/14/systemConfig/cucm_b_system-configuration-guide-14su2/cucm_b_system-configuration-guide-14_chapter_011110.html
retrieved_at: 2026-08-16T16:33:45.668801+00:00
---
...
You don't have to restart the CAPF service after regenerating or uploading the new CAPF certificate.
...
For any root or intermediate CA certificates, it should include the following default X509 extensions:
...
This Note is applicable from Release 14 SU3 onwards and only for IPSec certificates.

For any CA-signed IPSec certificates, it should not include the following extensions:
...
This field is enabled only if the Online CA Type is Microsoft CA.
...
FIPS enabled mode is not supported with Microsoft CA.

(Supported from Release 14SU2 onwards) EST Supported CA—Use this option when CA supports inbuilt EST server mode for automatic
                                                         enrollment.

Enrollment of CAPF LSC using EST supported CA is not supported in CC mode.
...
This field is enabled only if the Online CA Type is EST Supported CA.
...
Use the Command Line Interface to log in to the publisher node.

Step 2

Run the utils capf set keep_alive CLI command.
...
| Step 4 | Enter a Description for the certificate. For example, Certificate for External LSC-Signing CA . |
| Step 5 | Click Browse , navigate to the file, and then click Open . |
| Step 6 | Click Upload . Important This Note is applicable from Release 14 SU2 onwards. Note For any root or intermediate CA certificates, it should include the following default X509 extensions: X509v3 Basic Constraints: CA:TRUE, pathlen:0 X509v3 Key Usage: Digital Signature, Certificate Sign In the certificates if these extensions are missing, there will be TLS connection failure. Important This Note is applicable from Release 14 SU3 onwards and only for IPSec certificates. Note For any CA-signed IPSec certificates, it should not include the following extensions: X509v3 Basic Constraints: CA:TRUE | Important | This Note is applicable from Release 14 SU2 onwards. | Note | For any root or intermediate CA certificates, it should include the following default X509 extensions: X509v3 Basic Constraints: CA:TRUE, pathlen:0 X509v3 Key Usage: Digital Signature, Certificate Sign In the certificates if these extensions are missing, there will be TLS connection failure. | Important | This Note is applicable from Release 14 SU3 onwards and only for IPSec certificates. | Note | For any CA-signed IPSec certificates, it should not include the following extensions: X509v3 Basic Constraints: CA:TRUE |
| Important | This Note is applicable from Release 14 SU2 onwards. |
| Note | For any root or intermediate CA certificates, it should include the following default X509 extensions: X509v3 Basic Constraints: CA:TRUE, pathlen:0 X509v3 Key Usage: Digital Signature, Certificate Sign In the certificates if these extensions are missing, there will be TLS connection failure. |
| Important | This Note is applicable from Release 14 SU3 onwards and only for IPSec certificates. |
| Note | For any CA-signed IPSec certificates, it should not include the following extensions: X509v3 Basic Constraints: CA:TRUE |
...
| Note | FIPS enabled mode is not supported with Microsoft CA. |
| Note | Enrollment of CAPF LSC using EST supported CA is not supported in CC mode. |
...
| Step 1 | Use the Command Line Interface to log in to the publisher node. |
|---|---|
| Step 2 | Run the utils capf set keep_alive CLI command. |
