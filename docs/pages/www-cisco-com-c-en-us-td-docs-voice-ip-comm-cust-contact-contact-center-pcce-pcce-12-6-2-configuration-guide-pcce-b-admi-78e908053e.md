...
Run CCEDataProtect Tool from <Install Directory>:\icm\bin\CCEDataProtectTool.exe .
...
Initialize the Packaged CCE deployment using Unified CCE
                                    				Administration.
...
Once a Packaged CCE deployment is initialized, you cannot switch to a different Packaged CCE deployment type. However, you
                                                can switch to a Unified CCE deployment type. After the switch, any existing departments and their associations are retained,
                                                allowing you to maintain associations within the same department. Any new department created thereafter will be considered
                                                Global.
...
EXTERNAL_THIRD_PARTY_GATEWAY for main site and DC_EXTERNAL_THIRD_PARTY_GATEWAY for remote site.
...
| Step 1 | Run CCEDataProtect Tool from <Install Directory>:\icm\bin\CCEDataProtectTool.exe . |
|---|---|
...
| Note | Once a Packaged CCE deployment is initialized, you cannot switch to a different Packaged CCE deployment type. However, you
                                                can switch to a Unified CCE deployment type. After the switch, any existing departments and their associations are retained,
                                                allowing you to maintain associations within the same department. Any new department created thereafter will be considered
                                                Global. |
|---|---|
...
| name | Unique identifier for the machine | Yes | Name must start with an alphabet. Maximum length is limited to 128 characters. Valid characters are a-z, A-Z, 0-9, dot (.),
                                                               underscore (_), or hyphen (-). |
| machineType | MachineType Enum name | Yes | Mandatory machines are: CCE_ROGGER (applicable for 4000 Agents deployment) CCE_ROUTER (applicable for 12000 Agents deployment) CCE_LOGGER (applicable for 12000 Agents deployment) CCE_AW CUIC_PUBLISHER CUIC_SUBSCRIBER LIVE_DATA IDS_PUBLISHER IDS_SUBSCRIBER Optional machines: CCE_PG CVP FINESSE_PRIMARY FINESSE_SECONDARY CM_PUBLISHER CM_SUBSCRIBER HDS ECE (refers to ECE Data Server VM for ECE 400 agents and Services Server VM for ECE 1500 agents) ECE_WEB_SERVER CVP_REPORTING GATEWAYS CVVB CUSP CUSTOMER_COLLABORATION_ PLATFORM THIRD_PARTY_ MULTICHANNEL MEDIA_SERVER EXTERNAL_THIRD_PARTY_GATEWAY for main site and DC_EXTERNAL_THIRD_PARTY_GATEWAY for remote site. |
...
| Step 1 | Launch Microsoft SQL Server Management Studio  on the Administration and Data Server . |
|---|---|
