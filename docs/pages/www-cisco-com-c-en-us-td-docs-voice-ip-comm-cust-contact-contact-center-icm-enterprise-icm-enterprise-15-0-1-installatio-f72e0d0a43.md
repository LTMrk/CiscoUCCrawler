...
This chapter describes the prerequisites, procedures, and post-configuration required to migrate the Cisco Unified Contact Center Enterprise (Unified CCE) and its components from VMware to Nutanix.
...
Unified CCE components and Unified CVP are supported on Nutanix with version 15.0(1).
...
For more information, see the Technology Refresh Upgrade chapter.
...
Unified CCE 4000 Agent Deployment and above : Use a 3+ node Nutanix solution (minimum 5 nodes for Replication Factor 3 (RF3)) with NCI Core licensing.
...
For more information, see the Migration from VMware to Nutanix
...
Prerequisite: Ensure that the Cloud Connect destination instance is part of the Unified CCE Administrator Console inventory. If it is not, add the instance to the inventory.
...
For more information, see the Digital Channels Integration Using Webex Connect chapter in Cisco Unified Contact Center Enterprise Features Guide
...
If the Cisco VVB hostname or IP address changes during migration,
                                    verify that the destination Cisco VVB instance is listed in the Unified CCE inventory.

If the destination Cisco VVB instance is not present, configure
                                          Cisco VVB.

For more information, see the Configure Cisco VVB on Unified CVP
                                          section in the Cisco VVB Configuration chapter of the Configuration Guide for Cisco Unified Customer
                                             Voice Portal .
...
If the Cisco VVB hostname, IP address, or certificate changes during
                                    ...
                                    to your Unified CCE deployment.
...
For more information, see the show live-data aw-access section in the CLI Commands chapter.
...
For more information, see the set live-data aw-access section in the CLI Commands chapter.
...
For more information, see the set live-data machine-services section in the CLI Commands chapter.
...
For more information, see the utils live-data cors allowed_origin add section in the CLI Commands chapter.
...
For more information, see the set live-data cuic-datasource section in the CLI Commands chapter.
...
Reconfigure Single Sign-On (SSO) in Cisco IdS For more information, see the Cisco IdS for Single Sign-On chapter, see Cisco Unified Contact Center Enterprise Features Guide .
...
For more information, see the Configure Cross Origin Resource Sharing (CORS) for Live Data section in the Installation chapter.
...
For more information on the CLI commands, see the CLI Commands chapter.
...
| Step 1 | On the Network and Policy screen, select a test network. From the Test Network drop-down list, select the subnet to use for test migration. Note The test network must be non-routable and isolated from the rest of the network to avoid IP address or MAC address conflicts. Figure 1. Test Network Selection | Note | The test network must be non-routable and isolated from the rest of the network to avoid IP address or MAC address conflicts. |
|---|---|---|---|
| Note | The test network must be non-routable and isolated from the rest of the network to avoid IP address or MAC address conflicts. |
...
| Step 5 | From the Test Actions menu, select Create Test VM . Nutanix Move creates the test VM on the target Nutanix cluster. Test VM names are suffixed with -MoveTest in the target network. Figure 2. Create Test VM |
...
|---|---|
| Step 2 | AWDB Access configuration After Live Data migration, run the following CLI command: set live-data aw-access For more information, see the set live-data aw-access section in the CLI Commands chapter. |
| Step 3 | Machine Service Configuration (4000 and above deployment models only) The Machine Services CLI stores Live Data information. After Live Data migration, run the following command: set live-data machine-services For more information, see the set live-data machine-services section in the CLI Commands chapter. |
| Step 4 | CORS Configuration Live Data CORS configuration saves the hostnames where Finesse is hosted. These values are migrated during Live Data migration.
                                                If the Finesse hostnames remain the same on both source and target systems, no reconfiguration is required. If the Finesse hostnames differ between source and target, reconfiguration is necessary. Run the following CORS command: utils live-data cors allowed_origin add For more information, see the utils live-data cors allowed_origin add section in the CLI Commands chapter. |
| Step 5 | Data source Details in Unified Intelligence Center Store Live Data source details in Unified Intelligence Center to enable live data reporting. If Live Data hostnames change,
                                                update the data source details in Unified Intelligence Center by running the following CLI command on the Live Data server: set live-data cuic-datasource For more information, see the set live-data cuic-datasource section in the CLI Commands chapter. |
