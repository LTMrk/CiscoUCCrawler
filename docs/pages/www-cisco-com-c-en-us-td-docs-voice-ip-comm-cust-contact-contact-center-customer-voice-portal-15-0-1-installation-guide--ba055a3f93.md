...
If there is a change in platform of a later release of Unified CVP, migration from the existing release to the later release
                        is required. For example, moving from Unified CVP 12.5(1) on Windows Server 2016/2019 to Unified CVP 15.0(1) is considered
                        a migration because it involves a change in operating system (Windows Server 2016 to Windows Server 2019 or Windows Server
                        2019 to Windows Server 2022), platform, or architecture of the later release.
...
(Optional) Standalone distributed diagnostics and service network (SDDSN) is no longer required. If you have SDDSN servers,
                                    decommission these servers or use them for another purpose.
...
Restart Cisco Unified CVP Operations Console and Cisco CVP WebServicesManager service.
...
Select Cisco CVP WebServicesManager service.

Click Restart .

The Cisco CVP WebServicesManager service starts in the Services window.
...
If any CVP configuration exported from any version older than 12.6(1) is imported on a 15.0(1) OAMP, you must run the command mgr-init.bat -oamp from the command prompt from CVP’s bin folder:

Stop Cisco Unified CVP Operations Console and Cisco CVP WebServicesManager.

Go to the CVP bin folder.

Execute the command mgr-init.bat -oamp from the command prompt.

Restart Cisco Unified CVP Operations Console and Cisco CVP WebServicesManager.

Log in to Operations Console using the Operations Console login credentials of the previous version.
...
Review the list of Java changes that may affect backward compatibility and make any required updates. You can locate the compatibility
                                          page at https://developers.redhat.com/products/openjdk/download .
...
## Migrating Unified CVP to New Hardware Without Software Upgrade

This section describes the steps to follow when performing a hardware change (migrating VMs) for CVP, while retaining the
                              same CVP software version. No CVP software upgrade is performed. Follow the below steps to ensure a successful migration.

### Before you begin

Ensure that the hostname and IP address of the source and destination machines are identical.

Verify that FIPS is disabled on both source and destination. Import/Export of OAMP configuration is not supported with FIPS enabled.

For secure communication, reimport certificates to all relevant VMs on the destination.
...
Export the OAMP configuration.

The exported file is password protected. The administrator password (minimum 12 characters) must match on the destination
                                                OAMP server for import.

Back up sip.properties file.

Verify smart license registration in NOAMP.

Check the Smart License page in NOAMP to verify registration status.

VXML Server

Back up custom audio files.

Copy custom audio files from %CATALINA_HOME%/webapps/CVP/audio .

Back up custom applications.

Back up third-party libraries.

Copy any .class or .jar files from the following locations:
...
%CVP_HOME%\VXMLServer\applications\APP_NAME\java\util (Replace APP_NAME with the name of your deployed voice application.)

Call Server

Back up sip.properties file.

Copy the sip.properties file for use on the destination server.

Reporting Server

Perform a backup of reporting server data. For more information, see section Database Recovery of the Database Management
                                          chapter in the Reporting Guide for Cisco Unified Customer Voice Portal at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-customer-voice-portal/products-installation-and-configuration-guides-list.html .

When the source and destination VMs are running the same version (15.0(1)), perform a backup and restore. When the source
                                                      and destination are on different CVP versions (such as migrating from 12.5(1) or 12.6(2) to 15.0(1)), a data migration is
                                                      required. For more information, see section Migrate Reporting Server of the Unified CVP Migration chapter in the Installation and Upgrade Guide for Cisco Unified Customer Voice Portal at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-customer-voice-portal/products-installation-guides-list.html .

Step 2

Configure Destination VMs

Call Server

Restore sip.properties file.

Paste the sip.properties file into C:\Cisco\CVP\conf .

OAMP Server

Import the previously exported OAMP configuration.
...
Perform a Save and Deploy of all components, SIP server group, and dialed numbers.

Register Smart License

Import the WSM certificate of the Call Server into the OAMP keystore.

Restart the opsconsole server.

Access the Smart License page in NOAMP and verify registration status.

If necessary, perform a re-registration.

VXML Server

Restore custom audio files.

Copy custom audio files from %CATALINA_HOME%/webapps/CVP/audio .

Restore custom applications.

Deploy all custom applications and third-party libraries.

Restart services.

Restart the Cisco CVP VXML Server and the VXMLServer service.

Reporting Server

Perform a restore of reporting server data. For more information, see section Database Backup of the Database Management chapter
                                          in the Reporting Guide for Cisco Unified Customer Voice Portal at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-customer-voice-portal/products-installation-and-configuration-guides-list.html .
...
| Note | If you are using an older gateway or gatekeeper hardware, the version of Cisco IOS that is required in this release may no
                                             longer support the required hardware. Hence, you need to purchase new hardware. |
|---|---|

| Note | For more information on how to upgrade an existing Unified CVP version from Windows Server 2016 or Windows Server 2019 to
                                       Windows Server 2019 or Windows Server 2022, see Upgrade Path . |
|---|---|
...
| Step 6 | Restart Cisco Unified CVP Operations Console and Cisco CVP WebServicesManager service. Click Start > All Programs > Administrative Tools > Services . Select Cisco CVP Operations Console Server. Click Restart . The CVP Operations Console Server service starts in the Services window. Select Cisco CVP WebServicesManager service. Click Restart . The Cisco CVP WebServicesManager service starts in the Services window. |
...
| Step 1 | Prepare Source VMs OAMP Server Export the OAMP configuration. The exported file is password protected. The administrator password (minimum 12 characters) must match on the destination
                                                OAMP server for import. Back up sip.properties file. For more information, see section Migrate Operations Console of the Unified CVP Migration chapter in the Installation and Upgrade Guide for Cisco Unified Customer Voice Portal at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-customer-voice-portal/products-installation-guides-list.html . Verify smart license registration in NOAMP. Check the Smart License page in NOAMP to verify registration status. VXML Server Back up custom audio files. Copy custom audio files from %CATALINA_HOME%/webapps/CVP/audio . Back up custom applications. Back up third-party libraries. Copy any .class or .jar files from the following locations: %CVP_HOME%\VXMLServer\common\classes %CVP_HOME%\VXMLServer\common\lib %CVP_HOME%\VXMLServer\applications\APP_NAME\java\application\classes %CVP_HOME%\VXMLServer\applications\APP_NAME\java\application\lib %CVP_HOME%\VXMLServer\applications\APP_NAME\java\util (Replace APP_NAME with the name of your deployed voice application.) Call Server Back up sip.properties file. Copy the sip.properties file for use on the destination server. Reporting Server Perform a backup of reporting server data. For more information, see section Database Recovery of the Database Management
                                          chapter in the Reporting Guide for Cisco Unified Customer Voice Portal at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-customer-voice-portal/products-installation-and-configuration-guides-list.html . Note When the source and destination VMs are running the same version (15.0(1)), perform a backup and restore. When the source
                                                      and destination are on different CVP versions (such as migrating from 12.5(1) or 12.6(2) to 15.0(1)), a data migration is
                                                      required. For more information, see section Migrate Reporting Server of the Unified CVP Migration chapter in the Installation and Upgrade Guide for Cisco Unified Customer Voice Portal at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-customer-voice-portal/products-installation-guides-list.html . | Note | When the source and destination VMs are running the same version (15.0(1)), perform a backup and restore. When the source
                                                      and destination are on different CVP versions (such as migrating from 12.5(1) or 12.6(2) to 15.0(1)), a data migration is
                                                      required. For more information, see section Migrate Reporting Server of the Unified CVP Migration chapter in the Installation and Upgrade Guide for Cisco Unified Customer Voice Portal at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-customer-voice-portal/products-installation-guides-list.html . |
|---|---|---|---|
| Note | When the source and destination VMs are running the same version (15.0(1)), perform a backup and restore. When the source
                                                      and destination are on different CVP versions (such as migrating from 12.5(1) or 12.6(2) to 15.0(1)), a data migration is
                                                      required. For more information, see section Migrate Reporting Server of the Unified CVP Migration chapter in the Installation and Upgrade Guide for Cisco Unified Customer Voice Portal at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-customer-voice-portal/products-installation-guides-list.html . |
| Step 2 | Configure Destination VMs Call Server Restore sip.properties file. Paste the sip.properties file into C:\Cisco\CVP\conf . OAMP Server Import the previously exported OAMP configuration. The exported file is password protected. The administrator password (minimum 12 characters) must match on the destination
                                                OAMP server for import. Save and deploy configuration. Perform a Save and Deploy of all components, SIP server group, and dialed numbers. Register Smart License Import the WSM certificate of the Call Server into the OAMP keystore. Restart the opsconsole server. Access the Smart License page in NOAMP and verify registration status. If necessary, perform a re-registration. VXML Server Restore custom audio files. Copy custom audio files from %CATALINA_HOME%/webapps/CVP/audio . Restore custom applications. Deploy all custom applications and third-party libraries. Restart services. Restart the Cisco CVP VXML Server and the VXMLServer service. Reporting Server Perform a restore of reporting server data. For more information, see section Database Backup of the Database Management chapter
                                          in the Reporting Guide for Cisco Unified Customer Voice Portal at https://www.cisco.com/c/en/us/support/customer-collaboration/unified-customer-voice-portal/products-installation-and-configuration-guides-list.html . |
