---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-expressway-admin-guide-x14-3-exwy-b-cisco-expressway-administrator-guide-x14-2bb80acefc
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/expressway/admin_guide/X14-3/exwy_b_cisco-expressway-administrator-guide-x143/exwy_m_clustering-and-peers-x143.html
retrieved_at: 2026-08-16T22:36:36.844953+00:00
---
...
You can form an Expressway cluster using an <hostname> as the cluster name instead of a Fully Qualified Domain Name (FQDN). This formation does not cause any clustering issues (no cluster-related
                                       errors). But, when creating a Certificate Signing Request (CSR) (after the cluster is created), either using the cluster name
                                       as Common Name (CN) or Subject Alternative Name (SAN), the server will fail to generate a CSR since the cluster name is not
                                       in FQDN format. Hence, make sure that you set the cluster name to FQDN format.
...
Do not proceed any further with any upgrade operations.

SSH as root , Execute /sbin/verify-skey to establish encrypted entry(ies) that cannot be decrypted.

If possible, try to discover when and why it was incorrectly written and/or is incorrectly (re)written.

Workaround

Go to the User Interface and Overwrite the existing (bad) entry with a clean encrypted new entry.

Execute /sbin/verify-skey again to confirm that it is now clean.

Users will not encounter these issues post a successful upgrade to versions X14.0 and higher versions.
...
| Note | Make sure to address "Failed to update key file" alarm before adding the node to a cluster. Cluster needs to be recreated if "Failed to update key file" alarm is raised on all the Expressway nodes of a cluster. |
|---|---|