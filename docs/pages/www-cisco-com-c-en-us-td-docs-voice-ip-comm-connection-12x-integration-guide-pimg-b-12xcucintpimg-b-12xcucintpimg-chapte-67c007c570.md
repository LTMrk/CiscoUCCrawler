---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-connection-12x-integration-guide-pimg-b-12xcucintpimg-b-12xcucintpimg-chapte-67c007c570
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/connection/12x/integration/guide/pimg/b_12xcucintpimg/b_12xcucintpimg_chapter_0111.html
retrieved_at: 2026-08-16T18:44:14.321005+00:00
---
...
Chapter: Setting Up a Mitel
	 SX-2000 Digital PIMG Integration with Cisco Unity Connection
...
Setting Up a Mitel SX-2000 Digital PIMG Integration

## Task List for
                        	 Mitel SX-2000 PIMG Phone System Integration

Before doing the following tasks to integrate Unity Connection
                           		with the Mitel SX-2000 phone system using PIMG units (media gateways), confirm
...
Program the Mitel SX-2000 phone system and extensions. See the “Programming
                                    				Mitel SX-2000 PIMG Phone System for Integration” section.

Set up the PIMG units. See the “Setting
                                    				Up the Digital Mitel PIMG Units” section.

Create the integration. See the Creating an Integration section.
...
## Programming Mitel SX-2000 PIMG Phone System for Integration
...
Do the following steps to program the Mitel
                              		  SX-2000 phone system.

On the phone system console, go to the Main
                                       			 Forms screen, and select Multiline Set Forms .
...
Enter IP gateway hunt .

Hunt Mode

Select the hunt mode that you want
                                                      						  to use. For example, select “Circular.”

In the Hunt Group Assignment table of the
                                       			 Hunt Group Assignment screen, assign the directory number for each port on the
                                       			 PIMG units connected to the phone system as follows.
...
<the prime directory number>

Enter IP gateway Port 1 .

2

<the second directory number>

Enter IP gateway Port 2 .

3

<the third directory number>

Enter IP gateway Port 3 .

4

<the next directory number>

Enter IP gateway Port 4 .

5

<the next directory number>

Enter IP gateway Port 5 .

6

<the next directory number>

Enter IP gateway Port 6 .

7

<the next directory number>

Enter IP gateway Port 7 .

8

<the next directory number>

Enter IP gateway Port 8 .
...
<the last directory number>

Enter IP gateway Port <n> .
...
### Downloading PIMG Firmware Update Files for Digital Mitel PIMG
                           	 Units
...
In the right column, select the version of
                                                   				  the firmware for digital Mitel PIMG units.
...
On a Windows workstation with a high-speed
                                          			 Internet connection that have access to the PIMG units, go to the following
                                          			 link:

### Setting Up Digital Mitel PIMG Units (Firmware Version 6.x)
...
In the Choose File dialog box, browse to the
                                          			 file MTLDNI_Cfg_Mitel.ini.

Select MTLDNI_Cfg_Mitel.ini ,
                                          			 and select Open .
...
Enter the numbers of the PIMG ports
                                                         						  handling inbound calls. For example, enter “*” for all PIMG ports, or enter
...
Select the name of the VoIP host
                                                         						  group that you created for Cisco Unity Connection in Step 28 .
...
Under Call Progress Tone - Learn, in the
                                          			 Dial String field, enter the extension that you dialed in Step 79 c. from
                                          			 the third phone.
...
Under Call Progress Tone - Learn, in the
                                          			 Dial String field, enter the extension that you dialed in Step 82 a.
...
Repeat Step 2 through Step 91 on all remaining PIMG
                                          			 units.

## Configuring Unity
                        	 Connection Integration with Mitel SX-2000 Phone System

After ensuring that the Mitel
                           		SX-2000 phone system, the PIMG units, and Unity Connection are ready for the
...
| Step 1 | On the phone system console, go to the Main
                                       			 Forms screen, and select Multiline Set Forms . |
|---|---|
| Step 2 | On the Multiline Set Forms screen, select Multiline Set
                                          				Assignment . |
| Step 3 | On the Multiline Set Assignment screen,
                                       			 select DNI Circuit Assignment . |
| Step 4 | On the DNI Circuit Assignment screen, enter
                                       			 the following options. Table 1. DNI Circuit Assignment Options Field Setting Cabinet Enter 2 . Shelf Enter 1 . Slot Enter 7 . Circuit Enter 4 . Card Type Select DNI Line . Device Type: Channel #1 Select Superset 430 . | Field | Setting | Cabinet | Enter 2 . | Shelf | Enter 1 . | Slot | Enter 7 . | Circuit | Enter 4 . | Card Type | Select DNI Line . | Device Type: Channel #1 | Select Superset 430 . |
| Field | Setting |
| Cabinet | Enter 2 . |
| Shelf | Enter 1 . |
| Slot | Enter 7 . |
...
| Step 5 | Return to the Multiline Set Forms screen. |
| Step 6 | On the Multiline Set Forms screen, select Multiline Set Key
                                          				Assignment . |
| Step 7 | On the Multiline Set Key Assignment screen,
                                       			 enter the following options. Table 2. Multiline Set Key Assignment
                                                				Options Field Setting Prime Directory Number Enter the first directory number. Key Number: 2 Select not assigned . Key Number: 3 Select not assigned . Key Number: 4 Select not assigned . Key Number: 5 Select not assigned . Key Number: 6 Select not assigned . Key Number: 7 Select not assigned . Key Number: 8 Select not assigned . Key Number: 9 Select not assigned . Key Number: 10 Select not assigned . Key Number: 11 Select not assigned . | Field | Setting | Prime Directory Number | Enter the first directory number. | Key Number: 2 | Select not assigned . | Key Number: 3 | Select not assigned . | Key Number: 4 | Select not assigned . | Key Number: 5 | Select not assigned . | Key Number: 6 | Select not assigned . | Key Number: 7 | Select not assigned . | Key Number: 8 | Select not assigned . | Key Number: 9 | Select not assigned . | Key Number: 10 | Select not assigned . | Key Number: 11 | Select not assigned . |
| Field | Setting |
| Prime Directory Number | Enter the first directory number. |
| Key Number: 2 | Select not assigned . |
| Key Number: 3 | Select not assigned . |
| Key Number: 4 | Select not assigned . |
| Key Number: 5 | Select not assigned . |
| Key Number: 6 | Select not assigned . |
| Key Number: 7 | Select not assigned . |
| Key Number: 8 | Select not assigned . |
| Key Number: 9 | Select not assigned . |
| Key Number: 10 | Select not assigned . |
| Key Number: 11 | Select not assigned . |
| Step 8 | Return to the Form Groups screen. |
| Step 9 | On the Form Groups screen, select Group Forms . |
| Step 10 | On the Group Forms screen, select Hunt Group Assignment . |
| Step 11 | On the Hunt Group Assignment screen, enter
                                       			 the following options. Table 3. Hunt Group Assignment Options Field Setting Pilot Number Enter the pilot number for Unity
                                                      						  Connection. Name Enter IP gateway hunt . Hunt Mode Select the hunt mode that you want
                                                      						  to use. For example, select “Circular.” | Field | Setting | Pilot Number | Enter the pilot number for Unity
                                                      						  Connection. | Name | Enter IP gateway hunt . | Hunt Mode | Select the hunt mode that you want
                                                      						  to use. For example, select “Circular.” |
| Field | Setting |
| Pilot Number | Enter the pilot number for Unity
                                                      						  Connection. |
| Name | Enter IP gateway hunt . |
| Hunt Mode | Select the hunt mode that you want
                                                      						  to use. For example, select “Circular.” |
| Step 12 | In the Hunt Group Assignment table of the
                                       			 Hunt Group Assignment screen, assign the directory number for each port on the
                                       			 PIMG units connected to the phone system as follows. Table 4. Hunt Group Assignment Table Member Directory Number Name 1 <the prime directory number> Enter IP gateway Port 1 . 2 <the second directory number> Enter IP gateway Port 2 . 3 <the third directory number> Enter IP gateway Port 3 . 4 <the next directory number> Enter IP gateway Port 4 . 5 <the next directory number> Enter IP gateway Port 5 . 6 <the next directory number> Enter IP gateway Port 6 . 7 <the next directory number> Enter IP gateway Port 7 . 8 <the next directory number> Enter IP gateway Port 8 . . . . <additional directory numbers> . . . <last member> <the last directory number> Enter IP gateway Port <n> . | Member | Directory Number | Name | 1 | <the prime directory number> | Enter IP gateway Port 1 . | 2 | <the second directory number> | Enter IP gateway Port 2 . | 3 | <the third directory number> | Enter IP gateway Port 3 . | 4 | <the next directory number> | Enter IP gateway Port 4 . | 5 | <the next directory number> | Enter IP gateway Port 5 . | 6 | <the next directory number> | Enter IP gateway Port 6 . | 7 | <the next directory number> | Enter IP gateway Port 7 . | 8 | <the next directory number> | Enter IP gateway Port 8 . | . . . | <additional directory numbers> | . . . | <last member> | <the last directory number> | Enter IP gateway Port <n> . |
| Member | Directory Number | Name |
| 1 | <the prime directory number> | Enter IP gateway Port 1 . |
| 2 | <the second directory number> | Enter IP gateway Port 2 . |
| 3 | <the third directory number> | Enter IP gateway Port 3 . |
| 4 | <the next directory number> | Enter IP gateway Port 4 . |
| 5 | <the next directory number> | Enter IP gateway Port 5 . |
| 6 | <the next directory number> | Enter IP gateway Port 6 . |
| 7 | <the next directory number> | Enter IP gateway Port 7 . |
| 8 | <the next directory number> | Enter IP gateway Port 8 . |
| . . . | <additional directory numbers> | . . . |
| <last member> | <the last directory number> | Enter IP gateway Port <n> . |
...
| Note | To access the software download page, you must
                                             			 ...
                                             			 browser, the steps may differ. In the tree control on the Downloads Home
                                                   				  ...
                                                   				  the firmware for digital Mitel PIMG units. On the Download Image page, select Download . On the Supporting Document(s) page, select Agree . In the File Download dialog box, select Save . In the Save As dialog box, browse to the
                                                   				  Windows workstation that have access the PIMG units, browse to a directory
...
| Step 12 | Repeat Step 6 through Step 11 for each of the
                                          			 following files: Mtl_<xx>.fsh Run<xx>FskEcho.dsp Mtl<xx>.lca |
...
| Step 15 | In the Choose File dialog box, browse to the
                                          			 file MTLDNI_Cfg_Mitel.ini. |
| Step 16 | Select MTLDNI_Cfg_Mitel.ini ,
                                          			 and select Open . |
...
| Step 24 | On the Change Password page, enter the
                                          			 following settings. Table 8. Change Password Page Settings Field Setting Old Password Enter IpodAdmin . (This setting is
...
| Step 45 | Under Inbound VoIP Request Matching, enter
                                          			 the following settings. The rule that you created in Step 44 must be selected.
                                             				Otherwise, any changes you make apply to another rule. Table 14. Inbound VoIP Request Matching
...
| Step 55 | Under Outbound Routes, enter the following
                                          			 settings. The rule that you created in Step 53 must be selected.
                                             				Otherwise, any changes you make apply to another rule. Table 21. Outbound Routes Settings Field Settings Device
                                                         						  ...
                                                         						  group that you created for Cisco Unity Connection in Step 28 . CPID
                                                            							 Manipulation Calling Number Enter S. Calling Name Enter S. Called Number Enter D. Called Name Enter D. Redirect Number Enter R. Redirect Name Enter R. Select
                                                            							 Primary/Alternate Route Primary Select Primary . | Field | Settings | Device
                                                            							 Selection | Outbound Destination | Select VoIP . | Host Group | Select the name of the VoIP host
                                                         						  group that you created for Cisco Unity Connection in Step 28 . | CPID
...
| Step 59 | On the Configuration menu, select TDM > General . |
| Step 60 | On the TDM General Settings page, enter the
                                          			 following settings. Table 22. TDM General Settings Page
                                                   				Settings Field Settings PCM Coding Select uLaw . Minimum Call Party Delay (ms) Enter 500 . Maximum Call Party Delay (ms) Enter 2000 . Dial Digit on Time (ms) Enter 100 . Dial Inter-Digit Time (ms) Enter 100 . Dial Pause Time (ms) Enter 2000 . Turn MWI On FAC Enter the code that turns MWIs on. Turn MWI Off FAC Enter the code that turns MWIs off. Outbound Call Connect Timeout (ms) Enter 10000 . Wait for Ringback/Connect on Blind
                                                         						  Transfer Select Yes . Hunt Group Extension Enter the pilot number of the Unity
                                                         						  Connection voice messaging ports. Disconnect on Fax Cleardown Tone Select No . | Field | Settings | PCM Coding | Select uLaw . | Minimum Call Party Delay (ms) | Enter 500 . | Maximum Call Party Delay (ms) | Enter 2000 . | Dial Digit on Time (ms) | Enter 100 . | Dial Inter-Digit Time (ms) | Enter 100 . | Dial Pause Time (ms) | Enter 2000 . | Turn MWI On FAC | Enter the code that turns MWIs on. | Turn MWI Off FAC | Enter the code that turns MWIs off. | Outbound Call Connect Timeout (ms) | Enter 10000 . | Wait for Ringback/Connect on Blind
                                                         						  Transfer | Select Yes . | Hunt Group Extension | Enter the pilot number of the Unity
...
| Step 62 | On the Configuration menu, select TDM > Port Enable . |
| Step 63 | On the TDM Port Enabling page, select No for the ports that
                                          			 you want to disable on the PIMG unit. |
| Step 64 | Confirm that Yes is selected for all
                                          			 other ports on the PIMG unit. |
| Step 65 | Select Submit . |
| Step 66 | On the Configuration menu, select VoIP > General . |
| Step 67 | On the VoIP General Settings page, enter the
...
| Step 69 | On the Configuration menu, select VoIP > Media . |
| Step 70 | On the VoIP Media Settings page, enter the
...
| Step 72 | On the Configuration menu, select VoIP > QOS . |
| Step 73 | On the VoIP QOS Configuration page, enter
...
| Step 75 | On the Configuration menu, select IP . |
| Step 76 | On the IP Settings page, enter the following
...
| Step 78 | On the Configuration menu, select Tone Detection . |
| Step 79 | On the Tone Detection page, under Call
                                          			 Progress Tone - Learn, in the Learn Tone Event field, select Busy and do the
                                          			 following substeps to verify that the tone is correct. From a available phone, call a second
                                                				  ...
                                                				  handsets for the other two phones off. |
| Step 80 | Under Call Progress Tone - Learn, in the
                                          			 Dial String field, enter the extension that you dialed in Step 79 c. from
                                          			 the third phone. |
| Step 81 | Select Learn . |
...
| Step 88 | Select Submit . |
| Step 89 | Hang up the phones that you used in Step 79 . |
| Step 90 | On the System menu, select Restart . |
| Step 91 | On the Restart page, select Restart Unit Now . |
| Step 92 | Repeat Step 2 through Step 91 on all remaining PIMG
                                          			 units. |
