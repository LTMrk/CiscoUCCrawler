---
doc_id: www-cisco-com-c-en-us-td-docs-voice-ip-comm-cust-contact-contact-center-icm-enterprise-icm-enterprise-15-0-1-design-guid-b63f991f31
source_url: https://www.cisco.com/c/en/us/td/docs/voice_ip_comm/cust_contact/contact_center/icm_enterprise/icm_enterprise_15_0_1/design/guide/ucce_b_ucce_soldg-for-unified-cce-1501/rcct_m_sizing-and-operating-conditions_15_0.html
retrieved_at: 2026-08-16T19:50:30.933939+00:00
---
...
Size the solution for the number of ports in use for calls in a talking state to agents.
...
The sizing of the VXML server can vary depending on the custom remote server that is deployed and the custom code that is
                                                   run.
...
###### CCCSP Performance Benchmarks

When your solution uses CCCSP , remember the following points:

CCCSP baseline tests were done in isolation on the proxy. The capacity numbers from those tests are 450 TCP or 500 UDP transactions/second.
...
You can use the same Unified CVP Call Server for both video calls and traditional audio calls in a comprehensive call flow. For any of the Non-Reference call flows, you use separate Call Servers for the video and audio calls.

The basic video service uses the comprehensive call flow. It requires Call Server, VXML Server, and Cisco VVB . You size these components in the same way that you do for audio calls.
...
#### Sizing for Unified Contact Center Management Portal

Unified CCMP offers several deployments. All the deployments assume an n-sided server configuration that replicates data between
                                 sites.

Map the Unified CCMP folder structure to your solution's organizational structure to facilitate resource management.
...
LAN —Connect Unified CCMP systems to Unified CCE and other servers with gigabit (1000BASE-T) connections.

WAN —Allocate at least a 1.5-MB/s capacity dedicated link to connect Unified CCMP systems to Unified CCE or for a distributed
                                          CCMP deployment across a WAN.

Load Balancing —Distributed Unified CCMP systems can use a load balancer to distribute load across the sites. Use a dedicated load balancer,
                                          rather than using Windows built-in functionality. Any load balancing solution must support sticky connections to maintain
                                          the web session information between requests.
...
For a single-server system, install the software prerequisites and Unified CCMP components for both the Web Application Server
                                                and the Database Server on the single server. A single-server system can only support the smallest deployments.

### Cisco Virtualized Voice Browser and Media Gateway Sizing
...
Traditional Dual-Tone Multi-Frequency (DTMF) uses keypad tones to signal pressed keys over the phone line. Cisco VVB handles
                                 calls by recognizing these tones to navigate through menus and perform actions.

Traditional DTMF with ASR/TTS configuration, integrates Automatic Speech Recognition (ASR) and Text-to-Speech (TTS) technologies
                                 with traditional DTMF systems. ASR allows the system to understand spoken utterences, while TTS enables the system to provide
                                 spoken responses.

Virtual Agent Voice (VAV) provides a comprehensive conversational experience by integrating a suite of services including
                                 ASR, TTS, Natural Language Understanding (NLU), and DTMF signaling. This system is designed to handle the entire conversation
                                 with cloud based services without reverting to traditional self-service applications for ASR, TTS, or DTMF functionalities
                                 within the same interaction flow.

VAV with DTMF/ASR/TTS means the call can have a VAV interaction at any point in the call flow, while also using traditional
                                 DTMF, TTS, and ASR for self-service.

Calls that exceed the specified limit on the SIP interface will be rejected. The Calls Per Second (CPS) metric defines the number of calls the service can handle per second. This metric is crucial for understanding the capacity
                                             of the interface and ensuring it can manage the expected call volume without issues.

System Specification

CPS

DTMF with Wav and without TTS / ASR (Secure and Non-Secure Signaling)

DTMF with Wav / DTMF with TTS / ASR (Secure and Non-Secure Signaling with Secure and Non-Secure Media)

VAV / VAV with DTMF / ASR / TTS (Secure and Non-Secure)

Medium OVA (4 CPU, 10-GB RAM)

24 - DTMF with Wav and without TTS / ASR (Secure and Non-Secure Signaling)

12 - DTMF with Wav / DTMF with TTS / ASR (Secure and Non-Secure Signaling with Secure and Non-Secure Media)

12 - VAV / VAV with DTMF / ASR / TTS (Secure and Non-Secure Signaling with secure and Non-Secure Media)
...
DTMF with TTS / ASR (Secure and Non-Secure)
...
#### Reverse Proxy Server Sizing for VPN-less Finesse
...
Run the linux command dd if=/dev/zero of=/root/junk bs=512 count=1000 oflag=dsync to check if the throughput shows a minimum of 2 MB per second for a 2k cluster and completion time of less than 0.3 seconds
                                                to write the data out. For every additional 2k cluster, you must double the throughput. The disk speed is checked by the installer
                                                during startup and prompts the user, if the required speed is not met.
...
Select this deployment for type ICM Enterprise system where CCE PGs are deployed. The system should be distributed deployment
                                             with Router and Logger installed on different VMs which meet the specified requirements.
...
Select this deployment type for ICM Enterprise system where CCE PGs are deployed. The Router and Logger are colocated on a
                                             single VM which meets the specified requirements.
...
The Finesse desktop can support either IPv4 or IPv6 connections. Enterprise Chat and
                                    						Email agents must use IPv4 connections.

Cisco Finesse is the required dekstop for all other contact center enterprise solutions.
...
A supervisor’s team can include a mix of agents using Finesse desktops with either IPv4 or IPv6 connections.

You cannot use IPv6 to connect to a Finesse desktop through Citrix XenApp.

Supervisor reskilling using Unified CCE Administration requires a NAT64 deployment.
...
Administration Client (sometimes called a Client AW)
...
Parent/Child or System PG

No

No

You cannot use a System PG in an IPv6-enabled environment.
...
| Note | The definitions of
                                             		  ...
                                             		  You can ignore ASR and TTS processing when counting which calls are in which
                                             		  states for sizing purposes. However, ASR and TTS processing does come into call
                                             		  state counts for licensing. Size the solution for the number of ports in use for calls in a talking state to agents. |
|---|---|
...
| Note | The sizing of the VXML server can vary depending on the custom remote server that is deployed and the custom code that is
                                                   run. |
|---|---|
...
|---|---|
| CCMP Deployment | Agents (concurrent/configured) | Configured CCMP users | Folders | Folder depth |
| 4000 Agent Deployments | CCMP Small | 4000/24000 | 400 | 400 | 5 |
| 12000 Agent Deployments | CCMP Large | 12000/72000 | 1200 | 1000 | 6 |
| 24000 Agent Deployments | CCMP Large | 24000/72000 | 1200 | 1000 | 6 |

| Note | For a single-server system, install the software prerequisites and Unified CCMP components for both the Web Application Server
                                                and the Database Server on the single server. A single-server system can only support the smallest deployments. |
|---|---|
...
| Note | Calls that exceed the specified limit on the SIP interface will be rejected. The Calls Per Second (CPS) metric defines the number of calls the service can handle per second. This metric is crucial for understanding the capacity
                                             of the interface and ensuring it can manage the expected call volume without issues. |
|---|---|

| System Specification | CPS | DTMF with Wav and without TTS / ASR (Secure and Non-Secure Signaling) | DTMF with Wav / DTMF with TTS / ASR (Secure and Non-Secure Signaling with Secure and Non-Secure Media) | VAV / VAV with DTMF / ASR / TTS (Secure and Non-Secure) |
|---|---|---|---|---|
| Medium OVA (4 CPU, 10-GB RAM) | 24 - DTMF with Wav and without TTS / ASR (Secure and Non-Secure Signaling) 12 - DTMF with Wav / DTMF with TTS / ASR (Secure and Non-Secure Signaling with Secure and Non-Secure Media) 12 - VAV / VAV with DTMF / ASR / TTS (Secure and Non-Secure Signaling with secure and Non-Secure Media) | 1000 | 750 | 750 |
| Small OVA (4 CPU, 8-GB RAM) | 800 | 600 | 600 |

| System Specification | CPS | DTMF (Non-Secure) | DTMF with TTS / ASR (Secure and Non-Secure) |
|---|---|---|---|
| KVM [4451 (2 CPU - Gladen), 8-GB RAM] | 6 | 120 | 96 |
| KVM [4431 (6 CPU - Gladen), 8-GB RAM] | 3 | 80 | 70 |
| KVM [4351 (6 CPU - Rangley), 8-GB RAM] | 3 | 60 | 50 |
| KVM [4331 (6 CPU - Rangley), 8-GB RAM] | 2 | 40 | 30 |

| System Specification | CPS | DTMF with Wav and without TTS / ASR (Secure and Non-Secure Signaling) |
|---|---|---|
| Medium OVA (4 CPU, 16-GB RAM) | 10 | 1000 |
...
| Note | Disk slowness can hamper the proxy performance. Monitor the solution to ensure the disk has adequate IO throughput. Run the linux command dd if=/dev/zero of=/root/junk bs=512 count=1000 oflag=dsync to check if the throughput shows a minimum of 2 MB per second for a 2k cluster and completion time of less than 0.3 seconds
                                                to write the data out. For every additional 2k cluster, you must double the throughput. The disk speed is checked by the installer
                                                during startup and prompts the user, if the required speed is not met. To bypass the disk speed check, start the launcher with -y option, for example ./proxy_launcher.sh -e env_configs/elastigirl/pub/ast_envs/ -i env_configs/elastigirl/pub/installer.env -y start . |
|---|---|
...
| 2 | Contact Director | Select this deployment type for ICM instance which is dedicated to self-service call flows using Unified CVP or third-party
                                             ...
                                             specified requirements. No agents are allowed in this deployment type. If agents are configured and signed in, the capacity
                                             is adjusted to maximum capacity of an Enterprise Contact Center (Unified CCE 12000 Agents Router/ Logger). |
| 4 | ICM Router/Logger | Select this deployment for type ICM Enterprise system where CCE PGs are deployed. The system should be distributed deployment
                                             with Router and Logger installed on different VMs which meet the specified requirements. |
...
| 8 | ICM Rogger | Select this deployment type for ICM Enterprise system where CCE PGs are deployed. The Router and Logger are colocated on a
                                             single VM which meets the specified requirements. |
...
| Note | Cisco Finesse is the required dekstop for all other contact center enterprise solutions. |
|---|---|
...
|---|---|---|
| IPv6 | IPv4 |
| Cisco Remote Silent Monitoring | Yes | Yes |  |
| Enterprise Chat and
                                                      						Email | No | Yes |  |
...
| Non-Reference Design |
| Parent/Child or System PG | No | No | You cannot use a System PG in an IPv6-enabled environment. |
| Unified IP IVR | No | Yes |  |
